...
def main():
    ...
    if Version(syncmatrix.__version__) < Version("2.1"):
        # work queue is positional instead of a flag and requires creation
        subprocess.run(["syncmatrix", "work-queue", "create", "test"])
        try:
            subprocess.run(["syncmatrix", "agent", "start", "test"], timeout=30)
        except subprocess.TimeoutExpired:
            pass
    elif (
        Version(syncmatrix.__version__) > Version("2.6")
        ...
        return
    elif Version(syncmatrix.__version__) < Version("2.6"):
        # --run-once is not available so just run for a bit
        try:
            subprocess.run(
                ["syncmatrix", "agent", "start", "--work-queue", "default"], timeout=30
            )
        except subprocess.TimeoutExpired:
            pass
    else:
        subprocess.check_call(
            ["syncmatrix", "agent", "start", "--run-once", "--work-queue", "default"],
        )
