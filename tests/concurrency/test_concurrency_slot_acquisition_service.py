...
from syncmatrix.concurrency.services import ConcurrencySlotAcquisitionService
...
async def mocked_client(test_database_connection_url):
    async with get_client() as client:
        with mock.patch.object(client, "increment_concurrency_slots", autospec=True):
            ...
            with mock.patch(
                "syncmatrix.concurrency.services.get_client", lambda: wrapped_client
            ):
...
async def test_retries_failed_call_respects_retry_after_header(mocked_client):
    ...
    with mock.patch("syncmatrix.concurrency.asyncio.asyncio.sleep") as sleep:
