...
from syncmatrix.client.schemas.objects import Deployment
...
from syncmatrix.server.database.orm_models import ORMDeployment
...
async def _create_test_deployment_from_orm(
    syncmatrix_client: SyncmatrixClient, orm_deployment: ORMDeployment, **kwargs
) -> Deployment:
...
async def test_agent_cancel_run_not_called_for_other_states(
    syncmatrix_client: SyncmatrixClient, deployment: ORMDeployment, state
):
...
async def test_agent_cancel_run_called_for_each_cancelling_run_in_multiple_work_queues(
    ...
    deployment_foo = await _create_test_deployment_from_orm(
        syncmatrix_client, deployment, work_queue_name=wq1
    )
    deployment_bar = await _create_test_deployment_from_orm(
        syncmatrix_client, deployment, work_queue_name=wq2
    )

    flow_run_foo = await syncmatrix_client.create_flow_run_from_deployment(
        deployment_foo.id,
        state=cancelling_constructor(),
    )
    flow_run_bar = await syncmatrix_client.create_flow_run_from_deployment(
        deployment_bar.id,
        state=cancelling_constructor(),
    )

    async with SyncmatrixAgent(work_queues=[wq1, wq2], prefetch_seconds=10) as agent:
        agent.cancel_run = AsyncMock()
        await agent.check_for_cancelled_flow_runs()
...
async def test_agent_cancel_run_called_for_each_cancelling_run_in_a_work_queue(
    syncmatrix_client: SyncmatrixClient, deployment: ORMDeployment, cancelling_constructor
):
    deployment_foo = await _create_test_deployment_from_orm(
        syncmatrix_client, deployment, work_queue_name="foo"
    )
    ...
    async with SyncmatrixAgent(work_queues=["foo"], prefetch_seconds=10) as agent:
        agent.cancel_run = AsyncMock()
        await agent.check_for_cancelled_flow_runs()
...
async def test_agent_cancel_run_not_called_for_other_work_queues(
    syncmatrix_client: SyncmatrixClient, deployment, cancelling_constructor
):
    await syncmatrix_client.create_flow_run_from_deployment(
        deployment.id,
        state=cancelling_constructor(),
