...
from syncmatrix.client.schemas.objects import DEFAULT_AGENT_WORK_POOL_NAME
from syncmatrix.exceptions import Abort, CrashedRun, FailedRun
from syncmatrix.infrastructure.base import Infrastructure
from syncmatrix.server import models, schemas
from syncmatrix.states import Completed, Pending, Running, Scheduled, State, StateType
...
async def test_agent_start_will_not_run_without_start():
    agent = SyncmatrixAgent(work_queues=["foo"])
    mock = AsyncMock()
...
async def test_agent_start_and_shutdown():
    async with SyncmatrixAgent(work_queues=["foo"]) as agent:
...
async def test_agent_with_work_queue(syncmatrix_client, deployment):
    @flow
    def foo():
        pass

    def create_run_with_deployment(state):
        return syncmatrix_client.create_flow_run_from_deployment(
    ...
    flow_runs = [
        ...
        await syncmatrix_client.create_flow_run(foo, state=Scheduled()),
    ]
    flow_run_ids = [run.id for run in flow_runs]

    # Pull runs from the work queue to get expected runs
    work_queue_runs = await syncmatrix_client.get_runs_in_work_queue(
    ...
    agent = SyncmatrixAgent(work_queues=[deployment.work_queue.name], prefetch_seconds=10)
...
async def test_agent_with_work_queue_and_limit(syncmatrix_client, deployment):
    @flow
    def foo():
        pass
...
async def test_agent_matches_work_queues_dynamically(
    session, work_queue, syncmatrix_caplog
):
    assert await models.work_queues.read_work_queue_by_name(
        session=session, name=work_queue.name
    )
    async with SyncmatrixAgent(work_queue_prefix=["wq-"]) as agent:
        assert work_queue.name not in agent.work_queues
        await agent.get_and_submit_flow_runs()
        assert work_queue.name in agent.work_queues

    assert "Matched new work queues:" in syncmatrix_caplog.text
    assert work_queue.name in syncmatrix_caplog.text


async def test_agent_matches_multiple_work_queues_dynamically(syncmatrix_client):
    prod1 = "prod-deployment-1"
    prod2 = "prod-deployment-2"
    prod3 = "prod-deployment-3"
    dev1 = "dev-data-producer-1"
    await syncmatrix_client.create_work_queue(name=prod1)
    await syncmatrix_client.create_work_queue(name=prod2)

    async with SyncmatrixAgent(work_queue_prefix=["prod-"]) as agent:
        ...
        await syncmatrix_client.create_work_queue(name=prod3)
        await syncmatrix_client.create_work_queue(name=dev1)
        await agent.get_and_submit_flow_runs()
...
async def test_agent_matches_multiple_work_queue_prefixes(syncmatrix_client):
    prod = "prod-deployment-4"
    dev = "dev-data-producer-2"
    await syncmatrix_client.create_work_queue(name=prod)
    await syncmatrix_client.create_work_queue(name=dev)

    async with SyncmatrixAgent(work_queue_prefix=["prod-", "dev-"]) as agent:
        assert not agent.work_queues
        await agent.get_and_submit_flow_runs()
...
async def test_matching_work_queues_handes_work_queue_deletion(
    session, work_queue, syncmatrix_client, syncmatrix_caplog
):
    assert await models.work_queues.read_work_queue_by_name(
        session=session, name=work_queue.name
    )
    async with SyncmatrixAgent(work_queue_prefix=["wq-"]) as agent:
        ...
        assert "Matched new work queues:" in syncmatrix_caplog.text
        assert work_queue.name in syncmatrix_caplog.text
        await syncmatrix_client.delete_work_queue_by_id(work_queue.id)
        await agent.get_and_submit_flow_runs()
        assert work_queue.name not in agent.work_queues
        assert (
            f"Work queues no longer matched: {work_queue.name}" in syncmatrix_caplog.text
        )


async def test_agent_creates_work_queue_if_doesnt_exist(session, syncmatrix_caplog):
    name = f"hello-there-{uuid.uuid4()}"
    assert not await models.work_queues.read_work_queue_by_name(
        session=session, name=name
    )
    async with SyncmatrixAgent(work_queues=[name]) as agent:
        await agent.get_and_submit_flow_runs()
    assert await models.work_queues.read_work_queue_by_name(session=session, name=name)

    assert f"Created work queue '{name}'." in syncmatrix_caplog.text
...
async def test_agent_creates_work_queue_if_doesnt_exist_in_work_pool(
    ...
    async with SyncmatrixAgent(work_queues=[name], work_pool_name=work_pool.name) as agent:
        await agent.get_and_submit_flow_runs()
    ...
    assert (
        ...
        in syncmatrix_caplog.text
    )


async def test_agent_does_not_create_work_queues_if_matching_with_prefix(
    session, syncmatrix_caplog, syncmatrix_client: SyncmatrixClient
):
    default_work_pool = await syncmatrix_client.read_work_pool(
        DEFAULT_AGENT_WORK_POOL_NAME
    )
    if not default_work_pool:
        await syncmatrix_client.create_work_pool(
            WorkPoolCreate(name=DEFAULT_AGENT_WORK_POOL_NAME, type="syncmatrix-agent")
        )
    ...
    assert f"Created work queue '{name}'." not in syncmatrix_caplog.text
...
async def test_agent_gracefully_handles_error_when_creating_work_queue(
    ...
    monkeypatch.setattr("syncmatrix.client.SyncmatrixClient.create_work_queue", bad_create)
    ...
    assert "No!" in syncmatrix_caplog.text


async def test_agent_caches_work_queues(syncmatrix_client, deployment, monkeypatch):
    work_queue = await syncmatrix_client.read_work_queue(deployment.work_queue.id)

    async def read_queue(name, work_pool_name=None):
        return work_queue

    mock = AsyncMock(side_effect=read_queue)
    monkeypatch.setattr("syncmatrix.client.SyncmatrixClient.read_work_queue_by_name", mock)

    async with SyncmatrixAgent(
...
async def test_agent_internal_submit_run_called(syncmatrix_client, deployment):
...
async def test_agent_runs_multiple_work_queues(syncmatrix_client, session, flow):
    ...
    # create two runs
    flow_run_a = await syncmatrix_client.create_flow_run_from_deployment(
        deployment_a.id,
        state=Scheduled(scheduled_time=pendulum.now("utc")),
    )
    flow_run_b = await syncmatrix_client.create_flow_run_from_deployment(
        deployment_b.id,
        state=Scheduled(scheduled_time=pendulum.now("utc")),
    )
...
class TestInfrastructureIntegration:
    ...
    def mock_propose_state(self, monkeypatch):
        mock = AsyncMock()
        monkeypatch.setattr("syncmatrix.agent.propose_state", mock)

        yield mock

    async def test_agent_submits_using_the_retrieved_infrastructure(
        self, syncmatrix_client, deployment, mock_infrastructure_run
    ):
        ...
        flow = await syncmatrix_client.read_flow(deployment.flow_id)

        infra_document = await syncmatrix_client.read_block_document(infra_doc_id)
    ...
    async def test_agent_submit_run_aborts_without_raising_if_server_raises_abort(
        self, syncmatrix_client, deployment, mock_infrastructure_run, mock_propose_state
    ):
        flow_run = await syncmatrix_client.create_flow_run_from_deployment(
            deployment.id,
            state=Scheduled(scheduled_time=pendulum.now("utc")),
    ...
    async def test_agent_crashes_flow_if_infrastructure_returns_nonzero_status_code(
        self, syncmatrix_client, deployment, mock_infrastructure_run, caplog
    ):
        infra_doc_id = deployment.infrastructure_document_id
        infra_document = await syncmatrix_client.read_block_document(infra_doc_id)
        infrastructure = Block._from_block_document(infra_document)
...
async def test_agent_displays_message_on_work_queue_pause(
    syncmatrix_client, syncmatrix_caplog, deployment_in_default_work_pool
):
    async with SyncmatrixAgent(
        ...
        assert (
            ...
            not in syncmatrix_caplog.text
        ), "Message should not be displayed before pausing"

        await syncmatrix_client.update_work_queue(
...
async def test_agent_with_work_queue_and_work_pool(
    ...
    # Pull runs from the work queue to get expected runs
    responses = await syncmatrix_client.get_scheduled_flow_runs_for_work_pool(
...
async def test_agent_with_work_pool(
    ...
    # Pull runs from the work queue to get expected runs
    work_queue = await syncmatrix_client.read_work_queue_by_name(
        work_pool_name=work_pool.name,
...
async def test_agent_runs_high_priority_flow_runs_first(
    ...
    agent = SyncmatrixAgent(work_pool_name=work_pool.name, prefetch_seconds=10, limit=1)
