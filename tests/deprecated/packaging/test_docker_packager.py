...
from syncmatrix.deprecated.packaging.docker import DockerPackageManifest, DockerPackager
...
def silence_user_warnings_about_editable_packages():
    ...
    # syncmatrix.software.pip.current_environment_requirements about editable packages;
    # when working locally, the syncmatrix package is almost always editable
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
            message="The following requirements will not be installable.*",
            category=UserWarning,
            module="syncmatrix.software.pip",
        )
        yield
...
def test_base_image_defaults_to_syncmatrix_base():
    packager = DockerPackager()
    assert packager.base_image == get_syncmatrix_image_name()


def test_base_image_defaults_to_conda_flavor_of_syncmatrix_base():
    packager = DockerPackager(python_environment=CondaEnvironment())
    assert packager.base_image == get_syncmatrix_image_name(flavor="conda")
...
async def test_packaging_a_flow_to_local_docker_daemon(syncmatrix_base_image: str):
    packager = DockerPackager(
        base_image=syncmatrix_base_image,
...
@pytest.mark.service("docker")
async def test_packaging_a_flow_to_registry(syncmatrix_base_image: str, registry: str):
    packager = DockerPackager(
        base_image=syncmatrix_base_image,
        python_environment=PythonEnvironment(
            python_version="3.9",
...
async def test_packaging_a_flow_to_registry_without_scheme(
    syncmatrix_base_image: str, registry: str
):
    packager = DockerPackager(
        base_image=syncmatrix_base_image,
        python_environment=PythonEnvironment(
...
@pytest.fixture
def howdy_context(syncmatrix_base_image: str, tmp_path: Path) -> Path:
    (tmp_path / "Dockerfile").write_text(
        textwrap.dedent(
            f"""
            FROM {syncmatrix_base_image}
...
async def test_unpackaging_inside_container(
    syncmatrix_base_image: str, docker: DockerClient
):
    packager = DockerPackager(
        base_image=syncmatrix_base_image,
        python_environment=PythonEnvironment(
...
def assert_unpackaged_flow_works(docker: DockerClient, manifest: DockerPackageManifest):
    test_script = textwrap.dedent(
        f"""
    import asyncio
    from syncmatrix.deprecated.packaging.docker import DockerPackageManifest
