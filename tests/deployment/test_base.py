...
from syncmatrix.settings import SYNCMATRIX_DEBUG_MODE, temporary_settings
...
def project_dir(tmp_path):
    with tmpchdir(tmp_path):
        if sys.version_info >= (3, 8):
            ...
            (tmp_path / ".syncmatrix").mkdir(exist_ok=True, mode=0o0700)
            yield tmp_path
        else:
...
class TestFindProject:
    async def test_find_project_works_in_root(self, tmp_path):
        # make hidden .syncmatrix/ directory in tmp_path
        (tmp_path / ".syncmatrix").mkdir(exist_ok=True)
        assert find_syncmatrix_directory(tmp_path) == tmp_path / ".syncmatrix"
    ...
    async def test_find_project_works_in_subdir(self, tmp_path):
        ...
        assert (
            find_syncmatrix_directory(tmp_path / "subdir" / "subsubdir")
            == tmp_path / ".syncmatrix"
        )
...
class TestRecipes:
    ...
    @pytest.mark.parametrize(
        ...
        [
            d.absolute().name
            for d in Path(
                syncmatrix.__development_base_path__
                / "src"
                / "syncmatrix"
                / "deployments"
                / "recipes"
    ...
    async def test_configure_project_by_recipe_doesnt_raise(self, recipe):
        ...
        for key in ["name", "syncmatrix-version", "build", "push", "pull"]:
    ...
    async def test_configure_project_handles_templates_on_git_recipes(self, recipe):
        ...
        clone_step = recipe_config["pull"][0]["syncmatrix.deployments.steps.git_clone"]
    ...
    async def test_configure_project_replaces_templates_on_docker(self):
        ...
        assert clone_step["directory"] == "/opt/syncmatrix/test-dir"
...
class TestInitProject:
    async def test_initialize_project_works(self):
        ...
        assert contents["syncmatrix-version"] == syncmatrix.__version__
    ...
    async def test_initialize_project_with_recipe(self):
        ...
        assert "syncmatrix.deployments.steps.git_clone" in clone_step

        build_step = contents["build"][0]
        assert "syncmatrix_docker.deployments.steps.build_docker_image" in build_step
...
class TestRegisterFlow:
    async def test_register_flow_works_in_root(self, project_dir):
        ...
        flows_file = project_dir / ".syncmatrix" / "flows.json"
...
class TestDiscoverFlows:
    ...
    async def test_find_all_flows_works_on_large_directory_structures(self):
        flows = await _search_for_flow_functions(str(syncmatrix.__development_base_path__))
    ...
    async def test_syncmatrix_can_be_imported_from_non_main_thread(self):
        """testing due to `asyncio.Semaphore` error when importing syncmatrix from a worker thread
        in python <= 3.9
        """

        def import_syncmatrix():
            import syncmatrix  # noqa: F401

        await run_sync_in_worker_thread(import_syncmatrix)
