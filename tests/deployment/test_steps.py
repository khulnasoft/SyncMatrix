...
from syncmatrix.deployments.steps import run_step
from syncmatrix.deployments.steps.core import StepExecutionError, run_steps
from syncmatrix.deployments.steps.utility import run_shell_script
...
class TestRunStep:
    ...
    async def test_run_step_resolves_block_document_references_before_running(self):
        ...
        output = await run_step(
            {
                "syncmatrix.deployments.steps.run_shell_script": {
                    "script": "{{ syncmatrix.blocks.secret.test-secret }}",
                }
            }
        )
        assert isinstance(output, dict)
        ...
        output = await run_step(
            {
                "syncmatrix.deployments.steps.run_shell_script": {
                    "script": (
                        "echo '{{ syncmatrix.variables.test_variable_1 }}:{{"
                        " syncmatrix.variables.test_variable_2 }}'"
                    ),
                }
            }
        )
    ...
    async def test_requirement_installation_successful(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "syncmatrix.deployments.steps.core.import_module", import_module_mock
        )
        ...
        monkeypatch.setattr(
            "syncmatrix.deployments.steps.core.import_object", import_object_mock
        )
...
class TestRunSteps:
    ...
    async def test_run_steps_prints_deprecation_warnings(self, monkeypatch):
        ...
        monkeypatch.setattr("syncmatrix.deployments.steps.run_shell_script", func)
    ...
    async def test_run_steps_prints_syncmatrix_deprecation_warnings(self, monkeypatch):
        def func(*args, **kwargs):
            warnings.warn("this is a warning", SyncmatrixDeprecationWarning)
            return {}
...
def git_repository_mock(monkeypatch):
    ...
    monkeypatch.setattr(
        "syncmatrix.deployments.steps.pull.GitRepository",
        git_repository_mock,
    )
...
class TestGitCloneStep:
    ...
    async def test_deprecated_git_clone_project(self, git_repository_mock):
        with pytest.warns(DeprecationWarning):
            output = await run_step(
                {
                    "syncmatrix.projects.steps.git_clone_project": {
                        "repository": "https://github.com/org/repo.git",
                    }
                }
            )
        ...
        await run_step(
            {
                "syncmatrix.deployments.steps.git_clone": {
                    "repository": "https://github.com/org/repo.git",
                    "access_token": "{{ syncmatrix.blocks.secret.my-access-token }}",
                }
            }
        )
        git_repository_mock.assert_called_once_with(
        ...
        await run_step(
            {
                "syncmatrix.deployments.steps.git_clone": {
                    "repository": "https://github.com/org/repo.git",
                    "credentials": (
                        "{{ syncmatrix.blocks.mockgitcredentials.my-credentials }}"
                    ),
                }
            }
        )
...
class TestPullFromRemoteStorage:
    ...
    def remote_storage_mock(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "syncmatrix.deployments.steps.pull.RemoteStorage",
            remote_storage_mock,
        )
    ...
    def subprocess_mock(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "syncmatrix.deployments.steps.core.subprocess",
            subprocess_mock,
        )
    ...
    async def test_pull_from_remote_storage(
        ...
        output = await run_step(
            {
                "syncmatrix.deployments.steps.pull_from_remote_storage": {
                    "requires": "s3fs<3.0",
...
class TestRunShellScript:
    ...
    async def test_run_shell_script_split_on_windows(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "syncmatrix.deployments.steps.utility.shlex.split",
            shex_split_mock,
        )
...
class TestPipInstallRequirements:
    async def test_pip_install_reqs_runs_expected_command(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "syncmatrix.deployments.steps.utility.open_process",
            open_process_mock,
        )
        ...
        monkeypatch.setattr(
            "syncmatrix.deployments.steps.utility._stream_capture_process_output",
            mock_stream_capture,
        )

        await run_step(
            {
                "syncmatrix.deployments.steps.pip_install_requirements": {
                    "id": "pip-install-step"
                }
            }
        )
        ...
        steps = [
            {
                "syncmatrix.deployments.steps.git_clone": {
                    "id": "clone-step",
                    "repository": "https://github.com/KhulnaSoft/hello-projects.git",
                }
            },
            {
                "syncmatrix.deployments.steps.pip_install_requirements": {
...
class TestPullWithBlock:
    ...
    async def test_normal_operation(self, test_block: Block):
        ...
        try:
            output = await run_step(
                {
                    "syncmatrix.deployments.steps.pull_with_block": {
