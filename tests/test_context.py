...
from syncmatrix.results import ResultFactory
...
async def test_flow_run_context(syncmatrix_client):
    @flow
    def foo():
        pass
    ...
    async with anyio.create_task_group() as task_group:
        with FlowRunContext(
            ...
            assert ctx.client is syncmatrix_client
...
async def test_task_run_context(syncmatrix_client, flow_run):
    @task
    def foo():
        pass
...
async def test_get_run_context(syncmatrix_client, local_filesystem):
    @flow
    def foo():
        pass
    ...
    test_task_runner = SequentialTaskRunner()
    flow_run = await syncmatrix_client.create_flow_run(foo)
    task_run = await syncmatrix_client.create_task_run(bar, flow_run.id, dynamic_key="")
    ...
    async with anyio.create_task_group() as task_group:
        with FlowRunContext(
            ...
            task_runner=test_task_runner,
            background_tasks=task_group,
            result_factory=await ResultFactory.from_flow(foo, client=syncmatrix_client),
            parameters={"x": "y"},
        ) as flow_ctx:
...
class TestSettingsContext:
    ...
    def temporary_profiles_path(self, tmp_path):
        ...
        with temporary_settings(
            updates={SYNCMATRIX_HOME: tmp_path, SYNCMATRIX_PROFILES_PATH: path}
        ):
            yield path
    ...
    def test_settings_context_variable(self):
        with SettingsContext(
            ...
            settings=syncmatrix.settings.get_settings_from_env(),
        ) as context:
            assert get_settings_context() is context
            assert context.profile == Profile(name="test", settings={})
            assert context.settings == syncmatrix.settings.get_settings_from_env()
    ...
    def test_get_settings_context_missing(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "syncmatrix.context.SettingsContext.get", MagicMock(return_value=None)
        )
    ...
    def test_creates_home(self, tmp_path):
        ...
        with temporary_settings(updates={SYNCMATRIX_HOME: home}):
            pass
    ...
    def test_settings_context_uses_settings(self, temporary_profiles_path):
        temporary_profiles_path.write_text(
            textwrap.dedent(
                """
                [profiles.foo]
                SYNCMATRIX_API_URL="test"
                """
            )
        )
        with use_profile("foo") as ctx:
            assert syncmatrix.settings.SYNCMATRIX_API_URL.value() == "test"
            assert ctx.settings == syncmatrix.settings.get_current_settings()
            assert ctx.profile == Profile(
                name="foo",
                settings={SYNCMATRIX_API_URL: "test"},
    ...
    def test_settings_context_does_not_setup_logging(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "syncmatrix.logging.configuration.setup_logging", setup_logging
        )
    ...
    def test_settings_context_nesting(self, temporary_profiles_path):
        temporary_profiles_path.write_text(
            textwrap.dedent(
                """
                [profiles.foo]
                SYNCMATRIX_API_URL="foo"

                [profiles.bar]
                SYNCMATRIX_API_URL="bar"
                """
            )
        )
        with use_profile("foo") as foo_context:
            with use_profile("bar") as bar_context:
                assert bar_context.settings == syncmatrix.settings.get_current_settings()
                assert (
                    syncmatrix.settings.SYNCMATRIX_API_URL.value_from(bar_context.settings)
                    == "bar"
                )
                assert bar_context.profile == Profile(
                    name="bar",
                    settings={SYNCMATRIX_API_URL: "bar"},
                    source=temporary_profiles_path,
                )
            assert foo_context.settings == syncmatrix.settings.get_current_settings()
            assert (
                syncmatrix.settings.SYNCMATRIX_API_URL.value_from(foo_context.settings)
                == "foo"
            )
            assert foo_context.profile == Profile(
                name="foo",
                settings={SYNCMATRIX_API_URL: "foo"},
                source=temporary_profiles_path,
            )
    ...
    def foo_profile(self, temporary_profiles_path):
        profile = Profile(
            name="foo",
            settings={SYNCMATRIX_API_KEY: "xxx"},
            source=temporary_profiles_path,
        )
    ...
    def test_root_settings_context_default(self, monkeypatch):
        ...
        monkeypatch.setattr("syncmatrix.context.use_profile", use_profile)
    ...
    def test_root_settings_context_respects_cli(self, monkeypatch, foo_profile):
        use_profile = MagicMock()
        monkeypatch.setattr("syncmatrix.context.use_profile", use_profile)
        monkeypatch.setattr("sys.argv", ["/syncmatrix", "--profile", "foo"])
        result = root_settings_context()
        use_profile.assert_called_once_with(
            foo_profile,
    ...
    def test_root_settings_context_missing_cli(self, monkeypatch, capsys):
        use_profile = MagicMock()
        monkeypatch.setattr("syncmatrix.context.use_profile", use_profile)
        monkeypatch.setattr("sys.argv", ["/syncmatrix", "--profile", "bar"])
        root_settings_context()
    ...
    def test_root_settings_context_missing_environment_variables(
        ...
        use_profile = MagicMock()
        monkeypatch.setattr("syncmatrix.context.use_profile", use_profile)
        monkeypatch.setenv("SYNCMATRIX_PROFILE", "bar")
        root_settings_context()
        _, err = capsys.readouterr()
        assert (
