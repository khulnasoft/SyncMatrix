...
from syncmatrix.client.schemas.actions import DeploymentScheduleCreate
from syncmatrix.client.schemas.objects import MinimalDeploymentSchedule
from syncmatrix.client.schemas.schedules import CronSchedule, RRuleSchedule
from syncmatrix.deployments.deployments import load_flow_from_flow_run
...
from syncmatrix.deployments import Deployment, run_deployment
from syncmatrix.events.schemas import DeploymentTrigger
from syncmatrix.exceptions import BlockMissingCapabilities
from syncmatrix.filesystems import S3, GitHub, LocalFileSystem
from syncmatrix.infrastructure import DockerContainer, Infrastructure, Process
from syncmatrix.server.schemas import states
    ...
    if default_work_pool is None:
        await models.workers.create_work_pool(
            ...
            work_pool=schemas.actions.WorkPoolCreate(
                name=models.workers.DEFAULT_AGENT_WORK_POOL_NAME, type="syncmatrix-agent"
            ),
        )
...
def test_deployment_emits_deprecation_warning():
    with pytest.warns(
        ...
        match=(
            "syncmatrix.deployments.deployments.Deployment has been deprecated."
...
class TestDeploymentLoad:
    async def test_deployment_load_hydrates_with_server_settings(
        self, syncmatrix_client, flow, storage_document_id, infrastructure_document_id
    ):
        await syncmatrix_client.create_deployment(
            name="My Deployment",
            version="mint",
            path="/",
        ...
        infra_document = await syncmatrix_client.read_block_document(
            infrastructure_document_id
        )
        infrastructure_block = Block._from_block_document(infra_document)
        assert d.infrastructure == infrastructure_block

        storage_document = await syncmatrix_client.read_block_document(storage_document_id)
...
class TestDeploymentUpload:
    ...
    async def test_uploading_with_readonly_storage_raises(self, tmp_path):
        storage = GitHub(repository="syncmatrix")
...
class TestDeploymentBuild:
    ...
    async def test_build_from_flow_manages_docker_specific_path(self, flow_function):
        ...
        assert d.path == "/opt/syncmatrix/flows"
    ...
    async def test_build_from_flow_legacy_schedule_supported(
        self, flow_function, syncmatrix_client
    ):
        deployment = await Deployment.build_from_flow(
        ...
        refreshed = await syncmatrix_client.read_deployment(deployment_id)
...
def patch_import(monkeypatch):
    ...
    monkeypatch.setattr("syncmatrix.utilities.importtools.import_object", lambda path: fn)
...
class TestDeploymentApply:
    async def test_deployment_apply_updates_concurrency_limit(
        ...
        work_queue = await syncmatrix_client.read_work_queue_by_name(queue_name)
    ...
    async def test_deployment_apply_updates_schedules(
        ...
        dep = await syncmatrix_client.read_deployment(dep_id)
    ...
    async def test_deployment_build_from_flow_clears_multiple_schedules(
        ...
        modified_dep = await syncmatrix_client.read_deployment(dep_id)
    ...
    async def test_deployment_is_active_behaves_as_expected(
        self, flow_function, provided, expected, syncmatrix_client
    ):
        d = await Deployment.build_from_flow(
            flow_function,
            name="foo",
    ...
    async def test_deployment_apply_syncs_triggers(
        ...
        with temporary_settings(
            updates={
                SYNCMATRIX_API_URL: f"https://api.syncmatrix.cloud/api/accounts/{uuid4()}/workspaces/{uuid4()}",
                SYNCMATRIX_CLOUD_API_URL: "https://api.syncmatrix.cloud/api/",
            }
        ):
            with respx.mock(base_url=SYNCMATRIX_API_URL.value()) as router:
                ...
                delete_route = router.delete(
                    f"/automations/owned-by/syncmatrix.deployment.{created_deployment_id}"
    ...
    async def test_deployment_apply_with_dict_parameter(
        self, flow_function_dict_parameter, syncmatrix_client
    ):
        d = await Deployment.build_from_flow(
...
class TestRunDeployment:
    ...
    def test_running_a_deployment_blocks_until_termination(
        ...
        with respx.mock(
            base_url=SYNCMATRIX_API_URL.value(),
    ...
    def test_returns_flow_run_on_timeout(
        ...
        with respx.mock(
            base_url=SYNCMATRIX_API_URL.value(), assert_all_mocked=True
        ) as router:
            router.get("/csrf-token", params={"client": mock.ANY}).pass_through()
            router.get(f"/deployments/name/{d.flow_name}/{d.name}").pass_through()
            router.post(f"/deployments/{deployment_id}/create_flow_run").pass_through()
            flow_polls = router.request(
                "GET", re.compile(SYNCMATRIX_API_URL.value() + "/flow_runs/.*")
            ).mock(
    ...
    def test_accepts_tags(self, test_deployment):
        ...
        flow_run = run_deployment(
            f"{d.flow_name}/{d.name}",
            tags=["I", "love", "syncmatrix"],
            timeout=0,
            poll_interval=0,
        )

        assert sorted(flow_run.tags) == ["I", "love", "syncmatrix"]
    ...
    async def test_links_to_parent_flow_run_when_used_in_flow_by_default(
        self, test_deployment, use_hosted_api_server, syncmatrix_client: SyncmatrixClient
    ):
        d, deployment_id = test_deployment
        ...
        task_run = await syncmatrix_client.read_task_run(child_flow_run.parent_task_run_id)
    ...
    async def test_links_to_parent_flow_run_when_used_in_task_without_flow_context(
        self, test_deployment, syncmatrix_client
    ):
        """
        Regression test for deployments in a task on Dask and Ray task runners
        which do not have access to the flow run context - https://github.com/KhulnaSoft/syncmatrix/issues/9135
        """
        d, deployment_id = test_deployment
    ...
    async def test_tracks_dependencies_when_used_in_flow(
        self, test_deployment, use_hosted_api_server, syncmatrix_client
    ):
        d, deployment_id = test_deployment
...
class TestLoadFlowFromFlowRun:
    async def test_load_flow_from_module_entrypoint(
        self, syncmatrix_client: SyncmatrixClient, monkeypatch
    ):
        @flow
        def pretend_flow():
            pass
        ...
        monkeypatch.setattr(
            "syncmatrix.deployments.deployments.load_flow_from_entrypoint",
            load_flow_from_entrypoint,
        )

        flow_id = await syncmatrix_client.create_flow_from_name(pretend_flow.__name__)
        ...
        result = await load_flow_from_flow_run(flow_run, client=syncmatrix_client)
