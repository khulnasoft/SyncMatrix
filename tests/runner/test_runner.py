...
import syncmatrix.runner
from syncmatrix import flow, serve, task
from syncmatrix.client.orchestration import SyncmatrixClient
from syncmatrix.client.schemas.objects import MinimalDeploymentSchedule, StateType
from syncmatrix.client.schemas.schedules import CronSchedule, IntervalSchedule
...
from syncmatrix.logging.loggers import flow_run_logger
from syncmatrix.runner.runner import Runner
from syncmatrix.runner.server import perform_health_check
from syncmatrix.settings import (
    SYNCMATRIX_DEFAULT_DOCKER_BUILD_NAMESPACE,
    SYNCMATRIX_DEFAULT_WORK_POOL_NAME,
    SYNCMATRIX_RUNNER_POLL_FREQUENCY,
    SYNCMATRIX_RUNNER_PROCESS_LIMIT,
    SYNCMATRIX_RUNNER_SERVER_ENABLE,
    temporary_settings,
)
from syncmatrix.testing.utilities import AsyncMock
from syncmatrix.utilities.dockerutils import parse_image_tag
...
class MockStorage:
    ...
    def to_pull_step(self):
        return {"syncmatrix.fake.module": {}}


class TestInit:
    async def test_runner_respects_limit_setting(self):
        runner = Runner()
        assert runner.limit == SYNCMATRIX_RUNNER_PROCESS_LIMIT.value()

        runner = Runner(limit=50)
        assert runner.limit == 50

        with temporary_settings({SYNCMATRIX_RUNNER_PROCESS_LIMIT: 100}):
            runner = Runner()
            assert runner.limit == 100

    async def test_runner_respects_poll_setting(self):
        runner = Runner()
        assert runner.query_seconds == SYNCMATRIX_RUNNER_POLL_FREQUENCY.value()

        runner = Runner(query_seconds=50)
        assert runner.query_seconds == 50

        with temporary_settings({SYNCMATRIX_RUNNER_POLL_FREQUENCY: 100}):
            runner = Runner()
...
class TestServe:
    ...
    async def mock_runner_start(self, monkeypatch):
        mock = AsyncMock()
        monkeypatch.setattr("syncmatrix.runner.Runner.start", mock)
        return mock
    ...
    async def test_serve_prints_help_message_on_startup(self, capsys):
        ...
        assert "$ syncmatrix deployment run [DEPLOYMENT_NAME]" in captured.out
...
class TestRunner:
    async def test_add_flows_to_runner(self, syncmatrix_client: SyncmatrixClient):
        ...
        deployment_1 = await syncmatrix_client.read_deployment(deployment_id_1)
        deployment_2 = await syncmatrix_client.read_deployment(deployment_id_2)
    ...
    async def test_add_deployments_to_runner(self, syncmatrix_client: SyncmatrixClient):
    ...
    async def test_runner_can_pause_schedules_on_stop(
        self, syncmatrix_client: SyncmatrixClient, caplog
    ):
        runner = Runner()
        ...
        deployment_1 = await syncmatrix_client.read_deployment_by_name(
            name="dummy-flow-1/test_runner"
        )
        deployment_2 = await syncmatrix_client.read_deployment_by_name(
            name="dummy-flow-2/test_runner"
        )
    ...
    async def test_runner_executes_flow_runs(self, syncmatrix_client: SyncmatrixClient):
        runner = Runner()
        ...
        await runner.start(run_once=True)
        flow_run = await syncmatrix_client.read_flow_run(flow_run_id=flow_run.id)
    ...
    async def test_runner_respects_set_limit(
        ...
        good_run = await syncmatrix_client.create_flow_run_from_deployment(
            deployment_id=deployment_id
        )
        bad_run = await syncmatrix_client.create_flow_run_from_deployment(
            deployment_id=deployment_id
        )
        ...
        flow_run = await syncmatrix_client.read_flow_run(flow_run_id=bad_run.id)
    ...
    async def test_handles_spaces_in_sys_executable(self, monkeypatch, syncmatrix_client):
        """
        Regression test for https://github.com/KhulnaSoft/syncmatrix/issues/10820
        """
        import sys
        ...
        monkeypatch.setattr(syncmatrix.runner.runner, "run_process", mock_run_process_call)
        ...
        # ["C:/Program", "Files/Python38/python.exe", "-m", "syncmatrix.engine"]
    ...
    async def test_runner_sets_flow_run_env_var_with_dashes(
        self, monkeypatch, syncmatrix_client
    ):
        """
        Regression test for https://github.com/KhulnaSoft/syncmatrix/issues/10851
        """
        env_var_value = None
        ...
        def capture_env_var(*args, **kwargs):
            ...
            env_var_value = kwargs["env"].get("SYNCMATRIX__FLOW_RUN_ID")
            return mock_process
    ...
    @pytest.mark.usefixtures("use_hosted_api_server")
    async def test_runner_runs_a_remotely_stored_flow(self, syncmatrix_client):
        runner = Runner()
    ...
    @pytest.mark.usefixtures("use_hosted_api_server")
    async def test_runner_caches_adhoc_pulls(self, syncmatrix_client):
        runner = Runner()
    ...
    @pytest.mark.usefixtures("use_hosted_api_server")
    async def test_runner_does_not_raise_on_duplicate_submission(self, syncmatrix_client):
        """
        Regression test for https://github.com/KhulnaSoft/syncmatrix/issues/11093
...
class TestRunnerDeployment:
    ...
    async def test_apply(self, syncmatrix_client: SyncmatrixClient):
        deployment = RunnerDeployment.from_flow(dummy_flow_1, __file__, interval=3600)
    ...
    async def test_apply_with_work_pool(self, syncmatrix_client: SyncmatrixClient, work_pool):
        deployment = RunnerDeployment.from_flow(
            dummy_flow_1,
            __file__,
            interval=3600,
    ...
    async def test_apply_inactive_schedule(self, syncmatrix_client: SyncmatrixClient):
        deployment = RunnerDeployment.from_flow(
...
class TestServer:
    ...
    async def test_webserver_start_flag(self, enabled: bool):
        with temporary_settings(updates={SYNCMATRIX_RUNNER_SERVER_ENABLE: enabled}):
            with mock.patch("syncmatrix.runner.runner.threading.Thread") as mocked_thread:
                runner = Runner()
                await runner.start(run_once=True)
...
class TestDeploy:
    ...
    def mock_build_image(self, monkeypatch):
        mock = MagicMock()
        monkeypatch.setattr("syncmatrix.deployments.runner.build_image", mock)
        return mock
    ...
    def mock_docker_client(self, monkeypatch):
        ...
        monkeypatch.setattr("syncmatrix.deployments.runner.docker_client", mock)
        return mock
    ...
    def mock_generate_default_dockerfile(self, monkeypatch):
        mock = MagicMock()
        monkeypatch.setattr(
            "syncmatrix.deployments.runner.generate_default_dockerfile", mock
        )
        return mock
    ...
    async def test_deploy(
        ...
        assert deployment_2.pull_steps == [{"syncmatrix.fake.module": {}}]

        console_output = capsys.readouterr().out
        assert (
            f"syncmatrix worker start --pool {work_pool_with_image_variable.name!r}"
            in console_output
        )
        assert "syncmatrix deployment run [DEPLOYMENT_NAME]" in console_output
    ...
    async def test_deploy_to_default_work_pool(
        ...
        with temporary_settings(
            updates={SYNCMATRIX_DEFAULT_WORK_POOL_NAME: work_pool_with_image_variable.name}
        ):
            deployment_ids = await deploy(
                await dummy_flow_1.to_deployment(__file__),
                await (
    ...
    async def test_deploy_skip_build(
        ...
        deployment_1 = await syncmatrix_client.read_deployment(
        ...
        deployment_2 = await syncmatrix_client.read_deployment(
    ...
    async def test_deploy_do_not_print_next_steps(
        ...
        assert "syncmatrix deployment run [DEPLOYMENT_NAME]" not in capsys.readouterr().out
    ...
    async def test_deploy_push_work_pool(
        ...
        console_output = capsys.readouterr().out
        assert "syncmatrix worker start" not in console_output
        assert "syncmatrix deployment run [DEPLOYMENT_NAME]" not in console_output
    ...
    async def test_deploy_with_module_path_entrypoint(
        self, work_pool_with_image_variable, syncmatrix_client
    ):
        deployment_ids = await deploy(
        ...
        deployment = await syncmatrix_client.read_deployment(deployment_ids[0])
    ...
    async def test_deploy_with_partial_success(
        ...
        assert "syncmatrix worker start" in console_output
...
class TestDeploymentImage:
    def test_adds_default_registry_url(self):
        with temporary_settings(
            {SYNCMATRIX_DEFAULT_DOCKER_BUILD_NAMESPACE: "alltheimages.com/my-org"}
        ):
