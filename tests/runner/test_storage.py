...
from syncmatrix.deployments.steps.core import run_step
...
def mock_run_process(monkeypatch):
    ...
    monkeypatch.setattr("syncmatrix.runner.storage.run_process", mock_run_process)
...
class TestGitRepository:
    ...
    async def test_pull_code_with_username_and_password(
        ...
        Regression test for https://github.com/KhulnaSoft/syncmatrix/issues/11051
        """
        monkeypatch.setattr("pathlib.Path.exists", lambda x: False)
    ...
    async def test_git_clone_errors_obscure_access_token(self, monkeypatch, capsys):
        ...
        with pytest.raises(RuntimeError) as exc:
            ...
            await GitRepository(
                url="https://github.com/khulnasoft/syncmatrix.git",
    ...
    class TestCredentialFormatting:
        async def test_credential_formatting_maintains_secrets(
            ...
            """Regression test for https://github.com/KhulnaSoft/syncmatrix/issues/11135"""
        ...
        def test_to_pull_step_with_block_credentials(self):
            ...
            expected_output = {
                "syncmatrix.deployments.steps.git_clone": {
                    ...
                    "branch": None,
                    "credentials": (
                        "{{ syncmatrix.blocks.mockcredentials.test-credentials }}"
                    ),
                }
            }
        ...
        def test_to_pull_step_with_secret_access_token(self):
            ...
            expected_output = {
                "syncmatrix.deployments.steps.git_clone": {
                    ...
                    "credentials": {
                        "username": "testuser",
                        "access_token": "{{ syncmatrix.blocks.secret.test-access-token }}",
                    },
                }
            }
    ...
    def test_to_pull_step(self, monkeypatch):
        ...
        assert pull_step == {
            "syncmatrix.deployments.steps.pull_from_remote_storage": {
                "requires": "s3fs",
                "url": "s3://bucket/path",
                "key": "{{ syncmatrix.blocks.secret.aws-access-key-id }}",
                "secret": "{{ syncmatrix.blocks.secret.aws-secret-access-key }}",
                "token": "{{ syncmatrix.blocks.secret.aws-session-token }}",
            }
        }
