...
from syncmatrix.settings import SYNCMATRIX_API_KEY, SYNCMATRIX_API_URL, temporary_settings
...
async def test_subscriber_can_connect_with_defaults(
    ...
    with temporary_settings(
        updates={SYNCMATRIX_API_KEY: "my-token", SYNCMATRIX_API_URL: events_api_url}
    ):
        puppeteer.token = "my-token"
        puppeteer.outgoing_events = [example_event_1, example_event_2]

        async with SyncmatrixCloudEventSubscriber() as subscriber:
...
async def test_subscriber_complains_without_api_url_and_key(
    ...
    with temporary_settings(updates={SYNCMATRIX_API_KEY: "", SYNCMATRIX_API_URL: ""}):
        with pytest.raises(ValueError, match="must be provided or set"):
            SyncmatrixCloudEventSubscriber()
...
async def test_subscriber_can_connect_and_receive_one_event(
    ...
    async with SyncmatrixCloudEventSubscriber(
        events_api_url,
        "my-token",
        filter,
