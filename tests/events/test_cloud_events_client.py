...
from syncmatrix.events.clients import SyncmatrixCloudEventsClient
from syncmatrix.testing.fixtures import Puppeteer, Recorder


async def test_cloud_client_can_connect_and_emit(
    events_api_url: str, example_event_1: Event, recorder: Recorder
):
    async with SyncmatrixCloudEventsClient(events_api_url, "my-token") as client:
...
async def test_reconnects_and_resends_after_hard_disconnect(
    ...
    client = SyncmatrixCloudEventsClient(events_api_url, "my-token", checkpoint_every=1)
    async with client:
        assert recorder.connections == 1
...
async def test_gives_up_after_a_certain_amount_of_tries(
    ...
    client = SyncmatrixCloudEventsClient(
...
async def test_handles_api_url_with_trailing_slash(
    ...
    # Regression test for https://github.com/KhulnaSoft/syncmatrix/issues/9662
    # where a configuration that has a trailing slash on the SYNCMATRIX_API_URL
    # would cause the client to fail to connect with a 403 as the path would
    # contain a double slash, ie: `wss:api.syncmatrix.cloud/...//events/in`
...
async def test_recovers_from_temporary_error_reconnecting(
    ...
    SyncmatrixCloudEventsClient._emit because the websocket was None"""
    client = SyncmatrixCloudEventsClient(
