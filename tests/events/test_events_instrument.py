...
class InstrumentedClass:
    ...
    def _event_kind(self):
        return "syncmatrix.instrumented"

    def _event_method_called_resources(self) -> ResourceTuple:
        return (
            {
                "syncmatrix.resource.id": "syncmatrix.instrumented-class",
            },
            [
                {
                    "syncmatrix.resource.id": "syncmatrix.class.InstrumentedClass",
                    "syncmatrix.resource.role": "class",
                }
            ],
        )
...
async def test_instruments_methods(
    ...
    assert sync_event.event == "syncmatrix.instrumented.sync_method.called"
    assert sync_event.resource.id == "syncmatrix.instrumented-class"
    assert sync_event.related[0].id == "syncmatrix.class.InstrumentedClass"
    assert sync_event.related[0].role == "class"

    async_event = asserting_events_worker._client.events[1]
    assert async_event.event == "syncmatrix.instrumented.async_method.called"
    assert async_event.resource.id == "syncmatrix.instrumented-class"
    assert async_event.related[0].id == "syncmatrix.class.InstrumentedClass"
...
async def test_handles_method_failure(
    ...
    sync_event = asserting_events_worker._client.events[0]
    assert sync_event.event == "syncmatrix.instrumented.sync_fail.failed"
    assert sync_event.resource.id == "syncmatrix.instrumented-class"
    assert sync_event.related[0].id == "syncmatrix.class.InstrumentedClass"
    assert sync_event.related[0].role == "class"

    async_event = asserting_events_worker._client.events[1]
    assert async_event.event == "syncmatrix.instrumented.async_fail.failed"
    assert async_event.resource.id == "syncmatrix.instrumented-class"
    assert async_event.related[0].id == "syncmatrix.class.InstrumentedClass"
    assert async_event.related[0].role == "class"
...
async def test_instrument_idempotent(
    ...
    class AClass:
        def _event_kind(self):
            return "syncmatrix.a-class"

        def _event_method_called_resources(self) -> ResourceTuple:
            return ({"syncmatrix.resource.id": "some-id"}, [])
