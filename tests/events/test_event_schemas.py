...
from syncmatrix.events.actions import RunDeployment
...
def test_client_events_generate_an_id_by_default():
    event1 = Event(event="hello", resource={"syncmatrix.resource.id": "hello"})
    event2 = Event(event="hello", resource={"syncmatrix.resource.id": "hello"})
...
def test_client_events_generate_occurred_by_default(start_of_test: DateTime):
    event = Event(event="hello", resource={"syncmatrix.resource.id": "hello"})
...
def test_client_events_may_have_empty_related_resources():
    event = Event(
        ...
        event="hello",
        resource={"syncmatrix.resource.id": "hello"},
        id=uuid4(),
    )
...
def test_client_event_resources_have_correct_types():
    event = Event(
        ...
        resource={"syncmatrix.resource.id": "hello"},
        related=[
            {"syncmatrix.resource.id": "related-1", "syncmatrix.resource.role": "role-1"},
        ],
        id=uuid4(),
    )
...
def test_client_events_may_have_multiple_related_resources():
    event = Event(
        ...
        related=[
            {"syncmatrix.resource.id": "related-1", "syncmatrix.resource.role": "role-1"},
            {"syncmatrix.resource.id": "related-2", "syncmatrix.resource.role": "role-1"},
            {"syncmatrix.resource.id": "related-3", "syncmatrix.resource.role": "role-2"},
        ],
        id=uuid4(),
    )
...
def test_limit_on_labels(monkeypatch: pytest.MonkeyPatch):
    labels = {"syncmatrix.resource.id": "the.thing"}
    labels.update({str(i): str(i) for i in range(10)})

    monkeypatch.setattr("syncmatrix.events.schemas.MAXIMUM_LABELS_PER_RESOURCE", 10)
...
def test_limit_on_related_resources(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr("syncmatrix.events.schemas.MAXIMUM_RELATED_RESOURCES", 10)
    with pytest.raises(ValidationError, match="maximum number of related"):
        Event(
            occurred=pendulum.now("UTC"),
            event="anything",
            resource={"syncmatrix.resource.id": "the.thing"},
            related=[
                {
                    "syncmatrix.resource.id": f"another.thing.{i}",
                    "syncmatrix.resource.role": "related",
                }
                for i in range(11)
            ],
            id=uuid4(),
...
def test_deployment_trigger_as_automation():
    ...
    assert automation == Automation(
        ...
        owner_resource=f"syncmatrix.deployment.{trigger._deployment_id}",
    )
