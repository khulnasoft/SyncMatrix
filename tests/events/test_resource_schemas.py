...
from syncmatrix.events import RelatedResource, Resource
...
def test_resource_requires_resource_id(resource_class: Type[Resource]) -> None:
    with pytest.raises(ValidationError) as error:
        resource_class(
            __root__={
                "syncmatrix.resource.role": "any-role",
            }
        )
    ...
    assert error.value.errors() == [
        {
            "loc": ("__root__",),
            "msg": "Resources must include the syncmatrix.resource.id label",
            "type": "value_error",
        }
    ]
...
def test_related_resources_require_role() -> None:
    with pytest.raises(ValidationError) as error:
        RelatedResource(
            __root__={
                "syncmatrix.resource.id": "my.unique.resource",
            }
        )
    ...
    assert error.value.errors() == [
        {
            "loc": ("__root__",),
            "msg": "Related Resources must include the syncmatrix.resource.role label",
            "type": "value_error",
        },
    ]
...
def test_related_resources_require_non_empty_role() -> None:
    ...
    assert error.value.errors() == [
        {
            "loc": ("__root__",),
            "msg": "The syncmatrix.resource.role label must be non-empty",
            "type": "value_error",
        },
    ]
...
def test_resource_requires_non_empty_resource_id(
    ...
    assert error.value.errors() == [
        {
            "loc": ("__root__",),
            "msg": "The syncmatrix.resource.id label must be non-empty",
            "type": "value_error",
        }
    ]
