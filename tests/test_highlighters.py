...
from syncmatrix.logging.highlighters import (
    LevelHighlighter,
    NameHighlighter,
    SyncmatrixConsoleHighlighter,
    StateHighlighter,
    UrlHighlighter,
)
...
def test_highlight_url(url_kind):
    url = "https://www.khulnasoft.com/" if url_kind == "web" else "file://tests.py"
...
def test_highlight_console():
    ...
    highlighter = SyncmatrixConsoleHighlighter()
    highlighter.highlight(text)
    assert text.spans == [
