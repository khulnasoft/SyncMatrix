...
from syncmatrix.serializers import (
    ...
    PickleSerializer,
    Serializer,
    syncmatrix_json_object_decoder,
    syncmatrix_json_object_encoder,
)
...
class TestJSONSerializer:
    ...
    def test_uses_alternative_json_library(self, monkeypatch):
        ...
        serializer.dumps("test")
        serializer.loads(b"test")
        dumps_mock.assert_called_once_with("test", default=syncmatrix_json_object_encoder)
        loads_mock.assert_called_once_with(
            "test", object_hook=syncmatrix_json_object_decoder
        )

    def test_allows_custom_encoder(self, monkeypatch):
        fake_object_encoder = MagicMock(return_value="foobar!")
        syncmatrix_object_encoder = MagicMock()

        monkeypatch.setattr(
            "syncmatrix.fake_object_encoder", fake_object_encoder, raising=False
        )
        monkeypatch.setattr(
            "syncmatrix.serializers.syncmatrix_json_object_encoder",
            syncmatrix_object_encoder,
        )

        serializer = JSONSerializer(object_encoder="syncmatrix.fake_object_encoder")

        # Encoder hooks are only called for unsupported objects
        obj = uuid.uuid4()
        result = serializer.dumps(obj)
        assert result == b'"foobar!"'
        syncmatrix_object_encoder.assert_not_called()
    ...
    def test_allows_custom_decoder(self, monkeypatch):
        ...
        syncmatrix_object_decoder = MagicMock()

        monkeypatch.setattr(
            "syncmatrix.fake_object_decoder", fake_object_decoder, raising=False
        )

        monkeypatch.setattr(
            "syncmatrix.serializers.syncmatrix_json_object_decoder",
            syncmatrix_object_decoder,
        )

        serializer = JSONSerializer(object_decoder="syncmatrix.fake_object_decoder")
        ...
        syncmatrix_object_decoder.assert_not_called()
    ...
    def test_allows_custom_kwargs(self, monkeypatch):
        ...
        dumps_mock.assert_called_once_with(
            "test", default=syncmatrix_json_object_encoder, foo="bar"
        )
        loads_mock.assert_called_once_with(
            "test", object_hook=syncmatrix_json_object_decoder, bar="foo"
        )
