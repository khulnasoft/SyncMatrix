...
import syncmatrix.results
from syncmatrix import Task, task, unmapped
...
from syncmatrix.server.api.task_runs import TaskQueue
from syncmatrix.server.services.task_scheduling import TaskSchedulingTimeouts
from syncmatrix.settings import (
    ...
    SYNCMATRIX_TASK_SCHEDULING_PENDING_TASK_TIMEOUT,
    temporary_settings,
)
...
def local_filesystem():
    block = LocalFileSystem(basepath="~/.syncmatrix/storage/test")
...
def allow_experimental_task_scheduling():
    with temporary_settings(
        {
            SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING: True,
        }
    ):
        yield
...
async def clear_cached_filesystems():
    syncmatrix.results._default_task_scheduling_storages.clear()
    yield
    syncmatrix.results._default_task_scheduling_storages.clear()
...
def test_task_submission_fails_when_experimental_flag_off(foo_task):
    with temporary_settings({SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING: False}):
...
def test_task_submission_with_parameters_reuses_default_storage_block(
    ...
    with temporary_settings(
        {
            SYNCMATRIX_TASK_SCHEDULING_DEFAULT_STORAGE_BLOCK: "local-file-system/my-tasks",
            SYNCMATRIX_LOCAL_STORAGE_PATH: tmp_path / "some-storage",
        }
    ):
...
@pytest.fixture
async def syncmatrix_client() -> AsyncGenerator[SyncmatrixClient, None]:
    async with get_client() as client:
        yield client


async def test_scheduled_tasks_are_restored_at_server_startup(
    foo_task_with_result_storage: Task, syncmatrix_client: SyncmatrixClient
):
    ...
    # server pulls a task, then the syncmatrix server dies
    enqueued: TaskRun = await TaskQueue.for_key(task_run.task_key).get()
    ...
    # now emulate that we've restarted the Syncmatrix server by resetting the
    # TaskSchedulingTimeouts service
    service = TaskSchedulingTimeouts()
    await service.start(loops=1)

    # the task will still be SCHEDULED
    task_run = await syncmatrix_client.read_task_run(task_run.id)
...
async def test_stuck_pending_tasks_are_reenqueued(
    ...
    with pytest.raises(ValueError):
        with mock.patch(
            "syncmatrix.task_server.submit_autonomous_task_run_to_engine",
    ...
    with temporary_settings({SYNCMATRIX_TASK_SCHEDULING_PENDING_TASK_TIMEOUT: 1000000}):
    ...
    with temporary_settings({SYNCMATRIX_TASK_SCHEDULING_PENDING_TASK_TIMEOUT: 0.1}):
