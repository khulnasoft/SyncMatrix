...
import syncmatrix
from syncmatrix.plugins import load_extra_entrypoints
from syncmatrix.settings import SYNCMATRIX_EXTRA_ENTRYPOINTS, temporary_settings
...
def test_load_extra_entrypoints_imports_module():
    with temporary_settings({SYNCMATRIX_EXTRA_ENTRYPOINTS: "test_module_name"}):
...
def test_load_extra_entrypoints_unparsable_entrypoint(capsys):
    with temporary_settings({SYNCMATRIX_EXTRA_ENTRYPOINTS: "foo$bar"}):
        result = load_extra_entrypoints()
...
def test_load_extra_entrypoints_callable():
    with temporary_settings(
        {SYNCMATRIX_EXTRA_ENTRYPOINTS: "test_module_name:returns_test"}
    ):
        result = load_extra_entrypoints()
...
def test_load_extra_entrypoints_callable_given_no_arguments():
    with temporary_settings(
        {SYNCMATRIX_EXTRA_ENTRYPOINTS: "test_module_name:mock_function"}
    ):
        result = load_extra_entrypoints()
...
def test_load_extra_entrypoints_callable_that_raises(capsys):
    with temporary_settings(
        {SYNCMATRIX_EXTRA_ENTRYPOINTS: "test_module_name:raises_value_error"}
    ):
        result = load_extra_entrypoints()
...
def test_load_extra_entrypoints_callable_that_raises_base_exception():
    with temporary_settings(
        {SYNCMATRIX_EXTRA_ENTRYPOINTS: "test_module_name:raises_base_exception"}
    ):
...
def test_load_extra_entrypoints_error_on_import(capsys):
    with temporary_settings({SYNCMATRIX_EXTRA_ENTRYPOINTS: "raising_module_name"}):
        result = load_extra_entrypoints()
...
def test_load_extra_entrypoints_missing_module(capsys):
    with temporary_settings({SYNCMATRIX_EXTRA_ENTRYPOINTS: "nonexistant_module"}):
        result = load_extra_entrypoints()
...
def test_load_extra_entrypoints_missing_submodule(capsys):
    with temporary_settings(
        {SYNCMATRIX_EXTRA_ENTRYPOINTS: "test_module_name.missing_module"}
    ):
        result = load_extra_entrypoints()
...
def test_load_extra_entrypoints_missing_attribute(capsys):
    with temporary_settings(
        {SYNCMATRIX_EXTRA_ENTRYPOINTS: "test_module_name:missing_attr"}
    ):
        result = load_extra_entrypoints()
...
def test_plugin_load_on_syncmatrix_package_init(monkeypatch):
    ...
    monkeypatch.setattr(
        syncmatrix.plugins, "load_extra_entrypoints", mock_load_extra_entrypoints
    )

    importlib.reload(syncmatrix)

    mock_load_syncmatrix_collections.assert_not_called()  # We no longer load collections on import due to high performance cost
