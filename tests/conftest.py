"""
Base configuration of pytest for testing the 'syncmatrix' module.
...
- Inject a check for open Syncmatrix client lifespans after every test call
- Create a test Syncmatrix settings profile before test collection that will be used
  for the duration of the test run. This ensures tests are run in a temporary
  environment.

WARNING: Syncmatrix settings cannot be modified in async fixtures.
...
pytest.register_assert_rewrite("syncmatrix.testing.utilities")

import syncmatrix
import syncmatrix.settings
from syncmatrix.logging.configuration import setup_logging
from syncmatrix.settings import (
    SYNCMATRIX_API_BLOCKS_REGISTER_ON_START,
    SYNCMATRIX_API_DATABASE_CONNECTION_URL,
    SYNCMATRIX_API_SERVICES_CANCELLATION_CLEANUP_ENABLED,
    SYNCMATRIX_API_SERVICES_FLOW_RUN_NOTIFICATIONS_ENABLED,
    SYNCMATRIX_API_SERVICES_LATE_RUNS_ENABLED,
    SYNCMATRIX_API_SERVICES_PAUSE_EXPIRATIONS_ENABLED,
    SYNCMATRIX_API_SERVICES_SCHEDULER_ENABLED,
    SYNCMATRIX_API_URL,
    SYNCMATRIX_ASYNC_FETCH_STATE_RESULT,
    SYNCMATRIX_CLI_COLORS,
    SYNCMATRIX_CLI_WRAP_LINES,
    SYNCMATRIX_EXPERIMENTAL_ENABLE_ENHANCED_CANCELLATION,
    SYNCMATRIX_EXPERIMENTAL_ENABLE_WORKERS,
    SYNCMATRIX_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION,
    SYNCMATRIX_EXPERIMENTAL_WARN_WORKERS,
    SYNCMATRIX_HOME,
    SYNCMATRIX_LOCAL_STORAGE_PATH,
    SYNCMATRIX_LOGGING_INTERNAL_LEVEL,
    ...
    SYNCMATRIX_SERVER_ANALYTICS_ENABLED,
...
from syncmatrix.testing.cli import *
from syncmatrix.testing.fixtures import *
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        default=False,
        help=(
            "Do not build the syncmatrix Docker image during tests.  Tests that require "
            "the image will fail if the image is not present or has an outdated "
            "version of `syncmatrix` installed.  Used during CI to run the test suite "
...
def event_loop(request):
    ...
    if sys.version_info < (3, 8) and sys.platform != "win32":
        from syncmatrix.utilities.compat import ThreadedChildWatcher
...
def assert_lifespan_is_not_left_open():
    ...
    # across tests.
    from syncmatrix.client import APP_LIFESPANS
...
def pytest_sessionstart(session):
    ...
    """
    global TEST_SYNCMATRIX_HOME, TEST_PROFILE_CTX
    TEST_SYNCMATRIX_HOME = tempfile.mkdtemp()

    profile = syncmatrix.settings.Profile(
        name="test-session",
        settings={
            # Set SYNCMATRIX_HOME to a temporary directory to avoid clobbering
            # environments and settings
            SYNCMATRIX_HOME: TEST_SYNCMATRIX_HOME,
            SYNCMATRIX_LOCAL_STORAGE_PATH: Path(TEST_SYNCMATRIX_HOME) / "storage",
            SYNCMATRIX_PROFILES_PATH: "$SYNCMATRIX_HOME/profiles.toml",
            # Disable connection to an API
            SYNCMATRIX_API_URL: None,
            # Disable pretty CLI output for easier assertions
            SYNCMATRIX_CLI_COLORS: False,
            SYNCMATRIX_CLI_WRAP_LINES: False,
            # Enable future change
            SYNCMATRIX_ASYNC_FETCH_STATE_RESULT: True,
            # Enable debug logging
            SYNCMATRIX_LOGGING_LEVEL: "DEBUG",
            SYNCMATRIX_LOGGING_INTERNAL_LEVEL: "DEBUG",
            # Disable shipping logs to the API;
            # can be enabled by the `enable_api_log_handler` mark
            SYNCMATRIX_LOGGING_TO_API_ENABLED: False,
            # Disable services for test runs
            SYNCMATRIX_SERVER_ANALYTICS_ENABLED: False,
            SYNCMATRIX_API_SERVICES_LATE_RUNS_ENABLED: False,
            SYNCMATRIX_API_SERVICES_SCHEDULER_ENABLED: False,
            SYNCMATRIX_API_SERVICES_FLOW_RUN_NOTIFICATIONS_ENABLED: False,
            SYNCMATRIX_API_SERVICES_PAUSE_EXPIRATIONS_ENABLED: False,
            SYNCMATRIX_API_SERVICES_CANCELLATION_CLEANUP_ENABLED: False,
            # Disable block auto-registration memoization
            SYNCMATRIX_MEMOIZE_BLOCK_AUTO_REGISTRATION: False,
            # Disable auto-registration of block types as they can conflict
            SYNCMATRIX_API_BLOCKS_REGISTER_ON_START: False,
            # Code is being executed in a unit test context
            SYNCMATRIX_UNIT_TEST_MODE: True,
        },
        source=__file__,
    )

    TEST_PROFILE_CTX = syncmatrix.context.use_profile(
        profile,
    ...
    SYNCMATRIX_LOCAL_STORAGE_PATH.value().mkdir()
...
def pytest_sessionfinish(session):
    ...
    if TEST_SYNCMATRIX_HOME is not None:
        shutil.rmtree(TEST_SYNCMATRIX_HOME)
...
async def generate_test_database_connection_url(
    ...
    original_url = SYNCMATRIX_API_DATABASE_CONNECTION_URL.value()
    if not original_url:
        yield None
        return
    ...
    if scheme == "sqlite+aiosqlite":
        # SQLite databases will be scoped by the SYNCMATRIX_HOME setting, which will
...
def test_database_connection_url(generate_test_database_connection_url):
    ...
    if url is None:
        ...
        with temporary_settings({SYNCMATRIX_API_DATABASE_CONNECTION_URL: url}):
            yield url
...
def caplog(caplog):
    ...
    """
    from syncmatrix.logging.configuration import PROCESS_LOGGING_CONFIG
...
def enable_workers():
    with temporary_settings(
        {SYNCMATRIX_EXPERIMENTAL_ENABLE_WORKERS: 1, SYNCMATRIX_EXPERIMENTAL_WARN_WORKERS: 0}
    ):
        yield
...
def disable_workers():
    with temporary_settings(
        {SYNCMATRIX_EXPERIMENTAL_ENABLE_WORKERS: 0, SYNCMATRIX_EXPERIMENTAL_WARN_WORKERS: 1}
    ):
        yield
...
def enable_enhanced_cancellation():
    with temporary_settings(
        {
            SYNCMATRIX_EXPERIMENTAL_ENABLE_ENHANCED_CANCELLATION: 1,
            SYNCMATRIX_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION: 0,
        }
    ):
        yield
...
def disable_enhanced_cancellation():
    with temporary_settings(
        {
            SYNCMATRIX_EXPERIMENTAL_ENABLE_ENHANCED_CANCELLATION: 0,
            SYNCMATRIX_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION: 1,
        }
    ):
        yield
