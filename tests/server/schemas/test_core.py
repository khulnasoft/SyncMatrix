...
from syncmatrix.settings import SYNCMATRIX_API_TASK_CACHE_KEY_MAX_LENGTH, temporary_settings
...
class TestFlowRunPolicy:
    class OldFlowRunPolicy(SyncmatrixBaseModel):
...
class TestTaskRunPolicy:
    class OldTaskRunPolicy(SyncmatrixBaseModel):
        # Schemas ignore extras during normal execution, but raise errors during tests if not explicitly ignored.
        class Config:
            extra = "ignore"
...
class TestTaskRun:
    def test_task_run_cache_key_greater_than_user_configured_max_length(self):
        with temporary_settings({SYNCMATRIX_API_TASK_CACHE_KEY_MAX_LENGTH: 5}):
    ...
    def test_task_run_cache_key_within_user_configured_max_length(self):
        with temporary_settings({SYNCMATRIX_API_TASK_CACHE_KEY_MAX_LENGTH: 1000}):
