...
from syncmatrix.server.schemas.schedules import (
    MAX_ITERATIONS,
    MAX_RRULE_LENGTH,
    CronSchedule,
    IntervalSchedule,
...
class TestIntervalSchedule:
    ...
    async def test_get_dates_from_offset_naive_anchor(self):
        # Regression test for https://github.com/KhulnaSoft/orion/issues/2466
        clock = IntervalSchedule(
            interval=timedelta(days=1),
...
class TestCronScheduleDaylightSavingsTime:
    ...
    async def test_cron_schedule_handles_scheduling_near_dst_boundary(self):
        """
        Regression test for  https://github.com/KhulnaSoft/nebula/issues/4048
...
async def test_unanchored_rrule_schedules_are_idempotent(
    ...
    """Regression test for an issue discovered in Syncmatrix Cloud, where a schedule with
