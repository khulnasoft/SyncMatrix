...
import syncmatrix
from syncmatrix.server.services.telemetry import Telemetry
...
async def test_sens_o_matic_called_correctly(sens_o_matic_mock):
    ...
    assert request.headers["x-syncmatrix-event"] == "syncmatrix_server"

    heartbeat = json.loads(request.content.decode("utf-8"))
    assert heartbeat["type"] == "heartbeat"
    assert heartbeat["source"] == "syncmatrix_server"
    ...
    assert payload["syncmatrix_version"] == syncmatrix.__version__
...
async def test_errors_shutdown_service(error_sens_o_matic_mock, caplog):
    ...
    records = [
        ...
        if record.name == "syncmatrix.server.services.telemetry"
