...
from syncmatrix import states
from syncmatrix.server import models, schemas
from syncmatrix.server.services.scheduler import RecentDeploymentsScheduler, Scheduler
...
async def test_create_schedules_from_multiple_deployments_in_batches(flow, session):
    ...
    deployments_to_schedule = (
        ...
        // SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_RUNS.value()
    ) + 1
    ...
    assert (
        len(runs)
        == deployments_to_schedule * SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_RUNS.value()
    )
    assert len(runs) > SYNCMATRIX_API_SERVICES_SCHEDULER_INSERT_BATCH_SIZE.value()
