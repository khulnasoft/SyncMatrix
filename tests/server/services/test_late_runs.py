...
from syncmatrix.server.services.late_runs import MarkLateRuns
...
async def test_marks_late_run_at_buffer(session, late_run):
    ...
    with temporary_settings(updates={SYNCMATRIX_API_SERVICES_LATE_RUNS_AFTER_SECONDS: 60}):
        await MarkLateRuns(handle_signals=False).start(loops=1)
...
async def test_does_not_mark_run_late_if_within_buffer(session, late_run):
    ...
    with temporary_settings(updates={SYNCMATRIX_API_SERVICES_LATE_RUNS_AFTER_SECONDS: 61}):
        await MarkLateRuns(handle_signals=False).start(loops=1)
