...
from syncmatrix.server.utilities.schemas import (
    DateTimeTZ,
    FieldFrom,
    IDBaseModel,
    ORMBaseModel,
...
@contextmanager
def reload_syncmatrix_base_model(
    test_mode_value,
) -> Generator[Type[SyncmatrixBaseModel], None, None]:
    import syncmatrix.server.utilities.schemas.bases

    original_base_model = syncmatrix.server.utilities.schemas.bases.SyncmatrixBaseModel
    original_environment = os.environ.get("SYNCMATRIX_TEST_MODE")
    if test_mode_value is not None:
        os.environ["SYNCMATRIX_TEST_MODE"] = test_mode_value
    else:
        os.environ.pop("SYNCMATRIX_TEST_MODE")

    try:
        # We must re-execute the module since the setting is configured at base model
        # definition time
        importlib.reload(syncmatrix.server.utilities.schemas.bases)

        from syncmatrix.server.utilities.schemas.bases import SyncmatrixBaseModel

        yield SyncmatrixBaseModel
    finally:
        if original_environment is None:
            os.environ.pop("SYNCMATRIX_TEST_MODE")
        else:
            os.environ["SYNCMATRIX_TEST_MODE"] = original_environment

        # We must restore this type or `isinstance` checks will fail later
        syncmatrix.server.utilities.schemas.bases.SyncmatrixBaseModel = original_base_model


class TestExtraForbidden:
    def test_extra_attributes_are_forbidden_during_unit_tests(self):
        class Model(SyncmatrixBaseModel):
            x: int
    ...
    def test_extra_attributes_are_allowed_outside_test_mode(self, falsey_value):
        with reload_syncmatrix_base_model(falsey_value) as SyncmatrixBaseModel:
    ...
    def test_extra_attributes_are_not_allowed_with_truthy_test_mode(self, truthy_value):
        with reload_syncmatrix_base_model(truthy_value) as SyncmatrixBaseModel:
...
class TestNestedDict:
    ...
    def nested(self):
        ...
        class Parent(SyncmatrixBaseModel):
            x: int
            y: Child
...
class TestCopyModelFields:
    class MyModel(SyncmatrixBaseModel):
    ...
    async def test_validators_are_retained(self):
        @copy_model_fields
        class MyValidatedModel(SyncmatrixBaseModel):
    ...
    async def test_allows_type_to_be_optional(self):
        @copy_model_fields
        class MyOptionalTypeModel(SyncmatrixBaseModel):
