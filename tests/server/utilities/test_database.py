...
from syncmatrix.server.database.configurations import AioSqliteConfiguration
from syncmatrix.server.database.interface import SyncmatrixDBInterface
from syncmatrix.server.database.orm_models import AioSqliteORMConfiguration
from syncmatrix.server.database.query_components import AioSqliteQueryComponents
...
async def test_error_thrown_if_sqlite_version_is_below_minimum(monkeypatch):
    ...
    with pytest.raises(
        RuntimeError,
        match="Syncmatrix requires sqlite >= 3.24.0 but we found version 3.23.9",
    ):
