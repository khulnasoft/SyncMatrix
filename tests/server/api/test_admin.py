...
async def test_version(client):
    ...
    assert syncmatrix.__version__
    assert response.json() == syncmatrix.__version__
...
class TestSettings:
    async def test_read_settings(self, client):
        ...
        assert response.status_code == status.HTTP_200_OK
        parsed_settings = syncmatrix.settings.Settings.parse_obj(response.json())
        syncmatrix_settings = (
            syncmatrix.settings.get_current_settings().with_obfuscated_secrets()
        )

        assert parsed_settings == syncmatrix_settings
