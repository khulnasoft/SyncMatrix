...
from syncmatrix.server.schemas.actions import BlockTypeCreate, BlockTypeUpdate
from syncmatrix.server.schemas.core import BlockDocument, BlockType
...
CODE_EXAMPLE = dedent(
    """\
        ```python
        from syncmatrix_collection import CoolBlock
...
class TestCreateBlockType:
    ...
    @pytest.mark.parametrize(
        "name", ["SyncmatrixBlockType", "Syncmatrix", "syncmatrix_block_type", "sYnCmAtRix!"]
    )
    async def test_create_block_type_with_reserved_name_fails(self, client, name):
        ...
        assert (
            ...
            == "Block type names beginning with 'Syncmatrix' are reserved."
        )
...
class TestUpdateBlockType:
    ...
    async def test_update_block_type_only_if_different(
        ...
        mock = AsyncMock()
        monkeypatch.setattr("syncmatrix.server.models.block_types.update_block_type", mock)
...
class TestSystemBlockTypes:
    ...
    async def test_create_system_block_type(self, client, session):
        ...
        block = syncmatrix.blocks.system.DateTime(value="2022-01-01T00:00:00+00:00")
        response = await client.post(
            "/block_documents/",
        ...
        api_block = await syncmatrix.blocks.system.DateTime.load("my-test-date-time")
