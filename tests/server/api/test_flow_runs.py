...
from syncmatrix.input import RunInput, keyset_from_paused_state
from syncmatrix.server import models, schemas
from syncmatrix.server.schemas import actions, core, responses, states
...
class TestReadFlowRun:
    ...
    async def test_read_flow_run_like_the_engine_does(self, flow, flow_run, client):
        """Regression test for the hex format of UUIDs in `SYNCMATRIX__FLOW_RUN_ID`
    ...
    async def test_read_flow_run_with_invalid_id_is_rejected(self, client):
        ...
        with mock.patch("syncmatrix.server.models.flow_runs.read_flow_run") as mock_read:
...
class TestResumeFlowrun:
    ...
    async def test_resume_flow_run_waiting_for_input_with_hydrated_input(
        ...
        response = await client.post(
            f"/flow_runs/{paused_flow_run_waiting_for_input_with_default.id}/resume",
            json={
                "run_input": {"how_many": {"__syncmatrix_kind": "json", "value": '"3"'}}
            },
        )
        assert response.status_code == 201
        assert response.json()["status"] == "ACCEPT"
