...
from syncmatrix.server.exceptions import FlowRunGraphTooLarge, ObjectNotFoundError
from syncmatrix.server.models.flow_runs import read_flow_run_graph
from syncmatrix.server.schemas.graph import Edge, Graph, GraphArtifact, GraphState, Node
...
from syncmatrix.settings import (
    ...
    SYNCMATRIX_EXPERIMENTAL_ENABLE_ARTIFACTS_ON_FLOW_RUN_GRAPH,
    SYNCMATRIX_EXPERIMENTAL_ENABLE_STATES_ON_FLOW_RUN_GRAPH,
    temporary_settings,
)
...
def enable_artifacts_on_flow_run_graph():
    with temporary_settings(
        {SYNCMATRIX_EXPERIMENTAL_ENABLE_ARTIFACTS_ON_FLOW_RUN_GRAPH: True}
    ):
        yield
...
def disable_artifacts_on_flow_run_graph():
    with temporary_settings(
        {SYNCMATRIX_EXPERIMENTAL_ENABLE_ARTIFACTS_ON_FLOW_RUN_GRAPH: False}
    ):
        yield
...
async def test_artifacts_on_flow_run_graph_limited_by_setting(
    ...
    with temporary_settings(
        {SYNCMATRIX_API_MAX_FLOW_RUN_GRAPH_ARTIFACTS: test_max_artifacts_setting}
    ):
        graph = await read_flow_run_graph(
            session=session,
            flow_run_id=flow_run.id,
...
def enable_states_on_flow_run_graph():
    with temporary_settings(
        {SYNCMATRIX_EXPERIMENTAL_ENABLE_STATES_ON_FLOW_RUN_GRAPH: True}
    ):
        yield
...
def disable_states_on_flow_run_graph():
    with temporary_settings(
        {SYNCMATRIX_EXPERIMENTAL_ENABLE_STATES_ON_FLOW_RUN_GRAPH: False}
    ):
        yield
...
def model_method_mock(graph: Graph, monkeypatch: pytest.MonkeyPatch) -> AsyncMock:
    ...
    monkeypatch.setattr("syncmatrix.server.api.flow_runs.read_flow_run_graph", mock)
    return mock
...
async def test_reading_graph_for_flow_run_with_linked_tasks_too_many_nodes(
    ...
    with temporary_settings(
        updates={SYNCMATRIX_API_MAX_FLOW_RUN_GRAPH_NODES: 4},
    ):
