...
from syncmatrix.client.schemas.objects import WorkPool, WorkQueue
...
class TestWorkPoolStatus:
    ...
    async def test_work_pool_status_for_syncmatrix_agent_work_pool(
        self, client, syncmatrix_agent_work_pool
    ):
        """Work pools that are Syncmatrix Agent work pools should have `null` for status."""
        response = await client.get(f"/work_pools/{syncmatrix_agent_work_pool.name}")
        assert response.status_code == status.HTTP_200_OK
        ...
        for wq in [wq_aa, wq_ab, wq_ac, wq_ba, wq_bb, wq_bc, wq_ca, wq_cb, wq_cc]:
            ...
            await models.flow_runs.create_flow_run(
                ...
                flow_run=schemas.core.FlowRun(
                    flow_id=flow.id,
                    state=syncmatrix.states.Running(),
                    work_queue_id=wq.id,
                ),
            )
            ...
            for i in range(3, -2, -1):
                await models.flow_runs.create_flow_run(
                    ...
                    flow_run=schemas.core.FlowRun(
                        flow_id=flow.id,
                        state=syncmatrix.states.Scheduled(
