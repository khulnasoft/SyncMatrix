...
from syncmatrix._vendor.fastapi import APIRouter, status, testclient

from syncmatrix.client.constants import SERVER_API_VERSION
from syncmatrix.server.api.server import (
    API_ROUTERS,
    SQLITE_LOCKED_MSG,
...
class TestMemoizeBlockAutoRegistration:
    ...
    def enable_memoization(self, tmp_path):
        with temporary_settings(
            {
                SYNCMATRIX_MEMOIZE_BLOCK_AUTO_REGISTRATION: True,
                SYNCMATRIX_MEMO_STORE_PATH: tmp_path / "memo_store.toml",
            }
        ):
            yield
    ...
    def memo_store_with_mismatched_key(self):
        SYNCMATRIX_MEMO_STORE_PATH.value().write_text(
    ...
    async def test_runs_wrapped_function_on_missing_key(
        ...
        assert not SYNCMATRIX_MEMO_STORE_PATH.value().exists()
        assert (
            SYNCMATRIX_MEMOIZE_BLOCK_AUTO_REGISTRATION.value()
        ...
        with patch("syncmatrix.server.api.server.hash_objects") as mock:
            mock.return_value = current_block_registry_hash
            await _memoize_block_auto_registration(test_func)()

        test_func.assert_called_once()

        assert SYNCMATRIX_MEMO_STORE_PATH.value().exists(), "Memo store was not created"
        assert (
            toml.load(SYNCMATRIX_MEMO_STORE_PATH.value()).get("block_auto_registration")
    ...
    async def test_does_not_fail_on_read_only_filesystem(self, enable_memoization):
        try:
            SYNCMATRIX_MEMO_STORE_PATH.value().parent.chmod(744)
            ...
            SYNCMATRIX_MEMO_STORE_PATH.value().parent.chmod(777)
    ...
    async def test_changing_database_breaks_cache(self, enable_memoization):
        ...
        with temporary_settings(
            {
                SYNCMATRIX_API_DATABASE_CONNECTION_URL: "something else",
            }
        ):
            await _memoize_block_auto_registration(test_func)()
