...
from syncmatrix.settings import SYNCMATRIX_SERVER_CSRF_PROTECTION_ENABLED, temporary_settings
...
async def test_deletes_expired_tokens(
    db: SyncmatrixDBInterface, session: AsyncSession, client: AsyncClient
):
    # Create some tokens
    for i in range(5):
