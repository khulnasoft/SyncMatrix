...
from syncmatrix.server.models import concurrency_limits, flow_runs
from syncmatrix.server.orchestration.core_policy import (
    AddUnknownResult,
...
from syncmatrix.server.schemas import actions, states
from syncmatrix.server.schemas.responses import SetStateStatus
...
class TestFlowRetryingRule:
    async def test_retries(
        ...
        monkeypatch.setattr(
            "syncmatrix.server.models.task_runs.read_task_runs", read_task_runs
        )
        set_task_run_state = AsyncMock()
        monkeypatch.setattr(
            "syncmatrix.server.models.task_runs.set_task_run_state", set_task_run_state
        )
        ...
        with pytest.raises(ObjectNotFoundError, match="Unable to read flow run"):
            async with contextlib.AsyncExitStack() as stack:
                for rule in update_policy:
                    ...
                    monkeypatch.setattr(
                        "syncmatrix.server.orchestration.rules.TaskOrchestrationContext.flow_run",
                        missing_flow_run,
                    )
...
class TestPermitRerunningFailedTaskRuns:
    """
    Following https://github.com/KhulnaSoft/syncmatrix/pull/9152 some of these test names
...
class TestTaskRetryingRule:
    ...
    async def test_retries_can_jitter_sleeps(
        ...
        monkeypatch.setattr(
            "syncmatrix.server.orchestration.core_policy.clamped_poisson_interval",
            randomizer,
        )
...
class TestTaskConcurrencyLimits:
    ...
    async def test_concurrency_limiting_aborts_transitions_with_zero_limit(
        ...
        # instead of a WAIT response, Syncmatrix should direct the client to ABORT
        assert ctx.response_status == SetStateStatus.ABORT
