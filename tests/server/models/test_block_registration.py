...
from syncmatrix.server.models.block_registration import (
    _load_collection_blocks_data,
    register_block_schema,
    register_block_type,
    run_block_auto_registration,
)
from syncmatrix.server.models.block_schemas import read_block_schema_by_checksum
from syncmatrix.server.models.block_types import read_block_type_by_slug, read_block_types
from syncmatrix.settings import SYNCMATRIX_API_BLOCKS_REGISTER_ON_START, temporary_settings
...
class TestRunAutoRegistration:
    ...
    async def test_registration_works_with_populated_database(
        ...
        with temporary_settings({SYNCMATRIX_API_BLOCKS_REGISTER_ON_START: True}):
            await run_block_auto_registration(session=session)
            await session.commit()
