...
from syncmatrix.server.schemas import filters
from syncmatrix.server.schemas.states import StateType
from syncmatrix.settings import SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_RUNS
...
class TestCreateDeployment:
    ...
    async def test_creating_a_deployment_does_not_create_work_queue(
        ...
        # This test ensures that this no longer happens. See: https://github.com/KhulnaSoft/syncmatrix/pull/9046
        wq = await models.work_queues.read_work_queue_by_name(
...
class TestScheduledRuns:
    async def test_schedule_runs_inserts_in_db(self, flow, deployment, session, db):
        ...
        assert len(scheduled_runs) == SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_RUNS.value()
        ...
        expected_times = {
            ...
            for i in range(SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_RUNS.value())
        }
    ...
    async def test_schedule_runs_is_idempotent(self, flow, deployment, session, db):
        ...
        db_scheduled_runs = query_result.scalars().all()
        assert len(db_scheduled_runs) == SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_RUNS.value()
