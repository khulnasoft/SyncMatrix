...
class TestGetRunsInQueueQuery:
    ...
    async def test_get_runs_in_queue_limit_sorts_correctly(
        ...
        https://github.com/KhulnaSoft/syncmatrix/pull/7457
        """
...
class TestGetRunsFromWorkQueueQuery:
    ...
    async def test_get_all_runs(
        self, session, db: SyncmatrixDBInterface, work_pools, work_queues
    ):
    ...
    async def test_get_all_runs_with_limit(
        self, session, db: SyncmatrixDBInterface, work_pools, work_queues, limit
    ):
    ...
    async def test_scheduled_before(
        self, session, db: SyncmatrixDBInterface, hours, expected
    ):
        runs = await db.queries.get_scheduled_flow_runs_from_work_pool(
            session=session,
            db=db,
    ...
    async def test_scheduled_before_and_after(self, session, db: SyncmatrixDBInterface):
        runs = await db.queries.get_scheduled_flow_runs_from_work_pool(
            session=session,
            db=db,
