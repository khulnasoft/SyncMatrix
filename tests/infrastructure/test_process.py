...
import syncmatrix
import syncmatrix.infrastructure
import syncmatrix.infrastructure.process
import syncmatrix.utilities.processutils
...
async def test_prepare_for_flow_run_uses_sys_executable(
    ...
    assert infrastructure.command == [sys.executable, "-m", "syncmatrix.engine"]
...
def test_process_logs_exit_code_help_message(
    ...
    monkeypatch.setattr("syncmatrix.infrastructure.process.run_process", mock)
...
def test_windows_process_run_sets_process_group_creation_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        syncmatrix.infrastructure.process, "run_process", mock_run_process_call
    )

    syncmatrix.infrastructure.Process(command=["echo", "hello world"]).run()
