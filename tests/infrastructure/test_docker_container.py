...
from syncmatrix.testing.utilities import assert_does_not_warn
...
def test_allows_unsetting_environment_variables(
    ...
    DockerContainer(command=["echo", "hello"], env={"SYNCMATRIX_TEST_MODE": None}).run()
    mock_docker_client.containers.create.assert_called_once()
    call_env = mock_docker_client.containers.create.call_args[1].get("environment")
    assert "SYNCMATRIX_TEST_MODE" not in call_env
...
def test_network_mode_defaults_to_host_if_using_localhost_api_on_linux(
    ...
    DockerContainer(
        command=["echo", "hello"], env=dict(SYNCMATRIX_API_URL=f"http://{localhost}/test")
    ).run()
    mock_docker_client.containers.create.assert_called_once()
    network_mode = mock_docker_client.containers.create.call_args[1].get("network_mode")
...
def test_network_mode_defaults_to_none_if_using_networks(mock_docker_client):
    ...
    DockerContainer(
        command=["echo", "hello"],
        env=dict(SYNCMATRIX_API_URL="http://localhost/test"),
        networks=["test"],
    ).run()
    mock_docker_client.containers.create.assert_called_once()
    network_mode = mock_docker_client.containers.create.call_args[1].get("network_mode")
...
def test_network_mode_defaults_to_none_if_using_nonlocal_api(mock_docker_client):
    DockerContainer(
        command=["echo", "hello"], env=dict(SYNCMATRIX_API_URL="http://foo/test")
    ).run()
    mock_docker_client.containers.create.assert_called_once()
    network_mode = mock_docker_client.containers.create.call_args[1].get("network_mode")
    assert network_mode is None
...
def test_network_mode_defaults_to_none_if_not_on_linux(mock_docker_client, monkeypatch):
    ...
    DockerContainer(
        command=["echo", "hello"], env=dict(SYNCMATRIX_API_URL="http://localhost/test")
    ).run()
...
def test_network_mode_defaults_to_none_if_api_url_cannot_be_parsed(
    ...
    with pytest.warns(UserWarning, match="Failed to parse host"):
        DockerContainer(
            command=["echo", "hello"], env=dict(SYNCMATRIX_API_URL="foo")
        ).run()
...
def test_replaces_localhost_api_with_dockerhost_when_not_using_host_network(
    ...
    mock_docker_client.containers.create.assert_called_once()
    call_env = mock_docker_client.containers.create.call_args[1].get("environment")
    assert "SYNCMATRIX_API_URL" in call_env
    assert call_env["SYNCMATRIX_API_URL"] == hosted_api_server.replace(
...
def test_does_not_replace_localhost_api_when_using_host_network(
    ...
    call_env = mock_docker_client.containers.create.call_args[1].get("environment")
    assert "SYNCMATRIX_API_URL" in call_env
    assert call_env["SYNCMATRIX_API_URL"] == hosted_api_server
...
def test_warns_at_runtime_when_using_host_network_mode_on_non_linux_platform(
    ...
    with assert_does_not_warn(ignore_warnings=[SyncmatrixDeprecationWarning]):
...
def test_does_not_override_user_provided_api_host(
    ...
    DockerContainer(
        command=["echo", "hello"], env={"SYNCMATRIX_API_URL": "http://localhost/api"}
    ).run()
    mock_docker_client.containers.create.assert_called_once()
    call_env = mock_docker_client.containers.create.call_args[1].get("environment")
    assert call_env.get("SYNCMATRIX_API_URL") == "http://localhost/api"
...
def test_default_image_pull_policy_pulls_image_with_latest_tag(
    ...
    DockerContainer(command=["echo", "hello"], image="syncmatrix:latest").run()
    mock_docker_client.images.pull.assert_called_once()
    mock_docker_client.images.pull.assert_called_with("syncmatrix", "latest")


def test_default_image_pull_policy_pulls_image_with_no_tag(
    mock_docker_client,
):
    DockerContainer(command=["echo", "hello"], image="syncmatrix").run()
    mock_docker_client.images.pull.assert_called_once()
    mock_docker_client.images.pull.assert_called_with("syncmatrix", None)
...
def test_default_image_pull_policy_pulls_image_with_tag_other_than_latest_if_not_present(
    ...
    DockerContainer(command=["echo", "hello"], image="syncmatrix:omega").run()
    mock_docker_client.images.pull.assert_called_once()
    mock_docker_client.images.pull.assert_called_with("syncmatrix", "omega")
...
def test_warns_if_docker_version_does_not_support_host_gateway_on_linux(
    ...
    with pytest.warns(
        ...
        DockerContainer(
            command=["echo", "hello"],
            env={"SYNCMATRIX_API_URL": explicit_api_url} if explicit_api_url else {},
        ).run()
...
def test_does_not_warn_about_gateway_if_user_has_provided_nonlocal_api_url(
    ...
    with assert_does_not_warn(ignore_warnings=[SyncmatrixDeprecationWarning]):
        DockerContainer(
            command=["echo", "hello"],
            env={"SYNCMATRIX_API_URL": "http://my-domain.test/api"},
        ).run()
...
def test_container_metadata(docker: "DockerClient"):
    ...
    assert container.image.tags[0] == get_syncmatrix_image_name()
