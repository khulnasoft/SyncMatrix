...
def mock_k8s_client(monkeypatch, mock_cluster_config):
    ...
    monkeypatch.setattr(
        "syncmatrix.infrastructure.kubernetes.KubernetesJob.get_client",
        get_client,
    )
    return mock
...
def mock_k8s_batch_client(monkeypatch, mock_cluster_config, mock_k8s_v1_job):
    ...
    monkeypatch.setattr(
        "syncmatrix.infrastructure.kubernetes.KubernetesJob.get_batch_client",
        get_batch_client,
    )
    return mock
...
def test_cluster_uid_uses_env_var_if_set(
    ...
    monkeypatch.setenv("SYNCMATRIX_KUBERNETES_CLUSTER_UID", "test-uid")
    fake_status = MagicMock(spec=anyio.abc.TaskStatus)
...
async def test_allows_unsetting_environment_variables(
    ...
    assert "SYNCMATRIX_TEST_MODE" in KubernetesJob._base_environment()
    await KubernetesJob(
        command=["echo", "hello"], env={"SYNCMATRIX_TEST_MODE": None}
    ).run()
    mock_k8s_batch_client.create_namespaced_job.assert_called_once()
    manifest = mock_k8s_batch_client.create_namespaced_job.call_args[0][1]
    pod = manifest["spec"]["template"]["spec"]
    ...
    assert "SYNCMATRIX_TEST_MODE" not in env_names
...
class TestCustomizingBaseJob:
    ...
    def test_user_can_supply_a_sidecar_container_and_volume(self):
        ...
        # the syncmatrix-job container is still populated
        assert pod["containers"][0]["name"] == "syncmatrix-job"
...
class TestCustomizingJob:
    ...
    def test_setting_pod_resource_requests(self):
        ...
        # syncmatrix's orchestration values are still there
