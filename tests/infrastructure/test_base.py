...
import syncmatrix
from syncmatrix import engine
from syncmatrix.blocks.core import BlockNotSavedError
from syncmatrix.infrastructure import (
    DockerContainer,
    Infrastructure,
    KubernetesJob,
    Process,
)
from syncmatrix.infrastructure.base import MIN_COMPAT_SYNCMATRIX_VERSION
from syncmatrix.server.schemas.core import Deployment
...
async def test_submission_adds_flow_run_metadata(
    ...
    MockInfrastructure._run.assert_called_once_with(
        {
            "type": "mock",
            "env": {"SYNCMATRIX__FLOW_RUN_ID": str(flow_run.id)},
            "labels": {
            ...
            "name": flow_run.name,
            "command": ["syncmatrix", "flow-run", "execute"],
        }
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                "khulnasoft.com/deployment-name": "test",
                "khulnasoft.com/deployment-updated": "2022-11-10T16:50:59.500000Z",
            },
        ),
        ({"name": "test", "updated": None}, {"khulnasoft.com/deployment-name": "test"}),
    ],
)
...
@pytest.mark.usefixtures("use_hosted_api_server")
@pytest.mark.skipif(
    (Version(MIN_COMPAT_SYNCMATRIX_VERSION) > Version(syncmatrix.__version__.split("+")[0])),
    reason=f"Expected breaking change in next version: {MIN_COMPAT_SYNCMATRIX_VERSION}",
)
async def test_execution_is_compatible_with_old_syncmatrix_container_version(
    flow_run,
    syncmatrix_client,
    deployment,
):
    ...
    running an old version of Syncmatrix. This tests for regression in the path of
    ...
    will exist. If so, bump MIN_COMPAT_SYNCMATRIX_VERSION past the current syncmatrix
    ...
    infrastructure = DockerContainer(
        image=get_syncmatrix_image_name(MIN_COMPAT_SYNCMATRIX_VERSION)
...
async def test_enabling_enhanced_cancellation_changes_default_command(
    deployment, syncmatrix_client, enable_enhanced_cancellation
):
    flow_run = await syncmatrix_client.create_flow_run_from_deployment(deployment.id)
    infrastructure = MockInfrastructure(name="test").prepare_for_flow_run(flow_run)
    assert infrastructure.command == ["syncmatrix", "flow-run", "execute"]
...
async def test_publish_as_work_pool(
    work_pool_name, monkeypatch, capsys, syncmatrix_client
):
    block = MockInfrastructure()
    block._block_document_id = uuid.uuid4()
    ...
    work_pool = await syncmatrix_client.read_work_pool(
