...
from syncmatrix.infrastructure.provisioners.modal import ModalPushProvisioner
...
def mock_run_process():
    with patch("syncmatrix.infrastructure.provisioners.modal.run_process") as mock:
        yield mock
...
def mock_modal(monkeypatch):
    mock = MagicMock()
    monkeypatch.setattr("syncmatrix.infrastructure.provisioners.modal.modal", mock)
    yield mock


@pytest.fixture
def mock_importlib():
    with patch("syncmatrix.infrastructure.provisioners.modal.importlib") as mock:
        yield mock


@pytest.fixture
def mock_confirm():
    with patch("syncmatrix.infrastructure.provisioners.modal.Confirm") as mock:
        yield mock
...
async def test_provision(
    ...
    result = await provisioner.provision(
        work_pool_name, base_job_template, client=syncmatrix_client
    )
