...
from syncmatrix.infrastructure.provisioners import (
    get_infrastructure_provisioner_for_work_pool_type,
)
from syncmatrix.infrastructure.provisioners.ecs import (
...
def iam_policy_resource() -> IamPolicyResource:
    return IamPolicyResource(policy_name="syncmatrix-ecs-policy")
...
def existing_iam_policy():
    ...
    policy = iam_client.create_policy(
        PolicyName="syncmatrix-ecs-policy",
        PolicyDocument=json.dumps(
            {
                "Version": "2012-10-17",
                "Statement": [
                    {
                        "Sid": "SyncmatrixEcsPolicy",
                        "Effect": "Allow",
                        "Action": [
                            "ecs:DescribeTasks",
                        ],
...
def mock_run_process():
    with patch("syncmatrix.infrastructure.provisioners.ecs.run_process") as mock:
        yield mock
...
class TestIamPolicyResource:
    ...
    async def test_provision(self, iam_policy_resource):
        ...
        iam_client = boto3.client("iam")
        iam_client.create_user(UserName="syncmatrix-ecs-user")
        ...
        assert "syncmatrix-ecs-policy" in policy_names
    ...
    async def test_provision_preexisting_policy(self):
        ...
        iam_policy_resource = IamPolicyResource(policy_name="syncmatrix-ecs-policy")
        result = await iam_policy_resource.provision(
            advance=advance_mock, policy_document={}
        )
        # returns existing policy ARN
        assert result == "arn:aws:iam::123456789012:policy/syncmatrix-ecs-policy"
...
def iam_user_resource():
    return IamUserResource(user_name="syncmatrix-ecs-user")
...
def existing_iam_user():
    ...
    iam_client.delete_user(UserName="syncmatrix-ecs-user")
...
def existing_execution_role():
    ...
    iam_client.create_role(
        RoleName="SyncmatrixEcsTaskExecutionRole",
        AssumeRolePolicyDocument=json.dumps(
            {
                "Version": "2012-10-17",
                "Statement": [
                    {
                        "Sid": "SyncmatrixEcsExecutionRole",
                        "Effect": "Allow",
    ...
    iam_client.delete_role(RoleName="SyncmatrixEcsTaskExecutionRole")
...
class TestIamUserResource:
    ...
    async def test_provision(self, iam_user_resource):
        ...
        assert "syncmatrix-ecs-user" in user_names
    ...
    async def test_provision_preexisting_user(self):
        ...
        iam_user_resource = IamUserResource(user_name="syncmatrix-ecs-user")
...
def credentials_block_resource():
    return CredentialsBlockResource(
        user_name="syncmatrix-ecs-user", block_document_name="work-pool-aws-credentials"
    )


@pytest.fixture
async def existing_credentials_block(
    register_block_types, syncmatrix_client: SyncmatrixClient
):
    block_type = await syncmatrix_client.read_block_type_by_slug(slug="aws-credentials")
...
class TestCredentialsBlockResource:
    ...
    async def test_provision(self, syncmatrix_client, credentials_block_resource):
        advance_mock = MagicMock()
        ...
        block_document = await syncmatrix_client.read_block_document_by_name(
    ...
    @pytest.mark.usefixtures("existing_credentials_block")
    async def test_provision_preexisting_block(self, syncmatrix_client):
        advance_mock = MagicMock()
...
class TestAuthenticationResource:
    ...
    async def test_get_planned_actions(self, authentication_resource):
        ...
        assert (
            "Creating an IAM user for managing ECS tasks: [blue]syncmatrix-ecs-user[/]"
            in actions
        )
        assert (
            "Creating and attaching an IAM policy for managing ECS tasks:"
            " [blue]syncmatrix-ecs-policy[/]" in actions
        )
    ...
    async def test_get_planned_actions_existing_user(self, authentication_resource):
        ...
        assert actions == [
            (
                ...
                " [blue]SyncmatrixEcsTaskExecutionRole[/]"
            ),
            "Storing generated AWS credentials in a block",
        ]

    @pytest.mark.usefixtures("register_block_types")
    async def test_provision(self, authentication_resource, syncmatrix_client):
        advance_mock = MagicMock()
        base_job_template = {
            "variables": {
                "type": "object",
...
def cluster_resource():
    return ClusterResource(cluster_name="syncmatrix-ecs-cluster")


@pytest.fixture
def existing_cluster():
    ecs_client = boto3.client("ecs")
    ecs_client.create_cluster(clusterName="syncmatrix-ecs-cluster")

    yield

    ecs_client.delete_cluster(cluster="syncmatrix-ecs-cluster")
...
class TestClusterResource:
    ...
    async def test_get_planned_actions_cluster_does_not_exist(self, cluster_resource):
        ...
        assert actions == [
            "Creating an ECS cluster for running Syncmatrix flows:"
            " [blue]syncmatrix-ecs-cluster[/]"
        ]
    ...
    async def test_provision(self, cluster_resource):
        ...
        assert base_job_template["variables"]["properties"]["cluster"] == {
            "default": "syncmatrix-ecs-cluster",
        }
...
@pytest.fixture
def existing_syncmatrix_vpc(no_default_vpc):
    ec2 = boto3.resource("ec2")
    vpc = ec2.create_vpc(CidrBlock="172.31.0.0/16")
    vpc.create_tags(Tags=[{"Key": "Name", "Value": "syncmatrix-ecs-vpc"}])
...
class TestVpcResource:
    ...
    @pytest.mark.usefixtures("existing_syncmatrix_vpc")
    async def test_get_task_count_existing_syncmatrix_vpc(self, vpc_resource):
        count = await vpc_resource.get_task_count()
    ...
    @pytest.mark.usefixtures("existing_syncmatrix_vpc")
    async def test_requires_provisioning_syncmatrix_created_vpc_exists(self, vpc_resource):
        requires_provisioning = await vpc_resource.requires_provisioning()
    ...
    async def test_get_planned_actions_requires_provisioning(self, vpc_resource):
        ...
        assert actions == [
            ...
            " ECS tasks: [blue]syncmatrix-ecs-vpc[/]"
        ]
    ...
    async def test_provision(self, vpc_resource):
        ...
        assert vpc.tags[0]["Value"] == "syncmatrix-ecs-vpc"
    ...
    @pytest.mark.usefixtures("existing_syncmatrix_vpc")
    async def test_provision_existing_syncmatrix_vpc(self, vpc_resource):
        base_job_template = {
            "variables": {
                "type": "object",
                "properties": {"vpc_id": {}},
        ...
        syncmatrix_vpc = None
        for vpc in ec2.vpcs.all():
            if vpc.tags[0]["Value"] == "syncmatrix-ecs-vpc":
                syncmatrix_vpc = vpc
                break
...
class TestElasticContainerServicePushProvisioner:
    ...
    def mock_confirm(self):
        with patch("syncmatrix.infrastructure.provisioners.ecs.Confirm") as mock:
            yield mock

    @pytest.fixture
    def mock_importlib(self):
        with patch("syncmatrix.infrastructure.provisioners.ecs.importlib") as mock:
            yield mock
    ...
    async def test_provision_interactive_with_default_names(
        self, provisioner, mock_confirm, syncmatrix_client, mock_run_process, capsys
    ):
        provisioner.console.is_interactive = True
        mock_confirm.ask.side_effect = [
            False,
    ...
    async def test_provision_interactive_with_custom_names(
        ...
        monkeypatch.setattr(
            "syncmatrix.infrastructure.provisioners.ecs.prompt", mock_prompt
        )
...
def execution_role_resource():
    return ExecutionRoleResource(execution_role_name="SyncmatrixEcsTaskExecutionRole")
...
class TestExecutionRoleResource:
    ...
    async def test_provision_requires_provisioning(self, execution_role_resource):
        ...
        assert arn == "arn:aws:iam::123456789012:role/SyncmatrixEcsTaskExecutionRole"
        advance_mock.assert_called_once()
...
def container_repository_resource():
    return ContainerRepositoryResource(
        work_pool_name="test-work-pool", repository_name="syncmatrix-flows"
    )
...
def existing_ecr_repository():
    ...
    ecr_client.create_repository(repositoryName="syncmatrix-flows")

    yield

    ecr_client.delete_repository(repositoryName="syncmatrix-flows")
...
class TestContainerRepositoryResource:
    ...
    async def test_get_planned_actions_requires_provisioning(
        ...
        assert await container_repository_resource.get_planned_actions() == [
            "Creating an ECR repository for storing Syncmatrix images:"
            " [blue]syncmatrix-flows[/]"
        ]
    ...
    async def test_provision_requires_provisioning(
        ...
        new_repository = ecr.describe_repositories(repositoryNames=["syncmatrix-flows"])
        assert new_repository["repositories"][0]["repositoryName"] == "syncmatrix-flows"
        mock_run_process.assert_called_once_with(
            "docker login -u AWS -p 123456789012-auth-token"
            " https://123456789012.dkr.ecr.us-east-1.amazonaws.com"
        )
    ...
    async def test_update_next_steps(self, container_repository_resource):
        ...
        assert container_repository_resource.next_steps[0] == dedent(
            ...
            To build and push a Docker image to your newly created repository, use [blue]'syncmatrix-flows'[/] as your image name:
            """
        )
        assert container_repository_resource.next_steps[1].renderable.code == dedent(
            """\
                from syncmatrix import flow
                from syncmatrix.deployments import DeploymentImage
