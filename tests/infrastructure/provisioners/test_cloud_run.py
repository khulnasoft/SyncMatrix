...
from syncmatrix.infrastructure.provisioners.cloud_run import CloudRunPushProvisioner
        ...
        "properties": {
            ...
            "image": {
                ...
                "description": (
                    ...
                    " Syncmatrix image will be used. See"
                    " https://cloud.google.com/run/docs/deploying#images."
                ),
                "example": "docker.io/khulnasoft/syncmatrix:2-latest",
                "type": "string",
            },
            "cpu": {
                "title": "CPU",
            ...
            "timeout": {
                "title": "Job Timeout",
                "description": (
                    "The length of time that Syncmatrix will wait for Cloud Run Job state"
                    " changes."
                ),
                "default": 600,
                "exclusiveMinimum": 0,
...
def mock_run_process(monkeypatch):
    ...
    monkeypatch.setattr(
        "syncmatrix.infrastructure.provisioners.cloud_run.run_process", mock
    )
    return mock
...
async def test_provision(mock_run_process, syncmatrix_client: SyncmatrixClient):
    ...
    assert_commands(
        ...
        (
            "gcloud artifacts repositories create syncmatrix-images"
        ...
        (
            "gcloud iam service-accounts create syncmatrix-cloud-run --display-name"
            " 'Syncmatrix Cloud Run Service Account'"
        ),
        (
            "gcloud projects add-iam-policy-binding test-project"
            " --member=serviceAccount:syncmatrix-cloud-run@test-project.iam.gserviceaccount.com"
        ...
        re.compile(
            r"gcloud iam service-accounts keys create .*\/syncmatrix-cloud-run-key\.json"
            r" --iam-account=syncmatrix-cloud-run@test-project\.iam\.gserviceaccount\.com"
        ),
    )
    ...
    assert (
        load_current_profile().settings[SYNCMATRIX_DEFAULT_DOCKER_BUILD_NAMESPACE]
        == "us-central1-docker.pkg.dev/test-project/syncmatrix-images"
    )
...
async def test_provision_interactive_with_default_names(
    mock_run_process, syncmatrix_client: SyncmatrixClient, monkeypatch
):
    ...
    monkeypatch.setattr(
        "syncmatrix.infrastructure.provisioners.cloud_run.prompt_select_from_table",
        mock_prompt_select_from_table,
    )
    monkeypatch.setattr(
        "syncmatrix.infrastructure.provisioners.cloud_run.Confirm.ask", mock_confirm
    )
    provisioner = CloudRunPushProvisioner()
    ...
    assert_commands(
        ...
        re.compile(
            ...
            r" .*\/syncmatrix-cloud-run-key\.json"
            r" --iam-account=syncmatrix-cloud-run@test-project\.iam\.gserviceaccount\.com"
        ),
    )
...
async def test_provision_interactive_with_custom_names(
    ...
    monkeypatch.setattr(
        "syncmatrix.infrastructure.provisioners.cloud_run.prompt", mock_prompt
    )
    monkeypatch.setattr(
        "syncmatrix.infrastructure.provisioners.cloud_run.prompt_select_from_table",
        mock_prompt_select_from_table,
