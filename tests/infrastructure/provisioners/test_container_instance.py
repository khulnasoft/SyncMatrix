...
from syncmatrix.blocks.fields import SecretDict
from syncmatrix.client.orchestration import SyncmatrixClient
from syncmatrix.client.schemas.actions import BlockDocumentCreate
from syncmatrix.infrastructure.provisioners.container_instance import (
...
@pytest.fixture
async def existing_credentials_block(syncmatrix_client: SyncmatrixClient):
    block_type = await syncmatrix_client.read_block_type_by_slug(
        slug="azure-container-instance-credentials"
    )
    block_schema = await syncmatrix_client.get_most_recent_block_schema_for_block_type(
        block_type_id=block_type.id
    )
    assert block_schema is not None

    block_document = await syncmatrix_client.create_block_document(
    ...
    await syncmatrix_client.delete_block_document(block_document_id=block_document.id)
        ...
        "variables": {
            ...
            "properties": {
                ...
                "image": {
                    "title": "Image",
                    "description": (
                        "The image to use for the Syncmatrix container in the task. This"
                        " value defaults to a Syncmatrix base image matching your local"
                        " versions."
                    ),
                    "type": "string",
                },
                "resource_group_name": {
                    ...
                    "description": (
                        "The name of the Azure Resource Group in which to run Syncmatrix"
                        " ACI tasks."
                    ),
                    "type": "string",
                },
                ...
                "entrypoint": {
                    ...
                    "description": (
                        ...
                        " defaults to the entrypoint used by Syncmatrix images and should"
                        " only be changed when using a custom image that is not based"
                        " on an official Syncmatrix image. Any commands set on deployments"
                        " will be passed to the entrypoint as parameters."
                    ),
                    "default": "/opt/syncmatrix/entrypoint.sh",
                    "type": "string",
                },
                "image_registry": {
                ...
                "stream_output": {
                    "title": "Stream Output",
                    "description": (
                        "If `True`, logs will be streamed from the Syncmatrix container to"
                        " the local console."
                    ),
                    "default": True,
                    "type": "boolean",
...
async def test_aci_resource_group_creation_creates_new_group(provisioner):
    ...
    expected_calls = [
        call(
            "az group exists --name syncmatrix-aci-push-pool-rg --subscription None",
            return_json=True,
        ),
        call(
            (
                "az group create --name 'syncmatrix-aci-push-pool-rg' --location 'eastus'"
                " --subscription 'None'"
            ),
            success_message=(
                "Resource group 'syncmatrix-aci-push-pool-rg' created successfully"
            ),
            failure_message=(
                "Failed to create resource group 'syncmatrix-aci-push-pool-rg' in"
                " subscription 'None'"
            ),
            ignore_if_exists=True,
        ),
...
async def test_aci_app_registration_creates_new_app(provisioner):
    app_registration = {
        ...
        "displayName": "syncmatrix-aci-push-pool-app",
        "identifierUris": ["https://syncmatrix-aci-push-pool-app"],
    }
    provisioner.azure_cli.run_command.side_effect = [
    ...
    expected_calls = [
        call(
            "az ad app list --display-name syncmatrix-aci-push-pool-app --output json",
        ),
        call(
            "az ad app create --display-name syncmatrix-aci-push-pool-app --output json",
            success_message=(
                "App registration 'syncmatrix-aci-push-pool-app' created successfully"
            ),
            failure_message=(
                "Failed to create app registration with name"
                " 'syncmatrix-aci-push-pool-app'"
            ),
            ignore_if_exists=True,
        ),
    ]
    ...
    service_principal = [
        {
            ...
            "alternativeNames": [],
            "appDescription": None,
            "appDisplayName": "syncmatrix-aci-push-pool-app",
...
async def test_aci_assign_contributor_role_handles_existing_role(provisioner):
    ...
    role = "Contributor"
    scope = "/subscriptions/None/resourceGroups/syncmatrix-aci-push-pool-rg"
    ...
    expected_calls = [
        ...
        call(
            (
                ...
                " /subscriptions/None/resourceGroups/syncmatrix-aci-push-pool-rg"
...
async def test_get_or_create_registry_new_registry(provisioner):
    registry_name = "syncmatrix-registry"
    resource_group_name = "syncmatrix-rg"
    location = "westus"
    subscription_id = "12345678-1234-1234-1234-123456789012"

    provisioner.azure_cli.run_command.side_effect = [
        [],  # No existing registry
        {"name": "syncmatrix-registry"},  # Successful creation
    ]
    ...
    expected_calls = [
        call(
            (
                "az acr list --query \"[?starts_with(name, 'syncmatrix')]\" --subscription"
        ...
        call(
            (
                "az acr create --name syncmatrix-registry --resource-group syncmatrix-rg"
    ...
    assert response == {"name": "syncmatrix-registry"}
...
async def test_get_or_create_registry_existing_registry(provisioner):
    ...
    provisioner.azure_cli.run_command.side_effect = [
        [{"name": "syncmatrix-registry"}],  # Existing registry
    ]
...
async def test_aci_provision_no_existing_credentials_block(
    ...
    monkeypatch.setattr(
        provisioner,
        "_generate_acr_name",
        lambda *args, **kwargs: "syncmatrixacipushpoolregistry",
    )
    ...
    new_registry = {
        "id": "/subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/syncmatrix-aci-push-pool-rg/providers/Microsoft.ContainerRegistry/registries/syncmatrixacipushpoolregistry",
        "loginServer": "syncmatrixacipushpoolregistry.azurecr.io",
    }

    new_identity = {
        "id": "/subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/syncmatrix-aci-push-pool-rg/providers/Microsoft.ManagedIdentity/userAssignedIdentities/syncmatrix-aci-push-pool-identity",
        "principalId": "12345678-1234-1234-1234-123456789012",
    }
    ...
    expected_calls = [
        ...
        call(
            (
                "az group exists --name syncmatrix-aci-push-pool-rg --subscription"
                " 12345678-1234-1234-1234-123456789012"
            ),
            return_json=True,
        ),
        call(
            (
                "az group create --name 'syncmatrix-aci-push-pool-rg' --location 'westus'"
                " --subscription '12345678-1234-1234-1234-123456789012'"
            ),
            success_message=(
                "Resource group 'syncmatrix-aci-push-pool-rg' created successfully"
        ...
        call("az ad app list --display-name syncmatrix-aci-push-pool-app --output json"),
        call(
            "az ad app create --display-name syncmatrix-aci-push-pool-app --output json",
            success_message=(
                "App registration 'syncmatrix-aci-push-pool-app' created successfully"
        ...
        call(
            ...
            failure_message=(
                ...
                " `syncmatrix-aci-push-pool-app` resource and try again."
            ),
            ignore_if_exists=True,
            return_json=True,
        ),
        ...
        call(
            (
                ...
                " /subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/syncmatrix-aci-push-pool-rg"
                " --subscription 12345678-1234-1234-1234-123456789012 --output json"
            ),
            return_json=True,
        ),
        ...
        call(
            (
                "az acr create --name syncmatrixacipushpoolregistry --resource-group"
                " syncmatrix-aci-push-pool-rg --subscription"
        ...
        call(
            (
                "az acr login --name syncmatrixacipushpoolregistry.azurecr.io"
                " --subscription 12345678-1234-1234-1234-123456789012"
            ),
            success_message=(
                "Logged into registry syncmatrixacipushpoolregistry.azurecr.io"
            ),
            failure_message=(
                "Failed to log into registry syncmatrixacipushpoolregistry.azurecr.io"
            ),
        ),
        # _get_or_create_identity
        call(
            (
                "az identity list --query \"[?name=='syncmatrix-acr-identity']\""
                " --resource-group syncmatrix-aci-push-pool-rg --subscription"
                " 12345678-1234-1234-1234-123456789012 --output json"
            ),
            return_json=True,
        ),
        call(
            (
                "az identity create --name syncmatrix-acr-identity --resource-group"
                " syncmatrix-aci-push-pool-rg --subscription"
                " 12345678-1234-1234-1234-123456789012 --output json"
            ),
            success_message="Identity 'syncmatrix-acr-identity' created",
            failure_message="Failed to create identity 'syncmatrix-acr-identity'",
            return_json=True,
        ),
        ...
        call(
            (
                ...
                " /subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/syncmatrix-aci-push-pool-rg/providers/Microsoft.ContainerRegistry/registries/syncmatrixacipushpoolregistry"
    ...
    block_doc = await syncmatrix_client.read_block_document(new_block_doc_id)
    ...
    new_base_job_template["variables"]["properties"]["resource_group_name"][
        "default"
    ] = "syncmatrix-aci-push-pool-rg"

    new_base_job_template["variables"]["properties"]["image_registry"]["default"] = {
        "registry_url": "syncmatrixacipushpoolregistry.azurecr.io",
        "identity": "/subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/syncmatrix-aci-push-pool-rg/providers/Microsoft.ManagedIdentity/userAssignedIdentities/syncmatrix-aci-push-pool-identity",
    }

    new_base_job_template["variables"]["properties"]["identities"]["default"] = [
        "/subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/syncmatrix-aci-push-pool-rg/providers/Microsoft.ManagedIdentity/userAssignedIdentities/syncmatrix-aci-push-pool-identity"
    ]
...
async def test_aci_provision_interactive_default_provisioning(
    ...
    monkeypatch.setattr(
        "syncmatrix.infrastructure.provisioners.container_instance.prompt_select_from_table",
    ...
    monkeypatch.setattr(
        "syncmatrix.infrastructure.provisioners.container_instance.Confirm.ask",
        mock_confirm,
    )
    monkeypatch.setattr(
        provisioner,
...
async def test_aci_provision_interactive_custom_resource_names(
    ...
    monkeypatch.setattr(
        "syncmatrix.infrastructure.provisioners.container_instance.prompt", mock_prompt
    )
