...
from syncmatrix import SyncmatrixClient
from syncmatrix._internal.compatibility.deprecated import SyncmatrixDeprecationWarning
from syncmatrix.settings import SYNCMATRIX_AGENT_PREFETCH_SECONDS, temporary_settings
from syncmatrix.testing.cli import invoke_and_assert
from syncmatrix.testing.utilities import MagicMock
...
def test_start_agent_emits_deprecation_warning():
    invoke_and_assert(
        ...
        expected_output_contains=[
            ...
            " Use `syncmatrix worker start` instead.",
...
async def test_start_agent_creates_work_queue(syncmatrix_client: SyncmatrixClient):
    ...
    queue = await syncmatrix_client.read_work_queue_by_name("test")
    assert queue
...
def test_start_agent_with_prefetch_seconds(monkeypatch):
    ...
    monkeypatch.setattr(syncmatrix.cli.agent, "SyncmatrixAgent", mock_agent)
    invoke_and_assert(
        command=[
            "agent",
            "start",
...
def test_start_agent_with_prefetch_seconds_from_setting_by_default(monkeypatch):
    mock_agent = MagicMock()
    monkeypatch.setattr(syncmatrix.cli.agent, "SyncmatrixAgent", mock_agent)
    with temporary_settings({SYNCMATRIX_AGENT_PREFETCH_SECONDS: 100}):
        invoke_and_assert(
            command=[
                "agent",
                "start",
