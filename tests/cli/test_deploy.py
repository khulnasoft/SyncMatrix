...
import syncmatrix
from syncmatrix.blocks.system import JSON, Secret
from syncmatrix.cli.deploy import (
    _check_for_matching_deployment_name_and_entrypoint_in_syncmatrix_file,
...
from syncmatrix.client.schemas.schedules import (
    CronSchedule,
    IntervalSchedule,
    RRuleSchedule,
)
from syncmatrix.deployments import register_flow
from syncmatrix.deployments.base import (
    _save_deployment_to_syncmatrix_file,
    create_default_syncmatrix_yaml,
    initialize_project,
)
from syncmatrix.deployments.steps.core import StepExecutionError
from syncmatrix.events.schemas import DeploymentTrigger, Posture
from syncmatrix.exceptions import ObjectAlreadyExists, ObjectNotFound
from syncmatrix.infrastructure.container import DockerRegistry
from syncmatrix.server.schemas.actions import (
    BlockDocumentCreate,
    BlockSchemaCreate,
    BlockTypeCreate,
    WorkPoolCreate,
...
from syncmatrix.settings import (
    SYNCMATRIX_DEFAULT_WORK_POOL_NAME,
    SYNCMATRIX_UI_URL,
    temporary_settings,
)
...
from syncmatrix.utilities.filesystem import tmpchdir
from syncmatrix.utilities.slugify import slugify

TEST_PROJECTS_DIR = syncmatrix.__development_base_path__ / "tests" / "test-projects"


@pytest.fixture
def interactive_console(monkeypatch):
    monkeypatch.setattr("syncmatrix.cli.deploy.is_interactive", lambda: True)
...
def project_dir(tmp_path):
    with tmpchdir(tmp_path):
        if sys.version_info >= (3, 8):
            ...
            syncmatrix_home = tmp_path / ".syncmatrix"
            syncmatrix_home.mkdir(exist_ok=True, mode=0o0700)
            initialize_project()
            yield tmp_path
        else:
            shutil.copytree(TEST_PROJECTS_DIR, tmp_path / "three-seven")
            syncmatrix_home = tmp_path / "three-seven" / ".syncmatrix"
            syncmatrix_home.mkdir(exist_ok=True, mode=0o0700)
            initialize_project()
...
def project_dir_with_single_deployment_format(tmp_path):
    with tmpchdir(tmp_path):
        if sys.version_info >= (3, 8):
            ...
            with open("syncmatrix.yaml", "r") as f:
            ...
            shutil.copytree(TEST_PROJECTS_DIR, tmp_path / "three-seven")
            (tmp_path / "three-seven" / ".syncmatrix").mkdir(exist_ok=True, mode=0o0700)
            initialize_project()
...
def uninitialized_project_dir(project_dir):
    Path(project_dir, "syncmatrix.yaml").unlink()
    return project_dir
...
@pytest.fixture
async def default_agent_pool(syncmatrix_client):
    try:
        return await syncmatrix_client.create_work_pool(
            WorkPoolCreate(name="default-agent-pool", type="syncmatrix-agent")
        )
    except ObjectAlreadyExists:
        return await syncmatrix_client.read_work_pool("default-agent-pool")


@pytest.fixture
async def docker_work_pool(syncmatrix_client: SyncmatrixClient) -> WorkPool:
    return await syncmatrix_client.create_work_pool(
...
async def mock_prompt(monkeypatch):
    ...
    original_prompt = syncmatrix.cli._prompts.prompt
    monkeypatch.setattr("syncmatrix.cli._prompts.prompt", new_prompt)
...
def mock_provide_password(monkeypatch):
    ...
    original_prompt = syncmatrix.cli._prompts.prompt
    monkeypatch.setattr("syncmatrix.cli.deploy.prompt", new_prompt)
...
def mock_build_docker_image(monkeypatch):
    ...
    monkeypatch.setattr(
        "syncmatrix.deployments.steps.core.import_object",
        lambda x: mock_build,
    )
    monkeypatch.setattr(
        "syncmatrix.deployments.steps.core.import_module",
        lambda x: None,
    )
...
@pytest.fixture
async def aws_credentials(syncmatrix_client):
    aws_credentials_type = await syncmatrix_client.create_block_type(
    ...
    aws_credentials_schema = await syncmatrix_client.create_block_schema(
    ...
    return await syncmatrix_client.create_block_document(
...
@pytest.fixture
def set_ui_url():
    with temporary_settings({SYNCMATRIX_UI_URL: "http://gimmedata.com"}):
        yield
...
class TestProjectDeploySingleDeploymentYAML:
    ...
    async def test_project_deploy(
        self, project_dir_with_single_deployment_format, syncmatrix_client
    ):
        await syncmatrix_client.create_work_pool(
        ...
        result = await run_sync_in_worker_thread(
            ...
            expected_output_contains=[
                ...
                "Using a `deployment.yaml` file with `syncmatrix deploy`",
                (
                    "Please use the `syncmatrix.yaml` file instead by copying the"
                    " contents of your `deployment.yaml` file into your `syncmatrix.yaml`"
                    " file."
                ),
            ],
        )
        ...
        deployment = await syncmatrix_client.read_deployment_by_name(
            "An important name/test-name"
        )
    ...
    async def test_project_deploy_with_no_deployment_file(
        self, project_dir, syncmatrix_client
    ):
    ...
    async def test_project_deploy_with_empty_dep_file(
        self, project_dir_with_single_deployment_format, syncmatrix_client, work_pool
    ):
    ...
    async def test_project_deploy_templates_values(
        ...
        # update syncmatrix.yaml to include a new build step
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            syncmatrix_config = yaml.safe_load(f)

        # test step that returns a dictionary of inputs and output1, output2
        syncmatrix_config["build"] = [
            {"syncmatrix.testing.utilities.a_test_step": {"input": "foo"}}
        ]

        # save it back
        with syncmatrix_file.open(mode="w") as f:
            yaml.safe_dump(syncmatrix_config, f)
    ...
    async def test_project_deploy_templates_pull_step_safely(
        ...
        syncmatrix_config["pull"] = [
            {
                "syncmatrix.testing.utilities.a_test_step": {
                    "input": "{{ output1 }}",
                    "secret-input": "{{ syncmatrix.blocks.secret.test-secret }}",
                }
            },
        ]
        # save it back
    ...
    async def test_migrate_from_deployment_yaml_to_syncmatrix_yaml(self, work_pool):
        deployment_file = Path("deployment.yaml")
        with deployment_file.open(mode="r") as f:
            deploy_config = yaml.safe_load(f)
...
class TestProjectDeploy:
    ...
    async def test_project_deploy(self, project_dir, syncmatrix_client):
        await syncmatrix_client.create_work_pool(
            WorkPoolCreate(name="test-pool", type="test")
        )
        await run_sync_in_worker_thread(
            ...
            expected_output_contains=[
                "An important name/test-name",
                "syncmatrix worker start --pool 'test-pool'",
            ],
        )
    ...
    async def test_project_deploy_with_default_work_pool(
        ...
        with temporary_settings(updates={SYNCMATRIX_DEFAULT_WORK_POOL_NAME: "test-pool"}):
            await run_sync_in_worker_thread(
                invoke_and_assert,
                command=(
    ...
    async def test_project_deploy_with_no_syncmatrix_yaml(self, project_dir, work_pool):
        Path(project_dir, "syncmatrix.yaml").unlink()
        ...
        await run_sync_in_worker_thread(
            ...
            expected_code=0,
            expected_output_contains=[
                "Your Syncmatrix workers will attempt to load your flow from:",
    ...
    async def test_deploy_does_not_prompt_storage_when_pull_step_exists(
        ...
        # write a pull step to the syncmatrix.yaml
        with open("syncmatrix.yaml", "r") as f:
            config = yaml.safe_load(f)

        config["pull"] = [
            {"syncmatrix.deployments.steps.set_working_directory": {"directory": "."}}
        ]

        with open("syncmatrix.yaml", "w") as f:
    ...
    class TestGeneratedPullAction:
        async def test_project_deploy_generates_pull_action(
            self, work_pool, syncmatrix_client, uninitialized_project_dir
        ):
            await run_sync_in_worker_thread(
                invoke_and_assert,
                command=(
            ...
            assert deployment.pull_steps == [
                {
                    "syncmatrix.deployments.steps.set_working_directory": {
                        "directory": str(uninitialized_project_dir)
                    }
                }
            ]

        async def test_project_deploy_with_no_syncmatrix_yaml_git_repo_no_remote(
            self,
            work_pool,
            syncmatrix_client,
            uninitialized_project_dir_with_git_no_remote,
        ...
        async def test_project_deploy_with_no_syncmatrix_yaml_git_repo_user_rejects(
            self,
            work_pool,
            syncmatrix_client,
        ...
        async def test_project_deploy_with_no_syncmatrix_yaml_git_repo(
            self, work_pool, syncmatrix_client
        ):
            await run_sync_in_worker_thread(
                invoke_and_assert,
                command=(
            ...
            assert deployment.pull_steps == [
                {
                    "syncmatrix.deployments.steps.git_clone": {
            ...
            syncmatrix_file_contents = yaml.safe_load(Path("syncmatrix.yaml").read_text())
            assert syncmatrix_file_contents["pull"] == [
                {
                    "syncmatrix.deployments.steps.git_clone": {
                        "repository": "https://example.com/org/repo.git",
                        "branch": "main",
        ...
        async def test_project_deploy_with_no_syncmatrix_yaml_git_repo_user_overrides(
            self, work_pool, syncmatrix_client
        ):
            await run_sync_in_worker_thread(
                invoke_and_assert,
        ...
        async def test_project_deploy_with_no_syncmatrix_yaml_git_repo_with_token(
            self,
            work_pool,
            syncmatrix_client,
        ):
            ...
            assert deployment.pull_steps == [
                {
                    "syncmatrix.deployments.steps.git_clone": {
                        ...
                        "branch": "main",
                        "access_token": (
                            "{{ syncmatrix.blocks.secret.deployment-test-name-an-important-name-repo-token }}"
                        ),
                    }
                }
            ]
        ...
        async def test_deploy_with_blob_storage_select_existing_credentials(
            ...
            monkeypatch.setattr(
                "syncmatrix.deployments.steps.core.import_object", lambda x: mock_step
            )
            monkeypatch.setattr(
                "syncmatrix.deployments.steps.core.import_module",
                lambda x: None,
            ...
            assert deployment.pull_steps == [
                {
                    "syncmatrix_aws.deployments.steps.pull_from_s3": {
                        "bucket": "my-bucket",
                        "folder": "",
                        "credentials": (
                            "{{ syncmatrix.blocks.aws-credentials.bezos-creds }}"
                        ),
                    }
                }
            ]
            ...
            assert deployment.pull_steps == [
                {
                    "syncmatrix_aws.deployments.steps.pull_from_s3": {
                        ...
                        "folder": "",
                        "credentials": (
                            "{{ syncmatrix.blocks.aws-credentials.s3-storage-credentials }}"
                        ),
                    }
                }
            ]
        ...
        async def test_build_docker_image_step_auto_build_dockerfile(
            ...
            syncmatrix_yaml = {
                "build": [
                    {
                        "syncmatrix_docker.deployments.steps.build_docker_image": {
                            "requires": "syncmatrix-docker",
            ...
            await run_sync_in_worker_thread(
                ...
                expected_output_contains=[
                    "syncmatrix deployment run 'An important name/test-name'"
                ],
            )

            syncmatrix_file = Path("syncmatrix.yaml")
            assert syncmatrix_file.exists()

            with open(syncmatrix_file, "r") as f:
                config = yaml.safe_load(f)
            ...
            assert config["deployments"][0]["pull"] == [
                {
                    "syncmatrix.deployments.steps.set_working_directory": {
                        "directory": f"/opt/syncmatrix/{dir_name}"
                    }
                }
            ]
            ...
            # check to make sure syncmatrix-docker is not installed
            with pytest.raises(ImportError):
                import syncmatrix_docker  # noqa
        ...
        async def test_build_docker_image_step_custom_dockerfile_remote_flow_code_reject(
            ...
            assert config["deployments"][0]["pull"] == [
                {
                    "syncmatrix.deployments.steps.set_working_directory": {
                        "directory": "/opt/syncmatrix/hello-projects/"
                    }
                }
            ]
    ...
    class TestGeneratedPushAction:
        ...
        async def test_deploy_select_blob_storage_configures_push_step(
            ...
            assert config["push"] == [
                {
                    "syncmatrix_aws.deployments.steps.push_to_s3": {
                        "bucket": "my-bucket",
                        "folder": "",
                        "credentials": (
                            "{{ syncmatrix.blocks.aws-credentials.bezos-creds }}"
    ...
    async def test_project_deploy_with_empty_dep_file(
        self, project_dir, syncmatrix_client, work_pool
    ):
    ...
    async def test_project_deploy_templates_values(self, work_pool, syncmatrix_client):
        # prepare a templated deployment
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            contents = yaml.safe_load(f)
    ...
    async def test_project_deploy_templates_env_var_values(
        self, syncmatrix_client, work_pool, monkeypatch
    ):
        # prepare a templated deployment
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
        ...
        # update syncmatrix.yaml to include some new build steps
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            syncmatrix_config = yaml.safe_load(f)
        ...
        syncmatrix_config["build"] = [
            {
                "syncmatrix.deployments.steps.run_shell_script": {
                    "id": "get-dir",
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_project_deploy_with_default_parameters(
        self, syncmatrix_client, work_pool
    ):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            deploy_config = yaml.safe_load(f)
    ...
    async def test_project_deploy_with_default_parameters_from_cli(
        self, project_dir, syncmatrix_client, work_pool, option
    ):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            config = yaml.safe_load(f)
    ...
    async def test_project_deploy_templates_pull_step_safely(
        ...
        syncmatrix_config["pull"] = [
            {
                "syncmatrix.testing.utilities.b_test_step": {
                    "id": "b-test-step",
                    "input": "{{ output1 }}",
                    "secret-input": "{{ syncmatrix.blocks.secret.test-secret }}",
                },
    ...
    async def test_project_deploy_templates_pull_step_in_deployments_section_safely(
        ...
        syncmatrix_config["deployments"][0]["pull"] = [
            {
                "syncmatrix.testing.utilities.b_test_step": {
                    "id": "b-test-step",
                    "input": "{{ output1 }}",
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_project_deploy_reads_flow_name_from_syncmatrix_yaml(self, work_pool):
        await register_flow("flows/hello.py:my_flow")
        create_default_syncmatrix_yaml(".")
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_project_deploy_reads_entrypoint_from_syncmatrix_yaml(self, work_pool):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            deploy_config = yaml.safe_load(f)
    ...
    async def test_deploy_without_name_interactive(self, work_pool, syncmatrix_client):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=(
    ...
    async def test_deploy_without_work_pool_non_interactive(self):
        await run_sync_in_worker_thread(
            ...
            expected_output_contains=[
                ...
                " work pool name via the '--pool' flag or in your syncmatrix.yaml file."
            ],
        )
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_deploy_with_syncmatrix_agent_work_pool_non_interactive(
        ...
        await run_sync_in_worker_thread(
            ...
            expected_output_contains=(
                ...
                " 'syncmatrix-agent'. If you wish to use an agent with your deployment,"
                " please use the `syncmatrix deployment build` command."
            ),
        )

    @pytest.mark.usefixtures("interactive_console", "project_dir")
    async def test_deploy_with_syncmatrix_agent_work_pool_interactive(
        self, work_pool, syncmatrix_client, default_agent_pool
    ):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=(
            ...
            expected_output_contains=[
                (
                    "You've chosen a work pool with type 'syncmatrix-agent' which cannot"
    ...
    async def test_deploy_with_push_pool_no_worker_start_message(self, push_work_pool):
        await run_sync_in_worker_thread(
            ...
            expected_output_does_not_contain=[
                f"$ syncmatrix worker start --pool {push_work_pool.name!r}",
            ],
        )

    @pytest.mark.usefixtures("interactive_console", "project_dir")
    async def test_deploy_with_no_available_work_pool_interactive(self, syncmatrix_client):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command="deploy ./flows/hello.py:my_flow -n test-name --interval 3600",
            expected_code=0,
    ...
    async def test_migrate_from_deployment_yaml_to_syncmatrix_yaml(self, work_pool):
        ...
        await run_sync_in_worker_thread(
            ...
            expected_output_contains=[
                ...
                " syncmatrix.yaml file! Once you've verified that all your deployment"
                " configurations in your syncmatrix.yaml file are correct, you can delete"
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_deploy_with_entrypoint_does_not_fail_with_missing_syncmatrix_folder(
        self, work_pool
    ):
        Path(".syncmatrix").rmdir()
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=f"deploy ./flows/hello.py:my_flow -n test-name -p {work_pool.name}",
            expected_code=0,
    ...
    async def test_deploy_does_not_prompt_schedule_when_empty_schedule_syncmatrix_yaml(
        self, schedule_value, work_pool, syncmatrix_client
    ):
        syncmatrix_yaml_file = Path("syncmatrix.yaml")
        with syncmatrix_yaml_file.open(mode="r") as f:
            deploy_config = yaml.safe_load(f)
        ...
        with syncmatrix_yaml_file.open(mode="w") as f:
            yaml.safe_dump(deploy_config, f)
    ...
    @pytest.mark.usefixtures("project_dir", "interactive_console")
    async def test_deploy_does_not_prompt_build_docker_image_when_empty_build_action_syncmatrix_yaml(
        self, build_value, work_pool, syncmatrix_client
    ):
        syncmatrix_yaml_file = Path("syncmatrix.yaml")
        with syncmatrix_yaml_file.open(mode="r") as f:
            deploy_config = yaml.safe_load(f)
        ...
        assert await syncmatrix_client.read_deployment_by_name(
            "An important name/test-name"
        )
    ...
    async def test_deploy_templates_env_vars(
        self, syncmatrix_client, monkeypatch, work_pool
    ):
        # set up environment variables
        monkeypatch.setenv("WORK_POOL", work_pool.name)
        monkeypatch.setenv("MY_VAR", "my-value")

        # set up syncmatrix.yaml that has env var placeholders for the work pool name
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            syncmatrix_config = yaml.safe_load(f)

        syncmatrix_config["deployments"] = [
            {
        ...
        syncmatrix_config["build"] = [
            {"syncmatrix.testing.utilities.a_test_step": {"input": "{{ $MY_VAR }}"}}
        ]

        # save config to syncmatrix.yaml
        with syncmatrix_file.open(mode="w") as f:
            yaml.safe_dump(syncmatrix_config, f)
        ...
        result = await run_sync_in_worker_thread(
            ...
            expected_output_does_not_contain=(
                ...
                (
                    ...
                    " create a work pool in the Syncmatrix UI."
                ),
            ),
        )
        assert result.exit_code == 0

        deployments = await syncmatrix_client.read_deployments()
        ...
        assert (
            config["build"][0]["syncmatrix.testing.utilities.a_test_step"]["input"]
            == "{{ $MY_VAR }}"
        )
...
class TestSchedules:
    @pytest.mark.usefixtures("project_dir")
    async def test_passing_cron_schedules_to_deploy(self, work_pool, syncmatrix_client):
        result = await run_sync_in_worker_thread(
            invoke_and_assert,
            command=(
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_deployment_yaml_cron_schedule(self, work_pool, syncmatrix_client):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            deploy_config = yaml.safe_load(f)
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_interval_schedule_deployment_yaml(self, syncmatrix_client, work_pool):
        syncmatrix_yaml = Path("syncmatrix.yaml")
        with syncmatrix_yaml.open(mode="r") as f:
            deploy_config = yaml.safe_load(f)
        ...
        with syncmatrix_yaml.open(mode="w") as f:
            yaml.safe_dump(deploy_config, f)
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_rrule_deployment_yaml(self, work_pool, syncmatrix_client):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            deploy_config = yaml.safe_load(f)
    ...
    @pytest.mark.usefixtures("interactive_console", "project_dir")
    async def test_deploy_cron_schedule_interactive(self, syncmatrix_client, work_pool):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=(
                f"deploy ./flows/hello.py:my_flow -n test-name --pool {work_pool.name}"
    ...
    @pytest.mark.usefixtures("interactive_console", "project_dir")
    async def test_deploy_rrule_schedule_interactive(self, syncmatrix_client, work_pool):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=(
                f"deploy ./flows/hello.py:my_flow -n test-name --pool {work_pool.name}"
    ...
    @pytest.mark.usefixtures("interactive_console", "project_dir")
    async def test_deploy_no_schedule_interactive(self, syncmatrix_client, work_pool):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=(
                f"deploy ./flows/hello.py:my_flow -n test-name --pool {work_pool.name}"
    ...
    async def test_deploy_no_schedule_interactive_with_ci_flag(
        ...
        await run_sync_in_worker_thread(
            ...
            expected_output_contains=(
                ...
                " Dec 2023. Please use the global `--no-prompt` flag instead: `syncmatrix"
                " --no-prompt deploy`."
            ),
        )
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_deploy_with_inactive_schedule(self, work_pool, syncmatrix_client):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            deploy_config = yaml.safe_load(f)
...
class TestMultiDeploy:
    @pytest.mark.usefixtures("project_dir")
    async def test_deploy_all(self, syncmatrix_client, work_pool):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            contents = yaml.safe_load(f)
        ...
        deployment1 = await syncmatrix_client.read_deployment_by_name(
            "An important name/test-name-1"
        )
        deployment2 = await syncmatrix_client.read_deployment_by_name(
            "An important name/test-name-2"
        )
    ...
    async def test_deploy_selected_deployments(
        ...
        with pytest.raises(ObjectNotFound):
            await syncmatrix_client.read_deployment_by_name(
    ...
    async def test_deploy_multiple_with_cli_options(
        self, project_dir, syncmatrix_client, work_pool, deployment_selector_options
    ):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            contents = yaml.safe_load(f)
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_deploy_without_name_in_syncmatrix_yaml(self, syncmatrix_client, work_pool):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            contents = yaml.safe_load(f)
    ...
    @pytest.mark.usefixtures("interactive_console", "project_dir")
    async def test_deploy_without_name_in_syncmatrix_yaml_interactive(
        self, syncmatrix_client, work_pool
    ):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
    ...
    @pytest.mark.usefixtures("interactive_console", "project_dir")
    async def test_deploy_without_name_in_syncmatrix_yaml_interactive_user_skips(
        self, syncmatrix_client: SyncmatrixClient, work_pool
    ):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            contents = yaml.safe_load(f)
        ...
        assert len(await syncmatrix_client.read_deployments()) == 1

    async def test_deploy_with_name_not_in_syncmatrix_yaml(
        self, project_dir, syncmatrix_client, work_pool
    ):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
    ...
    async def test_deploy_existing_deployment_and_nonexistent_deployment_deploys_former(
        self, deploy_names, project_dir, syncmatrix_client, work_pool
    ):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            contents = yaml.safe_load(f)
...
class TestDeployPattern:
    ...
    async def test_pattern_deploy_multiple_existing_deployments(
        self, deploy_name, project_dir, syncmatrix_client, work_pool
    ):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            contents = yaml.safe_load(f)
    ...
    async def test_pattern_deploy_one_existing_deployment_one_nonexistent_deployment(
        self, project_dir, syncmatrix_client, work_pool, deploy_name
    ):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            contents = yaml.safe_load(f)
    ...
    async def test_deploy_multiple_nonexistent_deployments_raises(
        self, deploy_names, work_pool, syncmatrix_client
    ):
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            contents = yaml.safe_load(f)
...
class TestSaveUserInputs:
    def test_save_user_inputs_no_existing_syncmatrix_file(self):
        syncmatrix_file = Path("syncmatrix.yaml")
        syncmatrix_file.unlink()
        assert not syncmatrix_file.exists()
        ...
        invoke_and_assert(
            ...
            expected_output_contains=[
                ...
                "Deployment configuration saved to syncmatrix.yaml",
            ],
        )
    ...
    def test_save_user_inputs_existing_syncmatrix_file(self):
        syncmatrix_file = Path("syncmatrix.yaml")
        assert syncmatrix_file.exists()
    ...
    def test_save_user_inputs_with_interval_schedule(self):
        ...
        with open("syncmatrix.yaml", mode="r") as f:
            config = yaml.safe_load(f)
    ...
    def test_deploy_existing_deployment_with_no_changes_does_not_prompt_save(self):
        ...
        # assert that the deployment was updated in the syncmatrix.yaml
        with open("syncmatrix.yaml", mode="r") as f:
            config = yaml.safe_load(f)
    ...
    async def test_save_user_inputs_with_actions(self):
        ...
        build_steps = [
            {
                "syncmatrix.steps.set_working_directory": {
    ...
    def test_save_new_deployment_with_same_name_as_existing_deployment_overwrites(self):
        # Set up initial 'syncmatrix.yaml' file with a deployment
        initial_deployment = {
        ...
        _save_deployment_to_syncmatrix_file(initial_deployment)
        ...
        _save_deployment_to_syncmatrix_file(new_deployment)
    ...
    async def test_deploy_resolves_block_references_in_deployments_section(
        ...
        """
        Ensure block references are resolved in deployments section of syncmatrix.yaml
        """
        await JSON(value={"work_pool_name": work_pool.name}).save(
            name="test-json-block"
        )

        # add block reference to syncmatrix.yaml
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            syncmatrix_config = yaml.safe_load(f)
        ...
        syncmatrix_config["deployments"] = [
            {
                ...
                "work_pool": {
                    "name": (
                        "{{ syncmatrix.blocks.json.test-json-block.value.work_pool_name }}"
                    ),
                },
            }
        ]
        ...
        assert (
            syncmatrix_config["deployments"][0]["work_pool"]["name"]
            == "{{ syncmatrix.blocks.json.test-json-block.value.work_pool_name }}"
        )
        ...
        assert syncmatrix_config["deployments"][0]["work_pool"]["name"] == work_pool.name
    ...
    async def test_deploy_resolves_variables_in_deployments_section(
        ...
        """
        Ensure deployments section of syncmatrix.yaml placeholders are resolved
        """
        # create variable
        await syncmatrix_client._client.post(
            "/variables/", json={"name": "my_work_pool", "value": work_pool.name}
        )

        # add variable to deployments section of syncmatrix.yaml
        syncmatrix_file = Path("syncmatrix.yaml")
        with syncmatrix_file.open(mode="r") as f:
            syncmatrix_config = yaml.safe_load(f)
        ...
        syncmatrix_config["deployments"] = [
            {
                ...
                "entrypoint": "flows/hello.py:my_flow",
                "work_pool": {
                    "name": "{{ syncmatrix.variables.my_work_pool }}",
                },
            }
        ]
        ...
        assert (
            syncmatrix_config["deployments"][0]["work_pool"]["name"]
            == "{{ syncmatrix.variables.my_work_pool }}"
        )
        ...
        # ensure variable is resolved in syncmatrix.yaml
        with syncmatrix_file.open(mode="r") as f:
            syncmatrix_config = yaml.safe_load(f)
...
class TestDeployWithoutEntrypoint:
    async def test_deploy_without_entrypoint(self, syncmatrix_client: SyncmatrixClient):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command="deploy",
            user_input=(
...
class TestCheckForMatchingDeployment:
    async def test_matching_deployment_in_syncmatrix_file_returns_true(self):
        deployment = {
            "name": "existing_deployment",
            "entrypoint": "flows/existing_flow.py:my_flow",
            "schedule": None,
        ...
        _save_deployment_to_syncmatrix_file(deployment)
    ...
    async def test_no_matching_deployment_in_syncmatrix_file_returns_false(self):
        deployment = {
            "name": "existing_deployment",
            "entrypoint": "flows/existing_flow.py:my_flow",
            "schedule": None,
    ...
    class TestDeploymentTriggerSyncing:
        async def test_initialize_named_deployment_triggers(self):
            trigger_spec = {
                ...
                "enabled": True,
                "match": {"syncmatrix.resource.id": "syncmatrix.flow-run.*"},
                "expect": ["syncmatrix.flow-run.Completed"],
                "match_related": {
                    "syncmatrix.resource.name": "seed",
        ...
        async def test_create_deployment_triggers(self):
            ...
            client.delete_resource_owned_automations.assert_called_once_with(
                f"syncmatrix.deployment.{deployment_id}"
            )
        ...
        async def test_triggers_creation_orchestrated(
            ...
            with mock.patch(
                "syncmatrix.cli.deploy._create_deployment_triggers",
                AsyncMock(),
            ) as create_triggers:
                await run_sync_in_worker_thread(
                    invoke_and_assert,
        ...
        async def test_deploy_command_warns_triggers_not_created_not_cloud(
            ...
            await run_sync_in_worker_thread(
                ...
                expected_code=0,
                expected_output_contains=[
                    "Deployment triggers are only supported on Syncmatrix Cloud"
                ],
            )
    ...
    class TestDeploymentTriggerPassedViaCLI:
        ...
        async def test_multiple_trigger_flags(self, docker_work_pool):
            ...
            trigger_spec_2 = {
                "enabled": False,
                "match": {"syncmatrix.resource.id": "syncmatrix.flow-run.*"},
                "expect": ["syncmatrix.flow-run.Failed"],
            }
            ...
            contents["deployments"] = [
                {
                    ...
                    "triggers": [
                        {**cli_trigger_spec, "expect": ["syncmatrix.flow-run.Completed"]}
                    ],
                }
            ]
        ...
        @pytest.mark.usefixtures("interactive_console", "project_dir")
        async def test_triggers_saved_to_syncmatrix_yaml(self, docker_work_pool):
            client = AsyncMock()
            client.server_type = ServerType.CLOUD
            ...
            # Read the updated syncmatrix.yaml
            syncmatrix_file = Path("syncmatrix.yaml")
            with syncmatrix_file.open(mode="r") as f:
                contents = yaml.safe_load(f)
    ...
    async def test_prompt_build_custom_docker_image_accepted_use_existing_dockerfile_accepted(
        ...
        result = await run_sync_in_worker_thread(
            ...
            expected_output_contains=[
                ...
                "Image khulnasoft/syncmatrix/test-name:latest will be built",
        ...
        assert config["deployments"][1]["build"] == [
            {
                "syncmatrix_docker.deployments.steps.build_docker_image": {
                    "id": "build-image",
                    "requires": "syncmatrix-docker>=0.3.1",
                    "dockerfile": "Dockerfile",
                    "image_name": "khulnasoft/syncmatrix/test-name",
                    "tag": "latest",
                }
            }
        ]
...
class TestDeployInfraOverrides:
    @pytest.fixture
    async def work_pool(self, syncmatrix_client):
        await syncmatrix_client.create_work_pool(
            WorkPoolCreate(name="test-pool", type="test")
        )

    async def test_uses_infra_overrides(self, project_dir, work_pool, syncmatrix_client):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command=(
                "deploy ./flows/hello.py:my_flow -n test-name -p test-pool --version"
...
class TestDeployDockerPushSteps:
    ...
    async def test_prompt_push_custom_docker_image_accepted_public_registry(
        ...
        result = await run_sync_in_worker_thread(
            ...
            expected_output_does_not_contain=[
                "Would you like use syncmatrix-docker to manage Docker registry"
                " credentials?"
            ],
        )
        ...
        assert config["deployments"][1]["build"] == [
            {
                "syncmatrix_docker.deployments.steps.build_docker_image": {
                    ...
                    "requires": "syncmatrix-docker>=0.3.1",
                    "dockerfile": "auto",
                    "image_name": "https://hub.docker.com/khulnasoft/syncmatrix/test-name",
                    "tag": "latest",
                }
            }
        ]
        ...
        assert config["deployments"][1]["push"] == [
            {
                "syncmatrix_docker.deployments.steps.push_docker_image": {
                    "requires": "syncmatrix-docker>=0.3.1",
    ...
    async def test_prompt_push_docker_image_accepted_private_registry_use_existing_core_creds(
        ...
        result = await run_sync_in_worker_thread(
            ...
            expected_output_does_not_contain=["Installing syncmatrix-docker..."],
        )
        ...
        assert config["deployments"][1]["build"] == [
            {
                "syncmatrix_docker.deployments.steps.build_docker_image": {
                    ...
                    "dockerfile": "auto",
                    "image_name": (
                        "https://private.docker.com/khulnasoft/syncmatrix/test-name"
                    ),
                    "tag": "latest",
                }
            }
        ...
        assert config["deployments"][1]["push"] == [
            {
                "syncmatrix_docker.deployments.steps.push_docker_image": {
                    ...
                    "tag": "{{ build-image.tag }}",
                    "credentials": (
                        "{{ syncmatrix.docker-registry.docker_registry_creds_name }}"
                    ),
                }
            }
        ]
        ...
        assert config["deployments"][1]["build"] == [
            {
                "syncmatrix_docker.deployments.steps.build_docker_image": {
                    ...
                    "dockerfile": "auto",
                    "image_name": (
                        "https://private2.docker.com/khulnasoft/syncmatrix/test-name"
                    ),
                    "tag": "latest",
                }
            }
...
class TestDeployingUsingCustomSyncmatrixFile:
    def customize_from_existing_syncmatrix_file(
        self,
        existing_file: Path,
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_deploying_using_custom_syncmatrix_file(
        self, syncmatrix_client: SyncmatrixClient, work_pool: WorkPool
    ):
        # Create and use a temporary syncmatrix.yaml file
        with tempfile.NamedTemporaryFile("w+") as fp:
            self.customize_from_existing_syncmatrix_file(
                Path("syncmatrix.yaml"), fp, work_pool
            )

            await run_sync_in_worker_thread(
                invoke_and_assert,
                command=f"deploy --all --syncmatrix-file {fp.name}",
                expected_code=0,
                user_input=(
                    # decline remote storage
                    "n"
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_deploying_using_missing_syncmatrix_file(self):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command="deploy --all --syncmatrix-file THIS_FILE_DOES_NOT_EXIST",
            expected_code=1,
            expected_output_contains=[
    ...
    async def test_deploying_using_malformed_syncmatrix_file(self, content: str):
        with tempfile.NamedTemporaryFile("w+") as fp:
            fp.write(content)
    ...
    @pytest.mark.usefixtures("project_dir")
    async def test_deploying_directory_as_syncmatrix_file(self):
        await run_sync_in_worker_thread(
            invoke_and_assert,
            command="deploy --all --syncmatrix-file ./",
            expected_code=1,
            expected_output_contains=[
