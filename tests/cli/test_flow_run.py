...
from syncmatrix.cli.flow_run import LOGS_WITH_LIMIT_FLAG_DEFAULT_NUM_LOGS
...
@sync_compatible
async def assert_flow_run_is_deleted(syncmatrix_client, flow_run_id: UUID):
    """
    ...
    with pytest.raises(syncmatrix.exceptions.ObjectNotFound):
        await syncmatrix_client.read_flow_run(flow_run_id)
...
@pytest.fixture
async def scheduled_flow_run(syncmatrix_client):
    return await syncmatrix_client.create_flow_run(
        name="scheduled_flow_run", flow=hello_flow, state=Scheduled()
    )


@pytest.fixture
async def completed_flow_run(syncmatrix_client):
    return await syncmatrix_client.create_flow_run(
...
@pytest.fixture
async def running_flow_run(syncmatrix_client):
    return await syncmatrix_client.create_flow_run(
...
@pytest.fixture
async def late_flow_run(syncmatrix_client):
    return await syncmatrix_client.create_flow_run(
...
def test_delete_flow_run_succeeds(syncmatrix_client, flow_run):
    invoke_and_assert(
    ...
    assert_flow_run_is_deleted(syncmatrix_client, flow_run.id)
...
class TestCancelFlowRun:
    ...
    async def test_non_terminal_states_set_to_cancelling(self, syncmatrix_client, state):
        ...
        """
        before = await syncmatrix_client.create_flow_run(
        ...
        after = await syncmatrix_client.read_flow_run(before.id)
    ...
    async def test_scheduled_states_set_to_cancelled(self, syncmatrix_client, state):
...
@pytest.fixture()
def flow_run_factory(syncmatrix_client):
...
class TestFlowRunExecute:
    ...
    async def test_execute_flow_run_via_argument(self, syncmatrix_client: SyncmatrixClient):
    ...
    async def test_execute_flow_run_via_environment_variable(
        ...
        monkeypatch.setenv("SYNCMATRIX__FLOW_RUN_ID", str(flow_run.id))
