...
from syncmatrix.client.schemas.actions import WorkPoolUpdate
from syncmatrix.client.schemas.objects import WorkPool
from syncmatrix.context import get_settings_context
from syncmatrix.exceptions import ObjectNotFound
from syncmatrix.settings import SYNCMATRIX_DEFAULT_WORK_POOL_NAME, load_profile
...
from syncmatrix.workers.base import BaseWorker
from syncmatrix.workers.process import ProcessWorker
...
def interactive_console(monkeypatch):
    monkeypatch.setattr("syncmatrix.cli.work_pool.is_interactive", lambda: True)
...
class TestCreate:
    ...
    async def test_create_work_pool(self, syncmatrix_client, mock_collection_registry):
        pool_name = "my-pool"
        res = await run_sync_in_worker_thread(
            invoke_and_assert,
            f"work-pool create {pool_name} -t syncmatrix-agent",
        )
        assert res.exit_code == 0
        assert f"Created work pool {pool_name!r}" in res.output
        client_res = await syncmatrix_client.read_work_pool(pool_name)
    ...
    async def test_create_work_pool_with_base_job_template(
        self, syncmatrix_client, mock_collection_registry
    ):
    ...
    async def test_create_work_pool_with_empty_name(
        ...
        await run_sync_in_worker_thread(
            invoke_and_assert,
            "work-pool create '' -t syncmatrix-agent",
            expected_code=1,
    ...
    @pytest.mark.usefixtures("mock_collection_registry")
    async def test_default_template(self, syncmatrix_client):
        pool_name = "my-pool"
        res = await run_sync_in_worker_thread(
            invoke_and_assert,
            f"work-pool create {pool_name} -t syncmatrix-agent",
    ...
    @pytest.mark.usefixtures("mock_collection_registry")
    async def test_default_paused(self, syncmatrix_client):
        pool_name = "my-pool"
        res = await run_sync_in_worker_thread(
            invoke_and_assert,
            f"work-pool create {pool_name} -t syncmatrix-agent",
    ...
    @pytest.mark.usefixtures("mock_collection_registry")
    async def test_paused_true(self, syncmatrix_client):
        pool_name = "my-pool"
        res = await run_sync_in_worker_thread(
            invoke_and_assert,
            f"work-pool create {pool_name} --paused -t syncmatrix-agent",
        )
        assert res.exit_code == 0
        client_res = await syncmatrix_client.read_work_pool(pool_name)
    ...
    @pytest.mark.usefixtures("mock_collection_registry")
    async def test_create_work_pool_from_registry(self, syncmatrix_client):
        pool_name = "fake-work"
        res = await run_sync_in_worker_thread(
            invoke_and_assert,
    ...
    @pytest.mark.usefixtures("mock_collection_registry")
    async def test_create_process_work_pool(self, syncmatrix_client):
    ...
    async def test_create_interactive_first_type(self, syncmatrix_client):
        ...
        client_res = await syncmatrix_client.read_work_pool(work_pool_name)
        assert client_res.name == work_pool_name
        assert client_res.type == "syncmatrix-agent"
        assert isinstance(client_res, WorkPool)

    @pytest.mark.usefixtures("interactive_console", "mock_collection_registry")
    async def test_create_interactive_second_type(self, syncmatrix_client):
        work_pool_name = "test-interactive"
        await run_sync_in_worker_thread(
            invoke_and_assert,
            ["work-pool", "create", work_pool_name],
    ...
    @pytest.mark.usefixtures("mock_collection_registry")
    async def test_create_set_as_default(self, syncmatrix_client):
        settings_context = get_settings_context()
        assert (
            settings_context.profile.settings.get(SYNCMATRIX_DEFAULT_WORK_POOL_NAME)
            is None
        )
        pool_name = "my-pool"
        res = await run_sync_in_worker_thread(
        ...
        assert profile.settings.get(SYNCMATRIX_DEFAULT_WORK_POOL_NAME) == pool_name
    ...
    async def test_create_with_provision_infra(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "syncmatrix.cli.work_pool.get_infrastructure_provisioner_for_work_pool_type",
    ...
    async def test_create_prompt_table_only_displays_push_pool_types_using_provision_infra_flag(
        self, syncmatrix_client, monkeypatch
    ):
        mock_provision = AsyncMock()
...
class TestInspect:
    async def test_inspect(self, syncmatrix_client, work_pool):
        res = await run_sync_in_worker_thread(
            invoke_and_assert,
...
class TestPause:
    async def test_pause(self, syncmatrix_client, work_pool):
        ...
        assert res.exit_code == 0
        client_res = await syncmatrix_client.read_work_pool(work_pool.name)
        assert client_res.is_paused is True


class TestSetConcurrencyLimit:
    async def test_set_concurrency_limit(self, syncmatrix_client, work_pool):
...
class TestClearConcurrencyLimit:
    async def test_clear_concurrency_limit(self, syncmatrix_client, work_pool):
        await syncmatrix_client.update_work_pool(
            work_pool_name=work_pool.name,
            work_pool=WorkPoolUpdate(concurrency_limit=10),
        )
        work_pool = await syncmatrix_client.read_work_pool(work_pool.name)
...
class TestResume:
    async def test_resume(self, syncmatrix_client, work_pool):
        assert work_pool.is_paused is False
...
class TestDelete:
    async def test_delete(self, syncmatrix_client, work_pool):
        res = await run_sync_in_worker_thread(
            invoke_and_assert,
        ...
        with pytest.raises(ObjectNotFound):
            await syncmatrix_client.read_work_pool(work_pool.name)


class TestLS:
    async def test_ls(self, syncmatrix_client, work_pool):
        res = await run_sync_in_worker_thread(
            invoke_and_assert,
            "work-pool ls",
        )
    ...
    async def test_verbose(self, syncmatrix_client, work_pool):
        res = await run_sync_in_worker_thread(
            invoke_and_assert,
...
class TestUpdate:
    async def test_update_description(self, syncmatrix_client, work_pool):
    ...
    async def test_update_concurrency_limit(self, syncmatrix_client, work_pool):
        assert work_pool.description is None
        assert work_pool.type is not None
        assert work_pool.base_job_template is not None
        assert work_pool.is_paused is not None
    ...
    async def test_update_base_job_template(self, syncmatrix_client, work_pool):
        assert work_pool.description is None
        assert work_pool.type is not None
        assert work_pool.base_job_template is not None
        assert work_pool.is_paused is not None
    ...
    async def test_update_multi(self, syncmatrix_client, work_pool):
        assert work_pool.description is None
        assert work_pool.type is not None
        assert work_pool.base_job_template is not None
        assert work_pool.is_paused is not None
...
class TestPreview:
    async def test_preview(self, syncmatrix_client, work_pool):
        res = await run_sync_in_worker_thread(
            invoke_and_assert,
...
class TestProvisionInfrastructure:
    async def test_provision_infra(self, monkeypatch, push_work_pool, syncmatrix_client):
        client_res = await syncmatrix_client.read_work_pool(push_work_pool.name)
        assert client_res.base_job_template != FAKE_DEFAULT_BASE_JOB_TEMPLATE
