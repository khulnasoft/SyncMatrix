...
@sync_compatible
async def read_queue(syncmatrix_client, name, pool=None):
    return await syncmatrix_client.read_work_queue_by_name(name=name, work_pool_name=pool)
...
class TestCreateWorkQueue:
    ...
    def test_create_work_queue_without_pool_uses_default_pool(self, syncmatrix_client):
        queue_name = "q-name"
        invoke_and_assert(
...
class TestSetConcurrencyLimit:
    def test_set_concurrency_limit(self, syncmatrix_client, work_queue):
        assert work_queue.concurrency_limit is None
        invoke_and_assert(
            command=f"work-queue set-concurrency-limit {work_queue.name} 5",
            expected_code=0,
        )
        q = read_queue(syncmatrix_client, work_queue.name)
        assert q.concurrency_limit == 5

    def test_set_concurrency_limit_by_id(self, syncmatrix_client, work_queue):
        assert work_queue.concurrency_limit is None
        invoke_and_assert(
    ...
    def test_set_concurrency_limit_with_pool_with_name(
        ...
        q = read_queue(
            syncmatrix_client, work_queue_1.name, pool=work_queue_1.work_pool.name
        )
        assert q.concurrency_limit == 5
...
class TestClearConcurrencyLimit:
    def test_clear_concurrency_limit(self, syncmatrix_client, work_queue):
        invoke_and_assert(
            command=f"work-queue set-concurrency-limit {work_queue.name} 5"
        )
        invoke_and_assert(
    ...
    def test_clear_concurrency_limit_by_id(self, syncmatrix_client, work_queue):
        invoke_and_assert(
            command=f"work-queue set-concurrency-limit {work_queue.name} 5"
        )
        invoke_and_assert(
    ...
    async def test_clear_concurrency_limit_with_pool(
        ...
        await syncmatrix_client.update_work_queue(id=work_queue_1.id, concurrency_limit=5)
        ...
        q = await read_queue(syncmatrix_client, work_queue_1.name, pool=pool_name)
        assert q.concurrency_limit is None
...
class TestPauseWorkQueue:
    def test_pause(self, syncmatrix_client, work_queue):
    ...
    def test_pause_by_id(self, syncmatrix_client, work_queue):
        assert not work_queue.is_paused
        invoke_and_assert(
...
class TestResumeWorkQueue:
    def test_resume(self, syncmatrix_client, work_queue):
    ...
    def test_resume_by_id(self, syncmatrix_client, work_queue):
        invoke_and_assert(command=f"work-queue pause {work_queue.name}")
        invoke_and_assert(
...
class TestDelete:
    def test_delete(self, syncmatrix_client, work_queue):
        invoke_and_assert(
            command=f"work-queue delete {work_queue.name}",
            expected_code=0,
        )
        with pytest.raises(syncmatrix.exceptions.ObjectNotFound):
            read_queue(syncmatrix_client, work_queue.name)

    def test_delete_by_id(self, syncmatrix_client, work_queue):
        invoke_and_assert(
    ...
    def test_delete_with_pool(
        ...
        with pytest.raises(syncmatrix.exceptions.ObjectNotFound):
            read_queue(syncmatrix_client, work_queue_1.name, pool=pool_name)
    ...
    def test_delete_with_bad_pool(
        ...
        assert read_queue(syncmatrix_client, work_queue_1.name, pool=pool_name)

    def test_delete_with_bad_queue_name(self, syncmatrix_client, work_queue):
        invoke_and_assert(
            command=f"work-queue delete {work_queue.name}bad",
            expected_code=1,
        )
        assert read_queue(syncmatrix_client, work_queue.name)
...
class TestReadRuns:
    @sync_compatible
    async def create_runs_in_queue(self, syncmatrix_client, queue, count: int):
        ...
        for _ in range(count):
            await syncmatrix_client.create_flow_run_from_deployment(deployment_id)

    def test_read_wq(self, syncmatrix_client, work_queue):
        n_runs = 3
        self.create_runs_in_queue(syncmatrix_client, work_queue, n_runs)
    ...
    def test_read_wq_with_pool(self, syncmatrix_client, work_queue):
        n_runs = 3
        self.create_runs_in_queue(syncmatrix_client, work_queue, n_runs)
        cmd = (
