...
from syncmatrix.exceptions import FlowRunWaitTimeout
from syncmatrix.states import Completed, Failed
...
@pytest.fixture
async def deployment_name(deployment, syncmatrix_client):
...
async def test_run_deployment_only_creates_one_flow_run(
    deployment_name: str, syncmatrix_client: syncmatrix.SyncmatrixClient, deployment
):
...
async def test_start_at_option_schedules_flow_run(
    ...
    syncmatrix_client: syncmatrix.SyncmatrixClient,
):
...
async def test_date_as_start_in_option_schedules_flow_run_equal_to_start_at(
    ...
    Example:
    `syncmatrix deployment run --start-in 12/20/2022` and
    `syncmatrix deployment run --start-at 12/20/2022`
...
async def test_run_deployment_watch(
    ...
    monkeypatch.setattr(
        "syncmatrix.cli.deployment.wait_for_flow_run", mock_wait_for_flow_run
    )
    ...
    assert len(flow_runs := await syncmatrix_client.read_flow_runs()) == 1
    flow_run = flow_runs[0]
    assert flow_run.deployment_id == deployment.id
