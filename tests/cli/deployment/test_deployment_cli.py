...
from syncmatrix.server.schemas.filters import DeploymentFilter, DeploymentFilterId
from syncmatrix.server.schemas.schedules import IntervalSchedule
from syncmatrix.settings import SYNCMATRIX_UI_URL, temporary_settings
...
def test_deployment_apply_prints_deprecation_warning(tmp_path, patch_import):
    ...
    invoke_and_assert(
        ...
        expected_output_contains=(
            ...
            "Use 'syncmatrix deploy' to deploy flows via YAML instead.",
        ),
    )
...
class TestOutputMessages:
    def test_message_with_work_queue_name_from_python_build(
        ...
        invoke_and_assert(
            ...
            expected_output_contains=[
                ...
                f"$ syncmatrix agent start -q {d.work_queue_name!r}",
            ],
        )

    def test_message_with_syncmatrix_agent_work_pool(
        self, patch_import, tmp_path, syncmatrix_agent_work_pool
    ):
        Deployment.build_from_flow(
            flow=my_flow,
            name="TEST",
            flow_name="my_flow",
            output=str(tmp_path / "test.yaml"),
            work_pool_name=syncmatrix_agent_work_pool.name,
        )
        invoke_and_assert(
            [
                "deployment",
            ...
            expected_output_contains=[
                (
                    ...
                    f" pulls work from the {syncmatrix_agent_work_pool.name!r} work pool:"
                ),
                f"$ syncmatrix agent start -p {syncmatrix_agent_work_pool.name!r}",
            ],
        )
    ...
    def test_message_with_process_work_pool(
        ...
        invoke_and_assert(
            ...
            expected_output_contains=[
                ...
                f"$ syncmatrix worker start -p {process_work_pool.name!r}",
            ],
        )
    ...
    def test_message_with_process_work_pool_without_workers_enabled(
        ...
        invoke_and_assert(
            ...
            expected_output_contains=[
                ...
                (
                    "$ syncmatrix config set SYNCMATRIX_EXPERIMENTAL_ENABLE_WORKERS=True\n"
                    f"$ syncmatrix worker start -p {process_work_pool.name!r}"
                ),
            ],
        )
    ...
    def test_linking_to_deployment_in_ui(
        ...
        with temporary_settings({SYNCMATRIX_UI_URL: "http://foo/bar"}):
            Deployment.build_from_flow(
                flow=my_flow,
                name="TEST",
                flow_name="my_flow",
        ...
        invoke_and_assert(
            ...
            expected_output_contains=(
                [
                    ...
                    "$ syncmatrix work-pool create 'gibberish'",
                ]
            ),
        )
...
class TestDeploymentSchedules:
    @pytest.fixture
    async def flojo(self, syncmatrix_client):
        @flow
        async def rence_griffith():
            pass

        flow_id = await syncmatrix_client.create_flow(rence_griffith)
    ...
    @pytest.fixture
    async def flojo_deployment(self, flojo, syncmatrix_client):
        return await syncmatrix_client.read_deployment(flojo)
    ...
    def test_pause_schedule_multiple_schedules_raises(self, flojo):
        ...
        invoke_and_assert(
            ...
            expected_output_contains=[
                ...
                " Use `syncmatrix deployment schedule pause <deployment_name>"
                " <schedule_id>`"
            ],
        )
...
class TestDeploymentRun:
    @pytest.fixture
    async def deployment_name(self, deployment, syncmatrix_client):
