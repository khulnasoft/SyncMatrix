...
def patch_import(monkeypatch):
    ...
    monkeypatch.setattr(
        "syncmatrix.cli.deployment.load_flow_from_entrypoint", lambda path: fn
    )
    return fn
...
def mock_build_from_flow(monkeypatch):
    ...
    monkeypatch.setattr(
        "syncmatrix.cli.deployment.Deployment.build_from_flow", mock_build_from_flow
    )

    # not needed for test
    monkeypatch.setattr(
        "syncmatrix.cli.deployment.create_work_queue_and_set_concurrency_limit",
        AsyncMock(),
    )
...
def mock_create_default_ignore_file(monkeypatch):
    ...
    monkeypatch.setattr(
        "syncmatrix.cli.deployment.create_default_ignore_file",
    ...
    def test_success_message_with_work_queue_name(self, patch_import, tmp_path):
        ...
        invoke_and_assert(
            ...
            expected_output_contains=[
                ...
                "$ syncmatrix agent start -q 'default'",
            ],
        )
    ...
    def test_message_with_syncmatrix_agent_work_pool(
        ...
        invoke_and_assert(
            [
                ...
                str(tmp_path / "test.yaml"),
                "-p",
                syncmatrix_agent_work_pool.name,
                "--apply",
            ],
            expected_output_contains=[
                (
...
class TestWorkQueueConcurrency:
    async def test_setting_work_queue_concurrency_limits_with_build(
        self, built_deployment_with_queue_and_limit_overrides, syncmatrix_client
    ):
        queue = await syncmatrix_client.read_work_queue_by_name(
            "the-queue-to-end-all-queues"
        )
        assert queue.concurrency_limit == 424242

    async def test_setting_work_queue_concurrency_limits_with_apply(
        self, applied_deployment_with_queue_and_limit_overrides, syncmatrix_client
    ):
        queue = await syncmatrix_client.read_work_queue_by_name("the-mother-of-all-queues")
