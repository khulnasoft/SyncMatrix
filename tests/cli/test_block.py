...
from syncmatrix.blocks import system
...
from syncmatrix.blocks.core import Bloc
...
def test_register_blocks_from_module_with_ui_url():
    with temporary_settings(set_defaults={SYNCMATRIX_UI_URL: "https://app.syncmatrix.cloud"}):
        invoke_and_assert(
            ["block", "register", "-m", "syncmatrix.blocks.core"],
            expected_code=0,
            expected_output_contains=[
                "Successfully registered",
                "blocks",
                "Syncmatrix UI: https://app.syncmatrix.cloud/blocks/catalog",
            ],
        )


def test_register_blocks_from_module_without_ui_url():
    with temporary_settings(set_defaults={SYNCMATRIX_UI_URL: None}):
        invoke_and_assert(
            ["block", "register", "-m", "syncmatrix.blocks.core"],
            expected_code=0,
            expected_output_contains=[
            ...
            expected_output_does_not_contain=["Syncmatrix UI: https://"],
        )


def test_register_blocks_from_nonexistent_module():
    invoke_and_assert(
        ["block", "register", "-m", "syncmatrix.blocks.blorp"],
        expected_code=1,
        expected_output_contains=(
            "Unable to load syncmatrix.blocks.blorp. Please make sure "
...
def test_register_blocks_from_invalid_module():
    invoke_and_assert(
        ["block", "register", "-m", "syncmatrix-aws.credentials"],
        expected_code=1,
        expected_output_contains=(
            "Unable to load syncmatrix-aws.credentials. Please make sure "
            "the module is installed in your current environment."
        ),
    )


def test_register_blocks_from_file(tmp_path, syncmatrix_client: SyncmatrixClient):
    ...
    block_type = asyncio.run(
        syncmatrix_client.read_block_type_by_slug(slug="testforfileregister")
    )
...
def test_register_fails_on_multiple_options():
    invoke_and_assert(
        ["block", "register", "-m", "syncmatrix.blocks.blorp", "-f", "fake_file.py"],
        expected_code=1,
        expected_output_contains=(
            "Please specify either a module or a file containing blocks to be"
            " registered, but not both."
...
def test_deleting_a_block_type(tmp_path, syncmatrix_client):
    test_file_path = tmp_path / "test.py"
    ...
    with pytest.raises(ObjectNotFound):
        asyncio.run(syncmatrix_client.read_block_type_by_slug(slug="testforfileregister"))


def test_deleting_a_protected_block_type(
    tmp_path, syncmatrix_client, install_system_block_types
):
