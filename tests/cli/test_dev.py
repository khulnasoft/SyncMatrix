...
import syncmatrix
from syncmatrix.cli.dev import agent_process_entrypoint, start_agent
...
def test_dev_start_runs_all_services(monkeypatch):
    """
    Test that `syncmatrix dev start` runs all services. This test mocks out the
    ...
    monkeypatch.setattr(syncmatrix.cli.dev, "run_process", mock_run_process)
    ...
    mock_kill = MagicMock()
    monkeypatch.setattr(syncmatrix.cli.dev.os, "kill", mock_kill)
...
def test_agent_subprocess_entrypoint_runs_agent_with_valid_params(monkeypatch):
    ...
    monkeypatch.setattr(syncmatrix.cli.dev, "start_agent", mock_agent_start)
        ...
        parameters=[
            inspect.Parameter(  # Typer.OptionInfo
                ...
                default=Option(
                    None,
                    "--api",
                    help="The URL of the Syncmatrix API server",
                ),
            ),
...
def test_agent_subprocess_entrypoint_adds_typer_console(monkeypatch):
    """
    Ensures a Rich console is added to the SyncmatrixTyper's global `app` instance.
    """
    start_agent_signature = inspect.signature(start_agent)
    ...
    mock_app = MagicMock()
    monkeypatch.setattr(syncmatrix.cli.dev, "app", mock_app)
