...
from syncmatrix.server import schemas as server_schemas
...
def interactive_console(monkeypatch):
    monkeypatch.setattr("syncmatrix.cli.project.is_interactive", lambda: True)
    ...
    deployment = await models.deployments.create_deployment(
        ...
        deployment=server_schemas.core.Deployment(
            ...
            pull_steps=[
                ...
                {
                    "syncmatrix.deployments.steps.git_clone": {
                        "repository": "https://github.com/KhulnaSoft/marvin.git"
                    }
                },
            ],
        ),
...
class TestProjectInit:
    def test_project_init(self):
        with TemporaryDirectory() as tempdir:
            ...
            assert result.exit_code == 0
            for file in ["syncmatrix.yaml", ".syncmatrixignore"]:
    ...
    def test_project_init_with_interactive_recipe(self):
        with TemporaryDirectory() as tempdir:
            ...
            syncmatrix_file = list(Path(tempdir).rglob("syncmatrix.yaml")).pop()
    ...
    def test_project_init_with_unknown_recipe(self):
        ...
        assert "syncmatrix init" in result.output
    ...
    def test_project_init_opt_out_of_recipe(self):
        with TemporaryDirectory() as tempdir:
            ...
            assert result.exit_code == 0
            assert any(Path(tempdir).rglob("syncmatrix.yaml"))
