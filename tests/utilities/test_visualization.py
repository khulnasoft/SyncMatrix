...
class TestTrackTaskRun:
    async def test_track_task_run_outside_ctx(self, monkeypatch):
        mock = Mock()
        monkeypatch.setattr(
            "syncmatrix.utilities.visualization.TaskVizTracker.add_task", mock
        )
        _track_viz_task(
            "my_task",
            {"a": 1},
...
class TestFlowVisualise:
    ...
    def test_visualize_does_not_raise(self, test_flow, monkeypatch):
        monkeypatch.setattr(
            "syncmatrix.flows.visualize_task_dependencies", MagicMock(return_value=None)
        )
    ...
    def test_visualize_graph_contents(self, test_flow, expected_nodes, monkeypatch):
        ...
        monkeypatch.setattr("syncmatrix.flows.visualize_task_dependencies", mock_visualize)
