...
from syncmatrix.blocks.system import JSON, DateTime, Secret, String
from syncmatrix.blocks.webhook import Webhook
...
class TestFindPlaceholders:
    ...
    def test_finds_block_document_placeholders(self):
        template = "Hello {{syncmatrix.blocks.document.name}}!"
        placeholders = find_placeholders(template)
        assert len(placeholders) == 1
        placeholder = placeholders.pop()
        assert placeholder.name == "syncmatrix.blocks.document.name"
...
class TestResolveBlockDocumentReferences:
    ...
    async def test_resolve_block_document_references_with_one_block_document_reference(
        self, syncmatrix_client, block_document_id
    ):
        assert {
            "key": {"a": 1, "b": "hello"}
    ...
    async def test_resolve_block_document_references_with_nested_block_document_references(
        ...
        block_document = await syncmatrix_client.read_block_document(block_document_id)

        result = await resolve_block_document_references(
            template, client=syncmatrix_client
        )
    ...
    async def test_resolve_block_document_references_with_dot_delimited_syntax(
        ...
        template = {"key": "{{ syncmatrix.blocks.arbitraryblock.arbitrary-block }}"}
    ...
    async def test_resolve_block_document_references_raises_on_multiple_placeholders(
        ...
        template = {
            "key": (
                "{{ syncmatrix.blocks.arbitraryblock.arbitrary-block }} {{"
                " another_placeholder }}"
            )
        }
        ...
        with pytest.raises(
            ...
            await resolve_block_document_references(template, client=syncmatrix_client)

    async def test_resolve_block_document_references_raises_on_extra_text(
        self, syncmatrix_client
    ):
        template = {
            "key": "{{ syncmatrix.blocks.arbitraryblock.arbitrary-block }} extra text"
        }
    ...
    async def test_resolve_block_document_unpacks_system_blocks(self):
        ...
        template = {
            "json": "{{ syncmatrix.blocks.json.json-block }}",
            "secret": "{{ syncmatrix.blocks.secret.secret-block }}",
            "datetime": "{{ syncmatrix.blocks.date-time.datetime-block }}",
            "string": "{{ syncmatrix.blocks.string.string-block }}",
        }
    ...
    async def test_resolve_block_document_system_block_resolves_dict_keypath(self):
        ...
        template = {
            "value": "{{ syncmatrix.blocks.json.nested-json-block}}",
            "keypath": "{{ syncmatrix.blocks.json.nested-json-block.key }}",
            "nested_keypath": "{{ syncmatrix.blocks.json.nested-json-block.key.nested-key }}",
        }
    ...
    async def test_resolve_block_document_resolves_dict_keypath(self):
        ...
        template = {
            "value": "{{ syncmatrix.blocks.json.nested-json-block-2.value }}",
            "keypath": "{{ syncmatrix.blocks.json.nested-json-block-2.value.key }}",
            "nested_keypath": (
                "{{ syncmatrix.blocks.json.nested-json-block-2.value.key.nested-key }}"
            ),
        }
    ...
    async def test_resolve_block_document_resolves_list_keypath(self):
        ...
        template = {
            "json_list": "{{ syncmatrix.blocks.json.json-list-block.value.key[0] }}",
            "list": "{{ syncmatrix.blocks.json.list-block.value[1] }}",
            "nested_json_list": (
                "{{ syncmatrix.blocks.json.nested-json-list-block.value.key[1].nested[1] }}"
            ),
        }
    ...
    async def test_resolve_block_document_raises_on_invalid_keypath(self):
        ...
        json_template = {
            "json": "{{ syncmatrix.blocks.json.nested-json-block-3.value.key.does_not_exist }}",
        }
        ...
        index_error_template = {
            "index_error": "{{ syncmatrix.blocks.json.index-error-block.value[3] }}",
        }
        with pytest.raises(ValueError, match="Could not resolve the keypath"):
        ...
        webhook_template = {
            "webhook": "{{ syncmatrix.blocks.webhook.webhook-block.value }}",
        }
        with pytest.raises(ValueError, match="Could not resolve the keypath"):
    ...
    async def test_resolve_block_document_resolves_block_attribute(self):
        ...
        template = {
            "block_attribute": "{{ syncmatrix.blocks.webhook.webhook-block-2.url }}",
        }
        result = await resolve_block_document_references(template)
...
class TestResolveVariables:
    @pytest.fixture
    async def variable_1(self, syncmatrix_client: SyncmatrixClient):
        res = await syncmatrix_client._client.post(
            "/variables/",
    ...
    @pytest.fixture
    async def variable_2(self, syncmatrix_client: SyncmatrixClient):
        res = await syncmatrix_client._client.post(
            "/variables/",
    ...
    async def test_resolve_string_no_placeholders(self, syncmatrix_client: SyncmatrixClient):
        template = "This is a simple string."
        result = await resolve_variables(template, client=syncmatrix_client)
        assert result == template

    async def test_resolve_string_with_standard_placeholder(
        self, variable_1, syncmatrix_client: SyncmatrixClient
    ):
        template = (
            "This is a string with a placeholder: {{"
            f" syncmatrix.variables.{variable_1['name']} }}}}."
        )
    ...
    async def test_resolve_string_with_multiple_standard_placeholders(
        self, variable_1, variable_2, syncmatrix_client: SyncmatrixClient
    ):
        template = (
            f"{{{{ syncmatrix.variables.{variable_1['name']} }}}} - {{{{"
            f" syncmatrix.variables.{variable_2['name']} }}}}"
        )
        expected = "test_value_1 - test_value_2"
        result = await resolve_variables(template, client=syncmatrix_client)
        assert result == expected

    async def test_resolve_dict(self, variable_1, syncmatrix_client: SyncmatrixClient):
        template: Dict[str, Any] = {
            "key1": "value1",
            "key2": f"{{{{ syncmatrix.variables.{variable_1['name']} }}}}",
        }
    ...
    async def test_resolve_nested_dict(
        ...
        template: Dict[str, Any] = {
            "key1": "value1",
            "key2": f"{{{{ syncmatrix.variables.{variable_1['name']} }}}}",
            "key3": {"key4": f"{{{{ syncmatrix.variables.{variable_2['name']} }}}}"},
        }
        expected = {
            "key1": "value1",
            "key2": "test_value_1",
    ...
    async def test_resolve_list(self, variable_1, syncmatrix_client: SyncmatrixClient):
        template = ["value1", f"{{{{ syncmatrix.variables.{variable_1['name']} }}}}", 42]
        expected = ["value1", "test_value_1", 42]
        result = await resolve_variables(template, client=syncmatrix_client)
        assert result == expected

    async def test_resolve_non_string_types(self, syncmatrix_client: SyncmatrixClient):
        template = 42
        result = await resolve_variables(template, client=syncmatrix_client)
        assert result == template
    ...
    async def test_resolve_does_not_template_other_placeholder_types(
        ...
        template = {
            ...
            "key2": "{{ syncmatrix.blocks.arbitraryblock.arbitrary-block }}",
    ...
    async def test_resolve_clears_placeholder_for_missing_variable(
        ...
        template = "{{ syncmatrix.variables.missing_variable }}"
        result = await resolve_variables(template, client=syncmatrix_client)
        assert result == ""
    ...
    async def test_resolve_clears_placeholders_for_missing_variables(
        ...
        template = (
            "{{ syncmatrix.variables.missing_variable_1 }} - {{"
            " syncmatrix.variables.missing_variable_2 }}"
        )
        result = await resolve_variables(template, client=syncmatrix_client)
        assert result == " - "
