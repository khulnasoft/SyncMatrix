...
from syncmatrix.utilities.annotations import BaseAnnotation, quote
from syncmatrix.utilities.collections import (
    AutoEnum,
    StopVisiting,
    dict_to_flatdict,
    flatdict_to_dict,
...
class TestPydanticObjects:
    ...
    These tests do not cover Syncmatrix functionality and may break if Pydantic introduces
    breaking changes.
    """
...
class TestVisitCollection:
    ...
    @pytest.mark.parametrize(
        ...
        [
            {"name": "syncmatrix"},
            {"name": "syncmatrix", "num": 1},
            {"name": "syncmatrix", "num": 1, "val": uuid.UUID(int=0)},
            {"name": "syncmatrix", "val": uuid.UUID(int=0)},
        ],
    )
