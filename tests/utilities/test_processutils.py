...
from syncmatrix.utilities.processutils import open_process, run_process
...
class TestOpenProcess:
    ...
    async def test_adds_ctrl_c_handler_to_win32_process_group(self, monkeypatch):
        ...
        monkeypatch.setattr(
            syncmatrix.utilities.processutils, "_win32_ctrl_handler", mock_ctrl_c_handler
        )
        mock_set_console_ctrl_handler = mock.Mock()
        monkeypatch.setattr(
            syncmatrix.utilities.processutils.windll.kernel32,
        ...
        monkeypatch.setattr(
            syncmatrix.utilities.processutils, "_open_anyio_process", mock_open_process
        )

        await syncmatrix.utilities.processutils.run_process(
