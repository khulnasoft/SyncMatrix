...
import syncmatrix
from syncmatrix.blocks.core import Block, InvalidBlockRegistration
from syncmatrix.blocks.fields import SecretDict
from syncmatrix.blocks.system import JSON, Secret
from syncmatrix.client import SyncmatrixClient
from syncmatrix.exceptions import SyncmatrixHTTPStatusError
from syncmatrix.server import models
from syncmatrix.server.schemas.actions import BlockDocumentCreate
from syncmatrix.server.schemas.core import DEFAULT_BLOCK_SCHEMA_VERSION, BlockDocument
from syncmatrix.testing.utilities import AsyncMock
from syncmatrix.utilities.dispatch import lookup_type, register_type
from syncmatrix.utilities.names import obfuscate_string
...
class TestAPICompatibility:
    ...
    def test_create_block_schema_uses_syncmatrix_version_for_built_in_blocks(self):
        try:
            Secret.register_type_and_schema()
        except SyncmatrixHTTPStatusError as exc:
        ...
        assert block_schema.version == Version(syncmatrix.__version__).base_version
    ...
    async def test_block_load_loads__collections(
        ...
        mock_load_syncmatrix_collections = Mock()
        monkeypatch.setattr(
            syncmatrix.plugins, "load_syncmatrix_collections", mock_load_syncmatrix_collections
        )

        await Block.load(block_document.block_type.slug + "/" + block_document.name)
        mock_load_syncmatrix_collections.assert_called_once()
...
class TestRegisterBlockTypeAndSchema:
    ...
    async def test_register_type_and_schema(self, syncmatrix_client: SyncmatrixClient):
        await self.NewBlock.register_type_and_schema()

        block_type = await syncmatrix_client.read_block_type_by_slug(slug="newblock")
        assert block_type is not None
        assert block_type.name == "NewBlock"

        block_schema = await syncmatrix_client.read_block_schema_by_checksum(
    ...
    async def test_register_idempotent(self, syncmatrix_client: SyncmatrixClient):
        await self.NewBlock.register_type_and_schema()
        await self.NewBlock.register_type_and_schema()
    ...
    async def test_register_nested_block(self, syncmatrix_client: SyncmatrixClient):
        class Big(Block):
        ...
        big_block_type = await syncmatrix_client.read_block_type_by_slug(slug="big")
        assert big_block_type is not None
        big_block_schema = await syncmatrix_client.read_block_schema_by_checksum(
            checksum=Big._calculate_schema_checksum()
        )
        assert big_block_schema is not None

        bigger_block_type = await syncmatrix_client.read_block_type_by_slug(slug="bigger")
        assert bigger_block_type is not None
        bigger_block_schema = await syncmatrix_client.read_block_schema_by_checksum(
            checksum=Bigger._calculate_schema_checksum()
        )
        assert bigger_block_schema is not None

        biggest_block_type = await syncmatrix_client.read_block_type_by_slug(
            slug="biggest"
        )
        assert biggest_block_type is not None
        biggest_block_schema = await syncmatrix_client.read_block_schema_by_checksum(
            checksum=Biggest._calculate_schema_checksum()
        )
        assert biggest_block_schema is not None

    async def test_register_nested_block_union(self, syncmatrix_client: SyncmatrixClient):
        class A(Block):
            a: str
        ...
        a_block_type = await syncmatrix_client.read_block_type_by_slug(slug="a")
        assert a_block_type is not None
        b_block_type = await syncmatrix_client.read_block_type_by_slug(slug="b")
        assert b_block_type is not None
        c_block_type = await syncmatrix_client.read_block_type_by_slug(slug="c")
        assert c_block_type is not None
        umbrella_block_type = await syncmatrix_client.read_block_type_by_slug(
            slug="umbrella"
        )
        assert umbrella_block_type is not None

        a_block_schema = await syncmatrix_client.read_block_schema_by_checksum(
            checksum=A._calculate_schema_checksum()
        )
        assert a_block_schema is not None
        b_block_schema = await syncmatrix_client.read_block_schema_by_checksum(
            checksum=B._calculate_schema_checksum()
        )
        assert b_block_schema is not None
        c_block_schema = await syncmatrix_client.read_block_schema_by_checksum(
            checksum=C._calculate_schema_checksum()
        )
        assert c_block_schema is not None
        umbrella_block_schema = await syncmatrix_client.read_block_schema_by_checksum(
    ...
    async def test_register_updates_block_type(self, syncmatrix_client: SyncmatrixClient):
        # Ignore warning caused by matching key in registry
        warnings.filterwarnings("ignore", category=UserWarning)
        ...
        block_type = await syncmatrix_client.read_block_type_by_slug(slug="test-block")
    ...
    async def test_register_wont_update_same_block_type_values(
        ...
        mock = AsyncMock()
        syncmatrix_client.update_block_type = mock

        await After.register_type_and_schema(client=syncmatrix_client)
        ...
        await Before.register_type_and_schema(client=syncmatrix_client)

        # call count should not have increased
        assert mock.call_count == 1

    async def test_register_fails_on_abc(self, syncmatrix_client):
        ...
        with pytest.raises(
            ...
            await Interface.register_type_and_schema(client=syncmatrix_client)
    ...
    def test_to_block_type_with_description_and_example_docstring(self):
        class Cube(Block):
            ...
            Example:
                Calculate volume:
                ```python
                from syncmatrix_geometry import Cube
...
class TestTypeDispatch:
    ...
    async def test_block_type_slug_excluded_from_document(self, syncmatrix_client):
        await AChildBlock.register_type_and_schema(client=syncmatrix_client)
...
class TestBlockSchemaMigration:
    ...
    async def test_save_new_schema_with_overwrite(self, syncmatrix_client):
        class Baz(Block):
        ...
        updated_schema = await syncmatrix_client.read_block_document_by_name(
            name="test", block_type_slug="baz"
        )
        updated_schema_id = updated_schema.block_schema_id

        # new local schema ID should now be saved to Syncmatrix
