...
import syncmatrix
from syncmatrix.blocks.notifications import (
    SYNCMATRIX_NOTIFY_TYPE_DEFAULT,
...
def reload_modules():
    """
    Reloads the syncmatrix.blocks.notifications module so patches to modules it imports
    will be visible to the blocks under test.
    """
    try:
        reload(syncmatrix.blocks.notifications)
    except UserWarning:
        # ignore the warning Syncmatrix gives when reloading the notifications module
        # because we reload syncmatrix itself immediately afterward.
        pass
...
class TestAppriseNotificationBlock:
    ...
    async def test_notify_async(self, block_class: Type[AppriseNotificationBlock]):
        with patch("apprise.Apprise", autospec=True) as AppriseMock:
            ...
            notify_type = SYNCMATRIX_NOTIFY_TYPE_DEFAULT
    ...
    def test_notify_sync(self, block_class: Type[AppriseNotificationBlock]):
        with patch("apprise.Apprise", autospec=True) as AppriseMock:
            ...
            apprise_instance_mock.async_notify.assert_called_once_with(
                body="test", title=None, notify_type=SYNCMATRIX_NOTIFY_TYPE_DEFAULT
            )
...
class TestPagerDutyWebhook:
    async def test_notify_async(self):
        with patch("apprise.Apprise", autospec=True) as AppriseMock:
            ...
            AppriseMock.assert_called_once()
            apprise_instance_mock.add.assert_called_once_with(
                "pagerduty://int_key@api_key/Syncmatrix/Notification?region=us&"
                "image=yes&format=text&overflow=upstream&rto=4.0&cto=4.0&verify=yes"
            )
...
class TestTwilioSMS:
    ...
    async def test_twilio_notify_async(self, valid_apprise_url):
        with patch("apprise.Apprise", autospec=True) as AppriseMock:
            ...
            await twilio_sms_block.notify("hello from syncmatrix")
    ...
    def test_twilio_notify_sync(self, valid_apprise_url):
        with patch("apprise.Apprise", autospec=True) as AppriseMock:
            ...
            twilio_sms_block.notify("hello from syncmatrix")
...
class TestCustomWebhook:
    async def test_notify_async(self):
        with respx.mock as xmock:
            ...
            assert last_req.headers["user-agent"] == "Syncmatrix Notifications"
            assert (
                last_req.content
