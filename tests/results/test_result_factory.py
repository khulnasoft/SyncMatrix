...
from syncmatrix.results import (
    LiteralResult,
    PersistedResult,
    ResultFactory,
)
...
from syncmatrix.testing.utilities import assert_blocks_equal

DEFAULT_SERIALIZER = PickleSerializer


def DEFAULT_STORAGE():
    return LocalFileSystem(basepath=SYNCMATRIX_LOCAL_STORAGE_PATH.value())


@pytest.fixture
async def factory(syncmatrix_client):
    return await ResultFactory.default_factory(
        client=syncmatrix_client, persist_result=True
    )
...
def test_root_flow_default_result_serializer_can_be_overriden_by_setting():
    ...
    with temporary_settings({SYNCMATRIX_RESULTS_DEFAULT_SERIALIZER: "json"}):
        result_factory = foo()
...
def test_root_flow_default_persist_result_can_be_overriden_by_setting():
    ...
    with temporary_settings({SYNCMATRIX_RESULTS_PERSIST_BY_DEFAULT: True}):
        result_factory = foo()
...
async def test_root_flow_custom_storage_by_instance_unsaved(syncmatrix_client, tmp_path):
    storage = LocalFileSystem(basepath=tmp_path)
    ...
    storage_block_document = await syncmatrix_client.read_block_document(
...
async def test_child_flow_custom_storage_by_instance_unsaved(syncmatrix_client, tmp_path):
    storage = LocalFileSystem(basepath=tmp_path)
...
async def test_task_custom_storage_by_instance_unsaved(syncmatrix_client, tmp_path):
    storage = LocalFileSystem(basepath=tmp_path)
...
async def _verify_default_storage_creation_with_persistence(
    syncmatrix_client,
    result_factory: syncmatrix.results.ResultFactory,
):
...
async def test_default_storage_creation_for_flow_with_persistence_features(
    ...
    await _verify_default_storage_creation_with_persistence(
        syncmatrix_client, result_factory
    )
