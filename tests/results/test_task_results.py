...
from syncmatrix.results import LiteralResult
from syncmatrix.serializers import JSONSerializer, PickleSerializer
...
async def test_task_persisted_result_due_to_flow_feature(syncmatrix_client, options):
    @flow(**options)
    def foo():
    ...
    api_state = (
        await syncmatrix_client.read_task_run(task_state.state_details.task_run_id)
    ).state
    assert await api_state.result() == 1


@pytest.mark.parametrize("options", [{"cache_key_fn": lambda *_: "foo"}])
async def test_task_persisted_result_due_to_task_feature(syncmatrix_client, options):
    @flow()
    def foo():
        return bar(return_state=True)
...
async def test_task_persisted_result_due_to_opt_in(syncmatrix_client):
    @flow
    def foo():
        return bar(return_state=True)
...
async def test_task_with_uncached_and_unpersisted_result(syncmatrix_client):
    @flow
    def foo():
        return bar(return_state=True)
...
async def test_task_with_uncached_and_unpersisted_null_result(syncmatrix_client):
    @flow
    def foo():
        return bar(return_state=True)
...
async def test_task_with_uncached_but_persisted_result(syncmatrix_client):
    @flow
    def foo():
        return bar(return_state=True)
...
async def test_task_with_uncached_but_literal_result(syncmatrix_client):
    @flow
    def foo():
        return bar(return_state=True)
...
async def test_task_result_serializer(syncmatrix_client, source, serializer):
...
@pytest.mark.parametrize("source", ["child", "parent"])
async def test_task_result_storage(syncmatrix_client, source):
...
async def test_task_result_static_storage_key(syncmatrix_client):
    storage = LocalFileSystem(basepath=SYNCMATRIX_HOME.value() / "test-storage")
...
async def test_task_result_parameter_formatted_storage_key(syncmatrix_client):
    storage = LocalFileSystem(basepath=SYNCMATRIX_HOME.value() / "test-storage")
...
async def test_task_result_flow_run_formatted_storage_key(syncmatrix_client):
    storage = LocalFileSystem(basepath=SYNCMATRIX_HOME.value() / "test-storage")
...
async def test_task_result_missing_with_null_return(syncmatrix_client):
    @flow
    def foo():
        return bar(return_state=True)
...
async def test_task_exception_is_persisted(syncmatrix_client):
    @flow
    def foo():
