...
from syncmatrix.context import get_run_context
from syncmatrix.exceptions import MissingResult
from syncmatrix.filesystems import LocalFileSystem
from syncmatrix.results import PersistedResultBlob, UnpersistedResult
from syncmatrix.serializers import (
    CompressedSerializer,
    JSONSerializer,
    PickleSerializer,
    Serializer,
)
from syncmatrix.server import schemas
from syncmatrix.settings import (
    SYNCMATRIX_DEFAULT_RESULT_STORAGE_BLOCK,
    SYNCMATRIX_HOME,
    SYNCMATRIX_RESULTS_PERSIST_BY_DEFAULT,
    temporary_settings,
)
from syncmatrix.states import Cancelled, Completed, Failed
...
async def test_flow_with_unpersisted_result(syncmatrix_client, persist_result):
    ...
    api_state = (
        await syncmatrix_client.read_flow_run(state.state_details.flow_run_id)
    ).state
    with pytest.raises(MissingResult):
        await api_state.result()


async def test_flow_with_uncached_and_unpersisted_result(syncmatrix_client):
...
async def test_flow_with_uncached_and_unpersisted_null_result(syncmatrix_client):
    @flow(persist_result=False, cache_result_in_memory=False)
    def foo():
        return None
...
async def test_flow_with_uncached_but_persisted_result(syncmatrix_client):
...
async def test_flow_with_uncached_but_literal_result(syncmatrix_client):
    @flow(persist_result=True, cache_result_in_memory=False)
    def foo():
        return True
...
async def test_flow_result_missing_with_null_return(syncmatrix_client):
    @flow
    def foo():
        return None
...
async def test_flow_literal_result_is_available_but_not_serialized_or_persisted(
    syncmatrix_client, value
):
    @flow(
        persist_result=True,
        result_serializer="pickle",
        result_storage=LocalFileSystem(basepath=SYNCMATRIX_HOME.value()),
    )
    def foo():
        return value
...
async def test_flow_exception_is_persisted(syncmatrix_client):
...
async def test_flow_result_serializer(serializer, syncmatrix_client):
...
async def test_flow_result_storage_by_instance(syncmatrix_client):
    storage = LocalFileSystem(basepath=SYNCMATRIX_HOME.value() / "test-storage")
...
async def test_flow_result_storage_by_slug(syncmatrix_client):
    await LocalFileSystem(basepath=SYNCMATRIX_HOME.value() / "test-storage").save("test")
    ...
    state = foo(return_state=True)
    assert await state.result() == 1
    await assert_uses_result_storage(state, slug, client=syncmatrix_client)

    api_state = (
        await syncmatrix_client.read_flow_run(state.state_details.flow_run_id)
    ).state
    assert await api_state.result() == 1
    await assert_uses_result_storage(api_state, slug, client=syncmatrix_client)


@pytest.mark.parametrize("options", [{"retries": 3}])
async def test_child_flow_persisted_result_due_to_parent_feature(
    syncmatrix_client, options
):
    @flow(**options)
    def foo():
    ...
    api_state = (
        await syncmatrix_client.read_flow_run(child_state.state_details.flow_run_id)
    ).state
    assert await api_state.result() == 1


async def test_child_flow_persisted_result_due_to_opt_in(syncmatrix_client):
    @flow
    def foo():
        return bar(return_state=True)
...
async def test_child_flow_result_serializer(syncmatrix_client, source):
    serializer = "json"
...
@pytest.mark.parametrize("source", ["child", "parent"])
async def test_child_flow_result_storage(syncmatrix_client, source):
    storage = LocalFileSystem(basepath=SYNCMATRIX_HOME.value() / "test-storage")
...
async def test_child_flow_result_missing_with_null_return(syncmatrix_client):
    @flow
    def foo():
        return bar(return_state=True)
...
def test_flow_server_state_schema_result_is_respected(persist_result, return_state):
    ...
    with pytest.warns(DeprecationWarning, match="Use `syncmatrix.states.State` instead"):
        state = my_flow(return_state=True)
    ...
    if return_state.data:
        with pytest.warns(DeprecationWarning, match="use `syncmatrix.states.State`"):
            assert state.result(raise_on_failure=False) == return_state.data
...
async def test_root_flow_default_remote_storage():
    ...
    with temporary_settings(
        {
            SYNCMATRIX_RESULTS_PERSIST_BY_DEFAULT: True,
            SYNCMATRIX_DEFAULT_RESULT_STORAGE_BLOCK: "local-file-system/my-result-storage",
        }
    ):
...
async def test_root_flow_default_remote_storage_saves_correct_result():
    await LocalFileSystem(basepath="~/.syncmatrix/results").save("my-result-storage")
    ...
    result_bytes = await local_storage.read_path("~/.syncmatrix/results/my-result.pkl")
...
async def test_root_flow_explicit_result_storage_settings_overrides_default():
    await LocalFileSystem(basepath="~/.syncmatrix/results").save("explicit-storage")
    await LocalFileSystem(basepath="~/.syncmatrix/other-results").save(
