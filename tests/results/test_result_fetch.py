...
from syncmatrix.settings import SYNCMATRIX_ASYNC_FETCH_STATE_RESULT, temporary_settings
...
def disable_fetch_by_default():
    ...
    """
    with temporary_settings({SYNCMATRIX_ASYNC_FETCH_STATE_RESULT: False}):
        yield
...
async def test_async_result_returns_coroutine_with_setting():
    ...
    state = await foo(return_state=True)
    with temporary_settings({SYNCMATRIX_ASYNC_FETCH_STATE_RESULT: True}):
        coro = state.result(fetch=True)
