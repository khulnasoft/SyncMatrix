...
from syncmatrix import engine, flow, task
from syncmatrix._internal.compatibility.experimental import ExperimentalFeature
from syncmatrix.client.orchestration import SyncmatrixClient, get_client
from syncmatrix.client.schemas import OrchestrationResult
from syncmatrix.context import FlowRunContext, get_run_context
from syncmatrix.engine import (
    API_HEALTHCHECKS,
    begin_flow_run,
    begin_task_run,
    check_api_reachable,
...
from syncmatrix.exceptions import (
    Abort,
    CrashedRun,
    FailedRun,
    ParameterTypeError,
...
from syncmatrix.futures import SyncmatrixFuture
from syncmatrix.input import RunInput, read_flow_run_input
from syncmatrix.results import ResultFactory, UnknownResult
from syncmatrix.server.schemas.core import FlowRun
from syncmatrix.server.schemas.filters import FlowRunFilter
from syncmatrix.server.schemas.responses import (
    SetStateStatus,
    StateAbortDetails,
    StateAcceptDetails,
    StateRejectDetails,
...
from syncmatrix.server.schemas.states import StateDetails, StateType
from syncmatrix.settings import (
    SYNCMATRIX_FLOW_DEFAULT_RETRY_DELAY_SECONDS,
    SYNCMATRIX_TASK_DEFAULT_RETRY_DELAY_SECONDS,
    SYNCMATRIX_TASK_INTROSPECTION_WARN_THRESHOLD,
    temporary_settings,
)
from syncmatrix.states import (
    Cancelled,
    Completed,
    Failed,
    Paused,
...
from syncmatrix.task_runners import (
    BaseTaskRunner,
    SequentialTaskRunner,
    TaskConcurrencyType,
)
from syncmatrix.tasks import exponential_backoff
from syncmatrix.testing.utilities import AsyncMock, exceptions_equal
from syncmatrix.utilities.annotations import quote
from syncmatrix.utilities.pydantic import PartialModel


@pytest.fixture
async def result_factory(syncmatrix_client):
    return await ResultFactory.default_factory(
        client=syncmatrix_client,
    )
...
@pytest.fixture
async def get_flow_run_context(syncmatrix_client, result_factory, local_filesystem):
...
class TestBlockingPause:
    ...
    async def test_first_polling_interval_doesnt_grow_arbitrarily_large(
        ...
        monkeypatch.setattr("syncmatrix.engine.anyio.sleep", sleeper)
    ...
    async def test_paused_flows_block_execution_in_sync_flows(self, syncmatrix_client):
        completed = False
    ...
    async def test_paused_flows_block_execution_in_async_flows(self, syncmatrix_client):
        @task
        async def foo():
            return 42
        ...
        task_runs = await syncmatrix_client.read_task_runs(
            flow_run_filter=FlowRunFilter(id={"any_": [flow_run_id]})
        )
        assert len(task_runs) == 2, "only two tasks should have completed"

    async def test_paused_flows_can_be_resumed(self, syncmatrix_client):
        @task
        async def foo():
            return 42
        ...
        async def flow_resumer():
            await anyio.sleep(3)
            flow_runs = await syncmatrix_client.read_flow_runs(limit=1)
    ...
    async def test_paused_flows_can_receive_input(self, syncmatrix_client):
        flow_run_id = None
        ...
        async def flow_resumer():
            ...
            flow_run = await syncmatrix_client.read_flow_run(flow_run_id)
...
class TestNonblockingPause:
    ...
    async def test_paused_flows_do_not_block_execution_with_reschedule_flag(
        self, syncmatrix_client, deployment, session
    ):
        flow_run_id = None
        ...
        async def pausing_flow_without_blocking():
            ...
            from syncmatrix.server.models.flow_runs import update_flow_run
    ...
    async def test_paused_flows_gracefully_exit_with_reschedule_flag(
        ...
        async def pausing_flow_without_blocking():
            ...
            await update_flow_run(
                session,
                syncmatrix.runtime.flow_run.id,
                FlowRun.construct(deployment_id=deployment.id),
            )
            await session.commit()
...
class TestSuspendFlowRun:
    ...
    async def test_suspend_flow_run_by_id(self, syncmatrix_client, deployment, session):
        flow_run_id = None
        task_completions = 0
    ...
    async def test_suspend_can_receive_input(self, deployment, session, syncmatrix_client):
        flow_run_id = None
    ...
    async def test_suspend_can_receive_automatic_input(
        self, deployment, session, syncmatrix_client
    ):
        flow_run_id = None
...
class TestOrchestrateTaskRun:
    async def test_propose_state_does_not_recurse(
        self, monkeypatch, syncmatrix_client, mock_anyio_sleep, flow_run
    ):
        """
        Regression test for https://github.com/KhulnaSoft/syncmatrix/issues/8825.
        ...
        syncmatrix_client.set_task_run_state = AsyncMock(
            side_effect=[
                *[
                    OrchestrationResult(
        ...
        with mock_anyio_sleep.assert_sleeps_for(delay_seconds * num_waits):
            await propose_state(
                syncmatrix_client, State(type=StateType.RUNNING), task_run_id=task_run.id
            )

    async def test_raises_on_pause_with_reschedule(
        self, monkeypatch, syncmatrix_client, mock_anyio_sleep, flow_run, result_factory
    ):
    ...
    async def test_pending_in_pause_loop_submits_running_state(
        self, monkeypatch, syncmatrix_client, flow_run, result_factory
    ):
        paused_state = Paused()
    ...
    async def test_waits_for_awaiting_retry_scheduled_time(
        ...
        states = await syncmatrix_client.read_task_run_states(str(task_run.id))
    ...
    async def test_returns_not_ready_when_any_upstream_futures_resolve_to_incomplete(
        ...
        upstream_task_run = await syncmatrix_client.create_task_run(
            task=my_task,
            flow_run_id=flow_run.id,
    ...
    async def test_quoted_parameters_are_resolved(
        self, syncmatrix_client, flow_run, result_factory, local_filesystem
    ):
        # the flow run must be running prior to running tasks
        await syncmatrix_client.set_flow_run_state(
            flow_run_id=flow_run.id,
    ...
    async def test_global_task_retry_delay_seconds(
        self, mock_anyio_sleep, syncmatrix_client, flow_run, result_factory
    ):
        with temporary_settings(
            updates={SYNCMATRIX_TASK_DEFAULT_RETRY_DELAY_SECONDS: [3, 5, 9]}
        ):
            # the flow run must be running prior to running tasks
            await syncmatrix_client.set_flow_run_state(
                flow_run_id=flow_run.id,
    ...
    async def test_retry_condition_fn_retry_handler_returns_false_does_not_retry(
        self, mock_anyio_sleep, syncmatrix_client, flow_run, result_factory, caplog
    ):
        # the flow run must be running prior to running tasks
        await syncmatrix_client.set_flow_run_state(
            flow_run_id=flow_run.id,
...
class TestBeginTaskRun:
    async def test_begin_task_run_handles_pause_signal(
        self, monkeypatch, syncmatrix_client, result_factory, patch_manifest_load
    ):
        @task
        async def my_task():
            return 1
        ...
        flow_id = await syncmatrix_client.create_flow(my_flow)
        deployment_id = await syncmatrix_client.create_deployment(
            flow_id,
            name="test",
            manifest_path="file.json",
        )
        flow_run = await syncmatrix_client.create_flow_run_from_deployment(deployment_id)
        ...
        result = await syncmatrix_client.set_flow_run_state(
            flow_run_id=flow_run.id,
            state=Paused(reschedule=True),
        )
        print("RESULT ", result)

        with FlowRunContext.construct(client=syncmatrix_client, flow_run=flow_run):
            state = await begin_task_run(
                ...
                wait_for=[],
                log_prints=False,
                settings=syncmatrix.context.SettingsContext.get().copy(),
            )
...
class TestOrchestrateFlowRun:
    ...
    async def test_propose_state_does_not_recurse(
        self, monkeypatch, syncmatrix_client, mock_anyio_sleep
    ):
        """
        Regression test for https://github.com/KhulnaSoft/syncmatrix/issues/8825.
        Previously propose_state would make a recursive call. In extreme cases
        ...
        syncmatrix_client.set_flow_run_state = AsyncMock(
            side_effect=[
                *[
                    OrchestrationResult(
                        status=SetStateStatus.WAIT,
        ...
        with mock_anyio_sleep.assert_sleeps_for(delay_seconds * num_waits):
            await propose_state(
                syncmatrix_client, State(type=StateType.RUNNING), flow_run_id=flow_run.id
            )

    async def test_waits_until_scheduled_start_time(
        self, syncmatrix_client, mock_anyio_sleep, partial_flow_run_context
    ):
        @flow
        def foo():
            return 1
    ...
    async def test_waits_for_awaiting_retry_scheduled_time(
        ...
        # Check expected state transitions
        states = await syncmatrix_client.read_flow_run_states(str(flow_run.id))
        state_names = [state.type for state in states]
        assert state_names == [
            StateType.PENDING,
            StateType.RUNNING,
    ...
    async def test_global_flow_retry_delay_seconds(
        ...
        with temporary_settings(
            updates={SYNCMATRIX_FLOW_DEFAULT_RETRY_DELAY_SECONDS: "43"}
        ):
            flow_run_count = 0
...
class TestFlowRunCrashes:
    ...
    async def test_flow_timeouts_are_not_crashes(self, flow_run, syncmatrix_client):
        """
        ...
        flow_run = await syncmatrix_client.read_flow_run(flow_run.id)

        assert flow_run.state.is_failed()
        assert flow_run.state.type != StateType.CRASHED
        assert "exceeded timeout" in flow_run.state.message

    async def test_aborts_are_not_crashes(self, flow_run, syncmatrix_client):
        """
...
class TestTaskRunCrashes:
    ...
    async def test_interrupt_in_task_function_crashes_task_and_flow(
        self, flow_run, syncmatrix_client, interrupt_type
    ):
        @task
        async def my_task():
    ...
    async def test_interrupt_in_task_orchestration_crashes_task_and_flow(
        self, flow_run, syncmatrix_client, interrupt_type, monkeypatch
    ):
        monkeypatch.setattr(
            "syncmatrix.engine.orchestrate_task_run", AsyncMock(side_effect=interrupt_type)
        )
    ...
    async def test_error_in_task_orchestration_crashes_task_but_not_flow(
        self, flow_run, syncmatrix_client, monkeypatch
    ):
        exception = ValueError("Boo!")

        monkeypatch.setattr(
            "syncmatrix.engine.orchestrate_task_run", AsyncMock(side_effect=exception)
        )
        ...
        task_run = await syncmatrix_client.read_task_run(
...
class TestDeploymentFlowRun:
    ...
    async def test_completed_run(self, syncmatrix_client, patch_manifest_load):
        @flow
        def my_flow(x: int):
            return x

        await patch_manifest_load(my_flow)
        deployment_id = await self.create_deployment(syncmatrix_client, my_flow)
        ...
        state = await retrieve_flow_then_begin_flow_run(
            flow_run.id, client=syncmatrix_client, user_thread=threading.current_thread()
        )
        assert await state.result() == 1

    async def test_retries_loaded_from_flow_definition(
        self, syncmatrix_client, patch_manifest_load, mock_anyio_sleep
    ):
    ...
    async def test_failed_run(self, syncmatrix_client, patch_manifest_load):
        @flow
        def my_flow(x: int):
    ...
    async def test_parameters_are_cast_to_correct_type(
        self, syncmatrix_client, patch_manifest_load
    ):
        @flow
        def my_flow(x: int):
            return x
...
class TestDynamicKeyHandling:
    async def test_dynamic_key_increases_sequentially(self, syncmatrix_client):
        @task
        def my_task():
            pass
    ...
    async def test_subflow_resets_dynamic_key(self, syncmatrix_client):
        @task
        def my_task():
            pass
    ...
    async def test_dynamic_key_unique_per_task_key(self, syncmatrix_client):
        @task
        def task_one():
            pass
...
class TestCreateThenBeginFlowRun:
    async def test_handles_bad_parameter_types(
        self, syncmatrix_client, parameterized_flow
    ):
    ...
    async def test_handles_other_errors(
        self, syncmatrix_client, parameterized_flow, monkeypatch
    ):
        ...
        monkeypatch.setattr(
            syncmatrix.flows.Flow, "validate_parameters", raise_unspecified_exception
        )
...
class TestRetrieveFlowThenBeginFlowRun:
    async def test_handles_bad_parameter_types(
        self, syncmatrix_client, patch_manifest_load, parameterized_flow
    ):
        await patch_manifest_load(parameterized_flow)
        flow_id = await syncmatrix_client.create_flow(parameterized_flow)
        dep_id = await syncmatrix_client.create_deployment(
            flow_id,
            name="test",
            manifest_path="path/file.json",
        )
        new_flow_run = await syncmatrix_client.create_flow_run_from_deployment(
...
class TestAPIHealthcheck:
    ...
    async def test_healthcheck_for_ephemeral_client(self):
        ...
        # Check caching
        assert "http://ephemeral-syncmatrix/api/" in API_HEALTHCHECKS
        assert isinstance(API_HEALTHCHECKS["http://ephemeral-syncmatrix/api/"], float)

        assert API_HEALTHCHECKS["http://ephemeral-syncmatrix/api/"] == pytest.approx(
    ...
    async def test_healthcheck_not_reset_within_expiration(self):
        async with get_client() as client:
            await check_api_reachable(client, fail_message="test")
            value = API_HEALTHCHECKS["http://ephemeral-syncmatrix/api/"]
            for _ in range(2):
                await check_api_reachable(client, fail_message="test")
                assert API_HEALTHCHECKS["http://ephemeral-syncmatrix/api/"] == value

    async def test_healthcheck_reset_after_expiration(self):
        async with get_client() as client:
            await check_api_reachable(client, fail_message="test")
            value = API_HEALTHCHECKS[
                "http://ephemeral-syncmatrix/api/"
    ...
    async def test_failed_healthcheck(self):
        async with get_client() as client:
            ...
            with pytest.raises(
                RuntimeError,
                match="test. Failed to reach API at http://ephemeral-syncmatrix/api/.",
            ):
                await check_api_reachable(client, fail_message="test")

            # Not cached
            assert "http://ephemeral-syncmatrix/api/" not in API_HEALTHCHECKS
...
@patch(
    "syncmatrix.utilities.collections.visit_collection",
    wraps=syncmatrix.utilities.collections.visit_collection,
)
@patch("syncmatrix.engine.visit_collection", wraps=syncmatrix.engine.visit_collection)
async def test_collect_task_run_inputs_respects_quote(
    mock_outer_visit_collection, mock_recursive_visit_collection
):
    # Regression test for https://github.com/KhulnaSoft/syncmatrix/pull/10370
...
async def test_long_task_introspection_warning_on(
    syncmatrix_client, flow_run, result_factory, monkeypatch, caplog
):
    # the flow run must be running prior to running tasks
    await syncmatrix_client.set_flow_run_state(
        flow_run_id=flow_run.id,
    ...
    async def mock_resolve_inputs(*args, **kwargs):
        # sleep for longer than SYNCMATRIX_TASK_INTROSPECTION_WARN_THRESHOLD
        await anyio.sleep(0.5)
        return {}

    monkeypatch.setattr("syncmatrix.engine.resolve_inputs", mock_resolve_inputs)
    with caplog.at_level("WARNING"):
        with temporary_settings(
            updates={SYNCMATRIX_TASK_INTROSPECTION_WARN_THRESHOLD: "0.6"}
        ):
            await orchestrate_task_run(
                task=my_task,
                task_run=task_run,
...
async def test_long_task_introspection_warning_off(
    ...
    with caplog.at_level("WARNING"):
        with temporary_settings(
            updates={SYNCMATRIX_TASK_INTROSPECTION_WARN_THRESHOLD: "0"}
        ):
            await orchestrate_task_run(
                task=my_task,
                task_run=task_run,
