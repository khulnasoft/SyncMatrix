...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "syncmatrix.filesystems.S3 has been deprecated."
            " It will not be available after Sep 2024."
            " Use the `S3Bucket` block from syncmatrix-aws instead.",
        ),
        (
            GCS,
            {"bucket_path": "bucket/path"},
            "syncmatrix.filesystems.GCS has been deprecated."
            " It will not be available after Sep 2024."
            " Use the `GcsBucket` block from syncmatrix-gcp instead.",
        ),
        (
            Azure,
            {"bucket_path": "bucket/path"},
            "syncmatrix.filesystems.Azure has been deprecated."
            " It will not be available after Sep 2024."
            " Use the `AzureBlobStorageContainer` block from syncmatrix-azure instead.",
        ),
        (
            GitHub,
            {"repository": "https://github.com/org/repo.git"},
            "syncmatrix.filesystems.GitHub has been deprecated."
            " It will not be available after Sep 2024."
            " Use the `GitHubRepository` block from syncmatrix-github instead.",
        ),
    ],
)
...
class TestRemoteFileSystem:
    ...
    async def test_put_directory_flat(self):
        ...
        await fs.put_directory(
            ...
            ignore_file=os.path.join(
                TEST_PROJECTS_DIR, "flat-project", ".syncmatrixignore"
            ),
        )
    ...
    async def test_put_directory_tree(self):
        ...
        await fs.put_directory(
            ...
            ignore_file=os.path.join(
                TEST_PROJECTS_DIR, "tree-project", ".syncmatrixignore"
            ),
        )
    ...
    async def test_put_directory_put_file_count(self):
        ignore_file = os.path.join(TEST_PROJECTS_DIR, "tree-project", ".syncmatrixignore")
        ...
        included_files = syncmatrix.utilities.filesystem.filter_files(
            os.path.join(TEST_PROJECTS_DIR, "tree-project"),
            ignore_patterns,
            include_dirs=False,
        )
...
class TestGitHub:
    ...
    async def test_repository_default(self, monkeypatch):
        ...
        monkeypatch.setattr(syncmatrix.filesystems, "run_process", mock)
        g = GitHub(repository="syncmatrix")
        await g.get_directory()

        assert mock.await_count == 1
        expected_cmd = ["git", "clone", "syncmatrix"]
    ...
    async def test_reference_default(self, monkeypatch):
        ...
        mock = AsyncMock(return_value=p())
        monkeypatch.setattr(syncmatrix.filesystems, "run_process", mock)
        g = GitHub(repository="syncmatrix", reference="2.0.0")
        await g.get_directory()

        assert mock.await_count == 1
        expected_cmd = ["git", "clone", "syncmatrix", "-b", "2.0.0", "--depth", "1"]
        assert mock.await_args[0][0][: len(expected_cmd)] == expected_cmd
    ...
    async def test_token_added_correctly_from_credential(self, monkeypatch):
        ...
        monkeypatch.setattr(syncmatrix.filesystems, "run_process", mock)
        credential = "XYZ"
        repo = "https://github.com/KhulnaSoft/syncmatrix.git"
        g = GitHub(
            repository=repo,
        ...
        expected_cmd = [
            "git",
            "clone",
            f"https://{credential}@github.com/KhulnaSoft/syncmatrix.git",
            "--depth",
            "1",
        ]
        assert mock.await_args[0][0][: len(expected_cmd)] == expected_cmd
    ...
    async def test_ssh_fails_with_credential(self, monkeypatch):
        ...
        with pytest.raises(InvalidRepositoryURLError, match=error_msg):
            GitHub(
                repository="git@github.com:KhulnaSoft/syncmatrix.git",
                access_token=credential,
            )
    ...
    async def test_dir_contents_copied_correctly_with_get_directory(
        ...
        with TemporaryDirectory() as tmp_dst:
            monkeypatch.setattr(
                syncmatrix.filesystems,
                "TemporaryDirectory",
                self.MockTmpDir,
            )

            g = GitHub(
                repository="https://github.com/KhulnaSoft/syncmatrix.git",
            )
...
class TestAzure:
    def test_init(self, monkeypatch):
        ...
        monkeypatch.setattr("syncmatrix.filesystems.RemoteFileSystem", remote_storage_mock)
        Azure(
