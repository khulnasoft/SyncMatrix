...
from syncmatrix.client.schemas.objects import TaskRun
...
from syncmatrix.task_server import TaskServer, serve
...
def mock_task_server_start(monkeypatch):
    monkeypatch.setattr(
        "syncmatrix.task_server.TaskServer.start", mock_start := AsyncMock()
    )
    return mock_start
...
def mock_create_subscription(monkeypatch):
    monkeypatch.setattr(
        "syncmatrix.task_server.TaskServer._subscribe_to_task_scheduling",
...
class TestServe:
    async def test_serve_raises_if_task_scheduling_not_enabled(self, foo_task):
        with temporary_settings({SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING: False}):
            with pytest.raises(
                RuntimeError,
                match="set SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING to True",
            ):
...
async def test_task_server_can_execute_a_single_async_single_task_run(
    async_foo_task, syncmatrix_client
):
    ...
    updated_task_run = await syncmatrix_client.read_task_run(task_run.id)
...
class TestTaskServerTaskRunRetries:
    async def test_task_run_via_task_server_respects_retry_policy(self, syncmatrix_client):
        count = 0
    ...
    async def test_task_run_via_task_server_respects_retry_condition_fn(
        self, should_retry, syncmatrix_client
    ):
        count = 0
...
class TestTaskServerTaskResults:
    ...
    async def test_task_run_via_task_server_respects_persist_result(
        self, persist_result, syncmatrix_client
    ):
    ...
    async def test_task_run_via_task_server_respects_result_storage_key(
        self, storage_key, syncmatrix_client
    ):
    ...
    async def test_task_run_via_task_server_respects_caching(
        self, async_foo_task, syncmatrix_client, caplog
    ):
        count = 0
        ...
        new_updated_task_run = await syncmatrix_client.read_task_run(task_run.id)
...
class TestTaskServerTaskTags:
    async def test_task_run_via_task_server_respects_tags(
        self, async_foo_task, syncmatrix_client
    ):
        @task(tags=["foo", "bar"])
...
class TestTaskServerTaskStateHooks:
    async def test_task_run_via_task_server_runs_on_completion_hook(
        self, async_foo_task, syncmatrix_client, capsys
    ):
