...
from syncmatrix.client.base import SyncmatrixHttpxClient, SyncmatrixResponse
from syncmatrix.client.schemas.objects import CsrfToken
from syncmatrix.exceptions import SyncmatrixHTTPStatusError
from syncmatrix.settings import (
    SYNCMATRIX_CLIENT_MAX_RETRIES,
    SYNCMATRIX_CLIENT_RETRY_EXTRA_CODES,
    SYNCMATRIX_CLIENT_RETRY_JITTER_FACTOR,
    temporary_settings,
)
...
@pytest.fixture
def disable_jitter():
    with temporary_settings({SYNCMATRIX_CLIENT_RETRY_JITTER_FACTOR: 0}):
        yield


class TestSyncmatrixHttpxClient:
    ...
    async def test_syncmatrix_httpx_client_retries_on_extra_error_codes(
    ...
    @pytest.mark.usefixtures("mock_anyio_sleep", "disable_jitter")
    async def test_syncmatrix_httpx_client_raises_on_non_extra_error_codes(
        ...
        with temporary_settings({SYNCMATRIX_CLIENT_RETRY_EXTRA_CODES: "409"}):
            with pytest.raises(SyncmatrixHTTPStatusError):
                async with client:
                    await client.post(
                        url="fake.url/fake/route", data={"evenmorefake": "data"}
                    )
    ...
    async def test_syncmatrix_httpx_client_retries_on_designated_exceptions(
        self,
        monkeypatch,
        exception_type,
        caplog,
    ...
    async def test_syncmatrix_httpx_client_retries_up_to_five_times(
        self,
        monkeypatch,
        response_or_exc,
    ):
    ...
    async def test_syncmatrix_httpx_client_respects_max_retry_setting(
        self,
        monkeypatch,
        response_or_exc,
    ):
        ...
        with pytest.raises(Exception):
            with temporary_settings({SYNCMATRIX_CLIENT_MAX_RETRIES: 10}):
                async with client:
                    await client.post(
                        url="fake.url/fake/route",
                        data={"evenmorefake": "data"},
    ...
    async def test_syncmatrix_httpx_client_raises_final_error_after_retries(
    ...
    async def test_syncmatrix_httpx_client_respects_retry_header(
    ...
    async def test_syncmatrix_httpx_client_uses_exponential_backoff_without_retry_after_header(
    ...
    @pytest.mark.usefixtures("disable_jitter")
    async def test_syncmatrix_httpx_client_respects_retry_header_per_response(
    ...
    async def test_syncmatrix_httpx_client_adds_jitter_with_retry_header(
    ...
    async def test_syncmatrix_httpx_client_adds_jitter_with_exponential_backoff(
        self, mock_anyio_sleep, response_or_exc, monkeypatch
    ):
        base_client_send = AsyncMock()
        monkeypatch.setattr(AsyncClient, "send", base_client_send)
    ...
    async def test_syncmatrix_httpx_client_does_not_retry_other_exceptions(
        self, mock_anyio_sleep, monkeypatch
    ):
        base_client_send = AsyncMock()
        monkeypatch.setattr(AsyncClient, "send", base_client_send)
    ...
    async def test_syncmatrix_httpx_client_returns_syncmatrix_response(self, monkeypatch):
        """Test that the SyncmatrixHttpxClient returns a SyncmatrixResponse"""
        client = SyncmatrixHttpxClient()
        base_client_send = AsyncMock()
        monkeypatch.setattr(AsyncClient, "send", base_client_send)
        ...
        assert isinstance(response, SyncmatrixResponse)

    #
    async def test_syncmatrix_httpx_client_raises_syncmatrix_http_status_error(
        self, monkeypatch
    ):
        RESPONSE_400 = Response(
            status.HTTP_400_BAD_REQUEST,
        ...
        with pytest.raises(SyncmatrixHTTPStatusError) as exc:
            async with client:
                await client.post(
                    url="fake.url/fake/route", data={"evenmorefake": "data"}
                )
...
) -> AsyncGenerator[Tuple[SyncmatrixHttpxClient, mock.AsyncMock], None]:
    with mock.patch("httpx.AsyncClient.send", autospec=True) as send:
        send.side_effect = responses
        client = SyncmatrixHttpxClient(enable_csrf_support=True)
        async with client:
            try:
                yield client, send
            finally:
...
class TestCsrfSupport:
    async def test_no_csrf_headers_not_change_request(self):
        ...
        assert "Syncmatrix-Csrf-Token" not in request.headers
        assert "Syncmatrix-Csrf-Client" not in request.headers
    ...
    async def test_csrf_headers_on_change_request(self, method: str):
        ...
        assert request.headers["Syncmatrix-Csrf-Token"] == "test_token"
        assert request.headers["Syncmatrix-Csrf-Client"] == str(client.csrf_client_id)
    ...
    async def test_does_not_refresh_csrf_token_not_expired(self):
        ...
        assert isinstance(request, httpx.Request)
        assert request.url == httpx.URL("/fake.url/fake/route")
        assert request.headers["Syncmatrix-Csrf-Token"] == "fresh_token"
        assert request.headers["Syncmatrix-Csrf-Client"] == str(client.csrf_client_id)
