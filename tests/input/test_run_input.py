...
from syncmatrix.client.schemas.objects import FlowRunInput
from syncmatrix.context import FlowRunContext
from syncmatrix.input import (
    RunInput,
    RunInputMetadata,
...
from syncmatrix.input.run_input import (
    AutomaticRunInput,
    receive_input,
    run_input_subclass_from_type,
    send_input,
)
from syncmatrix.states import Paused, Running, Suspended


@pytest.fixture
def flow_run_context(flow_run, syncmatrix_client):
    with FlowRunContext.construct(flow_run=flow_run, client=syncmatrix_client) as context:
        yield context
...
async def test_load_from_flow_run_input(flow_run_context):
    flow_run_input = FlowRunInput(
        ...
        sender=f"syncmatrix.flow-run.{uuid4()}",
    )
...
async def test_respond(flow_run):
    flow_run_input = FlowRunInput(
        ...
        sender=f"syncmatrix.flow-run.{flow_run.id}",
    )
