...
be reraised to prevent Syncmatrix from swallowing important signals like keyboard
interrupts.
"""
import asyncio
import os
...
async def assert_flow_run_crashed(flow_run: FlowRun, expected_message: str):
    ...
    with pytest.raises(syncmatrix.exceptions.CrashedRun, match=expected_message):
        await flow_run.state.result()


async def test_anyio_cancellation_crashes_flow(syncmatrix_client):
    started = asyncio.Future()
    flow_run_id = None

    @syncmatrix.flow
    async def my_flow():
        started.set_result(syncmatrix.context.get_run_context().flow_run.id)
...
async def test_anyio_cancellation_crashes_flow_with_timeout_configured(syncmatrix_client):
    """
    ...
    @syncmatrix.flow(timeout_seconds=10)
    async def my_flow():
        started.set_result(syncmatrix.context.get_run_context().flow_run.id)
        await anyio.sleep_forever()
...
async def test_anyio_cancellation_crashes_parent_and_child_flow(syncmatrix_client):
    ...
    async def child_flow():
        child_started.set_result(syncmatrix.context.get_run_context().flow_run.id)
        await anyio.sleep_forever()

    @syncmatrix.flow
    async def parent_flow():
        parent_started.set_result(syncmatrix.context.get_run_context().flow_run.id)
        await child_flow()
    ...
    parent_flow_run = await syncmatrix_client.read_flow_run(parent_flow_run_id)
    await assert_flow_run_crashed(
        parent_flow_run,
        expected_message="Execution was cancelled by the runtime environment",
    )
...
async def test_anyio_cancellation_crashes_child_flow(syncmatrix_client):
    child_started = asyncio.Future()
    child_flow_run_id = parent_flow_run_id = None
    ...
    async def parent_flow():
        ...
        return syncmatrix.context.get_run_context().flow_run.id, child_flow_run_id
...
async def test_interrupt_crashes_flow(syncmatrix_client, interrupt_type):
    flow_run_id = None

    @syncmatrix.flow
    async def my_flow():
        nonlocal flow_run_id
        flow_run_id = syncmatrix.context.get_run_context().flow_run.id
...
async def test_interrupt_in_child_crashes_parent_and_child_flow(
    syncmatrix_client, interrupt_type
):
    child_flow_run_id = parent_flow_run_id = None
    ...
    async def child_flow():
        ...
        child_flow_run_id = syncmatrix.context.get_run_context().flow_run.id
        raise interrupt_type()

    @syncmatrix.flow
    async def parent_flow():
        nonlocal parent_flow_run_id
        parent_flow_run_id = syncmatrix.context.get_run_context().flow_run.id
        await child_flow()
...
async def test_sigterm_crashes_flow(syncmatrix_client, mock_sigterm_handler):
    flow_run_id = None
...
def test_sigterm_crashes_deployed_flow(
    syncmatrix_client, mock_sigterm_handler, monkeypatch, flow_run
):
    ...
    from syncmatrix.engine import enter_flow_run_engine_from_subprocess

    @syncmatrix.flow
    async def my_flow():
        assert syncmatrix.context.get_run_context().flow_run.id == flow_run.id
        os.kill(os.getpid(), signal.SIGTERM)

    # Patch `load_flow_from_flow_run` to bypass all deployment loading logic and use
    # our test flow
    monkeypatch.setattr(
        "syncmatrix.engine.load_flow_from_flow_run", AsyncMock(return_value=my_flow)
    )
    ...
    flow_run = asyncio.run(syncmatrix_client.read_flow_run(flow_run.id))
    asyncio.run(
