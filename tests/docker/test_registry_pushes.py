...
def howdy(docker: DockerClient, worker_id: str, frozen_now: pendulum.DateTime) -> str:
    ...
    with ImageBuilder("busybox") as image:
        image.add_line(f"LABEL io.syncmatrix.test-worker {worker_id}")
...
def test_pushing_with_owner(
    ...
    registry_tag = push_image(howdy, registry, "khulnasoft/howdy")
    assert registry_tag.startswith(f"localhost:5555/khulnasoft/howdy:{tag_prefix}")
