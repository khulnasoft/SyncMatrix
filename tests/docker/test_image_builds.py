...
def test_image_builder_allocates_temporary_context(syncmatrix_base_image: str):
    with ImageBuilder(syncmatrix_base_image) as image:
...
def test_from_syncmatrix_image(docker: DockerClient, syncmatrix_base_image: str):
    with ImageBuilder(syncmatrix_base_image) as image:
        image.add_line("RUN echo Woooo, building")
        image.add_line('ENTRYPOINT [ "syncmatrix", "--version" ]')
        image_id = image.build()

    output = docker.containers.run(image_id, remove=True).decode().strip()
    assert output == syncmatrix.__version__


def test_copying_file(contexts: Path, docker: DockerClient, syncmatrix_base_image: str):
    with ImageBuilder(syncmatrix_base_image) as image:
...
def test_copied_paths_are_resolved(
    contexts: Path, docker: DockerClient, syncmatrix_base_image: str
):
    with ImageBuilder(syncmatrix_base_image) as image:
        image.add_line("WORKDIR /tiny/")
...
def test_copying_file_from_alternative_base(
    ...
    with ImageBuilder(syncmatrix_base_image, base_directory=contexts / "tiny") as image:
        image.add_line("WORKDIR /tiny/")
...
def test_can_use_working_tree_as_context(
    ...
    with ImageBuilder(syncmatrix_base_image, context=contexts / "no-dockerfile") as image:
        image.add_line("WORKDIR /tiny/")
        image.copy("hello.txt", "hello.txt")
        image.add_line('ENTRYPOINT [ "/bin/cat", "hello.txt" ]')
        image_id = image.build()
...
def test_cannot_already_have_a_dockerfile_in_context(
    contexts: Path, syncmatrix_base_image: str
):
    with pytest.raises(ValueError, match="already a Dockerfile"):
        ImageBuilder(syncmatrix_base_image, context=contexts / "tiny")
...
def test_generate_dockerfile_with_no_requirements(contexts):
    with generate_default_dockerfile(contexts / "no-dockerfile") as dockerfile:
        ...
        assert dockerfile.read_text() == dedent(
            f"""\
                    FROM {get_syncmatrix_image_name()}
                    COPY . /opt/syncmatrix/no-dockerfile/
                    WORKDIR /opt/syncmatrix/no-dockerfile/
                    """
        )
...
def test_generate_dockerfile_with_requirements(contexts):
    with generate_default_dockerfile(contexts / "requirements") as dockerfile:
        ...
        assert dockerfile.read_text() == dedent(
            f"""\
                    FROM {get_syncmatrix_image_name()}
                    COPY requirements.txt /opt/syncmatrix/requirements/requirements.txt
                    RUN python -m pip install -r /opt/syncmatrix/requirements/requirements.txt
                    COPY . /opt/syncmatrix/requirements/
                    WORKDIR /opt/syncmatrix/requirements/
                    """
        )
