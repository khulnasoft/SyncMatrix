...
from syncmatrix.software.pip import PipRequirement, current_environment_requirements


class TestPipRequirement:
    def is_packaging_subclass(self):
        r = PipRequirement("syncmatrix")
    ...
    def test_can_be_used_in_pydantic_model(self):
        ...
        inst = MyModel(req="syncmatrix")
        assert inst.req == PipRequirement("syncmatrix")
        assert isinstance(inst.req, PipRequirement)

    def test_equality(self):
        assert PipRequirement("syncmatrix") == PipRequirement("syncmatrix")
        assert PipRequirement("syncmatrix") != PipRequirement("syncmatrix>=2")
...
def test_current_environment_requirements():
    ...
    assert "syncmatrix" not in names  # Editable install is excluded
    assert len(names) == len(set(names)), "Names should not be repeated"


def test_current_environment_requirements_warns_about_editable_syncmatrix():
    with pytest.warns(
        UserWarning,
        match=r"syncmatrix.*is an editable installation",
    ):
        requirements = current_environment_requirements(
    ...
    assert all(isinstance(r, PipRequirement) for r in requirements)
    names = [r.name for r in requirements]
    assert "syncmatrix" not in names
    assert len(names) == len(set(names)), "Names should not be repeated"


def test_current_environment_requirements_raises_on_editable_syncmatrix():
    with pytest.raises(
        ValueError,
        match=r"syncmatrix.*is an editable installation",
    ):
