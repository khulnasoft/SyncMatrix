"""
Tests for `syncmatrix._internal.pydantic.model_json_schema` function.
"""
import pytest
from pydantic import BaseModel

from syncmatrix._internal.pydantic import HAS_PYDANTIC_V2, model_json_schema
from syncmatrix.settings import (
    SYNCMATRIX_EXPERIMENTAL_ENABLE_PYDANTIC_V2_INTERNALS,
    temporary_settings,
)
...
def enable_v2_internals():
    with temporary_settings({SYNCMATRIX_EXPERIMENTAL_ENABLE_PYDANTIC_V2_INTERNALS: True}):
        yield
...
def test_model_json_schema_with_flag_disabled(caplog):
    ...
    with temporary_settings({SYNCMATRIX_EXPERIMENTAL_ENABLE_PYDANTIC_V2_INTERNALS: False}):
        schema = model_json_schema(TestModel)
