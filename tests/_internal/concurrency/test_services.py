...
from syncmatrix._internal.concurrency.services import (
    BatchedQueueService,
    QueueService,
    drain_on_exit,
    drain_on_exit_async,
...
from syncmatrix.settings import SYNCMATRIX_LOGGING_INTERNAL_LEVEL, temporary_settings
...
def test_drain_on_global_loop_shutdown():
    # Regression test https://github.com/KhulnaSoft/syncmatrix/issues/9275#issuecomment-1520468276
    for i in range(10):
        MockService.instance().send(i)
...
def test_queue_service_item_failure_contains_traceback_only_at_debug(
    ...
    with temporary_settings({SYNCMATRIX_LOGGING_INTERNAL_LEVEL: level}):
