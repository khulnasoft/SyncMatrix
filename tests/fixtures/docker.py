...
import syncmatrix
from syncmatrix.cli.dev import dev_app
from syncmatrix.infrastructure.container import CONTAINER_LABELS
...
def cleanup_all_new_docker_objects(docker: DockerClient, worker_id: str):
    IMAGE_LABELS["io.syncmatrix.test-worker"] = worker_id
    CONTAINER_LABELS["io.syncmatrix.test-worker"] = worker_id
    try:
        yield
    finally:
        try:
            for container in docker.containers.list(all=True):
                if container.labels.get("io.syncmatrix.test-worker") == worker_id:
                    _safe_remove_container(container)
                elif container.labels.get("io.syncmatrix.delete-me"):
                    _safe_remove_container(container)

            filters = {"label": f"io.syncmatrix.test-worker={worker_id}"}
...
@pytest.fixture(scope="session")
def syncmatrix_base_image(pytestconfig: "pytest.Config", docker: DockerClient):
    """Ensure that the syncmatrix dev image is available and up-to-date"""
    ...
    if image_exists:
        output = docker.containers.run(
            image_name, ["syncmatrix", "--version"], remove=True
        )
        image_version = output.decode().strip()
        version_is_right = image_version == syncmatrix.__version__
    ...
    if not image_exists or not version_is_right:
        if pytestconfig.getoption("--disable-docker-image-builds"):
            ...
            if not version_is_right:
                raise Exception(
                    "The --disable-docker-image-builds flag is set, but "
                    f"{image_name} includes {image_version}, not {syncmatrix.__version__}"
                )
        else:
