...
from syncmatrix.blocks.notifications import NotificationBlock
from syncmatrix.filesystems import LocalFileSystem
from syncmatrix.infrastructure import DockerContainer, Process
from syncmatrix.server import models, schemas
from syncmatrix.server.database.configurations import ENGINES, TRACKER
...
@pytest.fixture(scope="session", autouse=True)
async def database_engine(db: SyncmatrixDBInterface):
...
@pytest.fixture
async def storage_document_id(syncmatrix_client, tmpdir):
    return await LocalFileSystem(basepath=str(tmpdir)).save(
        name=f"local-test-{uuid.uuid4()}", client=syncmatrix_client
    )


@pytest.fixture
async def infrastructure_document_id(syncmatrix_client):
    return await Process(env={"MY_TEST_VARIABLE": 1})._save(
        is_anonymous=True, client=syncmatrix_client
    )


@pytest.fixture
async def infrastructure_document_id_2(syncmatrix_client):
...
@pytest.fixture
async def syncmatrix_agent_work_pool(session):
    model = await models.workers.create_work_pool(
        session=session,
        work_pool=schemas.actions.WorkPoolCreate(
            name="process-work-pool",
...
@pytest.fixture
async def notifier_block(syncmatrix_client):
