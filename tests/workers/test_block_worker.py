...
from syncmatrix.workers.block import (
    BlockWorker,
...
@pytest.fixture
async def block_work_pool(syncmatrix_client, mock_infra_block_doc_id):
...
async def test_block_worker_run(
    ...
    monkeypatch.setattr(
        "syncmatrix.workers.block.Block._from_block_document",
    ...
    assert {
        call.kwargs["env"]["SYNCMATRIX__FLOW_RUN_ID"]
...
async def test_block_worker_run_infra_overrides(
    ...
    monkeypatch.setattr(
        "syncmatrix.workers.block.Block._from_block_document", capture_block
    )
