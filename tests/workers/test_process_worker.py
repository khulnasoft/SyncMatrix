...
from syncmatrix.client.schemas import State
from syncmatrix.exceptions import InfrastructureNotAvailable
from syncmatrix.server.schemas.core import WorkPool
...
from syncmatrix.workers.process import (
...
def patch_run_process(monkeypatch):
    def patch_run_process(returncode=0, pid=1000):
        ...
        monkeypatch.setattr(syncmatrix.workers.process, "run_process", mock_run_process)
...
@pytest.fixture
async def flow_run(syncmatrix_client: SyncmatrixClient):
...
@pytest.fixture
async def flow_run_with_overrides(syncmatrix_client: SyncmatrixClient):
    flow_run = await syncmatrix_client.create_flow_run(
        flow=example_process_worker_flow,
        state=State(
            type=StateType.SCHEDULED,
    ...
    return await syncmatrix_client.read_flow_run(flow_run.id)
...
def mock_open_process(monkeypatch):
    if sys.platform == "win32":
        monkeypatch.setattr(
            "syncmatrix.utilities.processutils._open_anyio_process", AsyncMock()
        )
        syncmatrix.utilities.processutils._open_anyio_process.return_value.terminate = (  # noqa
            MagicMock()
        )

        yield syncmatrix.utilities.processutils._open_anyio_process  # noqa
    else:
...
def patch_client(monkeypatch, overrides: dict = None):
    ...
    monkeypatch.setattr("syncmatrix.workers.base.get_client", mock_get_client)
...
async def test_worker_process_run_flow_run(
    ...
    async with ProcessWorker(
        ...
        assert mock.call_args.kwargs["env"]["SYNCMATRIX__FLOW_RUN_ID"] == str(flow_run.id)
...
async def test_process_kill_early_return(monkeypatch, work_pool):
    ...
    monkeypatch.setattr("os.kill", os_kill)
    monkeypatch.setattr("syncmatrix.infrastructure.process.anyio.sleep", anyio_sleep)
