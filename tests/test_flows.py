...
from syncmatrix import flow, get_run_logger, runtime, tags, task
...
from syncmatrix.context import SyncmatrixObjectRegistry
from syncmatrix.deployments.runner import DeploymentImage, EntrypointType, RunnerDeployment
...
from syncmatrix.filesystems import LocalFileSystem
from syncmatrix.flows import Flow, load_flow_from_entrypoint
from syncmatrix.runtime import flow_run as flow_run_ctx
from syncmatrix.server.schemas.core import TaskRunResult
from syncmatrix.server.schemas.filters import FlowFilter, FlowRunFilter
from syncmatrix.server.schemas.sorting import FlowRunSort
from syncmatrix.settings import (
    SYNCMATRIX_FLOW_DEFAULT_RETRIES,
    temporary_settings,
)
...
from syncmatrix.task_runners import ConcurrentTaskRunner, SequentialTaskRunner
from syncmatrix.testing.utilities import (
    AsyncMock,
    exceptions_equal,
    get_most_recent_flow_run,
)
from syncmatrix.utilities.annotations import allow_failure, quote
from syncmatrix.utilities.callables import parameter_schema
from syncmatrix.utilities.collections import flatdict_to_dict
from syncmatrix.utilities.hashing import file_hash
...
class TestFlow:
    ...
    def test_version_none_if_source_file_cannot_be_determined(
        ...
        """
        monkeypatch.setattr(
            "syncmatrix.flows.inspect.getsourcefile", MagicMock(return_value=sourcefile)
        )
...
class TestFlowCall:
    ...
    async def test_call_execution_blocked_does_not_run_flow(self):
        ...
        with SyncmatrixObjectRegistry(block_code_execution=True):
            state = foo(1, 2)
...
class TestSubflowCalls:
    ...
    async def test_subflow_with_invalid_parameters_is_failed(self, syncmatrix_client):
        @flow
        def child(x: int):
            return x
        ...
        flow_run = await syncmatrix_client.read_flow_run(
    ...
    async def test_subflow_relationship_tracking(self, syncmatrix_client):
        @flow(version="inner")
        def child(x, y):
            return x + y
        ...
        child_flow_run = await syncmatrix_client.read_flow_run(child_flow_run_id)

        # This task represents the child flow run in the parent
        parent_flow_run_task = await syncmatrix_client.read_task_run(
        ...
        assert all(
            ...
            for state in await syncmatrix_client.read_flow_run_states(child_flow_run_id)
    ...
    async def test_sync_flow_with_async_subflow_and_task_that_awaits_result(self):
        """
        Regression test for https://github.com/KhulnaSoft/syncmatrix/issues/12053, where
...
class TestFlowRunTags:
    async def test_flow_run_tags_added_at_call(self, syncmatrix_client):
        @flow
        def my_flow():
            pass
    ...
    async def test_flow_run_tags_added_to_subflows(self, syncmatrix_client):
        @flow
        def my_flow():
            with tags("c", "d"):
...
class TestFlowTimeouts:
    ...
    async def test_subflow_timeout_waits_until_execution_starts(self, tmp_path):
        ...
        Fixes: https://github.com/KhulnaSoft/syncmatrix/issues/7903.
        """
...
class TestSubflowTaskInputs:
    async def test_subflow_with_one_upstream_task_future(self, syncmatrix_client):
        @task
        def child_task(x):
            return x
        ...
        flow_tracking_task_run = await syncmatrix_client.read_task_run(
    ...
    async def test_subflow_with_one_upstream_task_state(self, syncmatrix_client):
        @task
        def child_task(x):
            return x
    ...
    async def test_subflow_with_one_upstream_task_result(self, syncmatrix_client):
        @task
        def child_task(x):
            return x
    ...
    async def test_subflow_with_no_upstream_tasks(self, syncmatrix_client):
        @flow
        def bar(x, y):
            return x + y
...
class TestFlowRunLogs:
    async def test_user_logs_are_sent_to_orion(self, syncmatrix_client):
        @flow
        def my_flow():
    ...
    async def test_repeated_flow_calls_send_logs_to_orion(self, syncmatrix_client):
        @flow
        async def my_flow(i):
            logger = get_run_logger()
    ...
    async def test_exception_info_is_included_in_log(self, syncmatrix_client):
        @flow
        def my_flow():
            logger = get_run_logger()
            try:
    ...
    async def test_raised_exceptions_include_tracebacks(self, syncmatrix_client):
        @flow
        def my_flow():
    ...
    async def test_opt_out_logs_are_not_sent_to_api(self, syncmatrix_client):
        @flow
        def my_flow():
            logger = get_run_logger()
            logger.info(
    ...
    async def test_logs_are_given_correct_id(self, syncmatrix_client):
        @flow
        def my_flow():
            logger = get_run_logger()
            logger.info("Hello world!")
...
class TestSubflowRunLogs:
    async def test_subflow_logs_are_written_correctly(self, syncmatrix_client):
        @flow
        def my_subflow():
            logger = get_run_logger()
    ...
    async def test_subflow_logs_are_written_correctly_with_tasks(self, syncmatrix_client):
        @task
        def a_log_task():
            logger = get_run_logger()
...
class TestFlowRetries:
    ...
    @pytest.mark.xfail
    async def test_flow_retry_with_branched_tasks(self, syncmatrix_client):
        flow_run_count = 0
        ...
        result = await syncmatrix_client.retrieve_data(document)
    ...
    async def test_flow_retry_with_error_in_flow_and_one_failed_child_flow(
        ...
        child_flow_run = await syncmatrix_client.read_flow_run(
            child_state.state_details.flow_run_id
        )
        child_flow_runs = await syncmatrix_client.read_flow_runs(
    ...
    def test_global_retry_config(self):
        with temporary_settings(updates={SYNCMATRIX_FLOW_DEFAULT_RETRIES: "1"}):
            run_count = 0
...
def test_load_flow_from_entrypoint_with_module_path(monkeypatch):
    ...
    monkeypatch.setattr(
        "syncmatrix.flows.import_object",
        import_object_mock,
    )
...
async def test_handling_script_with_unprotected_call_in_flow_script(
    ...
    flow_code_with_call = """
    from syncmatrix import flow, get_run_logger
    ...
    flow_runs = await syncmatrix_client.read_flows()
...
class TestFlowRunName:
    ...
    async def test_sets_run_name_when_provided(self, syncmatrix_client):
    ...
    async def test_sets_run_name_with_params_including_defaults(self, syncmatrix_client):
    ...
    async def test_sets_run_name_with_function(self, syncmatrix_client):
        def generate_flow_run_name():
            return "hi"
...
class TestFlowHooksOnCancellation:
    ...
    async def test_on_cancellation_hook_called_on_sigterm_from_flow_with_cancelling_state(
        ...
        with pytest.raises(syncmatrix.exceptions.TerminationSignal):
            await my_flow._run()
    ...
    def test_on_cancellation_hooks_respect_env_var(self, monkeypatch):
        my_mock = MagicMock()
        monkeypatch.setenv("SYNCMATRIX__ENABLE_CANCELLATION_AND_CRASHED_HOOKS", "false")
...
class TestFlowServe:
    ...
    async def test_serve_prints_message(self, capsys):
        ...
        assert "$ syncmatrix deployment run 'test-flow/test'" in captured.out

    async def test_serve_creates_deployment(self, syncmatrix_client: SyncmatrixClient):
        ...
        deployment = await syncmatrix_client.read_deployment_by_name(name="test-flow/test")
    ...
    async def test_serve_can_user_a_module_path_entrypoint(self, syncmatrix_client):
    ...
    async def test_serve_handles__file__(self, syncmatrix_client: SyncmatrixClient):
    ...
    async def test_serve_passes_limit_specification_to_runner(self, monkeypatch):
        ...
        monkeypatch.setattr("syncmatrix.runner.Runner", runner_mock)
...
class TestFlowFromSource:
    ...
    async def test_load_flow_from_source_with_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "syncmatrix.flows.create_storage_from_url", mock_create_storage_from_url
...
class TestFlowDeploy:
    ...
    def mock_deploy(self, monkeypatch):
        mock = AsyncMock()
        monkeypatch.setattr("syncmatrix.flows.deploy", mock)
        return mock
    ...
    async def test_calls_deploy_with_expected_args(
        ...
        assert "syncmatrix worker start --pool" in console_output
        assert work_pool.name in console_output
        assert "syncmatrix deployment run 'local-flow-deploy/test'" in console_output
    ...
    async def test_no_worker_command_for_push_pool(
        ...
        assert "syncmatrix worker start" not in capsys.readouterr().out
