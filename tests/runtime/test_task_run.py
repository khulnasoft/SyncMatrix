...
from syncmatrix.runtime import task_run
...
class TestAttributeAccessPatterns:
    ...
    async def test_import_unknown_attribute_fails(self):
        with pytest.raises(ImportError, match="boop"):
            from syncmatrix.runtime.task_run import boop  # noqa
    ...
    async def test_new_attribute_via_env_var(self, monkeypatch):
        monkeypatch.setenv(name="SYNCMATRIX__RUNTIME__TASK_RUN__NEW_KEY", value="foobar")
    ...
    async def test_attribute_override_via_env_var(
        ...
        monkeypatch.setenv(
            name=f"SYNCMATRIX__RUNTIME__TASK_RUN__{attribute_name.upper()}",
            value=env_value,
        )
    ...
    async def test_attribute_override_via_env_var_not_allowed(
        ...
        monkeypatch.setenv(
            name=f"SYNCMATRIX__RUNTIME__TASK_RUN__{attribute_name.upper()}", value="foo"
        )
