...
from syncmatrix import flow, states, tags, task
from syncmatrix.client.schemas import FlowRun, TaskRun
from syncmatrix.context import FlowRunContext, TaskRunContext
from syncmatrix.flows import Flow
from syncmatrix.runtime import flow_run
from syncmatrix.settings import SYNCMATRIX_API_URL, SYNCMATRIX_UI_URL
...
class TestAttributeAccessPatterns:
    ...
    async def test_import_unknown_attribute_fails(self):
        with pytest.raises(ImportError, match="boop"):
            from syncmatrix.runtime.flow_run import boop  # noqa
    ...
    async def test_new_attribute_via_env_var(self, monkeypatch):
        monkeypatch.setenv(name="SYNCMATRIX__RUNTIME__FLOW_RUN__NEW_KEY", value="foobar")
    ...
    async def test_attribute_override_via_env_var(
        ...
        monkeypatch.setenv(
            name=f"SYNCMATRIX__RUNTIME__FLOW_RUN__{attribute_name.upper()}",
            value=env_value,
        )
    ...
    async def test_attribute_override_via_env_var_not_allowed(
        ...
        monkeypatch.setenv(
            name=f"SYNCMATRIX__RUNTIME__FLOW_RUN__{attribute_name.upper()}", value="foo"
        )
...
class TestID:
    ...
    async def test_id_uses_env_var_when_set(self, monkeypatch):
        monkeypatch.setenv(name="SYNCMATRIX__FLOW_RUN_ID", value="foo")
...
class TestTags:
    ...
    async def test_tags_pulls_from_api_when_needed(self, monkeypatch, syncmatrix_client):
        run = await syncmatrix_client.create_flow_run(
            flow=flow(lambda: None, name="test"), tags=["red", "green"]
        )
        assert flow_run.tags == []

        monkeypatch.setenv(name="SYNCMATRIX__FLOW_RUN_ID", value=str(run.id))
...
class TestRunCount:
    ...
    async def test_run_count_from_api(self, monkeypatch, syncmatrix_client):
        run = await syncmatrix_client.create_flow_run(
...
class TestName:
    ...
    async def test_name_pulls_from_api_when_needed(self, monkeypatch, syncmatrix_client):
        run = await syncmatrix_client.create_flow_run(
...
class TestParameters:
    ...
    async def test_parameters_from_api(self, monkeypatch, syncmatrix_client):
        run = await syncmatrix_client.create_flow_run(
    ...
    async def test_outside_flow_run_uses_serialized_parameters(self, monkeypatch):
        ...
        monkeypatch.setenv(name="SYNCMATRIX__FLOW_RUN_ID", value=flow_run_id)
...
class TestParentFlowRunId:
    ...
    async def test_parent_flow_run_id_returns_parent_flow_run_id_when_present_dynamically(
        ...
        parent_flow_run = await syncmatrix_client.create_flow_run(
        ...
        parent_task_run = await syncmatrix_client.create_task_run(
    ...
    async def test_parent_flow_run_id_pulls_from_api_when_needed(
        ...
        child_flow_run = await syncmatrix_client.create_flow_run(
            flow=Flow(fn=lambda: None, name="child"),
            parameters={"x": "foo", "y": "bar"},
            parent_task_run_id=parent_task_run.id,
        )

        monkeypatch.setenv(name="SYNCMATRIX__FLOW_RUN_ID", value=str(child_flow_run.id))
        assert (
            flow_run.parent_flow_run_id
            == parent_flow_run.id
            == parent_task_run.flow_run_id
        ...
        monkeypatch.setenv(name="SYNCMATRIX__FLOW_RUN_ID", value=str(parent_flow_run.id))
        assert flow_run.parent_flow_run_id is None
...
class TestParentDeploymentId:
    ...
    async def test_parent_deployment_id_returns_parent_deployment_id_when_present_dynamically(
        ...
        parent_flow_id = await syncmatrix_client.create_flow(parent)

        # Parent flow run that does not have a deployment
        parent_flow_run_no_deployment = await syncmatrix_client.create_flow_run(
            flow=parent,
        )
        parent_task_run_no_deployment = await syncmatrix_client.create_task_run(
        ...
        parent_flow_deployment_id = await syncmatrix_client.create_deployment(
        ...
        parent_task_run_with_deployment = await syncmatrix_client.create_task_run(
    ...
    async def test_parent_deployment_id_pulls_from_api_when_needed(
        ...
        child_flow_run_no_deployment = await syncmatrix_client.create_flow_run(
        ...
        monkeypatch.setenv(
            name="SYNCMATRIX__FLOW_RUN_ID", value=str(child_flow_run_no_deployment.id)
        )
        assert flow_run.parent_deployment_id is None
        ...
        child_flow_run_with_deployment = await syncmatrix_client.create_flow_run(
        ...
        monkeypatch.setenv(
            name="SYNCMATRIX__FLOW_RUN_ID", value=str(child_flow_run_with_deployment.id)
        )
        assert flow_run.parent_deployment_id == parent_flow_deployment_id

        # No parent flow run
        monkeypatch.setenv(
            name="SYNCMATRIX__FLOW_RUN_ID", value=str(parent_flow_run_no_deployment.id)
        )
        assert flow_run.parent_deployment_id is None
...
class TestURL:
    ...
    @pytest.mark.parametrize(
        ...
        [("api_url", SYNCMATRIX_API_URL.value()), ("ui_url", SYNCMATRIX_UI_URL.value())],
    )
    ...
    async def test_url_pulls_from_api_when_needed(
        ...
        run = await syncmatrix_client.create_flow_run(flow=flow(lambda: None, name="test"))
