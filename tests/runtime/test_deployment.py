...
@pytest.fixture
async def deployment_id(flow, syncmatrix_client):
    response = await syncmatrix_client.create_deployment(
        name="My Deployment",
        version="gold",
        flow_id=flow.id,
        tags=["foo"],
...
class TestAttributeAccessPatterns:
    ...
    async def test_import_unknown_attribute_fails(self):
        with pytest.raises(ImportError, match="boop"):
            from syncmatrix.runtime.deployment import boop  # noqa
    ...
    async def test_new_attribute_via_env_var(self, monkeypatch):
        monkeypatch.setenv(name="SYNCMATRIX__RUNTIME__DEPLOYMENT__NEW_KEY", value="foobar")
    ...
    async def test_attribute_override_via_env_var(
        ...
        monkeypatch.setenv(
            name=f"SYNCMATRIX__RUNTIME__DEPLOYMENT__{attribute_name.upper()}",
            value=env_value,
        )
    ...
    async def test_attribute_override_via_env_var_not_allowed(
        ...
        monkeypatch.setenv(
            name=f"SYNCMATRIX__RUNTIME__DEPLOYMENT__{attribute_name.upper()}", value="foo"
        )
...
class TestID:
    ...
    async def test_id_is_none_when_not_set(self, monkeypatch, syncmatrix_client):
    ...
    async def test_id_is_loaded_when_run_id_known(
        self, deployment_id, monkeypatch, syncmatrix_client
    ):
        flow_run = await syncmatrix_client.create_flow_run_from_deployment(deployment_id)

        assert deployment.id is None

        monkeypatch.setenv(name="SYNCMATRIX__FLOW_RUN_ID", value=str(flow_run.id))
...
class TestName:
    ...
    async def test_name_is_none_when_not_set(self, monkeypatch, syncmatrix_client):
...
class TestVersion:
    ...
    async def test_version_is_none_when_not_set(self, monkeypatch, syncmatrix_client):
