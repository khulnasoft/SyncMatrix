...
from syncmatrix._internal.concurrency.api import create_call, from_sync
from syncmatrix.context import FlowRunContext, TaskRunContext
from syncmatrix.deprecated.data_documents import _retrieve_result
from syncmatrix.exceptions import MissingContextError
from syncmatrix.infrastructure import Process
from syncmatrix.logging.configuration import (
    DEFAULT_LOGGING_SETTINGS_PATH,
    load_logging_config,
    setup_logging,
)
from syncmatrix.logging.filters import ObfuscateApiKeyFilter
from syncmatrix.logging.formatters import JsonFormatter
from syncmatrix.logging.handlers import APILogHandler, APILogWorker, SyncmatrixConsoleHandler
from syncmatrix.logging.highlighters import SyncmatrixConsoleHighlighter
from syncmatrix.logging.loggers import (
    SyncmatrixLogAdapter,
    disable_logger,
    disable_run_logger,
    flow_run_logger,
    get_logger,
...
from syncmatrix.server.schemas.actions import LogCreate
from syncmatrix.settings import (
    SYNCMATRIX_API_KEY,
    SYNCMATRIX_LOGGING_COLORS,
    SYNCMATRIX_LOGGING_LEVEL,
    SYNCMATRIX_LOGGING_MARKUP,
    SYNCMATRIX_LOGGING_SETTINGS_PATH,
    SYNCMATRIX_LOGGING_TO_API_BATCH_INTERVAL,
    SYNCMATRIX_LOGGING_TO_API_BATCH_SIZE,
    SYNCMATRIX_LOGGING_TO_API_ENABLED,
    SYNCMATRIX_LOGGING_TO_API_MAX_LOG_SIZE,
    SYNCMATRIX_LOGGING_TO_API_WHEN_MISSING_FLOW,
    temporary_settings,
)
from syncmatrix.testing.cli import temporary_console_width
...
def dictConfigMock(monkeypatch):
    ...
    old = syncmatrix.logging.configuration.PROCESS_LOGGING_CONFIG
    syncmatrix.logging.configuration.PROCESS_LOGGING_CONFIG = None
    yield mock
    syncmatrix.logging.configuration.PROCESS_LOGGING_CONFIG = old


@pytest.fixture
async def logger_test_deployment(syncmatrix_client):
    """
    A deployment with a flow that returns information about the given loggers
    """

    @syncmatrix.flow
    def my_flow(loggers=["foo", "bar", "syncmatrix"]):
        import logging
...
def test_setup_logging_uses_default_path(tmp_path, dictConfigMock):
    with temporary_settings(
        {SYNCMATRIX_LOGGING_SETTINGS_PATH: tmp_path.joinpath("does-not-exist.yaml")}
    ):
...
def test_setup_logging_uses_settings_path_if_exists(tmp_path, dictConfigMock):
    ...
    with temporary_settings({SYNCMATRIX_LOGGING_SETTINGS_PATH: config_file}):
        setup_logging()
...
def test_setup_logging_uses_env_var_overrides(tmp_path, dictConfigMock, monkeypatch):
    ...
    env["SYNCMATRIX_LOGGING_FOO"] = "IGNORED"
...
async def test_flow_run_respects_extra_loggers(syncmatrix_client, logger_test_deployment):
    """
    Runs a flow in a subprocess to check that SYNCMATRIX_LOGGING_EXTRA_LOGGERS works as
    ...
    assert (
        await Process(env={"SYNCMATRIX_LOGGING_EXTRA_LOGGERS": "foo"})
        .prepare_for_flow_run(flow_run)
        .run()
    )

    state = (await syncmatrix_client.read_flow_run(flow_run.id)).state
    settings = await _retrieve_result(state, syncmatrix_client)
    api_logs = await syncmatrix_client.read_logs()
    api_log_messages = [log.message for log in api_logs]

    extra_logger = logging.getLogger("syncmatrix.extra")

    # Configures 'foo' to match 'syncmatrix.extra'
...
def test_get_logger_returns_syncmatrix_logger_by_default(name):
    if name == "default":
        logger = get_logger()
    else:
        logger = get_logger(name)

    assert logger.name == "syncmatrix"


def test_get_logger_returns_syncmatrix_child_logger():
    logger = get_logger("foo")
    assert logger.name == "syncmatrix.foo"


def test_get_logger_does_not_duplicate_syncmatrix_prefix():
    logger = get_logger("syncmatrix.foo")
    assert logger.name == "syncmatrix.foo"


def test_default_level_is_applied_to_interpolated_yaml_values(dictConfigMock):
    with temporary_settings({SYNCMATRIX_LOGGING_LEVEL: "WARNING"}):
        expected_config = load_logging_config(DEFAULT_LOGGING_SETTINGS_PATH)
        expected_config["incremental"] = False

        assert expected_config["loggers"]["syncmatrix"]["level"] == "WARNING"
        assert expected_config["loggers"]["syncmatrix.extra"]["level"] == "WARNING"
...
def mock_log_worker(monkeypatch):
    mock = MagicMock()
    monkeypatch.setattr("syncmatrix.logging.handlers.APILogWorker", mock)
    return mock
...
class TestAPILogHandler:
    ...
    async def test_logs_can_still_be_sent_after_close(
        self, logger, handler, flow_run, syncmatrix_client
    ):
    ...
    def test_does_not_send_logs_that_opt_out_deprecated(
        ...
        with TaskRunContext.construct(task_run=task_run):
            with pytest.warns(
                ...
                SyncmatrixLogAdapter(logger, extra={}).info(
    ...
    def test_does_not_send_logs_when_handler_is_disabled(
        ...
        with temporary_settings(
            updates={SYNCMATRIX_LOGGING_TO_API_ENABLED: "False"},
        ):
            with TaskRunContext.construct(task_run=task_run):
                logger.info("test")
    ...
    def test_does_not_send_logs_outside_of_run_context_with_error_setting(
        ...
        with temporary_settings(
            updates={SYNCMATRIX_LOGGING_TO_API_WHEN_MISSING_FLOW: "error"},
        ):
            with pytest.raises(
                MissingContextError,
    ...
    def test_does_not_send_logs_outside_of_run_context_with_ignore_setting(
        ...
        with temporary_settings(
            updates={SYNCMATRIX_LOGGING_TO_API_WHEN_MISSING_FLOW: "ignore"},
        ):
            logger.info("test")
    ...
    def test_does_not_send_logs_outside_of_run_context_with_warn_setting(
        ...
        with temporary_settings(
            updates={SYNCMATRIX_LOGGING_TO_API_WHEN_MISSING_FLOW: "warn"},
        ):
            # Warns in the main process
            with pytest.warns(
                UserWarning, match="attempted to send logs .* without a flow run id"
    ...
    def test_writes_logging_errors_to_stderr(
        ...
        monkeypatch.setattr(
            "syncmatrix.logging.handlers.APILogHandler.prepare",
    ...
    async def test_does_not_enqueue_logs_that_are_too_big(
        ...
        with TaskRunContext.construct(task_run=task_run):
            with temporary_settings(updates={SYNCMATRIX_LOGGING_TO_API_MAX_LOG_SIZE: "1"}):
                logger.info("test")
...
class TestAPILogWorker:
    ...
    async def test_send_logs_single_record(self, log_dict, syncmatrix_client, worker):
    ...
    async def test_send_logs_many_records(self, log_dict, syncmatrix_client, worker):
        # Use the read limit as the count since we'd need multiple read calls otherwise
        count = syncmatrix.settings.SYNCMATRIX_API_DEFAULT_LIMIT.value()
    ...
    async def test_send_logs_writes_exceptions_to_stderr(
        ...
        monkeypatch.setattr(
            "syncmatrix.client.SyncmatrixClient.create_logs",
    ...
    async def test_send_logs_batches_by_size(self, log_dict, monkeypatch):
        ...
        monkeypatch.setattr(
            "syncmatrix.client.SyncmatrixClient.create_logs", mock_create_logs
        )
        ...
        with temporary_settings(
            updates={
                SYNCMATRIX_LOGGING_TO_API_BATCH_SIZE: log_size + 1,
                SYNCMATRIX_LOGGING_TO_API_MAX_LOG_SIZE: log_size,
            }
        ):
    ...
    async def test_logs_are_sent_immediately_when_stopped(
        self, log_dict, syncmatrix_client
    ):
        # Set a long interval
        start_time = time.time()
        with temporary_settings(updates={SYNCMATRIX_LOGGING_TO_API_BATCH_INTERVAL: "10"}):
            worker = APILogWorker.instance()
            worker.send(log_dict)
            worker.send(log_dict)
            await worker.drain()
    ...
    async def test_logs_are_sent_immediately_when_flushed(
        self, log_dict, syncmatrix_client, worker
    ):
        # Set a long interval
        start_time = time.time()
        with temporary_settings(updates={SYNCMATRIX_LOGGING_TO_API_BATCH_INTERVAL: "10"}):
...
def test_flow_run_logger(flow_run):
    ...
    assert logger.name == "syncmatrix.flow_runs"
    assert logger.extra == {
...
def test_task_run_logger(task_run):
    ...
    assert logger.name == "syncmatrix.task_runs"
    assert logger.extra == {
...
def test_run_logger_with_flow_run_context_without_parent_flow_run_id(caplog):
    ...
    with FlowRunContext.construct(flow_run=None, flow=None):
        ...
        assert "syncmatrix.flow_runs" in caplog.text
...
async def test_run_logger_with_task_run_context_without_parent_flow_run_id(
    ...
    task_run_context = TaskRunContext.construct(
        task=foo, task_run=task_run, client=syncmatrix_client
    )
    ...
    assert "syncmatrix.task_runs" in caplog.text
    assert "test3141592" in caplog.text
...
async def test_run_logger_with_explicit_context(
    syncmatrix_client, flow_run, local_filesystem
):
    @task
    def foo():
        pass

    task_run = await syncmatrix_client.create_task_run(foo, flow_run.id, dynamic_key="")
...
async def test_run_logger_in_flow(syncmatrix_client):
    @flow
    def test_flow():
...
async def test_run_logger_extra_data(syncmatrix_client):
    @flow
    def test_flow():
...
async def test_run_logger_in_nested_flow(syncmatrix_client):
    @flow
    def child_flow():
        return get_run_logger()
    ...
    flow_run = await syncmatrix_client.read_flow_run(child_state.state_details.flow_run_id)
...
async def test_run_logger_in_task(syncmatrix_client):
    @task
    def test_task():
        return get_run_logger()
    ...
    flow_run = await syncmatrix_client.read_flow_run(flow_state.state_details.flow_run_id)
...
class TestSyncmatrixConsoleHandler:
    @pytest.fixture
    def handler(self):
        yield SyncmatrixConsoleHandler()
    ...
    def test_init_defaults(self):
        handler = SyncmatrixConsoleHandler()
        console = handler.console
        assert isinstance(console, Console)
        assert isinstance(console.highlighter, SyncmatrixConsoleHighlighter)
        assert console._theme_stack._entries == [{}]  # inherit=False
        assert handler.level == logging.NOTSET

    def test_init_styled_console_disabled(self):
        with temporary_settings({SYNCMATRIX_LOGGING_COLORS: False}):
            handler = SyncmatrixConsoleHandler()
            console = handler.console
            assert isinstance(console, Console)
    ...
    def test_uses_stderr_by_default(self, capsys):
        ...
        logger.handlers = [SyncmatrixConsoleHandler()]
        logger.info("Test!")
    ...
    def test_respects_given_stream(self, capsys):
        logger = get_logger(uuid.uuid4().hex)
        logger.handlers = [SyncmatrixConsoleHandler(stream=sys.stdout)]
        logger.info("Test!")
        stdout, stderr = capsys.readouterr()
        assert stderr == ""
    ...
    def test_outputs_square_brackets_as_style(self, capsys):
        with temporary_settings({SYNCMATRIX_LOGGING_MARKUP: True}):
            logger = get_logger(uuid.uuid4().hex)
            handler = SyncmatrixConsoleHandler()
            logger.handlers = [handler]
...
class TestObfuscateApiKeyFilter:
    def test_filters_current_api_key(self):
        ...
        with temporary_settings({SYNCMATRIX_API_KEY: test_api_key}):
    ...
    def test_redact_substr_from_collections(
        ...
        """
        This is a regression test for https://github.com/KhulnaSoft/syncmatrix/issues/12139
        """
        ...
        with temporary_settings({SYNCMATRIX_API_KEY: "super-mega-admin-key"}):
            test_log_list()
...
def test_disable_run_logger(caplog):
    ...
    flow_run_logger = get_logger("syncmatrix.flow_run")
    task_run_logger = get_logger("syncmatrix.task_run")
...
def test_patch_print_writes_to_logger_with_task_run_context(caplog, capsys, task_run):
    ...
    assert record.name == "syncmatrix.task_runs"
