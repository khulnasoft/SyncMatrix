...
from syncmatrix.utilities.importtools import from_qualified_name, to_qualified_name
from syncmatrix.utilities.pydantic import add_type_dispatch

D = TypeVar("D")


def syncmatrix_json_object_encoder(obj: Any) -> Any:
    """
...
def syncmatrix_json_object_decoder(result: dict):
    """
    `JSONDecoder.object_hook` for decoding objects from JSON when previously encoded
    with `syncmatrix_json_object_encoder`
    """
...
class JSONSerializer(Serializer):
    ...
    object_encoder: Optional[str] = pydantic.Field(
        default="syncmatrix.serializers.syncmatrix_json_object_encoder",
        description=(
    ...
    object_decoder: Optional[str] = pydantic.Field(
        default="syncmatrix.serializers.syncmatrix_json_object_decoder",
        description=(
