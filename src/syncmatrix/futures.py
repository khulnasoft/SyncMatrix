...
from syncmatrix._internal.concurrency.event_loop import run_coroutine_in_loop_from_async
...
from syncmatrix.utilities.asyncutils import A, Async, Sync, sync
from syncmatrix.utilities.collections import StopVisiting, visit_collection
...
class SyncmatrixFuture(Generic[R, A]):
    """
    ...
    Examples:
        ...
        >>> @flow
        >>> def my_flow():
        >>>     future = my_task.submit()  # SyncmatrixFuture[str, Sync] includes result type
    ...
    @overload
    def wait(
        self: "SyncmatrixFuture[R, Async]", timeout: None = None
    ) -> Awaitable[State[R]]:
        ...

    @overload
    def wait(self: "SyncmatrixFuture[R, Sync]", timeout: None = None) -> State[R]:
        ...

    @overload
    def wait(
        self: "SyncmatrixFuture[R, Async]", timeout: float
    ) -> Awaitable[Optional[State[R]]]:
        ...

    @overload
    def wait(self: "SyncmatrixFuture[R, Sync]", timeout: float) -> Optional[State[R]]:
        ...
    ...
    @overload
    def get_state(
        self: "SyncmatrixFuture[R, Async]", client: SyncmatrixClient = None
    ) -> Awaitable[State[R]]:
        ...

    @overload
    def get_state(
        self: "SyncmatrixFuture[R, Sync]", client: SyncmatrixClient = None
    ) -> State[R]:
        ...

    def get_state(self, client: SyncmatrixClient = None):
        """
    ...
    @inject_client
    async def _get_state(self, client: SyncmatrixClient = None) -> State[R]:
    ...
    def __repr__(self) -> str:
        return f"SyncmatrixFuture({self.name!r})"

    def __bool__(self) -> bool:
        warnings.warn(
            (
                "A 'SyncmatrixFuture' from a task call was cast to a boolean; "
...
async def resolve_futures_to_data(
    expr: Union[SyncmatrixFuture[R, Any], Any],
    raise_on_failure: bool = True,
) -> Union[R, Any]:
    """
    Given a Python built-in collection, recursively find `SyncmatrixFutures` and build a
    ...
    """
    futures: Set[SyncmatrixFuture] = set()
