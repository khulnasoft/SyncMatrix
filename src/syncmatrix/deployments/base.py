"""
Core primitives for managing Syncmatrix projects.  Projects provide a minimally opinionated
...
from syncmatrix.client.schemas.schedules import IntervalSchedule
...
from syncmatrix.utilities.asyncutils import LazySemaphore, run_sync_in_worker_thread
from syncmatrix.utilities.filesystem import create_default_ignore_file, get_open_file_limit
from syncmatrix.utilities.templating import apply_values


def find_syncmatrix_directory(path: Path = None) -> Optional[Path]:
    """
    Given a path, recurses upward looking for .syncmatrix/ directories.

    Once found, returns absolute path to the ./syncmatrix directory, which is assumed to reside within the
    ...
    while path != parent:
        syncmatrix_dir = path.joinpath(".syncmatrix")
        if syncmatrix_dir.is_dir():
            return syncmatrix_dir

        path = parent.resolve()
        parent = path.parent.resolve()


def set_syncmatrix_hidden_dir(path: str = None) -> bool:
    """
    Creates default `.syncmatrix/` directory if one does not already exist.
...
def create_default_syncmatrix_yaml(
    path: str, name: str = None, contents: dict = None
) -> bool:
    """
    Creates default `syncmatrix.yaml` file in the provided path if one does not already exist;
    ...
    if syncmatrix_file.exists():
        return False
    default_file = Path(__file__).parent / "templates" / "syncmatrix.yaml"
    ...
    contents["syncmatrix-version"] = syncmatrix.__version__
    contents["name"] = name

    with syncmatrix_file.open(mode="w") as f:
        # write header
        f.write(
            "# Welcome to your syncmatrix.yaml file! You can use this file for storing and"
        ...
        yaml.dump({"syncmatrix-version": contents["syncmatrix-version"]}, f, sort_keys=False)
        f.write("\n")
...
def configure_project_by_recipe(recipe: str, **formatting_kwargs) -> dict:
    ...
    """
    # load the recipe
    recipe_path = Path(__file__).parent / "recipes" / recipe / "syncmatrix.yaml"
...
def initialize_project(
    ...
    if create_default_ignore_file("."):
        files.append(".syncmatrixignore")
    if create_default_syncmatrix_yaml(".", name=project_name, contents=configuration):
        files.append("syncmatrix.yaml")
    if set_syncmatrix_hidden_dir():
        files.append(".syncmatrix/")
...
async def register_flow(entrypoint: str, force: bool = False):
    ...
    try:
        ...
        if str(exc) == "not enough values to unpack (expected 2, got 1)":
            missing_flow_name_msg = (
                ...
                f" name, try `syncmatrix deploy -n {entrypoint}`."
            )
    ...
    if not syncmatrix_dir:
        raise FileNotFoundError(
            "No .syncmatrix directory could be found - run `syncmatrix project"
            " init` to create one."
        )

    entrypoint = f"{fpath.relative_to(syncmatrix_dir.parent)!s}:{obj_name}"

    flows_file = syncmatrix_dir / "flows.json"
    ...
    if flow.name in flows and flows[flow.name] != entrypoint:
        if not force:
            raise ValueError(
                ...
                f" {flow.name!r} from your [yellow]~/.syncmatrix/flows.json[/yellow]."
            )
...
def _copy_deployments_into_syncmatrix_file():
    """
    Copy deployments from the `deloyment.yaml` file into the `syncmatrix.yaml` file.

    Used to migrate users from the old `syncmatrix.yaml` + `deployment.yaml` structure
    to a single `syncmatrix.yaml` file.
    """
    syncmatrix_file = Path("syncmatrix.yaml")
    deployment_file = Path("deployment.yaml")
    if not deployment_file.exists() or not syncmatrix_file.exists():
        raise FileNotFoundError(
            "Could not find `syncmatrix.yaml` or `deployment.yaml` files."
        )
    ...
    with syncmatrix_file.open(mode="a") as f:
        # If deployment.yaml is empty, write an empty deployments list to syncmatrix.yaml.
        ...
        # Write all of deployment.yaml to syncmatrix.yaml.
        else:
            f.write("\n")
            f.write(raw_deployment_file_contents)


def _format_deployment_for_saving_to_syncmatrix_file(
    deployment: Dict,
) -> Dict:
    """
    Formats a deployment into a templated deploy config for saving to syncmatrix.yaml.
    ...
    # Parameter schema is not stored in syncmatrix.yaml
...
def _save_deployment_to_syncmatrix_file(
    deployment: Dict,
    ...
    """
    Save a deployment configuration to the `syncmatrix.yaml` file in the
    current directory.

    Will create a syncmatrix.yaml file if one does not already exist.

    Args:
        - deployment: a dictionary containing a deployment configuration
    """
    deployment = _format_deployment_for_saving_to_syncmatrix_file(deployment)

    current_directory_name = os.path.basename(os.getcwd())
    if not syncmatrix_file.exists():
        ...
        with syncmatrix_file.open(mode="r") as f:
            parsed_syncmatrix_file_contents = ryaml.load(f)

        if build_steps != parsed_syncmatrix_file_contents.get("build"):
            deployment["build"] = build_steps

        if push_steps != parsed_syncmatrix_file_contents.get("push"):
            deployment["push"] = push_steps

        if pull_steps != parsed_syncmatrix_file_contents.get("pull"):
            deployment["pull"] = pull_steps

        if triggers and triggers != parsed_syncmatrix_file_contents.get("triggers"):
            deployment["triggers"] = triggers

        deployments = parsed_syncmatrix_file_contents.get("deployments")
        if deployments is None:
            parsed_syncmatrix_file_contents["deployments"] = [deployment]
        else:
        ...
        with syncmatrix_file.open(mode="w") as f:
            ryaml.dump(parsed_syncmatrix_file_contents, f)
