...
try:
    from syncmatrix.server.schemas.schedules import SCHEDULE_TYPES as SERVER_SCHEDULE_TYPES

    SERVER_SCHEDULE_TYPES = get_args(SERVER_SCHEDULE_TYPES)
except ImportError:
    # `syncmatrix-client` does not have access to the server schemas.
        ...
        for obj in schedules:
            if isinstance(obj, get_args(SCHEDULE_TYPES)):
                ...
                raise ValueError(
                    ...
                    "the schedule objects from `syncmatrix.client.schemas.schedules`"
                )
            else:
                raise ValueError(
