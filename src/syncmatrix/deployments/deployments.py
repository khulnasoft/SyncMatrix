...
from syncmatrix.context import FlowRunContext, SyncmatrixObjectRegistry, TaskRunContext
from syncmatrix.deployments.schedules import (
...
from syncmatrix.infrastructure import Infrastructure, Process
from syncmatrix.logging.loggers import flow_run_logger, get_logger
...
from syncmatrix.utilities.asyncutils import run_sync_in_worker_thread, sync_compatible
from syncmatrix.utilities.callables import ParameterSchema, parameter_schema
from syncmatrix.utilities.filesystem import relative_path_to_current_platform, tmpchdir
...
def load_deployments_from_yaml(
    ...
    with SyncmatrixObjectRegistry(capture_failures=True) as registry:
        for node in nodes:
            with tmpchdir(path):
...
@deprecated_class(
    ...
    " https://docs.khulnasoft.com/latest/guides/upgrade-guide-agents-to-workers/.",
)
class Deployment(BaseModel):
    ...
    For upgrade instructions, see https://docs.khulnasoft.com/latest/guides/upgrade-guide-agents-to-workers/.

    A Syncmatrix Deployment definition, used for specifying and building deployments.
    ...
    Examples:
        ...
        >>> from my_project.flows import my_flow
        >>> from syncmatrix.deployments import Deployment
        >>> from syncmatrix.filesystems import S3
        ...
        ...     tags=["aws"],
        ...     storage=storage,
        ...     infra_overrides=dict("env.SYNCMATRIX_LOGGING_LEVEL"="DEBUG"),
        >>> )
        >>> deployment.apply()
    ...
    async def to_yaml(self, path: Path) -> None:
        ...
        with open(path, "w") as f:
            # write header
            f.write(
                "###\n### A complete description of a Syncmatrix Deployment for flow"
                f" {self.flow_name!r}\n###\n"
            )
    ...
    enforce_parameter_schema: Optional[bool] = Field(
        default=None,
        description=(
            "Whether or not the Syncmatrix API should enforce the parameter schema for"
            " this deployment."
        ),
    )
    ...
    async def upload_to_storage(
        self, storage_block: str = None, ignore_file: str = ".syncmatrixignore"
    ) -> Optional[int]:
        """
        ...
        Args:
            ...
            ignore_file: an optional path to a `.syncmatrixignore` file that specifies
                filename patterns to ignore when uploading to remote storage; if not
                provided, looks for `.syncmatrixignore` in the current working directory
        """
        file_count = None
        if storage_block:
    ...
    async def build_from_flow(
        ...
        ignore_file: str = ".syncmatrixignore",
        apply: bool = False,
        ...
        **kwargs,
        ...
        if not deployment.storage and not is_docker_based and not deployment.path:
            ...
            if not deployment.path:
                deployment.path = "/opt/syncmatrix/flows"
