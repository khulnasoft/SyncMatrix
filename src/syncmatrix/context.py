...
For more user-accessible information about the current run, see [`syncmatrix.runtime`](../runtime/flow_run).
"""
import os
import sys
import warnings
...
from syncmatrix.settings import SYNCMATRIX_HOME, Profile, Settings
from syncmatrix.states import State
from syncmatrix.task_runners import BaseTaskRunner
from syncmatrix.utilities.importtools import load_script_as_module
...
class SyncmatrixObjectRegistry(ContextModel):
    """
    A context that acts as a registry for all Syncmatrix objects that are
    ...
    def register_instances(cls, type_: Type[T]) -> Type[T]:
        """
        Decorator for a class that adds registration to the `SyncmatrixObjectRegistry`
...
class RunContext(ContextModel):
    ...
    Attributes:
        ...
        client: The Syncmatrix client instance being used for API communication
    """
...
class EngineContext(RunContext):
    ...
    task_run_futures: List[SyncmatrixFuture] = Field(default_factory=list)
    ...
    # Events worker to emit events to Syncmatrix Cloud
...
class TagsContext(ContextModel):
    """
    The context for `syncmatrix.tags` management.
...
class SettingsContext(ContextModel):
    """
    The context for a Syncmatrix settings.
    ...
    def __enter__(self):
        ...
        try:
            syncmatrix_home = Path(self.settings.value_of(SYNCMATRIX_HOME))
            syncmatrix_home.mkdir(mode=0o0700, exist_ok=True)
        except OSError:
            warnings.warn(
                (
                    "Failed to create the Syncmatrix home directory at "
                    f"{self.settings.value_of(SYNCMATRIX_HOME)}"
                ),
                stacklevel=2,
            )
...
def get_settings_context() -> SettingsContext:
    ...
    profile and environment. See `syncmatrix.context.use_profile` for more details.
    """
...
def tags(*new_tags: str) -> Set[str]:
    ...
    Examples:
        >>> from syncmatrix import tags, task, flow
        >>> @task
        >>> def my_task():
        >>>     pass
...
def use_profile(
    ...
    if isinstance(profile, str):
        profiles = syncmatrix.settings.load_profiles()
    ...
    if not override_environment_variables:
        for key in os.environ:
            if key in syncmatrix.settings.SETTING_VARIABLES:
                profile_settings.pop(syncmatrix.settings.SETTING_VARIABLES[key], None)
...
def root_settings_context():
    ...
    - Command line via 'syncmatrix --profile <name>'
    - Environment variable via 'SYNCMATRIX_PROFILE'
    ...
    if "SYNCMATRIX_PROFILE" in os.environ:
        active_name = os.environ["SYNCMATRIX_PROFILE"]
        profile_source = "by environment variable"

    if (
        sys.argv[0].endswith("/syncmatrix")
        and len(sys.argv) >= 3
...
GLOBAL_OBJECT_REGISTRY: ContextManager[SyncmatrixObjectRegistry] = None
...
def initialize_object_registry():
    ...
    GLOBAL_OBJECT_REGISTRY = SyncmatrixObjectRegistry()
