...
from syncmatrix import Task, get_client
...
from syncmatrix.client.subscriptions import Subscription
from syncmatrix.engine import emit_task_run_state_change_event, propose_state
from syncmatrix.logging.loggers import get_logger
...
from syncmatrix.settings import (
    ...
    SYNCMATRIX_TASK_SCHEDULING_DELETE_FAILED_SUBMISSIONS,
)
from syncmatrix.states import Pending
from syncmatrix.task_engine import submit_autonomous_task_run_to_engine
from syncmatrix.task_runners import (
    BaseTaskRunner,
    ConcurrentTaskRunner,
)
from syncmatrix.utilities.asyncutils import asyncnullcontext, sync_compatible
from syncmatrix.utilities.processutils import _register_signal
...
class TaskServer:
            ...
            try:
                ...
                if exc.status_code == 403:
                    logger.error(
                        ...
                        " `SYNCMATRIX_API_URL` and `SYNCMATRIX_API_KEY` environment variables."
                    )
                else:
                    raise
    ...
    async def _submit_scheduled_task_run(self, task_run: TaskRun):
        ...
        if not task:
            if SYNCMATRIX_TASK_SCHEDULING_DELETE_FAILED_SUBMISSIONS.value():
                logger.warning(
...
async def serve(*tasks: Task, task_runner: Optional[Type[BaseTaskRunner]] = None):
    ...
    Example:
        ...
        ```
    """
    if not SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING.value():
        raise RuntimeError(
            "To enable task scheduling, set SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING"
            " to True."
        )
