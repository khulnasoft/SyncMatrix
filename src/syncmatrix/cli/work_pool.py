...
from syncmatrix.client.schemas.actions import WorkPoolCreate, WorkPoolUpdate
...
from syncmatrix.settings import update_current_profile
from syncmatrix.workers.utilities import (
    get_available_work_pool_types,
    get_default_base_job_template_for_infrastructure_type,
)

work_pool_app = SyncmatrixTyper(
...
def set_work_pool_as_default(name: str):
    profile = update_current_profile({"SYNCMATRIX_DEFAULT_WORK_POOL_NAME": name})
    app.console.print(
    ...
    app.console.print(
        (
            "To change your default work pool, run:\n\n\t[blue]syncmatrix config set"
            " SYNCMATRIX_DEFAULT_WORK_POOL_NAME=<work-pool-name>[/]\n"
        ),
    )
...
async def create(
    ...
    base_job_template: typer.FileText = typer.Option(
        ...
        help=(
            ...
            " unspecified, Syncmatrix will use the default base job template for the given"
            " worker type."
        ),
    ),
    ...
    Examples:
        ...
        Create a Kubernetes work pool in a paused state:
            \b
            $ syncmatrix work-pool create "my-pool" --type kubernetes --paused
        \b
        Create a Docker work pool with a custom base job template:
            \b
            $ syncmatrix work-pool create "my-pool" --type docker --base-job-template ./base-job-template.json
    ...
    async with get_client() as client:
        ...
        try:
            ...
            if (
                ...
                app.console.print(
                    f"\t[blue]syncmatrix worker start --pool {work_pool.name}[/]\n"
                )
            if set_as_default:
...
async def inspect(
    ...
    \b
    Examples:
        $ syncmatrix work-pool inspect "my-pool"
...
async def pause(
    ...
    \b
    Examples:
        $ syncmatrix work-pool pause "my-pool"
...
async def resume(
    ...
    \b
    Examples:
        $ syncmatrix work-pool resume "my-pool"
...
async def update(
    ...
    \b
    Examples:
        $ syncmatrix work-pool update "my-pool"
...
async def provision_infrastructure(
    ...
    \b
    Examples:
        $ syncmatrix work-pool provision-infrastructure "my-pool"

        $ syncmatrix work-pool provision-infra "my-pool"
...
async def delete(
    ...
    \b
    Examples:
        $ syncmatrix work-pool delete "my-pool"
...
async def set_concurrency_limit(
    ...
    \b
    Examples:
        $ syncmatrix work-pool set-concurrency-limit "my-pool" 10
...
async def clear_concurrency_limit(
    ...
    \b
    Examples:
        $ syncmatrix work-pool clear-concurrency-limit "my-pool"
...
async def get_default_base_job_template(
    ...
    \b
    Examples:
        $ syncmatrix work-pool get-default-base-job-template --type kubernetes
    """
...
async def preview(
    ...
    \b
    Examples:
        $ syncmatrix work-pool preview "my-pool" --hours 24
