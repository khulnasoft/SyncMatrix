...
from syncmatrix.blocks.system import Secret
from syncmatrix.cli._prompts import (
    confirm,
    prompt,
...
from syncmatrix.cli._types import SettingsOption
from syncmatrix.cli._utilities import (
    exit_with_error,
)
...
from syncmatrix.client.schemas.objects import MinimalDeploymentSchedule
...
from syncmatrix.deployments import find_syncmatrix_directory, register_flow
from syncmatrix.deployments.base import (
    _copy_deployments_into_syncmatrix_file,
    _format_deployment_for_saving_to_syncmatrix_file,
    _get_git_branch,
    _get_git_remote_origin_url,
    _save_deployment_to_syncmatrix_file,
)
from syncmatrix.deployments.steps.core import run_steps
from syncmatrix.events.schemas import DeploymentTrigger
from syncmatrix.exceptions import ObjectNotFound
from syncmatrix.flows import load_flow_from_entrypoint
from syncmatrix.settings import (
    SYNCMATRIX_DEBUG_MODE,
    SYNCMATRIX_DEFAULT_WORK_POOL_NAME,
    SYNCMATRIX_UI_URL,
)
from syncmatrix.utilities.annotations import NotSet
...
from syncmatrix.utilities.collections import get_from_dict
from syncmatrix.utilities.slugify import slugify
from syncmatrix.utilities.templating import (
    apply_values,
...
async def deploy(
    ...
    syncmatrix_file: Path = typer.Option(
        Path("syncmatrix.yaml"),
        "--syncmatrix-file",
        help="Specify a custom path to a syncmatrix.yaml file",
    ),
    ci: bool = typer.Option(
        False,
        "--ci",
        help=(
            "DEPRECATED: Please use the global '--no-prompt' flag instead: 'syncmatrix"
    ...
    if ci:
        app.console.print(
            generate_deprecation_message(
                ...
                start_date="Jun 2023",
                help=(
                    "Please use the global `--no-prompt` flag instead: `syncmatrix"
                    " --no-prompt deploy`."
                ),
            ),
            style="yellow",
    ...
    try:
        deploy_configs, actions = _load_deploy_configs_and_actions(
            syncmatrix_file=syncmatrix_file, ci=ci
        )
        parsed_names = []
        for name in names:
            if "*" in name:
        ...
        if len(deploy_configs) > 1:
            ...
            await _run_multi_deploy(
                ...
                ci=ci,
                syncmatrix_file=syncmatrix_file,
            )
        else:
...
async def _run_single_deploy(
    ...
    ci: bool = False,
    client: SyncmatrixClient = None,
    syncmatrix_file: Path = Path("syncmatrix.yaml"),
):
    ...
    if not deploy_config.get("flow_name") and not deploy_config.get("entrypoint"):
        if not is_interactive() and not ci:
            raise ValueError(
                ...
                " \t[yellow]syncmatrix deploy path/to/file.py:flow_function\n\n"
                "You can also provide an entrypoint in a syncmatrix.yaml file."
            )
    ...
    if deploy_config.get("entrypoint"):
        try:
            ...
            if SYNCMATRIX_DEBUG_MODE:
                app.console.print(
                    "Could not find .syncmatrix directory. Flow entrypoint will not be"
                    " registered."
                )
        ...
        app.console.print(
            generate_deprecation_message(
                ...
                start_date="Jun 2023",
                help=(
                    "\nUse `syncmatrix deploy ./path/to/file.py:flow_fn_name` to specify"
                    " an entrypoint instead."
                ),
            )
        )
        syncmatrix_dir = find_syncmatrix_directory()
        if not syncmatrix_dir:
            raise ValueError(
                "No .syncmatrix directory could be found - run [yellow]`syncmatrix"
                " init`[/yellow] to create one."
            )
        if not (syncmatrix_dir / "flows.json").exists():
            raise ValueError(
                f"Flow {deploy_config['flow_name']!r} cannot be found;"
                " run\n\t[yellow]syncmatrix project register-flow"
                " ./path/to/file.py:flow_fn_name[/yellow]\nto register its location."
            )
        with open(syncmatrix_dir / "flows.json", "r") as f:
            flows = json.load(f)
    ...
    if work_pool_name:
        try:
            ...
            # dont allow submitting to syncmatrix-agent typed work pools
            if work_pool.type == "syncmatrix-agent":
                if not is_interactive() or ci:
                    raise ValueError(
                        "Cannot create a project-style deployment with work pool of"
                        " type 'syncmatrix-agent'. If you wish to use an agent with"
                        " your deployment, please use the `syncmatrix deployment"
                        " build` command."
                    )
                app.console.print(
                    "You've chosen a work pool with type 'syncmatrix-agent' which"
            ...
            raise ValueError(
                ...
                " work pool in the Syncmatrix UI."
            )
    else:
        if not is_interactive() or ci:
            raise ValueError(
                "A work pool is required to deploy this flow. Please specify a work"
                " pool name via the '--pool' flag or in your syncmatrix.yaml file."
            )
    ...
    docker_build_steps = [
        ...
        "syncmatrix_docker.projects.steps.build_docker_image",
    ]

    docker_push_steps = [
        "syncmatrix_docker.deployments.steps.push_docker_image",
        "syncmatrix_docker.projects.steps.push_docker_image",
    ]
    ...
    if (
        ...
        and confirm(
            (
                "Your Syncmatrix workers will need access to this flow's code in order to"
            ...
            app.console.print(
                Panel(
                    (
                        ...
                        "Syncmatrix Cloud. Any triggers defined for the deployment will "
                        "not be created."
                    ),
                ),
                style="yellow",
    ...
    if SYNCMATRIX_UI_URL:
        app.console.print(
            "\nView Deployment in UI:"
            f" {SYNCMATRIX_UI_URL.value()}/deployments/deployment/{deployment_id}\n"
        )

    identical_deployment_exists_in_syncmatrix_file = (
        _check_if_identical_deployment_in_syncmatrix_file(
            deploy_config_before_templating, syncmatrix_file
        )
    )
    if should_prompt_for_save and not identical_deployment_exists_in_syncmatrix_file:
        if confirm(
            (
            ...
            if matching_deployment_exists and not confirm(
                (
                    ...
                    f" in the [yellow]syncmatrix.yaml[/yellow] file at {syncmatrix_file}."
                ...
                app.console.print(
                    ...
                    f" deployment configuration file[/red] at {syncmatrix_file}"
                )
            else:
                _save_deployment_to_syncmatrix_file(
                ...
                app.console.print(
                    (
                        f"\n[green]Deployment configuration saved to {syncmatrix_file}![/]"
                        " You can now deploy using this deployment configuration"
                        " with:\n\n\t[blue]$ syncmatrix deploy -n"
    ...
    if not work_pool.is_push_pool and not work_pool.is_managed_pool:
        ...
        app.console.print(
            f"\n\t$ syncmatrix worker start --pool {deploy_config['work_pool']['name']!r}",
            style="blue",
        )
    app.console.print(
...
async def _run_multi_deploy(
    ...
    for deploy_config in deploy_configs:
        ...
        await _run_single_deploy(
            deploy_config, actions, ci=ci, syncmatrix_file=syncmatrix_file
        )
...
async def _generate_git_clone_pull_step(
    ...
    if confirm("Is this a private repository?", console=console):
        ...
        prompt_message = (
            ...
            " repository. This token will be saved as a secret via the Syncmatrix API"
        )
    ...
    if token_secret_block_name:
        git_clone_step["syncmatrix.deployments.steps.git_clone"]["access_token"] = (
            "{{ syncmatrix.blocks.secret." + token_secret_block_name + " }}"
        )
...
async def _generate_pull_step_for_build_docker_image(
    ...
    if auto:
        pull_step["directory"] = f"/opt/syncmatrix/{dir_name}"
    else:
        pull_step["directory"] = prompt(
            "What is the path to your flow code in your Dockerfile?",
            default=f"/opt/syncmatrix/{dir_name}",
            console=console,
        )

    return [{"syncmatrix.deployments.steps.set_working_directory": pull_step}]
...
async def _check_for_build_docker_image_step(
    ...
    build_docker_image_steps = [
        "syncmatrix_docker.projects.steps.build_docker_image",  # legacy
        "syncmatrix_docker.deployments.steps.build_docker_image",
    ]
...
async def _generate_actions_for_remote_flow_storage(
    ...
    storage_provider_to_collection = {
        "s3": "syncmatrix_aws",
        "gcs": "syncmatrix_gcp",
        "azure_blob_storage": "syncmatrix_azure",
    }
        ...
        if is_interactive() and not ci:
            if not confirm(
                ...
                exit_with_error(
                    ...
                    " Dockerfile: [yellow]COPY . /opt/syncmatrix/[/yellow]"
                )
            return await _generate_pull_step_for_build_docker_image(
...
def _handle_deployment_yaml_copy(syncmatrix_yaml_contents, ci):
    """
    Handle the deprecation of the deployment.yaml file by copying the
    deployment configurations into the syncmatrix.yaml file.

    Remove this after December 2023.

    Args:
        syncmatrix_yaml_contents: The contents of the syncmatrix.yaml file
        ci: Disables interactive mode if True
    """
    if is_interactive() and not ci and syncmatrix_yaml_contents:
        if confirm(
            generate_deprecation_message(
                ...
                help=(
                    ...
                    " file into your `syncmatrix.yaml` file now?"
                ),
            )
        ):
            try:
                ...
                app.console.print(
                    ...
                    " syncmatrix.yaml file! Once you've verified that all your"
                    " deployment configurations in your syncmatrix.yaml file are"
...
def _load_deploy_configs_and_actions(
    syncmatrix_file: Path, ci: bool = False
    ...
    """
    try:
        with syncmatrix_file.open("r") as f:
            syncmatrix_yaml_contents = yaml.safe_load(f)
        ...
        syncmatrix_yaml_contents = {}
    if not isinstance(syncmatrix_yaml_contents, dict):
        app.console.print(
    ...
    actions = {
        "build": syncmatrix_yaml_contents.get("build", []),
        "push": syncmatrix_yaml_contents.get("push", []),
        "pull": syncmatrix_yaml_contents.get("pull", []),
    }
    ...
    try:
        ...
        _handle_deployment_yaml_copy(syncmatrix_yaml_contents, ci)
    except FileNotFoundError:
        deploy_configs = syncmatrix_yaml_contents.get("deployments", [])
...
def _pick_deploy_configs(deploy_configs, names, deploy_all, ci=False):
    ...
    # e.g. `syncmatrix --no-prompt deploy`
    ...
    # e.g. `syncmatrix deploy -n flow-name/deployment-name -n deployment-name`
    elif len(names) >= 1:
        return _handle_pick_deploy_with_name(deploy_configs, names, ci=ci)

    # e.g. `syncmatrix deploy`
    elif is_interactive() and not ci:
        return _handle_pick_deploy_without_name(deploy_configs)

    # e.g `syncmatrix --no-prompt deploy` where we have multiple deployment configurations
...
def _check_for_matching_deployment_name_and_entrypoint_in_syncmatrix_file(
    deploy_config, syncmatrix_file: Path = Path("syncmatrix.yaml")
) -> bool:
    if syncmatrix_file.exists():
        with syncmatrix_file.open(mode="r") as f:
            parsed_syncmatrix_file_contents = yaml.safe_load(f)
            existing_deployments = parsed_syncmatrix_file_contents.get("deployments")
...
def _check_if_identical_deployment_in_syncmatrix_file(
    untemplated_deploy_config: Dict, syncmatrix_file: Path = Path("syncmatrix.yaml")
) -> bool:
    """
    Check if the given deploy config is identical to an existing deploy config in the
    syncmatrix.yaml file, meaning that there have been no updates and prompting to save is unnecessary.

    Args:
        untemplated_deploy_config: A deploy config that has not been templated.
    """

    user_specified_deploy_config = _format_deployment_for_saving_to_syncmatrix_file(
...
async def _create_deployment_triggers(
    client: SyncmatrixClient, deployment_id: UUID, triggers: List[DeploymentTrigger]
):
    if client.server_type == ServerType.CLOUD:
...
def _gather_deployment_trigger_definitions(
    ...
    """Parses trigger flags from CLI and existing deployment config in `syncmatrix.yaml`.
