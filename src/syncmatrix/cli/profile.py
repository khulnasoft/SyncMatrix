...
from syncmatrix.cli.cloud import CloudUnauthorizedError, get_cloud_client
...
profile_app = SyncmatrixTyper(
    name="profile", help="Commands for interacting with your Syncmatrix profiles."
)
...
def create(
        ...
        app.console.print(
            textwrap.dedent(
                ...
                To create a new profile, remove the existing profile first:
                    ...
                    syncmatrix profile delete {name!r}
                """
            ).strip()
        )
        raise typer.Exit(1)
    ...
    if from_name:
        ...
        profiles.add_profile(syncmatrix.settings.Profile(name=name, settings={}))

    syncmatrix.settings.save_profiles(profiles)
    ...
    app.console.print(
        textwrap.dedent(
            ...
            Use created profile for future, subsequent commands:
                syncmatrix profile use {name!r}

            Use created profile temporarily for a single command:
                syncmatrix -p {name!r} config view
            """
        )
    )
...
async def use(name: str):
    ...
    status_messages = {
        ConnectionStatus.CLOUD_CONNECTED: (
            exit_with_success,
            f"Connected to Syncmatrix Cloud using profile {name!r}",
        ),
        ConnectionStatus.CLOUD_ERROR: (
            exit_with_error,
            f"Error connecting to Syncmatrix Cloud using profile {name!r}",
        ),
        ConnectionStatus.CLOUD_UNAUTHORIZED: (
            exit_with_error,
            f"Error authenticating with Syncmatrix Cloud using profile {name!r}",
        ),
        ConnectionStatus.ORION_CONNECTED: (
            exit_with_success,
            f"Connected to Syncmatrix server using profile {name!r}",
        ),
        ConnectionStatus.ORION_ERROR: (
            exit_with_error,
            f"Error connecting to Syncmatrix server using profile {name!r}",
        ),
        ConnectionStatus.EPHEMERAL: (
            exit_with_success,
            (
                f"No Syncmatrix server specified using profile {name!r} - the API will run"
                " in ephemeral mode."
            ),
        ),
        ConnectionStatus.INVALID_API: (
            exit_with_error,
            "Error connecting to Syncmatrix API URL",
        ),
    }
...
def rename(name: str, new_name: str):
    ...
    syncmatrix.context.get_settings_context().profile
    if profiles.active_name == name:
        profiles.set_active(new_name)
    if os.environ.get("SYNCMATRIX_PROFILE") == name:
        app.console.print(
            f"You have set your current profile to {name!r} with the "
            "SYNCMATRIX_PROFILE environment variable. You must update this variable to "
...
async def check_orion_connection():
    ...
    try:
        ...
        if exc.response.status_code == status.HTTP_404_NOT_FOUND:
            # if the route does not exist, attempt to connect as a hosted Syncmatrix
            # instance
            try:
                # inform the user if Syncmatrix API endpoints exist, but there are
                # connection issues
                ...
                if connect_error is not None:
                    ...
                    # if the client is using an ephemeral Syncmatrix app, inform the user
        ...
        # if no Syncmatrix API URL has been set, httpx will throw a TypeError
        try:
            # try to connect with the client anyway, it will likely use an
            # ephemeral Syncmatrix instance
            client = get_client(httpx_settings=httpx_settings)
            async with client:
                connect_error = await client.api_healthcheck()
            if connect_error is not None:
