"""
Command line interface for interacting with Syncmatrix Cloud
"""
import signal
import traceback
import urllib.parse
...
from syncmatrix.client.cloud import CloudUnauthorizedError, get_cloud_client
from syncmatrix.client.schemas import Workspace
...
from syncmatrix.utilities.compat import raise_signal

# Set up the `syncmatrix cloud` and `syncmatrix cloud workspaces` CLI applications
cloud_app = SyncmatrixTyper(
    name="cloud", help="Commands for interacting with Syncmatrix Cloud"
)
workspace_app = SyncmatrixTyper(
    name="workspace", help="Commands for interacting with Syncmatrix Cloud Workspaces"
)
...
async def login_with_browser() -> str:
    ...
    async with anyio.create_task_group() as tg:
        ...
        ui_login_url = (
            SYNCMATRIX_CLOUD_UI_URL.value() + f"/auth/client?callback={callback}"
        )
...
async def login(
    key: Optional[str] = typer.Option(
        None, "--key", "-k", help="API Key to authenticate with Syncmatrix"
    ),
    ...
    """
    Log in to Syncmatrix Cloud.
    Creates a new profile configured to use the specified SYNCMATRIX_API_KEY.
    ...
    env_var_api_key = SYNCMATRIX_API_KEY.value()

    if env_var_api_key and key and env_var_api_key != key:
        exit_with_error(
            "Cannot log in with a key when a different SYNCMATRIX_API_KEY is present as an"
    ...
    if env_var_api_key and env_var_api_key == key:
        ...
        if not is_valid_key:
            ...
            exit_with_error(
                f"Unable to authenticate with Syncmatrix Cloud. {help_message}"
            )

    already_logged_in_profiles = []
    for name, profile in profiles.items():
        profile_key = profile.settings.get(SYNCMATRIX_API_KEY)
        if (
        ...
        try:
            ...
            if key.startswith("pcu"):
                help_message = (
                    ...
                    " (https://cloud.khulnasoft.com). Make sure that you generate API key"
                    " using Cloud 2 (https://app.syncmatrix.cloud)"
                )
            ...
            exit_with_error(f"Error connecting to Syncmatrix Cloud: {exc!r}")
        ...
        if prompt_switch_workspace:
            ...
            if current_workspace:
                ...
                exit_with_error(
                    ...
                    f" {SYNCMATRIX_CLOUD_UI_URL.value()} and try again."
                )

    update_current_profile(
        {
            SYNCMATRIX_API_KEY: key,
            SYNCMATRIX_API_URL: workspace.api_url(),
        }
    )

    exit_with_success(
        f"Authenticated with Syncmatrix Cloud! Using workspace {workspace.handle!r}."
    )
...
async def logout():
    ...
    Reset SYNCMATRIX_API_KEY and SYNCMATRIX_API_URL to default.
    """
    current_profile = syncmatrix.context.get_settings_context().profile
    if current_profile is None:
        exit_with_error("There is no current profile set.")

    if current_profile.settings.get(SYNCMATRIX_API_KEY) is None:
        exit_with_error("Current profile is not logged into Syncmatrix Cloud.")

    update_current_profile(
        {
            SYNCMATRIX_API_URL: None,
            SYNCMATRIX_API_KEY: None,
        },
    )

    exit_with_success("Logged out from Syncmatrix Cloud.")


@cloud_app.command()
async def open():
    """
    Open the Syncmatrix Cloud UI in the browser.
    """
    confirm_logged_in()
    ...
    if current_profile is None:
        exit_with_error(
            "There is no current profile set - set one with `syncmatrix profile create"
            " <name>` and `syncmatrix profile use <name>`."
        )

    current_workspace = get_current_workspace(
        await syncmatrix.get_cloud_client().read_workspaces()
    )
    if current_workspace is None:
        exit_with_error(
            "There is no current workspace set - set one with `syncmatrix cloud workspace"
...
async def set(
    ...
    profile = update_current_profile({SYNCMATRIX_API_URL: workspace.api_url()})
