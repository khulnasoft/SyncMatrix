...
from syncmatrix.cli.cloud import cloud_app, confirm_logged_in
from syncmatrix.cli.root import app
from syncmatrix.client.cloud import get_cloud_client
from syncmatrix.settings import SYNCMATRIX_API_URL

webhook_app = SyncmatrixTyper(
    name="webhook", help="Commands for interacting with Syncmatrix Cloud Webhooks"
)
...
async def ls():
    ...
    async with get_cloud_client(host=SYNCMATRIX_API_URL.value()) as client:
...
async def create(
    ...
    if not template:
        exit_with_error(
            ...
            ' "your.event.name", "resource": { "syncmatrix.resource.id":'
            ' "your.resource.id" } }'
            " \nhttps://docs.khulnasoft.com/latest/cloud/webhooks/#webhook-templates"
        )
