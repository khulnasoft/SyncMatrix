...
from syncmatrix.cli._utilities import exit_with_error, exit_with_success
from syncmatrix.cli.root import app
from syncmatrix.client.orchestration import SyncmatrixClient, ServerType, get_client
from syncmatrix.client.schemas.filters import FlowFilter
from syncmatrix.client.schemas.objects import DeploymentSchedule
...
from syncmatrix.deployments import Deployment, load_deployments_from_yaml
from syncmatrix.exceptions import (
    ObjectAlreadyExists,
    ObjectNotFound,
    SyncmatrixHTTPStatusError,
    ScriptError,
    exception_traceback,
)
from syncmatrix.flow_runs import wait_for_flow_run
from syncmatrix.flows import load_flow_from_entrypoint
from syncmatrix.settings import SYNCMATRIX_UI_URL
from syncmatrix.states import Scheduled
...
from syncmatrix.utilities.filesystem import create_default_ignore_file
...
deployment_app = SyncmatrixTyper(
    name="deployment", help="Commands for working with deployments."
)
schedule_app = SyncmatrixTyper(
...
async def get_deployment(client: SyncmatrixClient, name, deployment_id):
    if name is None and deployment_id is not None:
        try:
            deployment = await client.read_deployment(deployment_id)
        except SyncmatrixHTTPStatusError:
...
async def check_work_pool_exists(
    work_pool_name: Optional[str], client: SyncmatrixClient = None
):
    if work_pool_name is not None:
        try:
            ...
            app.console.print(
                f"$ syncmatrix work-pool create {work_pool_name!r}\n", style="blue"
            )
...
async def _print_deployment_work_pool_instructions(
    work_pool_name: str, client: SyncmatrixClient = None
):
    ...
    command = f"$ syncmatrix agent start -p {work_pool_name!r}"
    if work_pool.type != "syncmatrix-agent":
        if experiment_enabled("workers"):
            ...
            command = f"$ syncmatrix worker start -p {work_pool_name!r}"
        else:
            blurb = (
            ...
            command = (
                "$ syncmatrix config set"
                " SYNCMATRIX_EXPERIMENTAL_ENABLE_WORKERS=True\n$ syncmatrix"
...
async def inspect(name: str):
    ...
    Example:
        \b
        $ syncmatrix deployment inspect "hello-world/my-deployment"
        {
            ...
            'infrastructure': {
                ...
                'labels': {},
                'name': None,
                'command': ['python', '-m', 'syncmatrix.engine'],
                'stream_output': True
            }
        }
        ...
        if client.server_type == ServerType.CLOUD:
            deployment_json["automations"] = [
                ...
                for a in await client.read_resource_related_automations(
                    f"syncmatrix.deployment.{deployment.id}"
                )
            ]
...
@deployment_app.command(
    ...
    deprecated_help="Use 'syncmatrix deployment schedule create' instead.",
...
async def _set_schedule(
    ...
    This command is deprecated. Use 'syncmatrix deployment schedule create' instead.
    """
    assert_deployment_name_format(name)
        ...
        async with get_client() as client:
            ...
            if len(deployment.schedules) > 1:
                return exit_with_error(
                    ...
                    "Use `syncmatrix deployment schedules create` instead."
                )
...
@deployment_app.command(
    ...
    deprecated=True,
    deprecated_start_date="Mar 2024",
    deprecated_help="Use 'syncmatrix deployment schedule pause' instead.",
...
async def _pause_schedule(
    ...
    This command is deprecated. Use `syncmatrix deployment schedule pause` instead.
    """
    assert_deployment_name_format(name)
    ...
    async with get_client() as client:
        ...
        if len(deployment.schedules) > 1:
            return exit_with_error(
                f"Deployment {name!r} has multiple schedules. Use `syncmatrix deployment"
...
@deployment_app.command(
    ...
    deprecated=True,
    deprecated_start_date="Mar 2024",
    deprecated_help="Use 'syncmatrix deployment schedule resume' instead.",
...
async def _resume_schedule(
    ...
    This command is deprecated. Use `syncmatrix deployment schedule resume` instead.
    """
    ...
    async with get_client() as client:
        ...
        if len(deployment.schedules) > 1:
            return exit_with_error(
                f"Deployment {name!r} has multiple schedules. "
                "Use `syncmatrix deployment schedule pause <deployment_name> <schedule_id>"
            )
...
async def run(
    ...
    if SYNCMATRIX_UI_URL:
        run_url = f"{SYNCMATRIX_UI_URL.value()}/flow-runs/flow-run/{flow_run.id}"
    else:
...
def _load_deployments(path: Path, quietly=False) -> SyncmatrixObjectRegistry:
    """
...
@deployment_app.command(
    ...
    deprecated_help="Use 'syncmatrix deploy' to deploy flows via YAML instead.",
)
async def apply(
    ...
    async with get_client() as client:
        for path in paths:
            ...
            if client.server_type != ServerType.CLOUD and deployment.triggers:
                app.console.print(
                    (
                        ...
                        f"Syncmatrix Cloud. Triggers defined in {path!r} will be "
                        "ignored."
                    ),
                    style="red",
                )
            ...
            if deployment.work_pool_name is not None:
                ...
                app.console.print(
                    f"$ syncmatrix agent start -q {deployment.work_queue_name!r}",
                    style="blue",
                )
            else:
                app.console.print(
...
async def delete(
    ...
    Examples:
        \b
        $ syncmatrix deployment delete test_flow/test_deployment
        $ syncmatrix deployment delete --id dfd3e220-a130-4149-9af6-8d487e02fea6
    """
    async with get_client() as client:
        if name is None and deployment_id is not None:
            try:
...
async def build(
    ...
    if create_default_ignore_file(path="."):
        app.console.print(
            (
                "Default '.syncmatrixignore' file written to"
                f" {(Path('.') / '.syncmatrixignore').absolute()}"
            ),
            style="green",
        )
