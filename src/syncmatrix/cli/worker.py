...
from syncmatrix.cli._prompts import confirm
...
from syncmatrix.plugins import load_syncmatrix_collections
from syncmatrix.settings import (
    SYNCMATRIX_WORKER_HEARTBEAT_SECONDS,
    SYNCMATRIX_WORKER_PREFETCH_SECONDS,
    SYNCMATRIX_WORKER_QUERY_SECONDS,
)
from syncmatrix.utilities.dispatch import lookup_type
...
from syncmatrix.workers.server import start_healthcheck_server

worker_app = SyncmatrixTyper(
...
async def start(
    ...
    install_policy: InstallPolicy = typer.Option(
        ...
        "--install-policy",
        help="Install policy to use workers from Syncmatrix integration packages.",
    ...
    async with worker_cls(
        ...
        heartbeat_interval_seconds=SYNCMATRIX_WORKER_HEARTBEAT_SECONDS.value(),
        ...
        async with anyio.create_task_group() as tg:
            ...
            tg.start_soon(
                partial(
                    ...
                    interval=SYNCMATRIX_WORKER_QUERY_SECONDS.value(),
                    run_once=run_once,
            ...
            tg.start_soon(
                partial(
                    ...
                    interval=SYNCMATRIX_WORKER_QUERY_SECONDS.value() * 2,
                    run_once=run_once,
                    printer=app.console.print,
                    jitter_range=0.3,
                    backoff=4,
            ...
            if with_healthcheck:
                ...
                server_thread = threading.Thread(
                    ...
                    target=partial(
                        ...
                        worker=worker,
                        query_interval_seconds=SYNCMATRIX_WORKER_QUERY_SECONDS.value(),
                    ),
                    daemon=True,
                )
                server_thread.start()
...
async def _find_package_for_worker_type(worker_type: str) -> Optional[str]:
    ...
    worker_types_with_packages = {
        ...
        if worker_type != "syncmatrix-agent"
    }
...
async def _get_worker_class(
    ...
    if worker_type == "syncmatrix-agent":
        exit_with_error(
            "'syncmatrix-agent' typed work pools work with Syncmatrix Agents instead of"
            " Workers. Please use the 'syncmatrix agent start' to start a Syncmatrix Agent."
        )
