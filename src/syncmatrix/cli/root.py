"""
Base `syncmatrix` command-line application
"""
import asyncio
import platform
import sys
...
from syncmatrix.cli._utilities import with_cli_exception_handling
from syncmatrix.client.constants import SERVER_API_VERSION
from syncmatrix.client.orchestration import ServerType
...
app = SyncmatrixTyper(add_completion=False, no_args_is_help=True)


def version_callback(value: bool):
    if value:
        print(syncmatrix.__version__)
        raise typer.Exit()
...
def main(
    ...
    prompt: bool = SettingsOption(
        syncmatrix.settings.SYNCMATRIX_CLI_PROMPT,
        help="Force toggle prompts for this CLI run.",
    ),
):
    if profile and not syncmatrix.context.get_settings_context().profile.name == profile:
        ...
        settings_ctx = syncmatrix.context.use_profile(
    ...
    # https://github.com/KhulnaSoft/syncmatrix/issues/8206
...
@app.command()
async def version():
    """Get the current Syncmatrix version."""
    import sqlite3
    ...
    version_info = {
        "Version": syncmatrix.__version__,
        "API version": SERVER_API_VERSION,
        "Python version": platform.python_version(),
        "Git commit": syncmatrix.__version_info__["full-revisionid"][:8],
        "Built": pendulum.parse(
            syncmatrix.__version_info__["date"]
        ).to_day_datetime_string(),
        "OS/Arch": f"{sys.platform}/{platform.machine()}",
        "Profile": syncmatrix.context.get_settings_context().profile.name,
    }
    ...
    if server_type == ServerType.EPHEMERAL.value:
        database = get_dialect(SYNCMATRIX_API_DATABASE_CONNECTION_URL.value()).name
