...
from syncmatrix.settings import Setting
...
class SyncmatrixTyper(typer.Typer):
    """
    ...
    def add_typer(
        self,
        typer_instance: "SyncmatrixTyper",
        *args,
    ...
    def command(
        ...
        **kwargs,
        ...
        def wrapper(fn):
            ...
            command_decorator = super(SyncmatrixTyper, self).command(*args, **kwargs)
            original_command = command_decorator(fn)

            # register fn for each alias, e.g. @marvin_app.command(aliases=["r"])
            if aliases:
                for alias in aliases:
                    super(SyncmatrixTyper, self).command(
                        name=alias,
                        *args,
