...
help_message = """
    Commands for interacting with Syncmatrix settings.
"""

config_app = SyncmatrixTyper(name="config", help=help_message)
...
def set_(settings: List[str]):
    ...
    for item in settings:
        ...
        if setting not in syncmatrix.settings.SETTING_VARIABLES:
            exit_with_error(f"Unknown setting name {setting!r}.")

        # Guard against changing settings that tweak config locations
        if setting in {"SYNCMATRIX_HOME", "SYNCMATRIX_PROFILES_PATH"}:
            exit_with_error(
    ...
    try:
        new_profile = syncmatrix.settings.update_current_profile(parsed_settings)
    ...
    for setting, value in parsed_settings.items():
        ...
        if syncmatrix.settings.SETTING_VARIABLES[setting].deprecated:
            app.console.print(
                f"[yellow]{syncmatrix.settings.SETTING_VARIABLES[setting].deprecated_message}."
            )
...
def validate():
    ...
    profile = profiles[syncmatrix.context.get_settings_context().profile.name]
...
def unset(settings: List[str]):
    ...
    for setting in settings:
        ...
        parsed.add(syncmatrix.settings.SETTING_VARIABLES[setting])
...
def view(
    ...
    default_settings = syncmatrix.settings.get_default_settings()
    env_settings = syncmatrix.settings.get_settings_from_env()
    ...
    app.console.print(f"SYNCMATRIX_PROFILE={context.profile.name!r}")
