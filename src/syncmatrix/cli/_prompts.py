...
from syncmatrix.client.collections import get_collections_metadata_client
from syncmatrix.client.orchestration import SyncmatrixClient
from syncmatrix.client.schemas.actions import BlockDocumentCreate, WorkPoolCreate
...
from syncmatrix.utilities.processutils import get_sys_executable, run_process
...
async def prompt_build_custom_docker_image(
    ...
    return {"syncmatrix_docker.deployments.steps.build_docker_image": build_step}
...
async def prompt_push_custom_docker_image(
    ...
    build_docker_image_step["syncmatrix_docker.deployments.steps.build_docker_image"][
        "image_name"
    ] = full_image_name
    ...
    if confirm("Is this a private registry?", console=console):
        ...
        if confirm(
            "Would you like use syncmatrix-docker to manage Docker registry credentials?",
            console=console,
            default=False,
        ):
            try:
                import syncmatrix_docker
            except ImportError:
                console.print("Installing syncmatrix-docker...")
                await run_process(
                    [get_sys_executable(), "-m", "pip", "install", "syncmatrix-docker"],
                    stream_output=True,
                )
                import syncmatrix_docker

            credentials_block = syncmatrix_docker.DockerRegistryCredentials
            push_step[
                "credentials"
            ] = "{{ syncmatrix_docker.docker-registry-credentials.docker_registry_creds_name }}"
        else:
    ...
    return {
        "syncmatrix_docker.deployments.steps.push_docker_image": push_step
...
async def prompt_create_work_pool(
    ...
    if not confirm(
        ...
        raise ValueError(
            ...
            " name via the '--pool' flag or in your syncmatrix.yaml file."
        )
    ...
    selected_worker_row = prompt_select_from_table(
        ...
        data=[
            ...
            if worker["type"] != "syncmatrix-agent"
        ],
...
async def prompt_select_remote_flow_storage(
    console: Console, client: SyncmatrixClient = None
) -> Optional[str]:
...
async def prompt_select_blob_storage_credentials(
    console: Console, storage_provider: str, client: SyncmatrixClient = None
) -> str:
    """
    ...
    if existing_credentials_blocks:
        ...
        if selected_credentials_block and (
            ...
            return f"{{{{ syncmatrix.blocks.{creds_block_type_slug}.{selected_block} }}}}"
    ...
    if SYNCMATRIX_UI_URL:
        console.print(
            ...
            f"\n[blue]{SYNCMATRIX_UI_URL.value()}/blocks/block/{new_block_document.id}[/]\n"
        )
    return f"{{{{ syncmatrix.blocks.{creds_block_type_slug}.{new_block_document.name} }}}}"
