"""
Command line interface for working with Syncmatrix
"""
import os
import textwrap
...
from syncmatrix.settings import (
    ...
    SYNCMATRIX_SERVER_API_KEEPALIVE_TIMEOUT,
    SYNCMATRIX_SERVER_API_PORT,
    SYNCMATRIX_UI_ENABLED,
)
from syncmatrix.utilities.asyncutils import run_sync_in_worker_thread
from syncmatrix.utilities.processutils import (
    get_sys_executable,
    run_process,
    setup_signal_handlers_server,
)

server_app = SyncmatrixTyper(
    name="server",
    help="Commands for interacting with a self-hosted Syncmatrix server instance.",
)
database_app = SyncmatrixTyper(
...
def generate_welcome_blurb(base_url, ui_enabled: bool):
    blurb = textwrap.dedent(
        ...
        Configure Syncmatrix to communicate with the server with:

            syncmatrix config set SYNCMATRIX_API_URL={api_url}
    ...
    dashboard_not_built = textwrap.dedent(
        ...
        See `syncmatrix dev` for development commands.
        """
    )

    dashboard_disabled = textwrap.dedent(
        """
        The dashboard is disabled. Set `SYNCMATRIX_UI_ENABLED=1` to re-enable it.
        """
    )

    if not os.path.exists(syncmatrix.__ui_static_path__):
...
async def start(
    ...
    keep_alive_timeout: int = SettingsOption(SYNCMATRIX_SERVER_API_KEEPALIVE_TIMEOUT),
    log_level: str = SettingsOption(SYNCMATRIX_LOGGING_SERVER_LEVEL),
    scheduler: bool = SettingsOption(SYNCMATRIX_API_SERVICES_SCHEDULER_ENABLED),
    analytics: bool = SettingsOption(
        SYNCMATRIX_SERVER_ANALYTICS_ENABLED, "--analytics-on/--analytics-off"
    ),
    late_runs: bool = SettingsOption(SYNCMATRIX_API_SERVICES_LATE_RUNS_ENABLED),
    ui: bool = SettingsOption(SYNCMATRIX_UI_ENABLED),
):
    """Start a Syncmatrix server instance"""

    server_env = os.environ.copy()
    server_env["SYNCMATRIX_API_SERVICES_SCHEDULER_ENABLED"] = str(scheduler)
    server_env["SYNCMATRIX_SERVER_ANALYTICS_ENABLED"] = str(analytics)
    server_env["SYNCMATRIX_API_SERVICES_LATE_RUNS_ENABLED"] = str(late_runs)
    server_env["SYNCMATRIX_API_SERVICES_UI"] = str(ui)
    server_env["SYNCMATRIX_LOGGING_SERVER_LEVEL"] = log_level
    ...
    async with anyio.create_task_group() as tg:
        ...
        server_process_id = await tg.start(
            partial(
                ...
                command=[
                    ...
                    f'"{syncmatrix.__module_path__.parent}"',
                    "--factory",
        ...
        # https://github.com/KhulnaSoft/server/issues/2475

        setup_signal_handlers_server(
            server_process_id, "the Syncmatrix server", app.console.print
        )
...
async def reset(yes: bool = typer.Option(False, "--yes", "-y")):
    """Drop and recreate all Syncmatrix database tables"""
    ...
    if not yes:
        confirm = typer.confirm(
            "Are you sure you want to reset the Syncmatrix database located "
    ...
    await db.create_db()
    exit_with_success(f'Syncmatrix database "{engine.url!r}" reset!')
...
async def upgrade(
    ...
    """Upgrade the Syncmatrix database"""
    from syncmatrix.server.database.alembic_commands import alembic_upgrade
    from syncmatrix.server.database.dependencies import provide_database_interface
    ...
    if not yes:
        confirm = typer.confirm(
            f"Are you sure you want to upgrade the Syncmatrix database at {engine.url!r}?"
        )
        if not confirm:
    ...
    exit_with_success(f"Syncmatrix database at {engine.url!r} upgraded!")
...
async def downgrade(
    ...
    """Downgrade the Syncmatrix database"""
    from syncmatrix.server.database.alembic_commands import alembic_downgrade
    from syncmatrix.server.database.dependencies import provide_database_interface
    ...
    if not yes:
        confirm = typer.confirm(
            "Are you sure you want to downgrade the Syncmatrix "
    ...
    app.console.print("Migrations succeeded!")
    exit_with_success(f"Syncmatrix database at {engine.url!r} downgraded!")
...
async def revision(
    ...
    """Create a new migration for the Syncmatrix database"""
    from syncmatrix.server.database.alembic_commands import alembic_revision
...
async def stamp(revision: str):
    ...
    from syncmatrix.server.database.alembic_commands import alembic_stamp
