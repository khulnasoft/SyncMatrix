...
from syncmatrix.client.schemas.filters import WorkPoolFilter, WorkPoolFilterId
    ...
    if tags:
        ...
        output_msg = dedent(
            ...
            Start an agent to pick up flow runs from the work queue:
                syncmatrix agent start -q '{result.name}'

            Inspect the work queue:
                syncmatrix work-queue inspect '{result.name}'
            """
        )
    else:
        if not pool:
        ...
        output_msg = dedent(
            ...
            Created work queue with properties:
                ...
                concurrency limit - {limit}
            Start an agent to pick up flow runs from the work queue:
                syncmatrix agent start -q '{result.name} -p {pool}'
