"""
Command line interface for working with Syncmatrix Server
"""
import json
import os
import platform
...
from syncmatrix.cli.agent import start as start_agent
...
from syncmatrix.utilities.dockerutils import get_syncmatrix_image_name, python_version_minor
from syncmatrix.utilities.filesystem import tmpchdir
from syncmatrix.utilities.processutils import run_process
...
def exit_with_error_if_not_editable_install():
    if (
        syncmatrix.__module_path__.parent == "site-packages"
        or not (syncmatrix.__development_base_path__ / "setup.py").exists()
    ):
        exit_with_error(
            "Development commands require an editable Syncmatrix installation. "
            "Development commands require content outside of the 'syncmatrix' module  "
            "which is not available when installed into your site-packages. "
            f"Detected module path: {syncmatrix.__module_path__}."
        )
...
def agent_process_entrypoint(**kwargs):
    ...
    All kwargs are forwarded to  `syncmatrix.cli.agent.start`.
    """
    import inspect
    ...
    for name, param in start_agent_signature.parameters.items():
        if name not in kwargs:
            ...
            # Some defaults are Syncmatrix `SettingsOption.value` methods
    ...
    app.console = (
        rich.console.Console(
            highlight=False,
            color_system="auto" if SYNCMATRIX_CLI_COLORS else None,
            soft_wrap=not SYNCMATRIX_CLI_WRAP_LINES.value(),
        )
...
def build_docs(
    ...
    if not schema_path:
        schema_path = (
            syncmatrix.__development_base_path__ / "docs" / "api-ref" / "schema.json"
        ).absolute()
...
The built UI will be located at {syncmatrix.__development_base_path__ / "ui"}
...
def build_ui(
    ...
    exit_with_error_if_not_editable_install()
    with tmpchdir(syncmatrix.__development_base_path__ / "ui"):
        if not no_install:
    ...
    with tmpchdir(syncmatrix.__development_base_path__):
        if os.path.exists(syncmatrix.__ui_static_path__):
            app.console.print("Removing existing build files...")
            shutil.rmtree(syncmatrix.__ui_static_path__)

        app.console.print("Copying build into src...")
        shutil.copytree("ui/dist", syncmatrix.__ui_static_path__)
...
@dev_app.command()
async def api(
    host: str = SettingsOption(SYNCMATRIX_SERVER_API_HOST),
    port: int = SettingsOption(SYNCMATRIX_SERVER_API_PORT),
    log_level: str = "DEBUG",
    services: bool = True,
):
    """
    ...
    server_env["SYNCMATRIX_API_SERVICES_RUN_IN_APP"] = str(services)
    server_env["SYNCMATRIX_API_SERVICES_UI"] = "False"
    server_env["SYNCMATRIX_UI_API_URL"] = f"http://{host}:{port}/api"
    ...
    command = [
        ...
        "uvicorn",
        "--factory",
        "syncmatrix.server.api.server:create_app",
        "--host",
        str(host),
        "--port",
        str(port),
    ...
    async with anyio.create_task_group() as tg:
        if not exclude_api:
            tg.start_soon(
                partial(
                    api,
                    host=SYNCMATRIX_SERVER_API_HOST.value(),
                    port=SYNCMATRIX_SERVER_API_PORT.value(),
                )
            )
        if not exclude_ui:
            tg.start_soon(ui)
        if not exclude_agent:
            ...
            if not exclude_api:
                host = f"http://{SYNCMATRIX_SERVER_API_HOST.value()}:{SYNCMATRIX_SERVER_API_PORT.value()}/api"  # noqa
            else:
                host = SYNCMATRIX_API_URL.value()
...
def build_image(
    ...
    tag = get_syncmatrix_image_name(python_version=python_version, flavor=flavor)
    ...
    command = [
        "docker",
        "build",
        str(syncmatrix.__development_base_path__),
        "--tag",
        tag,
        "--platform",
        f"linux/{arch}",
        "--build-arg",
        "SYNCMATRIX_EXTRAS=[dev]",
        "--build-arg",
        f"PYTHON_VERSION={python_version}",
    ]

    if flavor:
        command += ["--build-arg", f"BASE_IMAGE=syncmatrix-{flavor}"]
...
@dev_app.command()
def container(bg: bool = False, name="syncmatrix-dev", api: bool = True, tag: str = None):
    """
    ...
    blocking_cmd = "syncmatrix dev api" if api else "sleep infinity"
    tag = tag or get_syncmatrix_image_name()
    ...
    container: Container = client.containers.create(
        ...
        command=[
            ...
            "-c",
            (  # noqa
                "pip install -e /opt/syncmatrix/repo\\[dev\\] && touch /READY &&"
                f" {blocking_cmd}"
            ),
        ],
        name=name,
        auto_remove=True,
        working_dir="/opt/syncmatrix/repo",
        volumes=[f"{syncmatrix.__development_base_path__}:/opt/syncmatrix/repo"],
        shm_size="4G",
    )
...
def kubernetes_manifest():
    ...
    Example:
        $ syncmatrix dev kubernetes-manifest | kubectl apply -f -
    """
    exit_with_error_if_not_editable_install()

    template = Template(
        (
            syncmatrix.__module_path__ / "cli" / "templates" / "kubernetes-dev.yaml"
        ).read_text()
    )
    manifest = template.substitute(
        {
            "syncmatrix_root_directory": syncmatrix.__development_base_path__,
            "image_name": get_syncmatrix_image_name(),
        }
    )
    print(manifest)
