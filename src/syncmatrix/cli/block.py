...
from syncmatrix.client import get_client
...
blocks_app = SyncmatrixTyper(name="block", help="Commands for working with blocks.")
blocktypes_app = SyncmatrixTyper(
...
async def register(
    ...
    Examples:
        ...
        $ syncmatrix block register -m syncmatrix_aws.credentials
        \b
        Register block types in a .py file:
        $ syncmatrix block register -f my_blocks.py
    """
    ...
    msg = (
        ...
        "go to the Blocks page in the Syncmatrix UI.\n"
    )
...
async def block_ls():
    ...
    table = Table(
        title="Blocks", caption="List Block Types using `syncmatrix block type ls`"
    )
...
async def block_create(
    block_type_slug: str = typer.Argument(
        ...,
        help="A block type slug. View available types with: syncmatrix block type ls",
        show_default=False,
    ),
):
    """
    Generate a link to the Syncmatrix UI to create a block.
    """
    async with get_client() as client:
        try:
        ...
        if not SYNCMATRIX_UI_URL:
            exit_with_error(
                "Syncmatrix must be configured to use a hosted Syncmatrix server or "
                "Syncmatrix Cloud to display the Syncmatrix UI"
            )

        block_link = f"{SYNCMATRIX_UI_URL.value()}/blocks/catalog/{block_type.slug}/create"
        app.console.print(
...
async def list_types():
    ...
    for blocktype in sorted(block_types, key=lambda x: x.name):
        table.add_row(
            ...
            f"syncmatrix block create {blocktype.slug}",
        )
