...
from syncmatrix.settings import (
    SYNCMATRIX_AGENT_PREFETCH_SECONDS,
    SYNCMATRIX_AGENT_QUERY_INTERVAL,
    SYNCMATRIX_API_URL,
)
from syncmatrix.utilities.processutils import setup_signal_handlers_agent
from syncmatrix.utilities.services import critical_service_loop

agent_app = SyncmatrixTyper(
    name="agent",
    help="Commands for starting and interacting with agent processes.",
    deprecated=True,
    deprecated_name="agent",
    deprecated_start_date="Mar 2024",
    deprecated_help="Use `syncmatrix worker start` instead. Refer to the upgrade guide for more information: https://docs.khulnasoft.com/latest/guides/upgrade-guide-agents-to-workers/. ",
)
...
async def start(
    ...
    api: str = SettingsOption(SYNCMATRIX_API_URL),
    run_once: bool = typer.Option(
        False, help="Run the agent loop once, instead of forever."
    ),
    prefetch_seconds: int = SettingsOption(SYNCMATRIX_AGENT_PREFETCH_SECONDS),
    # deprecated tags
    ...
    if work_queue is not None:
        ...
        app.console.print(
            (
                ...
                f" flag: `syncmatrix agent start -q {work_queue}`\n"
            ),
            style="blue",
        )
    ...
    if tags:
        ...
        app.console.print(
            (
                ...
                " of Syncmatrix, this agent will create a work queue named"
    ...
    if not hide_welcome:
        if api:
            app.console.print(
                f"Starting v{syncmatrix.__version__} agent connected to {api}..."
            )
        else:
            app.console.print(
                f"Starting v{syncmatrix.__version__} agent with ephemeral API..."
            )

    agent_process_id = os.getpid()
    setup_signal_handlers_agent(
        agent_process_id, "the Syncmatrix agent", app.console.print
    )
        ...
        async with anyio.create_task_group() as tg:
            tg.start_soon(
                partial(
                    ...
                    SYNCMATRIX_AGENT_QUERY_INTERVAL.value(),
            ...
            tg.start_soon(
                partial(
                    ...
                    SYNCMATRIX_AGENT_QUERY_INTERVAL.value() * 2,
                    printer=app.console.print,
                    run_once=run_once,
                    jitter_range=0.3,
                    backoff=4,
