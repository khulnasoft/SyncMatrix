...
from syncmatrix._internal.concurrency.api import (
    create_call,
    from_async,
    from_sync,
)
...
from syncmatrix.deployments.schedules import FlexibleScheduleList
from syncmatrix.engine import propose_state
...
from syncmatrix.logging.loggers import SyncmatrixLogAdapter, flow_run_logger, get_logger
from syncmatrix.runner.server import start_webserver
from syncmatrix.runner.storage import RunnerStorage
...
from syncmatrix.states import Crashed, Pending, exception_to_failed_state
from syncmatrix.utilities.asyncutils import (
    asyncnullcontext,
    is_async_fn,
    sync_compatible,
)
from syncmatrix.utilities.processutils import _register_signal, run_process
from syncmatrix.utilities.services import critical_service_loop
...
class Runner:
    def __init__(
        ...
        Args:
            ...
            query_seconds: The number of seconds to wait between querying for
                scheduled flow runs; defaults to `SYNCMATRIX_RUNNER_POLL_FREQUENCY`
        ...
        Examples:
            Set up a Runner to manage the execute of scheduled flow runs for two flows:
                ```python
                from syncmatrix import flow, Runner
        ...
        self.limit = limit or SYNCMATRIX_RUNNER_PROCESS_LIMIT.value()
        self.webserver = webserver

        self.query_seconds = query_seconds or SYNCMATRIX_RUNNER_POLL_FREQUENCY.value()
    ...
    async def add_deployment(
        ...
        """
        Registers the deployment with the Syncmatrix API and will monitor for work once
    ...
    async def add_flow(
        ...
        if any([interval, cron, rrule]) and not api:
            self._logger.warning(
                "Cannot schedule flows on an ephemeral server; run `syncmatrix server"
    ...
    async def start(
        ...
        if webserver or SYNCMATRIX_RUNNER_SERVER_ENABLE.value():
    ...
    def _get_flow_run_logger(self, flow_run: "FlowRun") -> SyncmatrixLogAdapter:
    ...
    async def _run_process(
        ...
        Args:
            flow_run: Flow run to execute via process. The ID of this flow run
                is stored in the SYNCMATRIX__FLOW_RUN_ID environment variable to
        ...
        """
        command = f"{shlex.quote(sys.executable)} -m syncmatrix.engine"
        ...
        env.update(
            {
                **{
                    ...
                    "SYNCMATRIX__STORAGE_BASE_PATH": str(self._tmp_dir),
                    "SYNCMATRIX__ENABLE_CANCELLATION_AND_CRASHED_HOOKS": "false",
                },
                **({"SYNCMATRIX__FLOW_ENTRYPOINT": entrypoint} if entrypoint else {}),
            }
        )
    ...
    def _acquire_limit_slot(self, flow_run_id: str) -> bool:
        ...
        try:
            ...
            self._logger.info(
                ...
                " to `serve` or adjusting the SYNCMATRIX_RUNNER_PROCESS_LIMIT setting."
            )
            return False
    ...
    **kwargs,
    ...
    if print_starting_message:
        ...
        if SYNCMATRIX_UI_URL:
            help_message_bottom += (
                "\nYou can also trigger your deployments via the Syncmatrix UI:"
                f" [blue]{SYNCMATRIX_UI_URL.value()}/deployments[/]\n"
            )
