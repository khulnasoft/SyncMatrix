...
from syncmatrix.client.schemas.filters import FlowRunFilter, TaskRunFilter
...
async def _submit_flow_to_runner(
    ...
    async with get_client() as client:
        ...
        response = await client._client.post(
            (
                f"http://{SYNCMATRIX_RUNNER_SERVER_HOST.value()}"
                f":{SYNCMATRIX_RUNNER_SERVER_PORT.value()}"
                "/flow/run"
            ),
            json={
...
def submit_to_runner(
    syncmatrix_callable: Union[Flow, Task],
    parameters: Dict[str, Any],
    retry_failed_submissions: bool = True,
) -> FlowRun:
    ...
...
async def submit_to_runner(
    ...
    Args:
        syncmatrix_callable: the callable to run (only flows are supported for now, but eventually tasks)
    ...
    """
    if not isinstance(syncmatrix_callable, (Flow, Task)):
        raise TypeError(
            "The `submit_to_runner` utility only supports submitting flows and tasks."
        )

    if not SYNCMATRIX_EXPERIMENTAL_ENABLE_EXTRA_RUNNER_ENDPOINTS.value():
        raise ValueError(
            ...
            " `SYNCMATRIX_EXPERIMENTAL_ENABLE_EXTRA_RUNNER_ENDPOINTS` setting to `True`."
        )
    ...
    for p in parameters:
        try:
            flow_run = await _submit_flow_to_runner(
                syncmatrix_callable, p, retry_failed_submissions
            )
    ...
    if unsubmitted_parameters:
        logger.warning(
            ...
            f" of the available {SYNCMATRIX_RUNNER_PROCESS_LIMIT.value()} slots were"
            " occupied. To increase the number of available slots, configure"
            " the`SYNCMATRIX_RUNNER_PROCESS_LIMIT` setting."
        )
