...
from syncmatrix.filesystems import ReadableDeploymentStorage, WritableDeploymentStorage
from syncmatrix.logging.loggers import get_logger
from syncmatrix.utilities.collections import visit_collection
...
class GitRepository:
    ...
    def to_pull_step(self) -> Dict:
        ...
        if isinstance(self._credentials, Block):
            pull_step["syncmatrix.deployments.steps.git_clone"][
                "credentials"
            ...
            if isinstance(self._credentials.get("access_token"), Secret):
                pull_step["syncmatrix.deployments.steps.git_clone"]["credentials"] = {
                    **self._credentials,
                    "access_token": (
                        "{{"
...
class RemoteStorage:
    ...
    Examples:
        ...
        ```python
        from syncmatrix.runner.storage import RemoteStorage
    ...
    def to_pull_step(self) -> dict:
        ...
        if required_package:
            step["syncmatrix.deployments.steps.pull_from_remote_storage"][
                "requires"
            ] = required_package
        return step
