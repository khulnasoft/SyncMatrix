...
from syncmatrix.utilities.math import bounded_poisson_interval, clamped_poisson_interval
...
async def app_lifespan_context(app: ASGIApp) -> AsyncGenerator[None, None]:
    ...
    #       See https://github.com/KhulnaSoft/orion/pull/1696
...
class SyncmatrixResponse(httpx.Response):
    """
    A Syncmatrix wrapper for the `httpx.Response` class.
    ...
    def raise_for_status(self) -> None:
        ...
        The `SyncmatrixHTTPStatusError` contains useful additional information that
        is not contained in the `HTTPStatusError`.
        """
        try:
            return super().raise_for_status()
        except HTTPStatusError as exc:
            raise SyncmatrixHTTPStatusError.from_httpx_error(exc) from exc.__cause__

    @classmethod
    def from_httpx_response(cls: Type[Self], response: httpx.Response) -> Self:
        """
        Create a `SyncmatrixReponse` from an `httpx.Response`.

        By changing the `__class__` attribute of the Response, we change the method
        resolution order to look for methods defined in SyncmatrixResponse, while leaving
...
class SyncmatrixHttpxClient(httpx.AsyncClient):
    """
    A Syncmatrix wrapper for the async httpx client with support for retry-after headers
    ...
    async def _send_with_retry(
        ...
        Sends the provided request and retries it up to SYNCMATRIX_CLIENT_MAX_RETRIES times
        ...
        while try_count <= SYNCMATRIX_CLIENT_MAX_RETRIES.value():
            try_count += 1
            retry_seconds = None
            exc_info = None
            ...
            try:
                ...
                if try_count > SYNCMATRIX_CLIENT_MAX_RETRIES.value():
                    raise
            ...
            # Add jitter
            jitter_factor = SYNCMATRIX_CLIENT_RETRY_JITTER_FACTOR.value()
            ...
            logger.debug(
                ...
                "This is attempt"
                f" {try_count}/{SYNCMATRIX_CLIENT_MAX_RETRIES.value() + 1}.",
                exc_info=exc_info,
            )
    ...
    async def send(self, request: Request, *args, **kwargs) -> Response:
        ...
        - Any additional status codes provided in `SYNCMATRIX_CLIENT_RETRY_EXTRA_CODES`
        """
        ...
        response = await self._send_with_retry(
            ...
            retry_exceptions=(
                ...
                httpx.WriteError,
                # Uvicorn bug, see https://github.com/KhulnaSoft/syncmatrix/issues/7512
                httpx.RemoteProtocolError,
                # HTTP2 bug, see https://github.com/KhulnaSoft/syncmatrix/issues/7442
                httpx.LocalProtocolError,
            ),
        )

        # Convert to a Syncmatrix response to add nicer errors messages
        response = SyncmatrixResponse.from_httpx_response(response)
    ...
    async def _add_csrf_headers(self, request: Request):
        ...
        request.headers["Syncmatrix-Csrf-Token"] = self.csrf_token
        request.headers["Syncmatrix-Csrf-Client"] = str(self.csrf_client_id)
