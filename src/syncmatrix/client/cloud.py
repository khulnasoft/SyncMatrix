...
from syncmatrix.client.base import SyncmatrixHttpxClient
...
def get_cloud_client(
    ...
    if infer_cloud_url is False:
        host = host or SYNCMATRIX_CLOUD_API_URL.value()
    else:
        configured_url = syncmatrix.settings.SYNCMATRIX_API_URL.value()
...
class CloudUnauthorizedError(SyncmatrixException):
    """
...
class CloudClient:
    def __init__(
        ...
        if not SYNCMATRIX_UNIT_TEST_MODE.value():
            httpx_settings.setdefault("follow_redirects", True)
        self._client = SyncmatrixHttpxClient(**httpx_settings, enable_csrf_support=False)
