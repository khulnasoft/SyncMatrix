...
from syncmatrix._internal.schemas.bases import IDBaseModel
...
class Subscription(Generic[S]):
    def __init__(
        ...
        base_url = SYNCMATRIX_API_URL.value().replace("http", "ws", 1)
    ...
    async def _ensure_connected(self):
        ...
        try:
            await websocket.send(
                orjson.dumps(
                    {"type": "auth", "token": SYNCMATRIX_API_KEY.value()}
                ).decode()
            )
            ...
            if isinstance(e, AssertionError) or e.code == WS_1008_POLICY_VIOLATION:
                raise Exception(
                    ...
                    "ensure the provided `SYNCMATRIX_API_KEY` you are using is "
