...
from syncmatrix._internal.compatibility.experimental import (
...
from syncmatrix.settings import (
    SYNCMATRIX_EXPERIMENTAL_WARN,
    SYNCMATRIX_EXPERIMENTAL_WARN_FLOW_RUN_INFRA_OVERRIDES,
)
...
from syncmatrix.client.schemas.schedules import SCHEDULE_TYPES
from syncmatrix.client.schemas.sorting import (
...
from syncmatrix.events.schemas import Automation, ExistingAutomation
from syncmatrix.logging import get_logger
from syncmatrix.settings import (
    SYNCMATRIX_API_DATABASE_CONNECTION_URL,
    SYNCMATRIX_API_ENABLE_HTTP2,
    SYNCMATRIX_API_KEY,
    SYNCMATRIX_API_REQUEST_TIMEOUT,
...
if TYPE_CHECKING:
    from syncmatrix.flows import Flow as FlowObject
    from syncmatrix.tasks import Task as TaskObject

from syncmatrix.client.base import ASGIApp, SyncmatrixHttpxClient, app_lifespan_context
...
def get_client(httpx_settings: Optional[dict] = None) -> "SyncmatrixClient":
    """
    Retrieve a HTTP client for communicating with the Syncmatrix REST API.
    ...
    ```
    """
    ctx = syncmatrix.context.get_settings_context()
    api = SYNCMATRIX_API_URL.value()
    ...
    return SyncmatrixClient(
        api,
        api_key=SYNCMATRIX_API_KEY.value(),
        httpx_settings=httpx_settings,
    )


class SyncmatrixClient:
    """
    An asynchronous client for interacting with the [Syncmatrix REST API](/api-ref/rest-api/).
    ...
    Examples:
        ...
        Say hello to a Syncmatrix REST API
    ...
    def __init__(
        ...
        *,
        ...
        if SYNCMATRIX_API_TLS_INSECURE_SKIP_VERIFY:
            httpx_settings.setdefault("verify", False)
        else:
            cert_file = SYNCMATRIX_API_SSL_CERT_FILE.value()
            if not cert_file:
        ...
        httpx_settings["headers"].setdefault("X-SYNCMATRIX-API-VERSION", api_version)
        if api_key:
        ...
        if isinstance(api, str):
            ...
            httpx_settings.setdefault(
                ...
                httpx.Limits(
                    ...
                    # The Syncmatrix Cloud LB will keep connections alive for 30s.
            ...
            httpx_settings.setdefault("http2", SYNCMATRIX_API_ENABLE_HTTP2.value())

            self.server_type = (
                ServerType.CLOUD
                if api.startswith(SYNCMATRIX_CLOUD_API_URL.value())
        ...
        # Connect to an in-process application
            ...
            # refs:
            # - https://github.com/KhulnaSoft/syncmatrix/pull/9637
            ...
            httpx_settings.setdefault("base_url", "http://ephemeral-syncmatrix/api")
        ...
        httpx_settings.setdefault(
            "timeout",
            httpx.Timeout(
                connect=SYNCMATRIX_API_REQUEST_TIMEOUT.value(),
                read=SYNCMATRIX_API_REQUEST_TIMEOUT.value(),
                write=SYNCMATRIX_API_REQUEST_TIMEOUT.value(),
                pool=SYNCMATRIX_API_REQUEST_TIMEOUT.value(),
            ),
        )

        if not SYNCMATRIX_UNIT_TEST_MODE:
            httpx_settings.setdefault("follow_redirects", True)

        enable_csrf_support = (
            self.server_type != ServerType.CLOUD
            and SYNCMATRIX_CLIENT_CSRF_SUPPORT_ENABLED.value()
        )

        self._client = SyncmatrixHttpxClient(
    ...
    async def create_flow(self, flow: "FlowObject") -> UUID:
        """
        Create a flow in the Syncmatrix API.

        Args:
            flow: a [Flow][syncmatrix.flows.Flow] object
    ...
    async def read_flow(self, flow_id: UUID) -> Flow:
        """
        Query the Syncmatrix API for a flow by id.

        Args:
            flow_id: the flow ID of interest

        Returns:
            a [Flow model][syncmatrix.client.schemas.objects.Flow] representation of the flow
        """
    ...
    async def read_flows(
        ...
        *,
        ...
        """
        Query the Syncmatrix API for flows. Only flows matching all criteria will
        be returned.
    ...
    async def read_flow_by_name(
        ...
        """
        Query the Syncmatrix API for a flow by name.
    ...
    async def create_flow_run_from_deployment(
        ...
        *,
        ...
        context: dict = None,
        state: syncmatrix.states.State = None,
        name: str = None,
        ...
        Raises:
            httpx.RequestError: if the Syncmatrix API does not successfully create a run for any reason
        ...
        if job_variables is not None and experiment_enabled("flow_run_infra_overrides"):
            if (
                SYNCMATRIX_EXPERIMENTAL_WARN
                and SYNCMATRIX_EXPERIMENTAL_WARN_FLOW_RUN_INFRA_OVERRIDES
            ):
                warnings.warn(
                    EXPERIMENTAL_WARNING.format(
                        feature="Flow run job variables",
                        group="flow_run_infra_overrides",
                        help="To use this feature, update your workers to Syncmatrix 2.16.4 or later. ",
                    ),
                    ExperimentalFeature,
                    stacklevel=3,
                )
        ...
        context = context or {}
        state = state or syncmatrix.states.Scheduled()
        tags = tags or []
    ...
    async def create_flow_run(
        ...
        if state is None:
            state = syncmatrix.states.Pending()
    ...
    async def delete_flow_run(
        ...
        Raises:
            syncmatrix.exceptions.ObjectNotFound: If request returns 404
        ...
        try:
            ...
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise syncmatrix.exceptions.ObjectNotFound(http_exc=e) from e
            else:
                raise
    ...
    async def create_concurrency_limit(
        ...
        """
        Create a tag concurrency limit in the Syncmatrix API. These limits govern concurrently
        running tasks.
    ...
    async def create_work_queue(
        ...
        Raises:
            syncmatrix.exceptions.ObjectAlreadyExists: If request returns 409
            httpx.RequestError: If request fails
        ...
        try:
            ...
            if e.response.status_code == status.HTTP_409_CONFLICT:
                raise syncmatrix.exceptions.ObjectAlreadyExists(http_exc=e) from e
    ...
    async def read_work_queue_by_name(
        ...
        Raises:
            syncmatrix.exceptions.ObjectNotFound: if no work queue is found
    ...
    async def match_work_queues(
        ...
        """
        Query the Syncmatrix API for work queues with names with a specific prefix.
    ...
    async def create_block_type(self, block_type: BlockTypeCreate) -> BlockType:
        """
        Create a block type in the Syncmatrix API.
        """
        try:
            response = await self._client.post(
                "/block_types/",
    ...
    async def create_block_schema(self, block_schema: BlockSchemaCreate) -> BlockSchema:
        """
        Create a block schema in the Syncmatrix API.
        """
        try:
            response = await self._client.post(
                "/block_schemas/",
    ...
    async def create_block_document(
        ...
        """
        Create a block document in the Syncmatrix API. This data is used to configure a
        corresponding Block.
    ...
    async def update_block_document(
        ...
        """
        Update a block document in the Syncmatrix API.
        """
        try:
    ...
    async def delete_block_type(self, block_type_id: UUID):
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                raise syncmatrix.exceptions.ProtectedBlockError(
                    "Protected block types cannot be deleted."
                ) from e
            else:
                raise
    ...
    async def read_deployment(
        ...
        """
        Query the Syncmatrix API for a deployment by id.

        Args:
            deployment_id: the deployment ID of interest

        Returns:
            a [Deployment model][syncmatrix.client.schemas.objects.Deployment] representation of the deployment
        """
        try:
    ...
    async def read_deployment_by_name(
        ...
        """
        Query the Syncmatrix API for a deployment by name.
    ...
    async def read_deployments(
        ...
        *,
        ...
        """
        Query the Syncmatrix API for deployments. Only deployments matching all
    ...
    async def read_deployment_schedules(
        ...
        """
        Query the Syncmatrix API for a deployment's schedules.
    ...
    async def read_flow_run(self, flow_run_id: UUID) -> FlowRun:
        """
        Query the Syncmatrix API for a flow run by id.
    ...
    async def read_flow_runs(
        ...
        *,
        ...
        """
        Query the Syncmatrix API for flow runs. Only flow runs matching all criteria will
        be returned.
    ...
    async def set_flow_run_state(
        ...
        Args:
            ...
            force: if True, disregard orchestration logic when setting the state,
                forcing the Syncmatrix API to accept the state
    ...
    async def read_flow_run_states(
        ...
        return pydantic.parse_obj_as(List[syncmatrix.states.State], response.json())
    ...
    async def read_task_run(self, task_run_id: UUID) -> TaskRun:
        """
        Query the Syncmatrix API for a task run by id.
    ...
    async def read_task_runs(
        ...
        *,
        ...
        """
        Query the Syncmatrix API for task runs. Only task runs matching all criteria will
        be returned.
    ...
    async def read_flow_run_notification_policies(
        ...
        """
        Query the Syncmatrix API for flow run notification policies. Only policies matching all criteria will
        be returned.
    ...
    async def read_artifacts(
        ...
        *,
        ...
        """
        Query the Syncmatrix API for artifacts. Only artifacts matching all criteria will
        be returned.
        Args:
    ...
    async def read_worker_metadata(self) -> Dict[str, Any]:
        """Reads worker metadata stored in Syncmatrix collection registry."""
    ...
    async def create_automation(self, automation: Automation) -> UUID:
        """Creates an automation in Syncmatrix Cloud."""
        if self.server_type != ServerType.CLOUD:
            raise RuntimeError("Automations are only supported for Syncmatrix Cloud.")
    ...
    async def __aenter__(self):
        ...
        if self._ephemeral_app:
            self.logger.debug(
                ...
                f"{SYNCMATRIX_API_DATABASE_CONNECTION_URL.value()}"
            )
        else:
    ...
    def __enter__(self):
        raise RuntimeError(
            "The `SyncmatrixClient` must be entered with an async context. Use 'async "
            "with SyncmatrixClient(...)' not 'with SyncmatrixClient(...)'"
        )
