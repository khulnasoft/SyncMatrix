...
from syncmatrix.infrastructure import Infrastructure, InfrastructureResult, Process
from syncmatrix.logging import get_logger
from syncmatrix.settings import SYNCMATRIX_AGENT_PREFETCH_SECONDS
from syncmatrix.states import Crashed, Pending, StateType, exception_to_failed_state


@deprecated_class(
    start_date="Mar 2024",
    help="Use a worker instead. Refer to the upgrade guide for more information: https://docs.khulnasoft.com/latest/guides/upgrade-guide-agents-to-workers/.",
)
class SyncmatrixAgent:
    def __init__(
        self,
    ...
    async def get_and_submit_flow_runs(self) -> List[FlowRun]:
        ...
        if not self.started:
            raise RuntimeError(
                "Agent is not started. Use `async with SyncmatrixAgent()...`"
            )

        self.logger.debug("Checking for scheduled flow runs...")

        before = pendulum.now("utc").add(
            seconds=self.prefetch_seconds or SYNCMATRIX_AGENT_PREFETCH_SECONDS.value()
        )
