"""
Module containing the base workflow class and decorator - for most use cases, using the [`@flow` decorator][syncmatrix.flows.flow] is preferred.
"""
...
from syncmatrix._vendor.fastapi.encoders import jsonable_encoder
from typing_extensions import Self

from syncmatrix._internal.compatibility.deprecated import deprecated_parameter
from syncmatrix._internal.concurrency.api import create_call, from_async
from syncmatrix._internal.pydantic import HAS_PYDANTIC_V2
from syncmatrix.client.orchestration import get_client
from syncmatrix.deployments.runner import DeploymentImage, EntrypointType, deploy
from syncmatrix.filesystems import ReadableDeploymentStorage
from syncmatrix.runner.storage import (
    BlockStorageAdapter,
    RunnerStorage,
...
from syncmatrix._internal.schemas.validators import raise_on_name_with_banned_characters
from syncmatrix.client.schemas.objects import Flow as FlowSchema
from syncmatrix.client.schemas.objects import FlowRun, MinimalDeploymentSchedule
from syncmatrix.client.schemas.schedules import SCHEDULE_TYPES
from syncmatrix.context import SyncmatrixObjectRegistry, registry_from_script
...
from syncmatrix.results import ResultSerializer, ResultStorage
...
from syncmatrix.task_runners import BaseTaskRunner, ConcurrentTaskRunner
from syncmatrix.utilities.annotations import NotSet
from syncmatrix.utilities.asyncutils import is_async_fn, sync_compatible
...
from syncmatrix.utilities.collections import listrepr
from syncmatrix.utilities.hashing import file_hash
from syncmatrix.utilities.importtools import import_object
from syncmatrix.utilities.visualization import (
...
if TYPE_CHECKING:
    from syncmatrix.deployments.runner import FlexibleScheduleList, RunnerDeployment


@SyncmatrixObjectRegistry.register_instances
class Flow(Generic[P, R]):
    """
    A Syncmatrix workflow definition.

    !!! note
        We recommend using the [`@flow` decorator][syncmatrix.flows.flow] for most use-cases.

    Wraps a function with an entrypoint to the Syncmatrix engine. To preserve the input
    ...
    Args:
        ...
        persist_result: An optional toggle indicating whether the result of this flow
            ...
            that Syncmatrix should choose whether the result should be persisted depending on
        ...
        result_serializer: An optional serializer to use to serialize the result of this
            ...
            in this flow. If not provided, the value of `SYNCMATRIX_RESULTS_DEFAULT_SERIALIZER`
    ...
    def __init__(
        ...
        self.retries = (
            retries if retries is not None else SYNCMATRIX_FLOW_DEFAULT_RETRIES.value()
        )

        self.retry_delay_seconds = (
            retry_delay_seconds
            if retry_delay_seconds is not None
            else SYNCMATRIX_FLOW_DEFAULT_RETRY_DELAY_SECONDS.value()
        )
        ...
        if module in ("__main__", "__syncmatrix_loader__"):
    ...
    async def to_deployment(
        ...
        Args:
            ...
            enforce_parameter_schema: Whether or not the Syncmatrix API should enforce the
        ...
        Examples:
            ...
            ```python
            from syncmatrix import flow, serve
            ...
            ```
        """
        from syncmatrix.deployments.runner import RunnerDeployment
    ...
    async def serve(
        ...
        Examples:
            ...
            ```
        """
        from syncmatrix.runner import Runner
        ...
        if print_starting_message:
            ...
            if SYNCMATRIX_UI_URL:
                help_message += (
                    "\nYou can also run your flow via the Syncmatrix UI:"
                    f" [blue]{SYNCMATRIX_UI_URL.value()}/deployments/deployment/{deployment_id}[/]\n"
                )
    ...
    async def from_source(
        ...
        Examples:
            ...
            ```python
            from syncmatrix import flow
            from syncmatrix.runner.storage import GitRepository
    ...
    async def deploy(
        ...
        and create a deployment via the Syncmatrix API that will run the flow on the given schedule.
        ...
        Args:
            ...
            work_pool_name: The name of the work pool to use for this deployment. Defaults to
                the value of `SYNCMATRIX_DEFAULT_WORK_POOL_NAME`.
        ...
        Examples:
            ...
            ```
        """
        work_pool_name = work_pool_name or SYNCMATRIX_DEFAULT_WORK_POOL_NAME.value()
        ...
        if print_next_steps:
            ...
            if not work_pool.is_push_pool and not work_pool.is_managed_pool:
                ...
                console.print(
                    f"\n\t$ syncmatrix worker start --pool {work_pool_name!r}",
                    style="blue",
                )
            console.print(
            ...
            console.print(
                f"\n\t$ syncmatrix deployment run '{self.name}/{name}'\n",
                style="blue",
            )
            if SYNCMATRIX_UI_URL:
                console.print(
                    "\nYou can also run your flow via the Syncmatrix UI:"
                    f" [blue]{SYNCMATRIX_UI_URL.value()}/deployments/deployment/{deployment_ids[0]}[/]\n"
                )
    ...
    def __call__(
        ...
        *args: "P.args",
        ...
        wait_for: Optional[Iterable[SyncmatrixFuture]] = None,
        **kwargs: "P.kwargs",
    ):
        """
        ...
        Args:
            *args: Arguments to run the flow with.
            return_state: Return a Syncmatrix State containing the result of the
                flow run.
        ...
        Returns:
            ...
            If `return_state` is True, returns the result of the flow run
                wrapped in a Syncmatrix State which provides error handling.
        ...
        Examples:
            ...
            >>> from syncmatrix import tags
            >>> with tags("db", "blue"):
            >>>     my_flow("foo")
        """
        from syncmatrix.engine import enter_flow_run_engine_from_flow_call
...
def flow(
    ...
    *,
    ...
    """
    Decorator to designate a function as a Syncmatrix workflow.
    ...
    Args:
        ...
        log_prints: If set, `print` statements in the flow will be redirected to the
            Syncmatrix logger for the flow run. Defaults to `None`, which indicates that
            the value from the parent flow should be used. If this is a parent flow,
            the default is pulled from the `SYNCMATRIX_LOGGING_LOG_PRINTS` setting.
    ...
    Examples:
        ...
        >>> from syncmatrix_dask.task_runners import DaskTaskRunner
        >>>
