...
async def get_default_result_storage() -> ResultStorage:
    ...
    """
    return (
        await Block.load(SYNCMATRIX_DEFAULT_RESULT_STORAGE_BLOCK.value())
        if SYNCMATRIX_DEFAULT_RESULT_STORAGE_BLOCK.value() is not None
        else LocalFileSystem(basepath=SYNCMATRIX_LOCAL_STORAGE_PATH.value())
    )
...
async def get_or_create_default_task_scheduling_storage() -> ResultStorage:
    ...
    default_storage_name, storage_path = cache_key = (
        SYNCMATRIX_TASK_SCHEDULING_DEFAULT_STORAGE_BLOCK.value(),
        SYNCMATRIX_LOCAL_STORAGE_PATH.value(),
    )
...
def get_default_result_serializer() -> ResultSerializer:
    ...
    Generate a default file system for result storage.
    """
    return SYNCMATRIX_RESULTS_DEFAULT_SERIALIZER.value()
...
def get_default_persist_setting() -> bool:
    ...
    """
    return SYNCMATRIX_RESULTS_PERSIST_BY_DEFAULT.value()
...
def _format_user_supplied_storage_key(key):
    ...
    runtime_vars = {key: getattr(syncmatrix.runtime, key) for key in dir(syncmatrix.runtime)}
    return key.format(**runtime_vars, parameters=syncmatrix.runtime.task_run.parameters)
...
class ResultFactory(pydantic.BaseModel):
    ...
    @classmethod
    @inject_client
    async def default_factory(cls, client: "SyncmatrixClient" = None, **kwargs):
        """
    ...
    @inject_client
    async def from_flow(
        cls: Type[Self], flow: "Flow", client: "SyncmatrixClient" = None
    ) -> Self:
        """
    ...
    @inject_client
    async def from_task(
        cls: Type[Self], task: "Task", client: "SyncmatrixClient" = None
    ) -> Self:
        """
...
class LiteralResult(BaseResult):
    ...
    These values are stored inline and JSON serialized when sent to the Syncmatrix API.
    ...
    async def create(
        ...
        description = f"Result with value `{obj}` persisted to Syncmatrix."
...
class PersistedResult(BaseResult):
    ...
    @sync_compatible
    @inject_client
    async def get(self, client: "SyncmatrixClient") -> R:
        """
    ...
    @inject_client
    async def _read_blob(self, client: "SyncmatrixClient") -> "PersistedResultBlob":
        assert (
            self.storage_block_id is not None
        ), "Unexpected storage block ID. Was it persisted?"
...
class PersistedResultBlob(pydantic.BaseModel):
    ...
    serializer: Serializer
    data: bytes
    syncmatrix_version: str = pydantic.Field(default=syncmatrix.__version__)
...
class UnknownResult(BaseResult):
    ...
    async def create(
        ...
        description = "Unknown result persisted to Syncmatrix."
        return cls(value=obj, artifact_type="result", artifact_description=description)
