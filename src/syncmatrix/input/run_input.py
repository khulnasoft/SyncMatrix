...
from syncmatrix.input import RunInput
...
from syncmatrix.input.actions import (
...
if HAS_PYDANTIC_V2:
    from syncmatrix._internal.pydantic.v2_schema import create_v2_schema
...
class RunInput(pydantic.BaseModel):
    ...
    async def respond(
        ...
        flow_run_id = None
        if self.metadata.sender and self.metadata.sender.startswith("syncmatrix.flow-run"):
