...
if HAS_PYDANTIC_V2:
    from syncmatrix._internal.pydantic.v2_schema import is_v2_model
...
async def create_flow_run_input_from_model(
    ...
    if sender is None:
        ...
        if context is not None and context.flow_run is not None:
            sender = f"syncmatrix.flow-run.{context.flow_run.id}"
...
async def read_flow_run_input(
    key: str, flow_run_id: Optional[UUID] = None, client: "SyncmatrixClient" = None
) -> Any:
