...
- **The flow is called by the user or an existing flow run is executed in a new process.**
    ...
    See `Flow.__call__` and `syncmatrix.engine.__main__` (`python -m syncmatrix.engine`)
...
- **The task run is orchestrated through states, calling the user's function as necessary.**
    ...
    user thread as we do for flows. See [#9855](https://github.com/KhulnaSoft/syncmatrix/pull/9855).
"""
...
import syncmatrix.plugins
from syncmatrix._internal.compatibility.deprecated import deprecated_parameter
from syncmatrix._internal.compatibility.experimental import experimental_parameter
from syncmatrix._internal.concurrency.api import create_call, from_async, from_sync
from syncmatrix._internal.concurrency.calls import get_current_call
from syncmatrix._internal.concurrency.cancellation import CancelledError, get_deadline
from syncmatrix._internal.concurrency.threads import wait_for_global_loop_exit
from syncmatrix.client.orchestration import SyncmatrixClient, get_client
from syncmatrix.client.schemas import FlowRun, OrchestrationResult, TaskRun
from syncmatrix.client.schemas.filters import FlowRunFilter
...
from syncmatrix.client.schemas.responses import SetStateStatus
from syncmatrix.client.schemas.sorting import FlowRunSort
from syncmatrix.client.utilities import inject_client
from syncmatrix.context import (
    FlowRunContext,
    SyncmatrixObjectRegistry,
    TagsContext,
    TaskRunContext,
)
from syncmatrix.deployments import load_flow_from_flow_run
from syncmatrix.events import Event, emit_event
...
from syncmatrix.flows import Flow, load_flow_from_entrypoint
from syncmatrix.futures import SyncmatrixFuture, call_repr, resolve_futures_to_states
from syncmatrix.input import keyset_from_paused_state
from syncmatrix.input.run_input import run_input_subclass_from_type
from syncmatrix.logging.configuration import setup_logging
from syncmatrix.logging.handlers import APILogHandler
...
from syncmatrix.utilities.annotations import allow_failure, quote, unmapped
from syncmatrix.utilities.asyncutils import (
    gather,
    is_async_fn,
    sync_compatible,
)
from syncmatrix.utilities.callables import (
    collapse_variadic_parameters,
    explode_variadic_parameter,
    get_parameter_defaults,
    parameters_to_args_kwargs,
)
from syncmatrix.utilities.collections import StopVisiting, isiterable, visit_collection
from syncmatrix.utilities.pydantic import PartialModel
from syncmatrix.utilities.text import truncated_to
...
def enter_flow_run_engine_from_flow_call(
    ...
    wait_for: Optional[Iterable[SyncmatrixFuture]],
    ...
    registry = SyncmatrixObjectRegistry.get()
...
def enter_flow_run_engine_from_subprocess(flow_run_id: UUID) -> State:
    ...
    syncmatrix.plugins.load_syncmatrix_collections()
...
async def create_then_begin_flow_run(
    ...
    ui_url = SYNCMATRIX_UI_URL.value()
    if ui_url:
        logger.info(
...
async def retrieve_flow_then_begin_flow_run(
    ...
    entrypoint = os.environ.get("SYNCMATRIX__FLOW_ENTRYPOINT")
...
async def begin_flow_run(
    ...
    display_state = repr(terminal_state) if SYNCMATRIX_DEBUG_MODE else str(terminal_state)
...
async def orchestrate_flow_run(
    ...
    while state.is_running():
        ...
        # block storage and send the resulting data document to the Syncmatrix API instead.
        # This prevents the pickled return value of flow runs
        # from being sent to the Syncmatrix API and stored in the Syncmatrix database.
        ...
        if state.type != terminal_state.type and SYNCMATRIX_DEBUG_MODE:
            logger.debug(
                (
...
def enter_task_run_engine(
    ...
    mapped: bool,
) -> Union[SyncmatrixFuture, Awaitable[SyncmatrixFuture], TaskRun]:
    ...
    if not flow_run_context:
        if (
            not SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING.value()
            ...
            raise RuntimeError(
                ...
                " `syncmatrix config set SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING=true`"
                " and use `your_task.submit()` instead of `your_task()`."
                " Mapping autonomous tasks is not yet supported."
            )
        from syncmatrix.task_engine import submit_autonomous_task_run_to_engine
...
) -> List[Union[SyncmatrixFuture, Awaitable[SyncmatrixFuture], TaskRun]]:
...
async def collect_task_run_inputs(expr: Any, max_depth: int = -1) -> Set[TaskRunInput]:
    ...
    def add_futures_and_states_to_inputs(obj):
        if isinstance(obj, SyncmatrixFuture):
...
async def begin_task_run(
    ...
    log_prints: bool,
    settings: syncmatrix.context.SettingsContext,
):
    """
    ...
    remote worker:
    --> Remote worker imports Syncmatrix (may not occur)
    ...
    """
    maybe_flow_run_context = syncmatrix.context.FlowRunContext.get()
    ...
    async with AsyncExitStack() as stack:
        ...
        if syncmatrix.context.SettingsContext.get() != settings:
...
async def orchestrate_task_run(
    ...
    Returns:
        The final state of the run
    """
    flow_run_context = syncmatrix.context.FlowRunContext.get()
    if flow_run_context:
    ...
    threshold = SYNCMATRIX_TASK_INTROSPECTION_WARN_THRESHOLD.value()
    if threshold and introspection_time > threshold:
        logger.warning(
            f"Task parameter introspection took {introspection_time} seconds "
            f", exceeding `SYNCMATRIX_TASK_INTROSPECTION_WARN_THRESHOLD` of {threshold}. "
            "Try wrapping large task parameters with "
            "`syncmatrix.utilities.annotations.quote` for increased performance, "
            "e.g. `my_task(quote(param))`. To disable this message set "
            "`SYNCMATRIX_TASK_INTROSPECTION_WARN_THRESHOLD=0`."
        )
    ...
    # Setting on task level overrules the Syncmatrix setting (env var)
    refresh_cache = (
        task.refresh_cache
        if task.refresh_cache is not None
        else SYNCMATRIX_TASKS_REFRESH_CACHE.value()
    )
    ...
    while state.is_running():
        ...
        with task_run_context.copy(
            ...
            try:
                ...
                if SYNCMATRIX_DEBUG_MODE.value():
    ...
    # If debugging, use the more complete `repr` than the usual `str` description
    display_state = repr(state) if SYNCMATRIX_DEBUG_MODE else str(state)
...
async def wait_for_task_runs_and_report_crashes(
    task_run_futures: Iterable[SyncmatrixFuture], client: SyncmatrixClient
) -> Literal[True]:
    crash_exceptions = []
...
@asynccontextmanager
async def report_flow_run_crashes(flow_run: FlowRun, client: SyncmatrixClient, flow: Flow):
    """
...
@asynccontextmanager
async def report_task_run_crashes(task_run: TaskRun, client: SyncmatrixClient):
    """
...
async def resolve_inputs(
    ...
    """
    Resolve any `Quote`, `SyncmatrixFuture`, or `State` types nested in parameters into
    data.
    ...
    def collect_futures_and_states(expr, context):
        ...
        if isinstance(expr, SyncmatrixFuture):
            futures.add(expr)
        if is_state(expr):
            states.add(expr)
...
async def propose_state(
    ...
    """
    Propose a new state for a flow run or task run, invoking Syncmatrix orchestration logic.

    If the proposed state is accepted, the provided `state` will be augmented with
     details and returned.

    If the proposed state is rejected, a new state returned by the Syncmatrix API will be
    returned.

    If the proposed state results in a WAIT instruction from the Syncmatrix API, the
    function will sleep and attempt to propose the state again.

    If the proposed state results in an ABORT instruction from the Syncmatrix API, an
    ...
    Returns:
        a [State model][syncmatrix.client.schemas.objects.State] representation of the
            flow or task run state

    Raises:
        ValueError: if neither task_run_id or flow_run_id is provided
        syncmatrix.exceptions.Abort: if an ABORT instruction is received from
            the Syncmatrix API
    """
    ...
    if response.status == SetStateStatus.ACCEPT:
        ...
        raise syncmatrix.exceptions.Abort(response.details.reason)
...
def link_state_to_result(state: State, result: Any) -> None:
    ...
    We do not set an attribute, e.g. `__syncmatrix_state__`, on the result because:
...
def should_log_prints(flow_or_task: Union[Flow, Task]) -> bool:
    ...
    if flow_or_task.log_prints is None:
        if flow_run_context:
            ...
            return SYNCMATRIX_LOGGING_LOG_PRINTS.value()
...
async def _run_flow_hooks(flow: Flow, flow_run: FlowRun, state: State) -> None:
    ...
    enable_cancellation_and_crashed_hooks = (
        os.environ.get("SYNCMATRIX__ENABLE_CANCELLATION_AND_CRASHED_HOOKS", "true").lower()
        == "true"
    )
...
async def check_api_reachable(client: SyncmatrixClient, fail_message: str):
...
def emit_task_run_state_change_event(
    ...
    return emit_event(
        ...
        event=f"syncmatrix.task-run.{validated_state.name}",
        payload={
            "intended": {
        ...
        resource={
            ...
            "syncmatrix.state-message": truncated_to(
                state_message_truncation_length, validated_state.message
            ),
            "syncmatrix.state-name": validated_state.name or "",
            "syncmatrix.state-timestamp": (
                validated_state.timestamp.isoformat()
                if validated_state and validated_state.timestamp
                else ""
            ),
            "syncmatrix.state-type": str(validated_state.type.value),
        },
        follows=follows,
    )
...
if __name__ == "__main__":
    try:
        flow_run_id = UUID(
            sys.argv[1] if len(sys.argv) > 1 else os.environ.get("SYNCMATRIX__FLOW_RUN_ID")
        )
    except Exception:
        engine_logger.error(
