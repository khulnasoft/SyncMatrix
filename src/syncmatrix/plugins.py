"""
Utilities for loading plugins that extend Syncmatrix's functionality.

Plugins are detected by entry point definitions in package setup files.

Currently supported entrypoints:
    - syncmatrix.collections: Identifies this package as a Syncmatrix collection that
        should be imported when Syncmatrix is imported.
"""
import sys
...
from syncmatrix.utilities.compat import EntryPoint, EntryPoints, entry_points
...
def load_extra_entrypoints() -> Dict[str, Union[Exception, Any]]:
    ...
    if not syncmatrix.settings.SYNCMATRIX_EXTRA_ENTRYPOINTS.value():
        return results

    values = {
        value.strip()
        for value in syncmatrix.settings.SYNCMATRIX_EXTRA_ENTRYPOINTS.value().split(",")
    }

    entrypoints = []
    for value in values:
        try:
            entrypoint = EntryPoint(name=None, value=value, group="syncmatrix-extra")
        except Exception as exc:
            print(
                (
    ...
    for value, result in zip(
        ...
        if isinstance(result, Exception):
            ...
            if syncmatrix.settings.SYNCMATRIX_DEBUG_MODE:
                print(
...
def load_syncmatrix_collections() -> Dict[str, ModuleType]:
    """
    Load all Syncmatrix collections that define an entrypoint in the group
    `syncmatrix.collections`.
    """
    collection_entrypoints: EntryPoints = entry_points(group="syncmatrix.collections")
