"""
Syncmatrix settings management.
...
The `SettingsContext` is set when the `syncmatrix` module is imported. This context is
...
management should be left to Syncmatrix application internals.
...
from syncmatrix.utilities.names import OBFUSCATED_PREFIX, obfuscate
from syncmatrix.utilities.pydantic import add_cloudpickle_reduction
...
REMOVED_EXPERIMENTAL_FLAGS = {
    ...
    "SYNCMATRIX_EXPERIMENTAL_ENABLE_ENHANCED_DEPLOYMENT_PARAMETERS",
}
...
class Setting(Generic[T]):
    ...
    def value(self, bypass_callback: bool = False) -> T:
        ...
        Example:
        ```python
        from syncmatrix.settings import SYNCMATRIX_API_URL
        SYNCMATRIX_API_URL.value()
        ```
        """
    ...
    def value_from(self, settings: "Settings", bypass_callback: bool = False) -> T:
        ...
        Example:
        ```python
        from syncmatrix.settings import get_default_settings
        SYNCMATRIX_API_URL.value_from(get_default_settings())
        ```
        """
    ...
    def name(self):
        ...
        raise ValueError("Setting not found in `syncmatrix.settings` module.")
...
def get_extra_loggers(_: "Settings", value: str) -> List[str]:
    """
    `value_callback` for `SYNCMATRIX_LOGGING_EXTRA_LOGGERS`that parses the CSV string into a
...
def debug_mode_log_level(settings, value):
    """
    `value_callback` for `SYNCMATRIX_LOGGING_LEVEL` that overrides the log level to DEBUG
    when debug mode is enabled.
    """
    if SYNCMATRIX_DEBUG_MODE.value_from(settings):
        return "DEBUG"
    else:
        return value
...
def only_return_value_in_test_mode(settings, value):
    """
    `value_callback` for `SYNCMATRIX_TEST_SETTING` that only allows access during test mode
    """
    if SYNCMATRIX_TEST_MODE.value_from(settings):
        return value
    else:
        return None
...
def default_ui_api_url(settings, value):
    """
    `value_callback` for `SYNCMATRIX_UI_API_URL` that sets the default value to
    ...
    return template_with_settings(
        SYNCMATRIX_SERVER_API_HOST, SYNCMATRIX_SERVER_API_PORT, SYNCMATRIX_API_URL
    )(settings, value)


def status_codes_as_integers_in_range(_, value):
    """
    `value_callback` for `SYNCMATRIX_CLIENT_RETRY_EXTRA_CODES` that ensures status codes
    ...
    if any(not v.isdigit() or int(v) < 100 or int(v) > 599 for v in values):
        raise ValueError(
            "SYNCMATRIX_CLIENT_RETRY_EXTRA_CODES must be a comma separated list of "
...
def max_log_size_smaller_than_batch_size(values):
    ...
    """
    if (
        values["SYNCMATRIX_LOGGING_TO_API_BATCH_SIZE"]
        < values["SYNCMATRIX_LOGGING_TO_API_MAX_LOG_SIZE"]
    ):
        raise ValueError(
            "`SYNCMATRIX_LOGGING_TO_API_MAX_LOG_SIZE` cannot be larger than"
            " `SYNCMATRIX_LOGGING_TO_API_BATCH_SIZE`"
        )
    return values
...
def warn_on_database_password_value_without_usage(values):
    ...
    """
    value = values["SYNCMATRIX_API_DATABASE_PASSWORD"]
    if (
        value
        and not value.startswith(OBFUSCATED_PREFIX)
        and (
            "SYNCMATRIX_API_DATABASE_PASSWORD"
            not in values["SYNCMATRIX_API_DATABASE_CONNECTION_URL"]
        )
    ):
        warnings.warn(
...
def check_for_deprecated_cloud_url(settings, value):
    deprecated_value = SYNCMATRIX_CLOUD_URL.value_from(settings, bypass_callback=True)
    if deprecated_value is not None:
        warnings.warn(
            (
                "`SYNCMATRIX_CLOUD_URL` is set and will be used instead of"
                " `SYNCMATRIX_CLOUD_API_URL` for backwards compatibility."
                " `SYNCMATRIX_CLOUD_URL` is deprecated, set `SYNCMATRIX_CLOUD_API_URL`"
                " instead."
            ),
            DeprecationWarning,
        )
...
def warn_on_misconfigured_api_url(values):
    ...
    """
    api_url = values["SYNCMATRIX_API_URL"]
    if api_url is not None:
        misconfigured_mappings = {
            "app.syncmatrix.cloud": (
                "`SYNCMATRIX_API_URL` points to `app.syncmatrix.cloud`. Did you"
                " mean `api.syncmatrix.cloud`?"
            ),
            "account/": (
                "`SYNCMATRIX_API_URL` uses `/account/` but should use `/accounts/`."
            ),
            "workspace/": (
                "`SYNCMATRIX_API_URL` uses `/workspace/` but should use `/workspaces/`."
            ),
        }
        warnings_list = []
        ...
        if parsed_url.path and not parsed_url.path.startswith("/api"):
            warnings_list.append(
                "`SYNCMATRIX_API_URL` should have `/api` after the base URL."
            )

        if warnings_list:
            example = 'e.g. SYNCMATRIX_API_URL="https://api.syncmatrix.cloud/api/accounts/[ACCOUNT-ID]/workspaces/[WORKSPACE-ID]"'
...
def default_database_connection_url(settings, value):
    templater = template_with_settings(SYNCMATRIX_HOME, SYNCMATRIX_API_DATABASE_PASSWORD)
    ...
    home = SYNCMATRIX_HOME.value_from(settings)

    old_default = home / "orion.db"
    new_default = home / "syncmatrix.db"
...
def default_ui_url(settings, value):
    ...
    ui_url = api_url = SYNCMATRIX_API_URL.value_from(settings)

    if not api_url:
        return None

    cloud_url = SYNCMATRIX_CLOUD_API_URL.value_from(settings)
    cloud_ui_url = SYNCMATRIX_CLOUD_UI_URL.value_from(settings)
...
def default_cloud_ui_url(settings, value):
    ...
    # Otherwise, infer a value from the API URL
    ui_url = api_url = SYNCMATRIX_CLOUD_API_URL.value_from(settings)

    if api_url.startswith("https://api.syncmatrix.cloud"):
        ui_url = ui_url.replace(
            "https://api.syncmatrix.cloud", "https://app.syncmatrix.cloud", 1
        )
...
SYNCMATRIX_HOME = Setting(
    Path,
    default=Path("~") / ".syncmatrix",
    value_callback=expanduser_in_path,
)
"""Syncmatrix's home directory. Defaults to `~/.syncmatrix`. This
directory may be created automatically when required.
"""

SYNCMATRIX_EXTRA_ENTRYPOINTS = Setting(
    str,
    default="",
)
"""
Modules for Syncmatrix to import when Syncmatrix is imported.
...
SYNCMATRIX_CLI_WRAP_LINES = Setting(
    bool,
    default=True,
)
...
SYNCMATRIX_UNIT_TEST_MODE = Setting(
    bool,
    default=False,
)
"""
...
SYNCMATRIX_TEST_SETTING = Setting(
    Any,
    default=None,
...
If accessed when `SYNCMATRIX_TEST_MODE` is not set, `None` is returned.
"""

SYNCMATRIX_API_TLS_INSECURE_SKIP_VERIFY = Setting(
    bool,
    default=False,
)
...
SYNCMATRIX_API_SSL_CERT_FILE = Setting(
    str,
...
SYNCMATRIX_API_URL = Setting(
    str,
    default=None,
)
"""
If provided, the URL of a hosted Syncmatrix API. Defaults to `None`.

When using Syncmatrix Cloud, this will include an account and workspace.
"""

SYNCMATRIX_SILENCE_API_URL_MISCONFIGURATION = Setting(
    bool,
    default=False,
)
"""If `True`, disable the warning when a user accidentally misconfigure its `SYNCMATRIX_API_URL`
Sometimes when a user manually set `SYNCMATRIX_API_URL` to a custom url,reverse-proxy for example,
we would like to silence this warning so we will set it to `FALSE`.
"""

SYNCMATRIX_API_KEY = Setting(
    str,
    default=None,
    is_secret=True,
)
"""API key used to authenticate with a the Syncmatrix API. Defaults to `None`."""

SYNCMATRIX_API_ENABLE_HTTP2 = Setting(bool, default=True)
"""
...
SYNCMATRIX_CLIENT_MAX_RETRIES = Setting(int, default=5)
"""
...
See `SYNCMATRIX_CLIENT_RETRY_EXTRA_CODES` for details on which HTTP status codes are
retried.
"""

SYNCMATRIX_CLIENT_RETRY_JITTER_FACTOR = Setting(float, default=0.2)
"""
...
SYNCMATRIX_CLIENT_RETRY_EXTRA_CODES = Setting(
...
SYNCMATRIX_CLIENT_CSRF_SUPPORT_ENABLED = Setting(bool, default=True)
"""
Determines if CSRF token handling is active in the Syncmatrix client for API
requests.
...
SYNCMATRIX_CLOUD_API_URL = Setting(
    str,
    default="https://api.syncmatrix.cloud/api",
    value_callback=check_for_deprecated_cloud_url,
)
"""API URL for Syncmatrix Cloud. Used for authentication."""


SYNCMATRIX_CLOUD_URL = Setting(
    str,
    default=None,
    deprecated=True,
    deprecated_start_date="Dec 2022",
    deprecated_help="Use `SYNCMATRIX_CLOUD_API_URL` instead.",
)
"""
DEPRECATED: Use `SYNCMATRIX_CLOUD_API_URL` instead.
"""

SYNCMATRIX_UI_URL = Setting(
    Optional[str],
    default=None,
    value_callback=default_ui_url,
)
"""
The URL for the UI. By default, this is inferred from the SYNCMATRIX_API_URL.

When using Syncmatrix Cloud, this will include the account and workspace.
When using an ephemeral server, this will be `None`.
"""


SYNCMATRIX_CLOUD_UI_URL = Setting(
    str,
    default=None,
    value_callback=default_cloud_ui_url,
)
"""
The URL for the Cloud UI. By default, this is inferred from the SYNCMATRIX_CLOUD_API_URL.

Note: SYNCMATRIX_UI_URL will be workspace specific and will be usable in the open source too.
      In contrast, this value is only valid for Cloud and will not include the workspace.
"""

SYNCMATRIX_API_REQUEST_TIMEOUT = Setting(
    float,
    default=60.0,
)
"""The default timeout for requests to the API"""

SYNCMATRIX_EXPERIMENTAL_WARN = Setting(bool, default=True)
"""
If enabled, warn on usage of experimental features.
"""

SYNCMATRIX_PROFILES_PATH = Setting(
    Path,
    default=Path("${SYNCMATRIX_HOME}") / "profiles.toml",
    value_callback=template_with_settings(SYNCMATRIX_HOME),
)
"""The path to a profiles configuration files."""

SYNCMATRIX_RESULTS_DEFAULT_SERIALIZER = Setting(
    str,
    default="pickle",
)
...
SYNCMATRIX_RESULTS_PERSIST_BY_DEFAULT = Setting(
    bool,
    default=False,
)
"""
...
SYNCMATRIX_TASKS_REFRESH_CACHE = Setting(
    bool,
    default=False,
)
"""
...
SYNCMATRIX_TASK_DEFAULT_RETRIES = Setting(int, default=0)
"""
...
SYNCMATRIX_FLOW_DEFAULT_RETRIES = Setting(int, default=0)
"""
...
SYNCMATRIX_FLOW_DEFAULT_RETRY_DELAY_SECONDS = Setting(Union[int, float], default=0)
"""
This value sets the retry delay seconds for all flows.
This value does not overwrite individually set retry delay seconds
"""

SYNCMATRIX_TASK_DEFAULT_RETRY_DELAY_SECONDS = Setting(
...
SYNCMATRIX_TASK_RUN_TAG_CONCURRENCY_SLOT_WAIT_SECONDS = Setting(int, default=30)
"""
The number of seconds to wait before retrying when a task run
cannot secure a concurrency slot from the server.
"""

SYNCMATRIX_LOCAL_STORAGE_PATH = Setting(
    Path,
    default=Path("${SYNCMATRIX_HOME}") / "storage",
    value_callback=template_with_settings(SYNCMATRIX_HOME),
)
"""The path to a block storage directory to store things in."""

SYNCMATRIX_MEMO_STORE_PATH = Setting(
    Path,
    default=Path("${SYNCMATRIX_HOME}") / "memo_store.toml",
    value_callback=template_with_settings(SYNCMATRIX_HOME),
)
"""The path to the memo store file."""

SYNCMATRIX_MEMOIZE_BLOCK_AUTO_REGISTRATION = Setting(
    bool,
    default=True,
)
"""
...
SYNCMATRIX_LOGGING_LEVEL = Setting(
    str,
    default="INFO",
    value_callback=debug_mode_log_level,
)
"""
The default logging level for Syncmatrix loggers. Defaults to
"INFO" during normal operation. Is forced to "DEBUG" during debug mode.
"""


SYNCMATRIX_LOGGING_INTERNAL_LEVEL = Setting(
    str,
    default="ERROR",
    value_callback=debug_mode_log_level,
)
"""
The default logging level for Syncmatrix's internal machinery loggers. Defaults to
"ERROR" during normal operation. Is forced to "DEBUG" during debug mode.
"""

SYNCMATRIX_LOGGING_SERVER_LEVEL = Setting(
    str,
    default="WARNING",
)
"""The default logging level for the Syncmatrix API server."""

SYNCMATRIX_LOGGING_SETTINGS_PATH = Setting(
    Path,
    default=Path("${SYNCMATRIX_HOME}") / "logging.yml",
    value_callback=template_with_settings(SYNCMATRIX_HOME),
)
"""
The path to a custom YAML logging configuration file. If
no file is found, the default `logging.yml` is used.
Defaults to a logging.yml in the Syncmatrix home directory.
"""

SYNCMATRIX_LOGGING_EXTRA_LOGGERS = Setting(
    str,
    default="",
    value_callback=get_extra_loggers,
)
"""
Additional loggers to attach to Syncmatrix logging at runtime.
Values should be comma separated. The handlers attached to the 'syncmatrix' logger
will be added to these loggers. Additionally, if the level is not set, it will
be set to the same level as the 'syncmatrix' logger.
"""

SYNCMATRIX_LOGGING_LOG_PRINTS = Setting(
    bool,
    default=False,
)
"""
If set, `print` statements in flows and tasks will be redirected to the Syncmatrix logger
for the given run. This setting can be overridden by individual tasks and flows.
"""

SYNCMATRIX_LOGGING_TO_API_ENABLED = Setting(
    bool,
    default=True,
)
"""
...
SYNCMATRIX_LOGGING_TO_API_BATCH_INTERVAL = Setting(float, default=2.0)
"""The number of seconds between batched writes of logs to the API."""

SYNCMATRIX_LOGGING_TO_API_BATCH_SIZE = Setting(
    int,
    default=4_000_000,
)
"""The maximum size in bytes for a batch of logs."""

SYNCMATRIX_LOGGING_TO_API_MAX_LOG_SIZE = Setting(
    int,
    default=1_000_000,
)
"""The maximum size in bytes for a single log."""

SYNCMATRIX_LOGGING_TO_API_WHEN_MISSING_FLOW = Setting(
...
SYNCMATRIX_SQLALCHEMY_POOL_SIZE = Setting(
    int,
    default=None,
)
"""
Controls connection pool size when using a PostgreSQL database with the Syncmatrix API. If not set, the default SQLAlchemy pool size will be used.
"""

SYNCMATRIX_SQLALCHEMY_MAX_OVERFLOW = Setting(
    int,
    default=None,
)
"""
Controls maximum overflow of the connection pool when using a PostgreSQL database with the Syncmatrix API. If not set, the default SQLAlchemy maximum overflow value will be used.
"""

SYNCMATRIX_LOGGING_COLORS = Setting(
    bool,
    default=True,
)
"""Whether to style console logs with color."""

SYNCMATRIX_LOGGING_MARKUP = Setting(
    bool,
    default=False,
)
"""
...
SYNCMATRIX_TASK_INTROSPECTION_WARN_THRESHOLD = Setting(
    float,
    default=10.0,
)
"""
...
and each element needs to be inspected. See `syncmatrix.utilities.annotations.quote`
for more details.
Defaults to `10.0`.
Set to `0` to disable logging the warning.
"""

SYNCMATRIX_AGENT_QUERY_INTERVAL = Setting(
    float,
    default=15,
)
"""
...
SYNCMATRIX_AGENT_PREFETCH_SECONDS = Setting(
    int,
    default=15,
)
"""
...
SYNCMATRIX_ASYNC_FETCH_STATE_RESULT = Setting(bool, default=False)
"""
Determines whether `State.result()` fetches results automatically or not.
In Syncmatrix 2.6.0, the `State.result()` method was updated to be async
...
SYNCMATRIX_API_BLOCKS_REGISTER_ON_START = Setting(
    bool,
    default=True,
)
"""
...
SYNCMATRIX_API_DATABASE_PASSWORD = Setting(
    str,
    default=None,
    is_secret=True,
)
"""
Password to template into the `SYNCMATRIX_API_DATABASE_CONNECTION_URL`.
...
SYNCMATRIX_API_DATABASE_CONNECTION_URL = Setting(
    str,
    default=None,
...
format. Syncmatrix currently supports SQLite and Postgres. Note that all
Syncmatrix database engines must use an async driver - for SQLite, use
...
Defaults to a sqlite database stored in the Syncmatrix home directory.

If you need to provide password via a different environment variable, you use
the `SYNCMATRIX_API_DATABASE_PASSWORD` setting. For example:

```
SYNCMATRIX_API_DATABASE_PASSWORD='mypassword'
SYNCMATRIX_API_DATABASE_CONNECTION_URL='postgresql+asyncpg://postgres:${SYNCMATRIX_API_DATABASE_PASSWORD}@localhost/syncmatrix'
```
"""

SYNCMATRIX_API_DATABASE_ECHO = Setting(
    bool,
    default=False,
)
"""If `True`, SQLAlchemy will log all SQL issued to the database. Defaults to `False`."""

SYNCMATRIX_API_DATABASE_MIGRATE_ON_START = Setting(
    bool,
    default=True,
)
"""If `True`, the database will be upgraded on application creation. If `False`, the database will need to be upgraded manually."""

SYNCMATRIX_API_DATABASE_TIMEOUT = Setting(
    Optional[float],
    default=10.0,
)
"""
...
SYNCMATRIX_API_DATABASE_CONNECTION_TIMEOUT = Setting(
    Optional[float],
    default=5,
)
...
SYNCMATRIX_API_SERVICES_SCHEDULER_LOOP_SECONDS = Setting(
    float,
    default=60,
)
...
SYNCMATRIX_API_SERVICES_SCHEDULER_DEPLOYMENT_BATCH_SIZE = Setting(
    int,
    default=100,
)
...
SYNCMATRIX_API_SERVICES_SCHEDULER_MAX_RUNS = Setting(
    int,
    default=100,
)
...
SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_RUNS = Setting(
    int,
    default=3,
)
...
SYNCMATRIX_API_SERVICES_SCHEDULER_MAX_SCHEDULED_TIME = Setting(
    timedelta,
...
SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_SCHEDULED_TIME = Setting(
    timedelta,
...
SYNCMATRIX_API_SERVICES_SCHEDULER_INSERT_BATCH_SIZE = Setting(
    int,
    default=500,
)
...
SYNCMATRIX_API_SERVICES_LATE_RUNS_LOOP_SECONDS = Setting(
    float,
    default=5,
)
...
SYNCMATRIX_API_SERVICES_LATE_RUNS_AFTER_SECONDS = Setting(
    timedelta,
...
SYNCMATRIX_API_SERVICES_PAUSE_EXPIRATIONS_LOOP_SECONDS = Setting(
    float,
    default=5,
)
...
SYNCMATRIX_API_SERVICES_CANCELLATION_CLEANUP_LOOP_SECONDS = Setting(
    float,
    default=20,
)
...
SYNCMATRIX_API_DEFAULT_LIMIT = Setting(
    int,
    default=200,
)
...
SYNCMATRIX_SERVER_API_HOST = Setting(
    str,
    default="127.0.0.1",
)
"""The API's host address (defaults to `127.0.0.1`)."""

SYNCMATRIX_SERVER_API_PORT = Setting(
    int,
    default=4200,
)
"""The API's port address (defaults to `4200`)."""

SYNCMATRIX_SERVER_API_KEEPALIVE_TIMEOUT = Setting(
    int,
    default=5,
)
"""
...
Note this setting only applies when calling `syncmatrix server start`; if hosting the
API with another tool you will need to configure this there instead.
"""

SYNCMATRIX_SERVER_CSRF_PROTECTION_ENABLED = Setting(bool, default=False)
"""
Controls the activation of CSRF protection for the Syncmatrix server API.
...
CSRF token management. See SYNCMATRIX_CLIENT_CSRF_SUPPORT_ENABLED for more.
"""

SYNCMATRIX_SERVER_CSRF_TOKEN_EXPIRATION = Setting(timedelta, default=timedelta(hours=1))
"""
...
SYNCMATRIX_UI_ENABLED = Setting(
    bool,
    default=True,
)
"""Whether or not to serve the Syncmatrix UI."""

SYNCMATRIX_UI_API_URL = Setting(
    str,
    default=None,
    value_callback=default_ui_api_url,
)
"""The connection url for communication from the UI to the API.
Defaults to `SYNCMATRIX_API_URL` if set. Otherwise, the default URL is generated from
`SYNCMATRIX_SERVER_API_HOST` and `SYNCMATRIX_SERVER_API_PORT`. If providing a custom value,
the aforementioned settings may be templated into the given string.
"""

SYNCMATRIX_SERVER_ANALYTICS_ENABLED = Setting(
    bool,
    default=True,
)
"""
When enabled, Syncmatrix sends anonymous data (e.g. count of flow runs, package version)
on server startup to help us improve our product.
"""

SYNCMATRIX_API_SERVICES_SCHEDULER_ENABLED = Setting(
    bool,
    default=True,
)
...
SYNCMATRIX_API_SERVICES_LATE_RUNS_ENABLED = Setting(
    bool,
    default=True,
)
...
SYNCMATRIX_API_SERVICES_FLOW_RUN_NOTIFICATIONS_ENABLED = Setting(
    bool,
    default=True,
)
...
SYNCMATRIX_API_SERVICES_PAUSE_EXPIRATIONS_ENABLED = Setting(
    bool,
    default=True,
)
...
SYNCMATRIX_API_TASK_CACHE_KEY_MAX_LENGTH = Setting(int, default=2000)
"""
...
SYNCMATRIX_API_SERVICES_CANCELLATION_CLEANUP_ENABLED = Setting(
    bool,
    default=True,
)
...
SYNCMATRIX_API_MAX_FLOW_RUN_GRAPH_NODES = Setting(int, default=10000)
"""
The maximum size of a flow run graph on the v2 API
"""

SYNCMATRIX_API_MAX_FLOW_RUN_GRAPH_ARTIFACTS = Setting(int, default=10000)
"""
The maximum number of artifacts to show on a flow run graph on the v2 API
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_ARTIFACTS_ON_FLOW_RUN_GRAPH = Setting(bool, default=True)
"""
Whether or not to enable artifacts on the flow run graph.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_STATES_ON_FLOW_RUN_GRAPH = Setting(bool, default=True)
"""
Whether or not to enable flow run states on the flow run graph.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_EVENTS_CLIENT = Setting(bool, default=True)
"""
Whether or not to enable experimental Syncmatrix work pools.
"""

SYNCMATRIX_EXPERIMENTAL_WARN_EVENTS_CLIENT = Setting(bool, default=False)
"""
Whether or not to warn when experimental Syncmatrix work pools are used.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_WORK_POOLS = Setting(bool, default=True)
"""
Whether or not to enable experimental Syncmatrix work pools.
"""

SYNCMATRIX_EXPERIMENTAL_WARN_WORK_POOLS = Setting(bool, default=False)
"""
Whether or not to warn when experimental Syncmatrix work pools are used.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_WORKERS = Setting(bool, default=True)
"""
Whether or not to enable experimental Syncmatrix workers.
"""

SYNCMATRIX_EXPERIMENTAL_WARN_WORKERS = Setting(bool, default=False)
"""
Whether or not to warn when experimental Syncmatrix workers are used.
"""

SYNCMATRIX_EXPERIMENTAL_WARN_VISUALIZE = Setting(bool, default=False)
"""
Whether or not to warn when experimental Syncmatrix visualize is used.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_ENHANCED_CANCELLATION = Setting(bool, default=True)
"""
Whether or not to enable experimental enhanced flow run cancellation.
"""

SYNCMATRIX_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION = Setting(bool, default=False)
"""
Whether or not to warn when experimental enhanced flow run cancellation is used.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_DEPLOYMENT_STATUS = Setting(bool, default=True)
"""
Whether or not to enable deployment status in the UI
"""

SYNCMATRIX_EXPERIMENTAL_WARN_DEPLOYMENT_STATUS = Setting(bool, default=False)
"""
Whether or not to warn when deployment status is used.
"""

SYNCMATRIX_EXPERIMENTAL_FLOW_RUN_INPUT = Setting(bool, default=False)
"""
Whether or not to enable flow run input.
"""

SYNCMATRIX_EXPERIMENTAL_WARN_FLOW_RUN_INPUT = Setting(bool, default=True)
"""
Whether or not to enable flow run input.
"""

SYNCMATRIX_RUNNER_PROCESS_LIMIT = Setting(int, default=5)
"""
Maximum number of processes a runner will execute in parallel.
"""

SYNCMATRIX_RUNNER_POLL_FREQUENCY = Setting(int, default=10)
"""
Number of seconds a runner should wait between queries for scheduled work.
"""

SYNCMATRIX_RUNNER_SERVER_MISSED_POLLS_TOLERANCE = Setting(int, default=2)
"""
Number of missed polls before a runner is considered unhealthy by its webserver.
"""

SYNCMATRIX_RUNNER_SERVER_HOST = Setting(str, default="localhost")
"""
The host address the runner's webserver should bind to.
"""

SYNCMATRIX_RUNNER_SERVER_PORT = Setting(int, default=8080)
"""
The port the runner's webserver should bind to.
"""

SYNCMATRIX_RUNNER_SERVER_LOG_LEVEL = Setting(str, default="error")
"""
The log level of the runner's webserver.
"""

SYNCMATRIX_RUNNER_SERVER_ENABLE = Setting(bool, default=False)
"""
Whether or not to enable the runner's webserver.
"""

SYNCMATRIX_WORKER_HEARTBEAT_SECONDS = Setting(float, default=30)
"""
Number of seconds a worker should wait between sending a heartbeat.
"""

SYNCMATRIX_WORKER_QUERY_SECONDS = Setting(float, default=10)
"""
Number of seconds a worker should wait between queries for scheduled flow runs.
"""

SYNCMATRIX_WORKER_PREFETCH_SECONDS = Setting(float, default=10)
"""
...
SYNCMATRIX_WORKER_WEBSERVER_HOST = Setting(
    str,
    default="0.0.0.0",
)
"""
...
SYNCMATRIX_WORKER_WEBSERVER_PORT = Setting(
    int,
    default=8080,
)
"""
...
SYNCMATRIX_TASK_SCHEDULING_DEFAULT_STORAGE_BLOCK = Setting(
    str,
    default="local-file-system/syncmatrix-task-scheduling",
)
"""The `block-type/block-document` slug of a block to use as the default storage
for autonomous tasks."""

SYNCMATRIX_TASK_SCHEDULING_DELETE_FAILED_SUBMISSIONS = Setting(
    bool,
    default=True,
)
"""
...
SYNCMATRIX_TASK_SCHEDULING_MAX_SCHEDULED_QUEUE_SIZE = Setting(
    int,
    default=1000,
)
"""
...
SYNCMATRIX_TASK_SCHEDULING_MAX_RETRY_QUEUE_SIZE = Setting(
    int,
    default=100,
)
"""
...
SYNCMATRIX_TASK_SCHEDULING_PENDING_TASK_TIMEOUT = Setting(
    timedelta,
...
SYNCMATRIX_EXPERIMENTAL_ENABLE_FLOW_RUN_INFRA_OVERRIDES = Setting(bool, default=False)
"""
Whether or not to enable infrastructure overrides made on flow runs.
"""

SYNCMATRIX_EXPERIMENTAL_WARN_FLOW_RUN_INFRA_OVERRIDES = Setting(bool, default=True)
"""
Whether or not to warn infrastructure when experimental flow runs overrides are used.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_EXTRA_RUNNER_ENDPOINTS = Setting(bool, default=False)
"""
Whether or not to enable experimental worker webserver endpoints.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_ARTIFACTS = Setting(bool, default=True)
"""
Whether or not to enable experimental Syncmatrix artifacts.
"""

SYNCMATRIX_EXPERIMENTAL_WARN_ARTIFACTS = Setting(bool, default=False)
"""
Whether or not to warn when experimental Syncmatrix artifacts are used.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_WORKSPACE_DASHBOARD = Setting(bool, default=True)
"""
Whether or not to enable the experimental workspace dashboard.
"""

SYNCMATRIX_EXPERIMENTAL_WARN_WORKSPACE_DASHBOARD = Setting(bool, default=False)
"""
Whether or not to warn when the experimental workspace dashboard is enabled.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING = Setting(bool, default=False)
"""
Whether or not to enable experimental task scheduling.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_WORK_QUEUE_STATUS = Setting(bool, default=True)
"""
Whether or not to enable experimental work queue status in-place of work queue health.
"""

SYNCMATRIX_EXPERIMENTAL_ENABLE_PYDANTIC_V2_INTERNALS = Setting(bool, default=False)
"""
...
SYNCMATRIX_DEFAULT_RESULT_STORAGE_BLOCK = Setting(
    str,
    default=None,
)
"""The `block-type/block-document` slug of a block to use as the default result storage."""

SYNCMATRIX_DEFAULT_WORK_POOL_NAME = Setting(str, default=None)
"""
The default work pool to deploy to.
"""

SYNCMATRIX_DEFAULT_DOCKER_BUILD_NAMESPACE = Setting(
    str,
    default=None,
)
"""
...
SYNCMATRIX_UI_SERVE_BASE = Setting(
    str,
    default="/",
)
"""
The base URL path to serve the Syncmatrix UI from.

Defaults to the root path.
"""

SYNCMATRIX_UI_STATIC_DIRECTORY = Setting(
    str,
    default=None,
)
"""
...
class Settings(SettingsFieldsMixin):
    ...
    @validator(SYNCMATRIX_LOGGING_LEVEL.name, SYNCMATRIX_LOGGING_SERVER_LEVEL.name)
    ...
    def post_root_validators(cls, values):
        ...
        if not values["SYNCMATRIX_SILENCE_API_URL_MISCONFIGURATION"]:
...
def get_current_settings() -> Settings:
    ...
    """
    from syncmatrix.context import SettingsContext
...
def temporary_settings(
    ...
    Examples:
        >>> from syncmatrix.settings import SYNCMATRIX_API_URL
        >>>
        >>> with temporary_settings(updates={SYNCMATRIX_API_URL: "foo"}):
        >>>    assert SYNCMATRIX_API_URL.value() == "foo"
        >>>
        >>>    with temporary_settings(set_defaults={SYNCMATRIX_API_URL: "bar"}):
        >>>         assert SYNCMATRIX_API_URL.value() == "foo"
        >>>
        >>>    with temporary_settings(restore_defaults={SYNCMATRIX_API_URL}):
        >>>         assert SYNCMATRIX_API_URL.value() is None
        >>>
        >>>         with temporary_settings(set_defaults={SYNCMATRIX_API_URL: "bar"})
        >>>             assert SYNCMATRIX_API_URL.value() == "bar"
        >>> assert SYNCMATRIX_API_URL.value() is None
    """
    import syncmatrix.context

    context = syncmatrix.context.get_settings_context()
    ...
    with syncmatrix.context.SettingsContext(
...
def load_profiles() -> ProfilesCollection:
    ...
    user_profiles_path = SYNCMATRIX_PROFILES_PATH.value()
...
def save_profiles(profiles: ProfilesCollection) -> None:
    ...
    """
    profiles_path = SYNCMATRIX_PROFILES_PATH.value()
...
def update_current_profile(settings: Dict[Union[str, Setting], Any]) -> Profile:
    ...
    current_profile = syncmatrix.context.get_settings_context().profile
