...
if TYPE_CHECKING:
    from syncmatrix.client.schemas import FlowRun as ClientFlowRun
    from syncmatrix.client.schemas.objects import FlowRun, TaskRun
    from syncmatrix.context import RunContext
    from syncmatrix.flows import Flow
    from syncmatrix.tasks import Task


class SyncmatrixLogAdapter(logging.LoggerAdapter):
    """
...
def get_logger(name: str = None) -> logging.Logger:
    """
    Get a `syncmatrix` logger. These loggers are intended for internal use within the
    `syncmatrix` package.
    ...
    """
    parent_logger = logging.getLogger("syncmatrix")

    if name:
        # Append the name if given but allow explicit full names e.g. "syncmatrix.test"
        # should not become "syncmatrix.syncmatrix.test"
...
def get_run_logger(
    ...
    """
    Get a Syncmatrix logger for the current task run or flow run.

    The logger will be named either `syncmatrix.task_runs` or `syncmatrix.flow_runs`.
    ...
    task_run_context = syncmatrix.context.TaskRunContext.get()
    flow_run_context = syncmatrix.context.FlowRunContext.get()

    # Apply the context override
    if context:
        if isinstance(context, syncmatrix.context.FlowRunContext):
            flow_run_context = context
        elif isinstance(context, syncmatrix.context.TaskRunContext):
    ...
    if task_run_context:
        ...
        get_logger("syncmatrix.flow_run").disabled
        and get_logger("syncmatrix.task_run").disabled
    ):
...
def flow_run_logger(
    ...
    **kwargs: str,
    ...
    """
    return SyncmatrixLogAdapter(
        get_logger("syncmatrix.flow_runs"),
        extra={
            **{
...
def task_run_logger(
    ...
    **kwargs: str,
    ...
    return SyncmatrixLogAdapter(
        get_logger("syncmatrix.task_runs"),
        extra={
            **{
...
def disable_run_logger():
    """
    Gets both `syncmatrix.flow_run` and `syncmatrix.task_run` and disables them
    within the context manager. Upon exiting the context manager, both loggers
    are returned to its original state.
    """
    with disable_logger("syncmatrix.flow_run"), disable_logger("syncmatrix.task_run"):
        yield


def print_as_log(*args, **kwargs):
    """
    A patch for `print` to send printed messages to the Syncmatrix run logger.
    ...
    not be forwarded to the Syncmatrix logger either.
    """
