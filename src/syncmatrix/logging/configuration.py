...
# This path will be used if `SYNCMATRIX_LOGGING_SETTINGS_PATH` is null
...
def load_logging_config(path: Path) -> dict:
    ...
    for key_tup, val in flat_config.items():
        env_val = os.environ.get(
            ...
            to_envvar("SYNCMATRIX_LOGGING_" + "_".join(key_tup)).upper()
        )
        if env_val:
            val = env_val
...
def setup_logging(incremental: Optional[bool] = None) -> dict:
    ...
    config = load_logging_config(
        (
            SYNCMATRIX_LOGGING_SETTINGS_PATH.value()
            if SYNCMATRIX_LOGGING_SETTINGS_PATH.value().exists()
    ...
    # Copy configuration of the 'syncmatrix.extra' logger to the extra loggers
    extra_config = logging.getLogger("syncmatrix.extra")

    for logger_name in SYNCMATRIX_LOGGING_EXTRA_LOGGERS.value():
