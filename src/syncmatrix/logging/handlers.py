...
from syncmatrix._internal.concurrency.event_loop import get_running_loop
from syncmatrix._internal.concurrency.services import BatchedQueueService
from syncmatrix._internal.concurrency.threads import in_global_loop
from syncmatrix.client.orchestration import get_client
from syncmatrix.client.schemas.actions import LogCreate
...
class APILogWorker(BatchedQueueService[Dict[str, Any]]):
    ...
    def _max_batch_size(self):
        return max(
            SYNCMATRIX_LOGGING_TO_API_BATCH_SIZE.value()
            - SYNCMATRIX_LOGGING_TO_API_MAX_LOG_SIZE.value(),
            SYNCMATRIX_LOGGING_TO_API_MAX_LOG_SIZE.value(),
        )

    @property
    def _min_interval(self):
        return SYNCMATRIX_LOGGING_TO_API_BATCH_INTERVAL.value()
    ...
    async def _handle_batch(self, items: List):
        try:
            ...
            if logging.raiseExceptions and sys.stderr:
                ...
                if SYNCMATRIX_LOGGING_INTERNAL_LEVEL.value() == "DEBUG":
...
class APILogHandler(logging.Handler):
    """
    A logging handler that sends logs to the Syncmatrix API.
    ...
    def emit(self, record: logging.LogRecord):
        ...
        """
        try:
            profile = syncmatrix.context.get_settings_context()

            if not SYNCMATRIX_LOGGING_TO_API_ENABLED.value_from(profile.settings):
    ...
    def handleError(self, record: logging.LogRecord) -> None:
        ...
        if isinstance(exc, MissingContextError):
            log_handling_when_missing_flow = (
                SYNCMATRIX_LOGGING_TO_API_WHEN_MISSING_FLOW.value()
            )
    ...
    def prepare(self, record: logging.LogRecord) -> Dict[str, Any]:
        ...
        if not flow_run_id:
            try:
                context = syncmatrix.context.get_run_context()
        ...
        if log_size > SYNCMATRIX_LOGGING_TO_API_MAX_LOG_SIZE.value():
            raise ValueError(
                f"Log of size {log_size} is greater than the max size of "
                f"{SYNCMATRIX_LOGGING_TO_API_MAX_LOG_SIZE.value()}"
            )
...
class SyncmatrixConsoleHandler(logging.StreamHandler):
    def __init__(
        self,
        stream=None,
        highlighter: Highlighter = SyncmatrixConsoleHighlighter,
        styles: Dict[str, str] = None,
        level: Union[int, str] = logging.NOTSET,
    ):
        """
        The default console handler for Syncmatrix, which highlights log levels,
        web and file URLs, flow and task (run) names, and state types in the
        local console (terminal).

        Highlighting can be toggled on/off with the SYNCMATRIX_LOGGING_COLORS setting.
        For finer control, use logging.yml to add or remove styles, and/or
        adjust colors.
        """
        super().__init__(stream=stream)

        styled_console = SYNCMATRIX_LOGGING_COLORS.value()
        markup_console = SYNCMATRIX_LOGGING_MARKUP.value()
        if styled_console:
