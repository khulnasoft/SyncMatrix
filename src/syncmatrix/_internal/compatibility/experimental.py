...
disabled per feature group with the setting `SYNCMATRIX_EXPERIMENTAL_WARN_<GROUP>`.
Warnings may also be disabled globally with the setting `SYNCMATRIX_EXPERIMENTAL_WARN`.

Some experimental features require opt-in to enable any usage. These require the setting
`SYNCMATRIX_EXPERIMENTAL_ENABLE_<GROUP>` to be set or an error will be thrown on use.
"""
import functools
import warnings
...
from syncmatrix.settings import SYNCMATRIX_EXPERIMENTAL_WARN, SETTING_VARIABLES, Setting
from syncmatrix.utilities.callables import get_call_parameters
...
EXPERIMENTAL_WARNING = (
    ...
    " SYNCMATRIX_EXPERIMENTAL_WARN_{group}."
)

EXPERIMENTAL_ERROR = (
    "{feature} is experimental and requires opt-in for usage. {help}"
    "To use this feature, enable SYNCMATRIX_EXPERIMENTAL_ENABLE_{group}."
)
...
class ExperimentalFeature(ExperimentalWarning):
    ...
    These can be globally disabled by the SYNCMATRIX_EXPIRIMENTAL_WARN setting.
    """
...
def _opt_in_setting_for_group(group: str) -> Setting[bool]:
    group_opt_in_setting_name = f"SYNCMATRIX_EXPERIMENTAL_ENABLE_{group.upper()}"
...
def _warn_setting_for_group(group: str) -> Setting[bool]:
    group_warn_setting_name = f"SYNCMATRIX_EXPERIMENTAL_WARN_{group.upper()}"
    ...
    *,
    ...
    def decorator(fn: T):
        ...
        def wrapper(*args, **kwargs):
            ...
            if SYNCMATRIX_EXPERIMENTAL_WARN and group_warn:
                warnings.warn(
                    warn_message,
                    ExperimentalFeature,
...
def enabled_experiments() -> Set[str]:
    ...
    """
    return {
        name[len("SYNCMATRIX_EXPERIMENTAL_ENABLE_") :].lower()
        for name, setting in SETTING_VARIABLES.items()
        if name.startswith("SYNCMATRIX_EXPERIMENTAL_ENABLE_") and setting.value()
    }
