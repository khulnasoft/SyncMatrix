...
from syncmatrix.settings import SYNCMATRIX_EXPERIMENTAL_ENABLE_PYDANTIC_V2_INTERNALS

IncEx: typing_extensions.TypeAlias = (
    "Union[Set[int], Set[str], Dict[int, Any], Dict[str, Any], None]"
)

logger = get_logger("syncmatrix._internal.pydantic")
...
def is_pydantic_v2_compatible(
    ...
    is explicitly enabled through a global setting (`SYNCMATRIX_EXPERIMENTAL_ENABLE_PYDANTIC_V2_INTERNALS`).
    ...
    should_dump_as_v2_model = (
        HAS_PYDANTIC_V2 and SYNCMATRIX_EXPERIMENTAL_ENABLE_PYDANTIC_V2_INTERNALS
    )
    ...
    if should_dump_as_v2_model:
        ...
        logger.debug(
            "Pydantic v2 compatibility layer is disabled. To enable, set `SYNCMATRIX_EXPERIMENTAL_ENABLE_PYDANTIC_V2_INTERNALS` to `True`."
        )
