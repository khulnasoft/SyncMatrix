...
[Task Runners](/concepts/task-runners/) in Syncmatrix are responsible for managing the execution of Syncmatrix task runs. Generally speaking, users are not expected to interact with task runners outside of configuring and initializing them for a flow.
...
from syncmatrix._internal.concurrency.primitives import Event
from syncmatrix.client.schemas.objects import State
from syncmatrix.logging import get_logger
from syncmatrix.states import exception_to_crashed_state
...
class BaseTaskRunner(metaclass=abc.ABCMeta):
    ...
    async def submit(
        ...
        """
        Submit a call for execution and return a `SyncmatrixFuture` that can be used to
        get the call result.
    ...
    async def wait(self, key: UUID, timeout: float = None) -> Optional[State]:
        """
        Given a `SyncmatrixFuture`, wait for its return state up to `timeout` seconds.
...
class ConcurrentTaskRunner(BaseTaskRunner):
    ...
    Example:
        ...
        >>> from syncmatrix.task_runners import ConcurrentTaskRunner
