...
from syncmatrix.server.task_queue import MultiQueue, TaskQueue
from syncmatrix.server.utilities import subscriptions
...
router = SyncmatrixRouter(prefix="/task_runs", tags=["Task Runs"])
...
async def scheduled_task_subscription(websocket: WebSocket):
    websocket = await subscriptions.accept_syncmatrix_socket(websocket)
    if not websocket:
        return
