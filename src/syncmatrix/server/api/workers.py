...
async def create_work_pool(
    ...
    if work_pool.name.lower().startswith("syncmatrix"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Work pools starting with 'Syncmatrix' are reserved for internal use.",
        )
...
async def update_work_pool(
    ...
    if work_pool_name.lower().startswith("syncmatrix") and (
        set(update_values).difference({"is_paused", "concurrency_limit"})
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=(
                "Work pools starting with 'Syncmatrix' are reserved for internal use "
...
async def delete_work_pool(
    ...
    if work_pool_name.lower().startswith("syncmatrix"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=(
                "Work pools starting with 'Syncmatrix' are reserved for internal use and"
                " can not be deleted."
            ),
        )
