"""
Defines the Syncmatrix REST API FastAPI app.
"""
...
from syncmatrix._vendor.fastapi import APIRouter, Depends, FastAPI, Request, status
from syncmatrix._vendor.fastapi.encoders import jsonable_encoder
from syncmatrix._vendor.fastapi.exceptions import RequestValidationError
from syncmatrix._vendor.fastapi.middleware.cors import CORSMiddleware
from syncmatrix._vendor.fastapi.middleware.gzip import GZipMiddleware
from syncmatrix._vendor.fastapi.openapi.utils import get_openapi
from syncmatrix._vendor.fastapi.responses import JSONResponse
from syncmatrix._vendor.fastapi.staticfiles import StaticFiles
from syncmatrix._vendor.starlette.exceptions import HTTPException

import syncmatrix
import syncmatrix.server.api as api
import syncmatrix.server.services as services
import syncmatrix.settings
from syncmatrix._internal.compatibility.experimental import enabled_experiments
from syncmatrix.client.constants import SERVER_API_VERSION
from syncmatrix.logging import get_logger
from syncmatrix.server.api.dependencies import EnforceMinimumAPIVersion
from syncmatrix.server.exceptions import ObjectNotFoundError
from syncmatrix.server.utilities.database import get_dialect
from syncmatrix.server.utilities.server import method_paths_from_routes
from syncmatrix.settings import (
    SYNCMATRIX_API_DATABASE_CONNECTION_URL,
    SYNCMATRIX_DEBUG_MODE,
    SYNCMATRIX_MEMO_STORE_PATH,
    SYNCMATRIX_MEMOIZE_BLOCK_AUTO_REGISTRATION,
    SYNCMATRIX_UI_SERVE_BASE,
)
from syncmatrix.utilities.hashing import hash_objects

TITLE = "Syncmatrix Server"
API_TITLE = "Syncmatrix Syncmatrix REST API"
UI_TITLE = "Syncmatrix Syncmatrix REST API UI"
API_VERSION = syncmatrix.__version__
...
async def syncmatrix_object_not_found_exception_handler(
...
def create_api_app(
    ...
    """
    Create a FastAPI app that includes the Syncmatrix REST API

    Args:
        router_prefix: a prefix to apply to all included routers
        dependencies: a list of global dependencies to add to each Syncmatrix REST API router
    ...
    Returns:
        a FastAPI app that serves the Syncmatrix REST API
    """
...
def create_ui_app(ephemeral: bool) -> FastAPI:
    ...
    base_url = syncmatrix.settings.SYNCMATRIX_UI_SERVE_BASE.value()
    stripped_base_url = base_url.rstrip("/")
    static_dir = (
        syncmatrix.settings.SYNCMATRIX_UI_STATIC_DIRECTORY.value()
        or syncmatrix.__ui_static_subpath__
    )
    ...
    def ui_settings():
        return {
            "api_url": syncmatrix.settings.SYNCMATRIX_UI_API_URL.value(),
    ...
    def create_ui_static_subpath():
        ...
        copy_directory(syncmatrix.__ui_static_path__, static_dir)
        replace_placeholder_string_in_files(
            static_dir,
            "/SYNCMATRIX_UI_SERVE_BASE_REPLACE_PLACEHOLDER",
            stripped_base_url,
        )
    ...
    if (
        os.path.exists(syncmatrix.__ui_static_path__)
        and syncmatrix.settings.SYNCMATRIX_UI_ENABLED.value()
        and not ephemeral
    ):
        ...
        ui_app.mount(
            SYNCMATRIX_UI_SERVE_BASE.value(),
...
APP_CACHE: Dict[Tuple[syncmatrix.settings.Settings, bool], FastAPI] = {}
...
def _memoize_block_auto_registration(fn: Callable[[], Awaitable[None]]):
    ...
    from syncmatrix.server.models.block_registration import _load_collection_blocks_data
    from syncmatrix.utilities.dispatch import get_registry_for_type

    @wraps(fn)
    async def wrapper(*args, **kwargs):
        if not SYNCMATRIX_MEMOIZE_BLOCK_AUTO_REGISTRATION.value():
        ...
        current_blocks_loading_hash = hash_objects(
            ...
            SYNCMATRIX_API_DATABASE_CONNECTION_URL.value(),
            hash_algo=sha256,
        )

        memo_store_path = SYNCMATRIX_MEMO_STORE_PATH.value()
        try:
            ...
            logger.warn(
                ""
                f"Unable to read memo_store.toml from {SYNCMATRIX_MEMO_STORE_PATH} during "
        ...
        if current_blocks_loading_hash is not None:
            try:
                ...
                logger.warn(
                    ...
                    f" {SYNCMATRIX_MEMO_STORE_PATH} after block auto-registration:"
...
def create_app(
    settings: syncmatrix.settings.Settings = None,
    ephemeral: bool = False,
    ignore_cache: bool = False,
) -> FastAPI:
    """
    Create an FastAPI app that includes the Syncmatrix REST API and UI
    ...
    """
    settings = settings or syncmatrix.settings.get_current_settings()
    ...
    async def run_migrations():
        ...
        if syncmatrix.settings.SYNCMATRIX_API_DATABASE_MIGRATE_ON_START:
    ...
    async def add_block_types():
        ...
        if not syncmatrix.settings.SYNCMATRIX_API_BLOCKS_REGISTER_ON_START:
            return

        from syncmatrix.server.database.dependencies import provide_database_interface
        from syncmatrix.server.models.block_registration import run_block_auto_registration
    ...
    async def start_services():
        """Start additional services when the Syncmatrix REST API starts up."""
        ...
        if syncmatrix.settings.SYNCMATRIX_API_SERVICES_SCHEDULER_ENABLED.value():
        ...
        if syncmatrix.settings.SYNCMATRIX_API_SERVICES_LATE_RUNS_ENABLED.value():
            service_instances.append(services.late_runs.MarkLateRuns())

        if syncmatrix.settings.SYNCMATRIX_API_SERVICES_PAUSE_EXPIRATIONS_ENABLED.value():
            service_instances.append(services.pause_expirations.FailExpiredPauses())

        if syncmatrix.settings.SYNCMATRIX_API_SERVICES_CANCELLATION_CLEANUP_ENABLED.value():
            service_instances.append(
                services.cancellation_cleanup.CancellationCleanup()
            )

        if syncmatrix.settings.SYNCMATRIX_SERVER_ANALYTICS_ENABLED.value():
            service_instances.append(services.telemetry.Telemetry())

        if syncmatrix.settings.SYNCMATRIX_API_SERVICES_FLOW_RUN_NOTIFICATIONS_ENABLED.value():
            service_instances.append(
                services.flow_run_notifications.FlowRunNotifications()
            )

        if syncmatrix.settings.SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING.value():
    ...
    async def stop_services():
        """Ensure services are stopped before the Syncmatrix REST API shuts down."""
    ...
    api_app = create_api_app(
        fast_api_app_kwargs={
            "exception_handlers": {
                ...
                ObjectNotFoundError: syncmatrix_object_not_found_exception_handler,
            }
        },
    )
    ...
    # concurrent writes
    if (
        get_dialect(syncmatrix.settings.SYNCMATRIX_API_DATABASE_CONNECTION_URL.value()).name
        == "sqlite"
    ):
        app.add_middleware(RequestLimitMiddleware, limit=100)

    if syncmatrix.settings.SYNCMATRIX_SERVER_CSRF_PROTECTION_ENABLED.value():
    ...
    def openapi():
        ...
        new_schema["info"]["x-logo"] = {"url": "static/syncmatrix-logo-mark-gradient.png"}
        return new_schema
