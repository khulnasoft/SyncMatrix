...
from syncmatrix._vendor.fastapi import Body, Depends, HTTPException, Path, Query, status

from syncmatrix.blocks.core import _should_update_block_type
from syncmatrix.server import models, schemas
from syncmatrix.server.api import dependencies
...
router = SyncmatrixRouter(prefix="/block_types", tags=["Block types"])
...
async def create_block_type(
    ...
    """
    # API-created blocks cannot start with the word "Syncmatrix"
    # as it is reserved for system use
    if block_type.name.lower().startswith("syncmatrix"):
        raise HTTPException(
            status.HTTP_403_FORBIDDEN,
            detail="Block type names beginning with 'Syncmatrix' are reserved.",
        )
    try:
