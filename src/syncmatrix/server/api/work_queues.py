...
router = SyncmatrixRouter(prefix="/work_queues", tags=["Work Queues"])
...
async def read_work_queue_runs(
    ...
    agent_id: Optional[UUID] = Body(
        ...
        description=(
            ...
            " provided, the Syncmatrix REST API will track the last time this agent polled"
            " the work queue."
        ),
    ),
    x_syncmatrix_ui: Optional[bool] = Header(
        default=False,
        description="A header to indicate this request came from the Syncmatrix UI.",
    ),
    db: SyncmatrixDBInterface = Depends(provide_database_interface),
    ...
    # The Syncmatrix UI often calls this route to see which runs are enqueued.
    # We do not want to record this as an actual poll event.
    if not x_syncmatrix_ui:
