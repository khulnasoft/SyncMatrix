"""
Routes for admin-level interactions with the Syncmatrix REST API.
"""
from syncmatrix._vendor.fastapi import Body, Depends, Response, status
...
router = SyncmatrixRouter(prefix="/admin", tags=["Admin"])


@router.get("/settings")
async def read_settings() -> syncmatrix.settings.Settings:
    """
    Get the current Syncmatrix REST API settings.

    Secret setting values will be obfuscated.
    """
    return syncmatrix.settings.get_current_settings().with_obfuscated_secrets()


@router.get("/version")
async def read_version() -> str:
    """Returns the Syncmatrix version number"""
    return syncmatrix.__version__
