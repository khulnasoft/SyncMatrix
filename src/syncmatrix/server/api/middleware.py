...
from syncmatrix._vendor.fastapi import status
...
from syncmatrix import settings
...
class CsrfMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if (
            settings.SYNCMATRIX_SERVER_CSRF_PROTECTION_ENABLED.value()
            and request_needs_csrf_protection
        ):
            incoming_token = request.headers.get("Syncmatrix-Csrf-Token")
            incoming_client = request.headers.get("Syncmatrix-Csrf-Client")
