...
from syncmatrix._vendor.fastapi import Body, Depends, Header, HTTPException, status

from syncmatrix.settings import SYNCMATRIX_API_DEFAULT_LIMIT


def provide_request_api_version(x_syncmatrix_api_version: str = Header(None)):
    if not x_syncmatrix_api_version:
        return

    # parse version
    try:
        major, minor, patch = [int(v) for v in x_syncmatrix_api_version.split(".")]
    except ValueError:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                "Invalid X-SYNCMATRIX-API-VERSION header format.Expected header in format"
                f" 'x.y.z' but received {x_syncmatrix_api_version}"
            ),
        )
    return Version(x_syncmatrix_api_version)
...
class EnforceMinimumAPIVersion:
    ...
    Looks for the header 'X-SYNCMATRIX-API-VERSION' in the request and compares it
    ...
    async def __call__(
        self,
        x_syncmatrix_api_version: str = Header(None),
    ):
        request_version = x_syncmatrix_api_version
    ...
    async def _notify_of_invalid_value(self, request_version: str):
        self.logger.error(
            f"Invalid X-SYNCMATRIX-API-VERSION header format: '{request_version}'"
        )

    async def _notify_of_outdated_version(self, request_version: str):
        self.logger.error(
            f"X-SYNCMATRIX-API-VERSION header specifies version '{request_version}' "
...
def LimitBody() -> Depends:
    ...
    def get_limit(
        limit: int = Body(
            None,
            description="Defaults to SYNCMATRIX_API_DEFAULT_LIMIT if not provided.",
        ),
    ):
        default_limit = SYNCMATRIX_API_DEFAULT_LIMIT.value()
