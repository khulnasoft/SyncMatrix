...
from syncmatrix._vendor.fastapi import HTTPException, status

from syncmatrix.server.utilities.server import SyncmatrixRouter

router = SyncmatrixRouter(prefix="/collections", tags=["Collections"])

GLOBAL_COLLECTIONS_VIEW_CACHE: TTLCache = TTLCache(maxsize=200, ttl=60 * 10)

REGISTRY_VIEWS = (
    "https://raw.githubusercontent.com/KhulnaSoft/syncmatrix-collection-registry/main/views"
)
KNOWN_VIEWS = {
...
async def read_view_content(view: str) -> Dict[str, Any]:
    """Reads the content of a view from the syncmatrix-collection-registry."""
    try:
