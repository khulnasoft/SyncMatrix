...
from syncmatrix.server.schemas import filters, states
from syncmatrix.server.services.loop_service import LoopService
from syncmatrix.settings import SYNCMATRIX_TASK_SCHEDULING_PENDING_TASK_TIMEOUT
...
class TaskSchedulingTimeouts(LoopService):
    ...
    def __init__(self, loop_seconds: float = None, **kwargs):
        ...
        super().__init__(
            ...
            or SYNCMATRIX_TASK_SCHEDULING_PENDING_TASK_TIMEOUT.value().total_seconds(),
            **kwargs,
        )
    ...
    async def reschedule_pending_runs(self, session: AsyncSession):
        ...
        older_than = (
            pendulum.now("UTC") - SYNCMATRIX_TASK_SCHEDULING_PENDING_TASK_TIMEOUT.value()
        )
