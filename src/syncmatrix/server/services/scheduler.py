...
from syncmatrix.server.services.loop_service import LoopService, run_multiple_services
from syncmatrix.settings import (
    SYNCMATRIX_API_SERVICES_SCHEDULER_DEPLOYMENT_BATCH_SIZE,
    SYNCMATRIX_API_SERVICES_SCHEDULER_INSERT_BATCH_SIZE,
    SYNCMATRIX_API_SERVICES_SCHEDULER_LOOP_SECONDS,
...
from syncmatrix.utilities.collections import batched_iterable
...
class Scheduler(LoopService):
    ...
    def __init__(self, loop_seconds: float = None, **kwargs):
        super().__init__(
            loop_seconds=(
                ...
                or SYNCMATRIX_API_SERVICES_SCHEDULER_LOOP_SECONDS.value()
            ),
            **kwargs,
        )
        self.deployment_batch_size: int = (
            SYNCMATRIX_API_SERVICES_SCHEDULER_DEPLOYMENT_BATCH_SIZE.value()
        )
        self.max_runs: int = SYNCMATRIX_API_SERVICES_SCHEDULER_MAX_RUNS.value()
        self.min_runs: int = SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_RUNS.value()
        ...
        self.min_scheduled_time: datetime.timedelta = (
            SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_SCHEDULED_TIME.value()
        )
        self.insert_batch_size = (
            SYNCMATRIX_API_SERVICES_SCHEDULER_INSERT_BATCH_SIZE.value()
        )

    @inject_db
    async def run_once(self, db: SyncmatrixDBInterface):
        """
    ...
    @inject_db
    def _get_select_deployments_to_schedule_query(self, db: SyncmatrixDBInterface):
        """
