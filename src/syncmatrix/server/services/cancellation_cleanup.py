...
from syncmatrix.settings import SYNCMATRIX_API_SERVICES_CANCELLATION_CLEANUP_LOOP_SECONDS
...
class CancellationCleanup(LoopService):
    ...
    def __init__(self, loop_seconds: float = None, **kwargs):
        super().__init__(
            ...
            or SYNCMATRIX_API_SERVICES_CANCELLATION_CLEANUP_LOOP_SECONDS.value(),
            **kwargs,
        )
    ...
    async def _cancel_child_runs(
        self, db: SyncmatrixDBInterface, flow_run: SyncmatrixDBInterface.FlowRun
    ) -> None:
        async with db.session_context() as session:
