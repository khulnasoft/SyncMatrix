...
from syncmatrix.server.models.workers import (
        ...
        if default_agent_work_pool:
            ...
            default_agent_work_pool = await models.workers.create_work_pool(
                ...
                work_pool=schemas.actions.WorkPoolCreate(
                    name=DEFAULT_AGENT_WORK_POOL_NAME, type="syncmatrix-agent"
                ),
            )
...
async def read_work_queue(
    session: AsyncSession, work_queue_id: UUID, db: SyncmatrixDBInterface
):
    """
...
async def read_work_queue_by_name(
    session: AsyncSession, name: str, db: SyncmatrixDBInterface
):
    """
    Reads a WorkQueue by id.
