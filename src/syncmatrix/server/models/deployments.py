...
Intended for internal use by the Syncmatrix REST API.
"""
...
from syncmatrix.server.api.workers import WorkerLookups
...
from syncmatrix.server.utilities.database import json_contains
from syncmatrix.settings import (
    SYNCMATRIX_API_SERVICES_SCHEDULER_MAX_RUNS,
    SYNCMATRIX_API_SERVICES_SCHEDULER_MAX_SCHEDULED_TIME,
    SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_RUNS,
    SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_SCHEDULED_TIME,
)
...
async def read_deployment(
    session: sa.orm.Session, deployment_id: UUID, db: SyncmatrixDBInterface
):
...
async def read_deployment_by_name(
    session: sa.orm.Session, name: str, flow_name: str, db: SyncmatrixDBInterface
):
...
async def schedule_runs(
    ...
    """
    if min_runs is None:
        min_runs = SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_RUNS.value()
    if max_runs is None:
        max_runs = SYNCMATRIX_API_SERVICES_SCHEDULER_MAX_RUNS.value()
    if start_time is None:
        start_time = pendulum.now("UTC")
    if end_time is None:
        end_time = start_time + (
            SYNCMATRIX_API_SERVICES_SCHEDULER_MAX_SCHEDULED_TIME.value()
        )
    if min_time is None:
        min_time = SYNCMATRIX_API_SERVICES_SCHEDULER_MIN_SCHEDULED_TIME.value()
...
async def _insert_scheduled_flow_runs(
    session: sa.orm.Session, runs: List[Dict], db: SyncmatrixDBInterface
) -> List[UUID]:
    """
...
async def check_work_queues_for_deployment(
    db: SyncmatrixDBInterface, session: sa.orm.Session, deployment_id: UUID
...
async def _update_deployment_last_polled(
    db: SyncmatrixDBInterface, session: sa.orm.Session, deployment_ids: List[UUID]
) -> None:
    """
...
async def delete_schedules_for_deployment(
    db: SyncmatrixDBInterface, session: AsyncSession, deployment_id: UUID
) -> bool:
    """
