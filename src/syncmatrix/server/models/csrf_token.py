...
from syncmatrix.server.database.dependencies import db_injector
from syncmatrix.server.database.interface import SyncmatrixDBInterface
from syncmatrix.server.schemas import core
...
async def create_or_update_csrf_token(
    ...
    expiration = (
        ...
        + settings.SYNCMATRIX_SERVER_CSRF_TOKEN_EXPIRATION.value()
    )
...
@db_injector
async def delete_expired_tokens(db: SyncmatrixDBInterface, session: AsyncSession) -> int:
