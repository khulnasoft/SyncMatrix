...
from syncmatrix.server.database.orm_models import ORMWorker, ORMWorkPool, ORMWorkQueue
...
async def read_work_pool(
    session: AsyncSession, work_pool_id: UUID, db: SyncmatrixDBInterface
) -> ORMWorkPool:
    """
...
async def read_work_pool_by_name(
    session: AsyncSession, work_pool_name: str, db: SyncmatrixDBInterface
) -> ORMWorkPool:
    """
