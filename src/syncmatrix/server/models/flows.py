...
async def create_flow(
    session: sa.orm.Session, flow: schemas.core.Flow, db: SyncmatrixDBInterface
):
    """
    Creates a new flow.
...
@inject_db
async def read_flow(session: sa.orm.Session, flow_id: UUID, db: SyncmatrixDBInterface):
    """
    Reads a flow by id.
...
@inject_db
async def read_flow_by_name(session: sa.orm.Session, name: str, db: SyncmatrixDBInterface):
    """
...
async def delete_flow(
    session: sa.orm.Session, flow_id: UUID, db: SyncmatrixDBInterface
) -> bool:
    """
    Delete a flow by id.
