...
class SyncmatrixBaseModel(BaseModel):
    """A base pydantic.BaseModel for all Syncmatrix schemas and pydantic models.

    As the basis for most Syncmatrix schemas, this base model usually ignores extra
    ...
    Syncmatrix client loading data from a server running a possibly-newer version
    of Syncmatrix will be able to process those new fields gracefully. However,
    when SYNCMATRIX_TEST_MODE is on, extra fields are forbidden in order to catch
    ...
    class Config:
        ...
        if os.getenv("SYNCMATRIX_TEST_MODE", "0").lower() in ["1", "true"]:
            extra = "forbid"
        else:
            extra = "ignore"
        ...
        json_encoders = {
            ...
            # for Secret dict in syncmatrix.blocks.fields
    ...
    def _reset_fields(self) -> Set[str]:
        """A set of field names that are reset when the SyncmatrixBaseModel is copied.
        These fields are also disregarded for equality comparisons.
        """
        return set()

    def __eq__(self, other: Any) -> bool:
        """Equaltiy operator that ignores the resettable fields of the SyncmatrixBaseModel.

        NOTE: this equality operator will only be applied if the SyncmatrixBaseModel is
        the left-hand operand. This is a limitation of Python.
        """
        copy_dict = self.dict(exclude=self._reset_fields())
        if isinstance(other, SyncmatrixBaseModel):
...
class IDBaseModel(SyncmatrixBaseModel):
    """
    A SyncmatrixBaseModel with an auto-generated UUID ID value.
...
class ORMBaseModel(IDBaseModel):
    """
    A SyncmatrixBaseModel with an auto-generated UUID ID value and created /
