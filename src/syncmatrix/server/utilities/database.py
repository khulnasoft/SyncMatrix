"""
Utilities for interacting with Syncmatrix REST API database and ORM layer.

Syncmatrix supports both SQLite and Postgres. Many of these utilities
allow the Syncmatrix REST API to seamlessly switch between the two.
"""
...
def get_dialect(
    ...
    Primary use case is figuring out whether the Syncmatrix REST API is communicating with
    SQLite or Postgres.
    ...
    Example:
        ...
        dialect = get_dialect(SYNCMATRIX_API_DATABASE_CONNECTION_URL.value())
        if dialect == "sqlite":
            print("Using SQLite!")
        else:
