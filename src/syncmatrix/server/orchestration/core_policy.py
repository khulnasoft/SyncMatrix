...
Syncmatrix enforces on a state transition.
"""
...
from syncmatrix.results import UnknownResult
...
from syncmatrix.server.models import concurrency_limits
from syncmatrix.server.orchestration.policies import BaseOrchestrationPolicy
from syncmatrix.server.orchestration.rules import (
...
from syncmatrix.server.schemas import core, filters, states
from syncmatrix.server.schemas.states import StateType
from syncmatrix.server.task_queue import TaskQueue
from syncmatrix.settings import (
    SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING,
    SYNCMATRIX_TASK_RUN_TAG_CONCURRENCY_SLOT_WAIT_SECONDS,
)
from syncmatrix.utilities.math import clamped_poisson_interval
...
class SecureTaskConcurrencySlots(BaseOrchestrationRule):
    ...
    specified by the "SYNCMATRIX_TASK_RUN_TAG_CONCURRENCY_SLOT_WAIT_SECONDS" setting
        ...
        for tag, cl in run_limits.items():
            ...
            if limit == 0:
                ...
                await self.delay_transition(
                    SYNCMATRIX_TASK_RUN_TAG_CONCURRENCY_SLOT_WAIT_SECONDS.value(),
...
class WaitForScheduledTime(BaseOrchestrationRule):
    ...
    used by the Syncmatrix REST API instance. This rule will identify transitions from scheduled
