"""
Syncmatrix's flow and task-run orchestration machinery.

This module contains all the core concepts necessary to implement Syncmatrix's state
orchestration engine. These states correspond to intuitive descriptions of all the
points that a Syncmatrix flow or task can observe executing user code and intervene, if
necessary. A detailed description of states can be found in our concept
[documentation](/concepts/states).

Syncmatrix's orchestration engine operates under the assumption that no governed user code
will execute without first requesting Syncmatrix REST API validate a change in state and record
metadata about the run. With all attempts to run user code being checked against a
Syncmatrix instance, the Syncmatrix REST API database becomes the unambiguous source of truth for managing
...
from syncmatrix.server.exceptions import OrchestrationError
from syncmatrix.server.models import artifacts, flow_runs
from syncmatrix.server.schemas import core, states
...
class OrchestrationContext(SyncmatrixBaseModel):
    """
    ...
    When a flow- or task- run attempts to change state, Syncmatrix REST API has an opportunity
...
class FlowOrchestrationContext(OrchestrationContext):
    ...
    When a flow- run attempts to change state, Syncmatrix REST API has an opportunity
    to decide whether this transition can proceed. All the relevant information
    associated with the state transition is stored in an `OrchestrationContext`,
    which is subsequently governed by nested orchestration rules implemented using
    the `BaseOrchestrationRule` ABC.
    ...
    async def validate_proposed_state(
        ...
        """
        # (circular import)
        from syncmatrix.server.api.server import is_client_retryable_exception
...
class TaskOrchestrationContext(OrchestrationContext):
    ...
    When a task- run attempts to change state, Syncmatrix REST API has an opportunity
    to decide whether this transition can proceed. All the relevant information
    associated with the state transition is stored in an `OrchestrationContext`,
    which is subsequently governed by nested orchestration rules implemented using
    the `BaseOrchestrationRule` ABC.
...
class BaseOrchestrationRule(contextlib.AbstractAsyncContextManager):
    ...
    Syncmatrix REST API to decide whether or not this transition can proceed. The current state of
