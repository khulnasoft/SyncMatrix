...
class CountByState(SyncmatrixBaseModel):
...
class State(StateBaseModel, Generic[R]):
    ...
    def from_orm_without_result(
        cls,
        orm_state: Union[
            "syncmatrix.server.database.orm_models.ORMFlowRunState",
            "syncmatrix.server.database.orm_models.ORMTaskRunState",
        ],
    ...
    def result(self, raise_on_failure: bool = True, fetch: Optional[bool] = None):
        ...
        warnings.warn(
            (
                ...
                " `syncmatrix.server.schemas.states.State` and will be removed in a future"
                " release. When result retrieval is needed, use `syncmatrix.states.State`."
            ),
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def to_state_create(self):
        ...
        warnings.warn(
            (
                "Use of `syncmatrix.server.schemas.states.State` from the client is"
                " deprecated and support will be removed in a future release. Use"
                " `syncmatrix.states.State` instead."
            ),
            DeprecationWarning,
            stacklevel=2,
        )
