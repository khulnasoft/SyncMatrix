"""
Full schemas of Syncmatrix REST API objects.
"""
...
import syncmatrix.server.database
from syncmatrix.server.schemas import schedules, states
from syncmatrix.server.utilities.schemas.bases import (
    ORMBaseModel,
    SyncmatrixBaseModel,
)
...
from syncmatrix.settings import (
    SYNCMATRIX_API_TASK_CACHE_KEY_MAX_LENGTH,
)
from syncmatrix.utilities.collections import dict_to_flatdict, flatdict_to_dict, listrepr
from syncmatrix.utilities.names import generate_slug, obfuscate, obfuscate_string
...
class TaskRun(ORMBaseModel):
    ...
    def validate_cache_key_length(cls, cache_key):
        if cache_key and len(cache_key) > SYNCMATRIX_API_TASK_CACHE_KEY_MAX_LENGTH.value():
            raise ValueError(
                "Cache key exceeded maximum allowed length of"
                f" {SYNCMATRIX_API_TASK_CACHE_KEY_MAX_LENGTH.value()} characters."
            )
        return cache_key
...
class BlockDocument(ORMBaseModel):
    ...
    async def from_orm_model(
        cls,
        session,
        orm_block_document: "syncmatrix.server.database.orm_models.ORMBlockDocument",
