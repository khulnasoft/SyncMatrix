...
from syncmatrix.settings import (
    SYNCMATRIX_API_DATABASE_CONNECTION_TIMEOUT,
    SYNCMATRIX_API_DATABASE_ECHO,
    SYNCMATRIX_API_DATABASE_TIMEOUT,
    SYNCMATRIX_SQLALCHEMY_MAX_OVERFLOW,
    SYNCMATRIX_SQLALCHEMY_POOL_SIZE,
    SYNCMATRIX_UNIT_TEST_MODE,
)
from syncmatrix.utilities.asyncutils import add_event_loop_shutdown_callback
...
class BaseDatabaseConfiguration(ABC):
    """
    Abstract base class used to inject database connection configuration into Syncmatrix.

    This configuration is responsible for defining how Syncmatrix REST API creates and manages
    ...
    def __init__(
        ...
        self.echo = echo or SYNCMATRIX_API_DATABASE_ECHO.value()
        self.timeout = timeout or SYNCMATRIX_API_DATABASE_TIMEOUT.value()
        self.connection_timeout = (
            connection_timeout or SYNCMATRIX_API_DATABASE_CONNECTION_TIMEOUT.value()
        )
        self.sqlalchemy_pool_size = (
            sqlalchemy_pool_size or SYNCMATRIX_SQLALCHEMY_POOL_SIZE.value()
        )
        self.sqlalchemy_max_overflow = (
            sqlalchemy_max_overflow or SYNCMATRIX_SQLALCHEMY_MAX_OVERFLOW.value()
        )
...
class AioSqliteConfiguration(BaseDatabaseConfiguration):
    ...
    async def engine(self) -> AsyncEngine:
        ...
        if sqlite3.sqlite_version_info < self.MIN_SQLITE_VERSION:
            ...
            raise RuntimeError(
                f"Syncmatrix requires sqlite >= {required} but we found version "
        ...
        if cache_key not in ENGINES:
            ...
            # see https://github.com/KhulnaSoft/syncmatrix/pull/6702
    ...
    def setup_sqlite(self, conn, record):
        ...
        if SYNCMATRIX_UNIT_TEST_MODE.value() is True:
