...
def provide_database_interface() -> SyncmatrixDBInterface:
    """
    Get the current Syncmatrix REST API database interface.

    If components of the interface are not set, defaults will be inferred
    based on the dialect of the connection URL.
    """
    connection_url = SYNCMATRIX_API_DATABASE_CONNECTION_URL.value()
    ...
    if interface_class is None:
        interface_class = SyncmatrixDBInterface
...
def db_injector(
    func: Callable[Concatenate[SyncmatrixDBInterface, P], R],
) -> Callable[P, R]:
    """
    Decorator to inject a SyncmatrixDBInterface instance as the first positional
    ...
    SyncmatrixDBInterface explicit in the function signature.
    ...
    Returns:
        A wrapped function with the SyncmatrixDBInterface instance injected as the
...
def temporary_database_config(tmp_database_config: BaseDatabaseConfiguration):
    """
    Temporarily override the Syncmatrix REST API database configuration.
    When the context is closed, the existing database configuration will
    be restored.

    Args:
        tmp_database_config: Syncmatrix REST API database configuration to inject.
...
def temporary_query_components(tmp_queries: BaseQueryComponents):
    """
    Temporarily override the Syncmatrix REST API database query components.
    When the context is closed, the existing query components will
    be restored.

    Args:
        tmp_queries: Syncmatrix REST API query components to inject.
...
def temporary_orm_config(tmp_orm_config: BaseORMConfiguration):
    """
    Temporarily override the Syncmatrix REST API ORM configuration.
    When the context is closed, the existing orm configuration will
    be restored.

    Args:
        tmp_orm_config: Syncmatrix REST API ORM configuration to inject.
...
@contextmanager
def temporary_interface_class(tmp_interface_class: Type[SyncmatrixDBInterface]):
    """
    Temporarily override the Syncmatrix REST API interface class When the context is closed,
    the existing interface will be restored.

    Args:
        tmp_interface_class: Syncmatrix REST API interface class to inject.
...
def temporary_database_interface(
    ...
    tmp_interface_class: Type[SyncmatrixDBInterface] = None,
):
    """
    Temporarily override the Syncmatrix REST API database interface.

    Any interface components that are not explicitly provided will be
    cleared and inferred from the Syncmatrix REST API database connection string
    dialect.
    ...
    Args:
        tmp_database_config: An optional Syncmatrix REST API database configuration to inject.
        tmp_orm_config: An optional Syncmatrix REST API ORM configuration to inject.
        tmp_queries: Optional Syncmatrix REST API query components to inject.
...
def set_database_config(database_config: BaseDatabaseConfiguration):
    """Set Syncmatrix REST API database configuration."""
    MODELS_DEPENDENCIES["database_config"] = database_config


def set_query_components(query_components: BaseQueryComponents):
    """Set Syncmatrix REST API query components."""
    MODELS_DEPENDENCIES["query_components"] = query_components


def set_orm_config(orm_config: BaseORMConfiguration):
    """Set Syncmatrix REST API orm configuration."""
    MODELS_DEPENDENCIES["orm"] = orm_config


def set_interface_class(interface_class: Type[SyncmatrixDBInterface]):
    """Set Syncmatrix REST API interface class."""
