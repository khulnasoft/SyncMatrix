...
def with_alembic_lock(fn):
    ...
    This issue occurred in https://github.com/KhulnaSoft/syncmatrix-dask/pull/50, where
...
def alembic_config():
    ...
    alembic_dir = Path(syncmatrix.server.database.__file__).parent
...
def alembic_upgrade(revision: str = "head", dry_run: bool = False):
    """
    Run alembic upgrades on Syncmatrix REST API database
...
def alembic_downgrade(revision: str = "-1", dry_run: bool = False):
    """
    Run alembic downgrades on Syncmatrix REST API database
