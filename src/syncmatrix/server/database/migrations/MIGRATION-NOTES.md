...
Adds a table for storing key / value configuration options for Syncmatrix REST API in the database.
...
Upgrading to an existing database to use migrations requires the use of `syncmatrix database alembic stamp` before a reset will drop existing tables.
