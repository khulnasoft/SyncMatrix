...
from syncmatrix.server.utilities.database import (
    JSON,
    UUID,
    GenerateUUID,
    Pydantic,
...
from syncmatrix.server.utilities.encryption import decrypt_fernet, encrypt_fernet
...
class BaseORMConfiguration(ABC):
    """
    Abstract base class used to inject database-specific ORM configuration into Syncmatrix.

    Modifications to core Syncmatrix REST API data structures can have unintended consequences.
    Use with caution.
    ...
    def _create_orm_models(
        ...
        """
        Defines the ORM models used in Syncmatrix REST API and binds them to the `self`. This method
...
class AsyncPostgresORMConfiguration(BaseORMConfiguration):
    ...
    def versions_dir(self) -> Path:
        """Directory containing migrations"""
        return (
            Path(syncmatrix.server.database.__file__).parent
            / "migrations"
            / "versions"
            / "postgresql"
        )
