...
from syncmatrix.blocks.abstract import NotificationBlock
from syncmatrix.blocks.fields import SecretDict
from syncmatrix.events.instrument import instrument_instance_method_call
from syncmatrix.utilities.asyncutils import sync_compatible
from syncmatrix.utilities.templating import apply_values, find_placeholders

SYNCMATRIX_NOTIFY_TYPE_DEFAULT = "syncmatrix_default"
...
class AbstractAppriseNotificationBlock(NotificationBlock, ABC):
    ...
    notify_type: Literal[
        "syncmatrix_default", "info", "success", "warning", "failure"
    ] = Field(
        default=SYNCMATRIX_NOTIFY_TYPE_DEFAULT,
        description=(
            "The type of notification being performed; the syncmatrix_default "
    ...
    def __init__(self, *args, **kwargs):
        ...
        if SYNCMATRIX_NOTIFY_TYPE_DEFAULT not in apprise.NOTIFY_TYPES:
            apprise.NOTIFY_TYPES += (SYNCMATRIX_NOTIFY_TYPE_DEFAULT,)
    ...
    def _start_apprise_client(self, url: SecretStr):
        ...
        # A custom `AppriseAsset` that ensures Syncmatrix Notifications
        # appear correctly across multiple messaging platforms
        syncmatrix_app_data = AppriseAsset(
            app_id="Syncmatrix Notifications",
            app_desc="Syncmatrix Notifications",
            app_url="https://khulnasoft.com",
        )

        self._apprise_client = Apprise(asset=syncmatrix_app_data)
...
class AppriseNotificationBlock(AbstractAppriseNotificationBlock, ABC):
    ...
    _documentation_url = "https://docs.khulnasoft.com/ui/notifications/"
    url: SecretStr = Field(
        default=...,
        title="Webhook URL",
...
# TODO: Move to syncmatrix-slack once collection block auto-registration is
# available
class SlackWebhook(AppriseNotificationBlock):
    ...
    Examples:
        ...
        ```python
        from syncmatrix.blocks.notifications import SlackWebhook

        slack_webhook_block = SlackWebhook.load("BLOCK_NAME")
        slack_webhook_block.notify("Hello from Syncmatrix!")
        ```
    """
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/blocks/notifications/#syncmatrix.blocks.notifications.SlackWebhook"
...
class MicrosoftTeamsWebhook(AppriseNotificationBlock):
    ...
    Examples:
        ...
        ```python
        from syncmatrix.blocks.notifications import MicrosoftTeamsWebhook
        teams_webhook_block = MicrosoftTeamsWebhook.load("BLOCK_NAME")
        teams_webhook_block.notify("Hello from Syncmatrix!")
        ```
    """
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/blocks/notifications/#syncmatrix.blocks.notifications.MicrosoftTeamsWebhook"
...
class PagerDutyWebHook(AbstractAppriseNotificationBlock):
    ...
    Examples:
        ...
        ```python
        from syncmatrix.blocks.notifications import PagerDutyWebHook
        pagerduty_webhook_block = PagerDutyWebHook.load("BLOCK_NAME")
        pagerduty_webhook_block.notify("Hello from Syncmatrix!")
        ```
    """
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/blocks/notifications/#syncmatrix.blocks.notifications.PagerDutyWebHook"

    # The default cannot be syncmatrix_default because NotifyPagerDuty's
    ...
    source: Optional[str] = Field(
        default="Syncmatrix", description="The source string as part of the payload."
    )
...
class TwilioSMS(AbstractAppriseNotificationBlock):
    ...
    Examples:
        ...
        ```python
        from syncmatrix.blocks.notifications import TwilioSMS
        twilio_webhook_block = TwilioSMS.load("BLOCK_NAME")
        twilio_webhook_block.notify("Hello from Syncmatrix!")
        ```
    """
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/blocks/notifications/#syncmatrix.blocks.notifications.TwilioSMS"
...
class OpsgenieWebhook(AbstractAppriseNotificationBlock):
    ...
    Examples:
        ...
        ```python
        from syncmatrix.blocks.notifications import OpsgenieWebhook
        opsgenie_webhook_block = OpsgenieWebhook.load("BLOCK_NAME")
        opsgenie_webhook_block.notify("Hello from Syncmatrix!")
        ```
    """
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/blocks/notifications/#syncmatrix.blocks.notifications.OpsgenieWebhook"
...
class MattermostWebhook(AbstractAppriseNotificationBlock):
    ...
    Examples:
        ...
        ```python
        from syncmatrix.blocks.notifications import MattermostWebhook

        mattermost_webhook_block = MattermostWebhook.load("BLOCK_NAME")

        mattermost_webhook_block.notify("Hello from Syncmatrix!")
        ```
    """
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/blocks/notifications/#syncmatrix.blocks.notifications.MattermostWebhook"
...
class DiscordWebhook(AbstractAppriseNotificationBlock):
    ...
    Examples:
        ...
        ```python
        from syncmatrix.blocks.notifications import DiscordWebhook

        discord_webhook_block = DiscordWebhook.load("BLOCK_NAME")

        discord_webhook_block.notify("Hello from Syncmatrix!")
        ```
    """
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/blocks/notifications/#syncmatrix.blocks.notifications.DiscordWebhook"
...
class CustomWebhookNotificationBlock(NotificationBlock):
    ...
    Examples:
        ...
        ```python
        from syncmatrix.blocks.notifications import CustomWebhookNotificationBlock

        custom_webhook_block = CustomWebhookNotificationBlock.load("BLOCK_NAME")

        custom_webhook_block.notify("Hello from Syncmatrix!")
        ```
    """
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/blocks/notifications/#syncmatrix.blocks.notifications.CustomWebhookNotificationBlock"
    ...
    async def notify(self, body: str, subject: Optional[str] = None):
        ...
        client = httpx.AsyncClient(
            headers={"user-agent": "Syncmatrix Notifications"}, cookies=cookies
        )
        async with client:
...
class SendgridEmail(AbstractAppriseNotificationBlock):
    ...
    Examples:
        ...
        ```python
        from syncmatrix.blocks.notifications import SendgridEmail

        sendgrid_block = SendgridEmail.load("BLOCK_NAME")

        sendgrid_block.notify("Hello from Syncmatrix!")
    """
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/blocks/notifications/#syncmatrix.blocks.notifications.SendgridEmail"
