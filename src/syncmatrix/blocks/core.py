...
from syncmatrix.client.schemas import (
...
from syncmatrix.events.instrument import (
    ResourceTuple,
    emit_instance_method_called_event,
    instrument_instance_method_call,
    instrument_method_calls_on_class_instances,
)
from syncmatrix.logging.loggers import disable_logger
from syncmatrix.utilities.asyncutils import sync_compatible
from syncmatrix.utilities.collections import listrepr, remove_nested_keys
from syncmatrix.utilities.dispatch import lookup_type, register_base_type
...
class Block(BaseModel, ABC):
    ...
    `_block_type_id` are reserved by Syncmatrix as Block metadata fields, but
    ...
    def _to_block_document(
        ...
        # reported from https://github.com/KhulnaSoft/syncmatrix-dbt/issues/54
    ...
    def _event_kind(self) -> str:
        return f"syncmatrix.block.{self.get_block_type_slug()}"
        ...
        return (
            {
                "syncmatrix.resource.id": (
                    f"syncmatrix.block-document.{self._block_document_id}"
                ),
                "syncmatrix.resource.name": self._block_document_name,
            },
            [
                {
                    "syncmatrix.resource.id": (
                        f"syncmatrix.block-type.{self.get_block_type_slug()}"
                    ),
                    "syncmatrix.resource.role": "block-type",
                }
            ],
        )
    ...
    async def _get_block_document(
        ...
        except syncmatrix.exceptions.ObjectNotFound as e:
            raise ValueError(
    ...
    @sync_compatible
    @inject_client
    async def register_type_and_schema(cls, client: "SyncmatrixClient" = None):
        """
        Makes block available for configuration with current Syncmatrix API.
        ...
        Args:
            client: Optional client to use for registering type and schema with the
                Syncmatrix API. A new client will be created and used if one is not
                provided.
        """
        if cls.__name__ == "Block":
        ...
        except syncmatrix.exceptions.ObjectNotFound:
    ...
    async def _save(
        ...
        except syncmatrix.exceptions.ObjectAlreadyExists as err:
            if overwrite:
    ...
    def get_block_placeholder(self) -> str:
        ...
        Returns:
            str: The block placeholder for the current block in the format
                `syncmatrix.blocks.{block_type_name}.{block_document_name}`
        ...
        return f"syncmatrix.blocks.{self.get_block_type_slug()}.{block_document_name}"
