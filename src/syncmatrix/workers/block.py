...
from syncmatrix.client.schemas.objects import BlockDocument
from syncmatrix.infrastructure.base import Infrastructure
from syncmatrix.utilities.collections import get_from_dict
from syncmatrix.workers.base import BaseWorker, BaseWorkerResult
...
from syncmatrix.events.related import object_as_related_resource, tags_as_related_resources
...
if TYPE_CHECKING:
    from syncmatrix.client.schemas.objects import Flow, FlowRun
...
class BlockWorkerJobConfiguration(BaseModel):
    ...
    def is_using_a_runner(self):
        return (
            ...
            and "syncmatrix flow-run execute" in shlex.join(self.block.command)
        )
    ...
    async def from_template_and_values(
        cls, base_job_template: dict, values: dict, client: "SyncmatrixClient" = None
    ):
...
class BlockWorker(BaseWorker):
    ...
    async def run(
        ...
        # logic for applying infra overrides taken from src/syncmatrix/agent.py
