...
```bash
syncmatrix worker start --pool 'my-work-pool' --type process
```
...
checkout out the [Syncmatrix docs](/concepts/work-pools/).
"""
import asyncio
import contextlib
import os
...
from syncmatrix.utilities.filesystem import relative_path_to_current_platform
from syncmatrix.utilities.processutils import get_sys_executable, run_process
from syncmatrix.workers.base import (
...
if TYPE_CHECKING:
    from syncmatrix.client.schemas.objects import Flow
...
class ProcessJobConfiguration(BaseJobConfiguration):
    ...
    def prepare_for_flow_run(
        ...
        self.command = (
            f"{get_sys_executable()} -m syncmatrix.engine"
...
class ProcessWorker(BaseWorker):
    ...
    _documentation_url = (
        "https://docs.khulnasoft.com/latest/api-ref/syncmatrix/workers/process/"
    )
    ...
    async def run(
        ...
        if not command:
            command = f"{get_sys_executable()} -m syncmatrix.engine"
