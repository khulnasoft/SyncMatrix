...
from syncmatrix.client.schemas.objects import StateType, WorkPool
...
from syncmatrix.utilities.dispatch import get_registry_for_type, register_base_type
...
class BaseJobConfiguration(BaseModel):
    ...
    def is_using_a_runner(self):
        return self.command is not None and "syncmatrix flow-run execute" in self.command
    ...
    def _base_flow_run_environment(flow_run: "FlowRun") -> Dict[str, str]:
        ...
        """
        return {"SYNCMATRIX__FLOW_RUN_ID": str(flow_run.id)}
...
class BaseWorker(abc.ABC):
    ...
    def __init__(
        ...
        *,
        ...
        """
        Base class for all Syncmatrix workers.
        ...
        self._prefetch_seconds: float = (
            prefetch_seconds or SYNCMATRIX_WORKER_PREFETCH_SECONDS.value()
        )
        self.heartbeat_interval_seconds = (
            heartbeat_interval_seconds or SYNCMATRIX_WORKER_HEARTBEAT_SECONDS.value()
        )

        self._work_pool: Optional[WorkPool] = None
        self._runs_task_group: Optional[anyio.abc.TaskGroup] = None
        self._client: Optional[SyncmatrixClient] = None
    ...
    def get_flow_run_logger(self, flow_run: "FlowRun") -> SyncmatrixLogAdapter:
    ...
    async def sync_with_backend(self):
        ...
        self._logger.debug("Worker synchronized with the Syncmatrix API server.")
    ...
    def _event_resource(self):
        return {
            "syncmatrix.resource.id": f"syncmatrix.worker.{self.type}.{self.get_name_slug()}",
            "syncmatrix.resource.name": self.name,
            "syncmatrix.version": syncmatrix.__version__,
            "syncmatrix.worker-type": self.type,
        }
    ...
    def _event_related_resources(
        ...
        if include_self:
            ...
            worker_resource["syncmatrix.resource.role"] = "worker"
    ...
    def _emit_flow_run_submitted_event(
        ...
        return emit_event(
            event="syncmatrix.worker.submitted-flow-run",
    ...
    def _emit_flow_run_executed_event(
        ...
        for resource in related:
            if resource.role == "flow-run":
                resource["syncmatrix.infrastructure.identifier"] = str(result.identifier)
                resource["syncmatrix.infrastructure.status-code"] = str(result.status_code)

        emit_event(
            event="syncmatrix.worker.executed-flow-run",
            resource=self._event_resource(),
            related=related,
    ...
    def _emit_flow_run_cancelled_event(
        ...
        for resource in related:
            if resource.role == "flow-run":
                resource["syncmatrix.infrastructure.identifier"] = str(
                    flow_run.infrastructure_pid
                )

        emit_event(
            event="syncmatrix.worker.cancelled-flow-run",
            resource=self._event_resource(),
            related=related,
        )
