...
from syncmatrix.deprecated.packaging.base import PackageManifest, Packager
from syncmatrix.deprecated.packaging.serializers import SourceSerializer
from syncmatrix.flows import Flow, load_flow_from_script
from syncmatrix.software import CondaEnvironment, PythonEnvironment
...
class DockerPackager(Packager):
    ...
    def set_default_base_image(cls, values):
        if not values.get("base_image") and not values.get("dockerfile"):
            values["base_image"] = get_syncmatrix_image_name(
                flavor=(
                    "conda"
