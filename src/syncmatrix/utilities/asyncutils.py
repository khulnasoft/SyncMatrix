...
SYNCMATRIX_THREAD_LIMITER: Optional[anyio.CapacityLimiter] = None

logger = get_logger()


def get_thread_limiter():
    global SYNCMATRIX_THREAD_LIMITER

    if SYNCMATRIX_THREAD_LIMITER is None:
        SYNCMATRIX_THREAD_LIMITER = anyio.CapacityLimiter(250)

    return SYNCMATRIX_THREAD_LIMITER
...
def sync_compatible(async_fn: T) -> T:
    ...
    def coroutine_wrapper(*args, **kwargs):
        ...
        from syncmatrix._internal.concurrency.calls import get_current_call, logger
        from syncmatrix._internal.concurrency.event_loop import get_running_loop
        from syncmatrix._internal.concurrency.threads import get_global_loop
        ...
        if current_thread.ident == global_thread_portal.thread.ident:
            ...
            # In the syncmatrix async context; return the coro for us to await
            ...
            # call in Syncmatrix's event loop thread
...
async def add_event_loop_shutdown_callback(coroutine_fn: Callable[[], Awaitable]):
    ...
    async def on_shutdown(key):
        ...
        # https://github.com/KhulnaSoft/syncmatrix/issues/7709#issuecomment-1560021109
