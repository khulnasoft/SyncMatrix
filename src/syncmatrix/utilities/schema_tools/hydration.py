...
from syncmatrix.server.models.variables import read_variables
...
SyncmatrixKind: TypeAlias = Optional[str]

_handlers: Dict[SyncmatrixKind, Handler] = {}
...
class KeyNotFound(HydrationError):
    @property
    def message(self):
        return f"Missing '{self.key}' key in __syncmatrix object"
...
def handler(kind: SyncmatrixKind) -> Callable:
...
def call_handler(kind: SyncmatrixKind, obj: Dict, ctx: HydrationContext) -> Any:
...
def json_handler(obj: Dict, ctx: HydrationContext):
    if "value" in obj:
        ...
        # the UI. For now if an object looks like {"__syncmatrix_kind": "json"}
        # We will remove it from the parent object. e.x.
        # {"a": {"__syncmatrix_kind": "json"}} -> {}
        # or
...
def workspace_variable_handler(obj: Dict, ctx: HydrationContext):
    if "variable_name" in obj:
        ...
        # If an object looks like {"__syncmatrix_kind": "workspace_variable"}
        # we will remove it from the parent object. e.x.
        # {"a": {"__syncmatrix_kind": "workspace_variable"}} -> {}
        # or
...
def _hydrate(obj, ctx: Optional[HydrationContext] = None):
    ...
    syncmatrix_object = isinstance(obj, dict) and "__syncmatrix_kind" in obj

    if syncmatrix_object:
        syncmatrix_kind = obj.get("__syncmatrix_kind")
        return call_handler(syncmatrix_kind, obj, ctx)
    else:
