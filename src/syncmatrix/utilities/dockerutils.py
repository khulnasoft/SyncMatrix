...
def get_syncmatrix_image_name(
    syncmatrix_version: str = None, python_version: str = None, flavor: str = None
) -> str:
    """
    Get the Syncmatrix image name matching the current Syncmatrix and Python versions.

    Args:
        syncmatrix_version: An optional override for the Syncmatrix version.
    ...
    """
    parsed_version = (syncmatrix_version or syncmatrix.__version__).split("+")
    is_prod_build = len(parsed_version) == 1
    syncmatrix_version = (
        parsed_version[0]
        if is_prod_build
        else "sha-" + syncmatrix.__version_info__["full-revisionid"][:7]
    )

    python_version = python_version or python_version_minor()

    tag = slugify(
        f"{syncmatrix_version}-python{python_version}" + (f"-{flavor}" if flavor else ""),
        lowercase=False,
        max_length=128,
    ...
    image = "syncmatrix" if is_prod_build else "syncmatrix-dev"
    return f"khulnasoft/{image}:{tag}"
...
# Labels to apply to all images built with Syncmatrix
IMAGE_LABELS = {
...
def parse_image_tag(name: str) -> Tuple[str, Optional[str]]:
    ...
    - If a tag exists, this function parses and returns the image registry and tag,
      ...
      - Example 1: 'khulnasoft/syncmatrix:latest' -> ('khulnasoft/syncmatrix', 'latest')
    ...
    if len(name_parts) == 1:
        ...
        # 1. Separates index (hostname.io or khulnasoft) from path:tag (folder/subfolder:latest or syncmatrix:latest)
...
def generate_default_dockerfile(context: Optional[Path] = None):
    ...
    if not context:
        context = Path.cwd()
    lines = []
    base_image = get_syncmatrix_image_name()
    lines.append(f"FROM {base_image}")
    dir_name = context.name

    if (context / "requirements.txt").exists():
        lines.append(f"COPY requirements.txt /opt/syncmatrix/{dir_name}/requirements.txt")
        lines.append(
            f"RUN python -m pip install -r /opt/syncmatrix/{dir_name}/requirements.txt"
        )

    lines.append(f"COPY . /opt/syncmatrix/{dir_name}/")
    lines.append(f"WORKDIR /opt/syncmatrix/{dir_name}/")
