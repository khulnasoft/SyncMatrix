...
BLOCK_DOCUMENT_PLACEHOLDER_PREFIX = "syncmatrix.blocks."
VARIABLE_PLACEHOLDER_PREFIX = "syncmatrix.variables."
...
async def resolve_block_document_references(
    template: T, client: "SyncmatrixClient" = None
    ...
    1. Accessing a nested dictionary:
       Format: syncmatrix.blocks.<block_type_slug>.<block_document_name>.value.key
       Example: Returns {"nested-key": "nested-value"}

    2. Accessing a specific nested value:
       Format: syncmatrix.blocks.<block_type_slug>.<block_document_name>.value.key.nested-key
       Example: Returns "nested-value"

    3. Accessing a list element's key-value:
       Format: syncmatrix.blocks.<block_type_slug>.<block_document_name>.value.list[0].list-key
...
@inject_client
async def resolve_variables(template: T, client: "SyncmatrixClient" = None):
    """
