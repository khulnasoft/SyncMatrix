...
class BaseAnnotation(
    ...
    """
    Base class for Syncmatrix annotation types.
...
class allow_failure(BaseAnnotation[T]):
    ...
    Generally, Syncmatrix will not allow a downstream run to start if any of its inputs
...
class quote(BaseAnnotation[T]):
    ...
    by Syncmatrix. For example, if you want to return a state from a flow without having
    the flow assume that state.

    quote will also instruct syncmatrix to ignore introspection of the wrapped object
