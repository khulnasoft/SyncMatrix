...
if TYPE_CHECKING:
    from syncmatrix._internal.schemas.bases import ObjectBaseModel
...
def tags_as_related_resources(tags: Iterable[str]) -> List[RelatedResource]:
    return [
        RelatedResource(
            __root__={
                "syncmatrix.resource.id": f"syncmatrix.tag.{tag}",
...
def object_as_related_resource(kind: str, role: str, object: Any) -> RelatedResource:
    resource_id = f"syncmatrix.{kind}.{object.id}"

    return RelatedResource(
        __root__={
            "syncmatrix.resource.id": resource_id,
            "syncmatrix.resource.role": role,
            "syncmatrix.resource.name": object.name,
        }
    )
