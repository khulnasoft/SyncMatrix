...
from syncmatrix._internal.concurrency.services import QueueService
from syncmatrix.settings import SYNCMATRIX_API_KEY, SYNCMATRIX_API_URL, SYNCMATRIX_CLOUD_API_URL
from syncmatrix.utilities.context import temporary_context

from .clients import EventsClient, NullEventsClient, SyncmatrixCloudEventsClient
...
def emit_events_to_cloud() -> bool:
    ...
    return (
        ...
        and api
        and api.startswith(SYNCMATRIX_CLOUD_API_URL.value())
    )
    ...
    def instance(
        ...
        if client_type is None:
            if emit_events_to_cloud():
                client_type = SyncmatrixCloudEventsClient
                client_kwargs = {
                    "api_url": SYNCMATRIX_API_URL.value(),
