...
from syncmatrix.events.actions import ActionTypes, RunDeployment
from syncmatrix.utilities.collections import AutoEnum

# These are defined by Syncmatrix Cloud
...
class ResourceSpecification(SyncmatrixBaseModel):
    __root__: Dict[str, Union[str, List[str]]]


class Labelled(SyncmatrixBaseModel):
...
class Resource(Labelled):
    ...
    def requires_resource_id(cls, values: Dict[str, Any]):
        ...
        if "syncmatrix.resource.id" not in labels:
            raise ValueError("Resources must include the syncmatrix.resource.id label")
        if not labels["syncmatrix.resource.id"]:
            raise ValueError("The syncmatrix.resource.id label must be non-empty")

        return values

    @property
    def id(self) -> str:
        return self["syncmatrix.resource.id"]
...
class RelatedResource(Resource):
    ...
    def requires_resource_role(cls, values: Dict[str, Any]):
        ...
        if "syncmatrix.resource.role" not in labels:
            raise ValueError(
                "Related Resources must include the syncmatrix.resource.role label"
            )
        if not labels["syncmatrix.resource.role"]:
            raise ValueError("The syncmatrix.resource.role label must be non-empty")

        return values

    @property
    def role(self) -> str:
        return self["syncmatrix.resource.role"]


class Event(SyncmatrixBaseModel):
...
class Trigger(SyncmatrixBaseModel, abc.ABC):
    """
...
class SyncmatrixMetric(Enum):
    lateness = "lateness"
    duration = "duration"
    successes = "successes"


class MetricTriggerQuery(SyncmatrixBaseModel):
...
class Automation(SyncmatrixBaseModel):
...
class AutomationCreateFromTrigger(SyncmatrixBaseModel):
...
class DeploymentTrigger(AutomationCreateFromTrigger):
    ...
    def owner_resource(self) -> Optional[str]:
        return f"syncmatrix.deployment.{self._deployment_id}"
