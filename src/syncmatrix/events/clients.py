...
try:
    ...
    pass
from syncmatrix._vendor.starlette.status import WS_1008_POLICY_VIOLATION
...
from syncmatrix.settings import SYNCMATRIX_API_KEY, SYNCMATRIX_API_URL

if TYPE_CHECKING:
    from syncmatrix.events.filters import EventFilter

logger = get_logger(__name__)


class EventsClient(abc.ABC):
    """The abstract interface for all Syncmatrix Events clients"""
...
class NullEventsClient(EventsClient):
    """A Syncmatrix Events client implementation that does nothing"""
...
class AssertingEventsClient(EventsClient):
    """A Syncmatrix Events client that records all events sent to it for inspection during
    tests."""
...
def _get_api_url_and_key(
    ...
    api_url = api_url or SYNCMATRIX_API_URL.value()
    api_key = api_key or SYNCMATRIX_API_KEY.value()

    if not api_url or not api_key:
        raise ValueError(
            "api_url and api_key must be provided or set in the Syncmatrix configuration"
        )

    return api_url, api_key


class SyncmatrixCloudEventsClient(EventsClient):
    """A Syncmatrix Events client that streams Events to a Syncmatrix Cloud Workspace"""
    ...
    def __init__(
        ...
        """
        Args:
            api_url: The base URL for a Syncmatrix Cloud workspace
    ...
    async def __aenter__(self) -> "SyncmatrixCloudEventsClient":
...
class SyncmatrixCloudEventSubscriber:
    """
    Subscribes to a Syncmatrix Cloud event stream, yielding events as they occur.

    Example:

        from syncmatrix.events.clients import SyncmatrixCloudEventSubscriber
        from syncmatrix.events.filters import EventFilter, EventNameFilter

        filter = EventFilter(event=EventNameFilter(prefix=["syncmatrix.flow-run."]))

        async with SyncmatrixCloudEventSubscriber(api_url, api_key, filter) as subscriber:
    ...
    def __init__(
        ...
        self._connect = connect(
            socket_url,
            subprotocols=["syncmatrix"],
        )
        self._websocket = None
        self._reconnection_attempts = reconnection_attempts

    async def __aenter__(self) -> "SyncmatrixCloudEventSubscriber":
        # Don't handle any errors in the initial connection, because these are most
        # likely a permission or configuration issue that should propagate
        await self._reconnect()
        return self
    ...
    async def _reconnect(self) -> None:
        ...
        from syncmatrix.events.filters import EventOccurredFilter
    ...
    def __aiter__(self) -> "SyncmatrixCloudEventSubscriber":
        return self
