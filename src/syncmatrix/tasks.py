"""
Module containing the base workflow task class and decorator - for most use cases, using the [`@task` decorator][syncmatrix.tasks.task] is preferred.
"""
...
from syncmatrix.client.schemas import TaskRun
from syncmatrix.context import FlowRunContext, SyncmatrixObjectRegistry
...
from syncmatrix.task_runners import BaseTaskRunner
from syncmatrix.utilities.annotations import NotSet
from syncmatrix.utilities.asyncutils import Async, Sync
...
if TYPE_CHECKING:
    from syncmatrix.context import TaskRunContext
...
class Task(Generic[P, R]):
    """
    A Syncmatrix task definition.

    !!! note
        We recommend using [the `@task` decorator][syncmatrix.tasks.task] for most use-cases.

    Wraps a function with an entrypoint to the Syncmatrix engine. Calling this class within a flow function
    ...
    Args:
        ...
        tags: An optional set of tags to be associated with runs of this task. These
            tags are combined with any tags defined by a `syncmatrix.tags` context at
            task runtime.
        ...
        log_prints: If set, `print` statements in the task will be redirected to the
            Syncmatrix logger for the task run. Defaults to `None`, which indicates
    ...
    def __init__(
        ...
        self.retries = (
            retries if retries is not None else SYNCMATRIX_TASK_DEFAULT_RETRIES.value()
        )
        if retry_delay_seconds is None:
            retry_delay_seconds = SYNCMATRIX_TASK_DEFAULT_RETRY_DELAY_SECONDS.value()
    ...
    def __call__(
        ...
        **kwargs: P.kwargs,
        ...
        the result is wrapped in a Syncmatrix State which provides error handling.
        """
        from syncmatrix.engine import enter_task_run_engine
        ...
        if (
            SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING.value()
            and not FlowRunContext.get()
        ):
            from syncmatrix import get_client
    ...
    def submit(
        ...
        *args: P.args,
        **kwargs: P.kwargs,
    ) -> Awaitable[SyncmatrixFuture[T, Async]]:
        ...
    ...
    def submit(
        ...
        *args: Any,
        ...
        wait_for: Optional[Iterable[SyncmatrixFuture]] = None,
        **kwargs: Any,
    ) -> Union[SyncmatrixFuture, Awaitable[SyncmatrixFuture], TaskRun, Awaitable[TaskRun]]:
        """
        ...
        Returns:
            ...
            If `return_state` is True a future wrapped in a Syncmatrix State allowing asynchronous access to
                the state of the task
        ...
        from syncmatrix.engine import create_autonomous_task_run, enter_task_run_engine
    ...
    def map(
        ...
        *args: P.args,
        **kwargs: P.kwargs,
    ) -> List[SyncmatrixFuture[None, Sync]]:
        # `NoReturn` matches if a type can't be inferred for the function which stops a
        # sync function from matching the `Coroutine` overload
        ...
    ...
    def map(
        ...
        *args: P.args,
        **kwargs: P.kwargs,
    ) -> Awaitable[List[SyncmatrixFuture[T, Async]]]:
        ...
        ...
        **kwargs: Any,
        ...
        Args:
            *args: Iterable and static arguments to run the tasks with
            return_state: Return a list of Syncmatrix States that wrap the results
                of each task run.
        ...
        Examples:
            ...
            >>> from syncmatrix import unmapped
            >>>
            >>> @task
        ...
        from syncmatrix.engine import begin_task_map, enter_task_run_engine
    ...
    def serve(self, task_runner: Optional[BaseTaskRunner] = None) -> "Task":
        ...
        establish a websocket connection with the Syncmatrix server and listen for
        ...
        if not SYNCMATRIX_EXPERIMENTAL_ENABLE_TASK_SCHEDULING:
            raise ValueError(
        ...
        from syncmatrix.task_server import serve
...
def task(
    ...
    *,
    ...
    """
    Decorator to designate a function as a task in a Syncmatrix workflow.
    ...
    Examples:
        ...
        >>> from syncmatrix.tasks import task_input_hash
