...
from syncmatrix.settings import SYNCMATRIX_API_URL, get_current_settings, temporary_settings
from syncmatrix.testing.utilities import AsyncMock
from syncmatrix.utilities.processutils import open_process


@pytest.fixture(autouse=True)
def add_syncmatrix_loggers_to_caplog(caplog):
    import logging
...
async def hosted_api_server(unused_tcp_port_factory):
    """
    Runs an instance of the Syncmatrix API server in a subprocess instead of the using the
    ...
    async with open_process(
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not response:
                raise RuntimeError(
                    "Timed out while attempting to connect to hosted test Syncmatrix API."
                )
...
def use_hosted_api_server(hosted_api_server):
    """
    Sets `SYNCMATRIX_API_URL` to the test session's hosted API endpoint.
    """
    with temporary_settings({SYNCMATRIX_API_URL: hosted_api_server}):
...
def mock_emit_events_to_cloud(monkeypatch):
    monkeypatch.setattr("syncmatrix.events.utilities.emit_events_to_cloud", lambda: True)
