...
from syncmatrix.client.schemas import sorting
from syncmatrix.client.utilities import inject_client
from syncmatrix.filesystems import ReadableFileSystem
from syncmatrix.results import PersistedResult
from syncmatrix.serializers import Serializer
from syncmatrix.server.database.dependencies import temporary_database_interface
...
def syncmatrix_test_harness():
    ...
    with TemporaryDirectory() as temp_dir:
        with ExitStack() as stack:
            ...
            DB_PATH = "sqlite+aiosqlite:///" + str(Path(temp_dir) / "syncmatrix-test.db")
            stack.enter_context(
                syncmatrix.settings.temporary_settings(
                    # Clear the SYNCMATRIX_API_URL
                    restore_defaults={syncmatrix.settings.SYNCMATRIX_API_URL},
                    # Use a temporary directory for the database
                    updates={
                        syncmatrix.settings.SYNCMATRIX_API_DATABASE_CONNECTION_URL: DB_PATH,
                        syncmatrix.settings.SYNCMATRIX_API_URL: None,
                    },
                )
            )
            yield
...
async def get_most_recent_flow_run(client: SyncmatrixClient = None):
    if client is None:
        client = get_client()
...
async def assert_uses_result_storage(
    state: State, storage: Union[str, ReadableFileSystem], client: "SyncmatrixClient"
):
    assert isinstance(state.data, PersistedResult)
    assert_blocks_equal(
