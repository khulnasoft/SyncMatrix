...
infrastructure using Azure Container Instances for Syncmatrix work pools.
...
from syncmatrix.cli._prompts import prompt, prompt_select_from_table
...
class ContainerInstancePushProvisioner:
    ...
    def __init__(self):
        ...
        self._identity_name = "syncmatrix-acr-identity"
        self.azure_cli = AzureCLI(self.console)
        self._credentials_block_name = None
        self._resource_group_name = "syncmatrix-aci-push-pool-rg"
        self._app_registration_name = "syncmatrix-aci-push-pool-app"
        self._registry_name_prefix = "syncmatrix"
    ...
    async def _get_or_create_registry(
        ...
        """
        # check to see if there are any registries starting with 'syncmatrix'
    ...
    async def _create_aci_credentials_block(
        ...
        Args:
            ...
            client (SyncmatrixClient): An instance of SyncmatrixClient.
    ...
    async def _aci_credentials_block_exists(
        self, block_name: str, client: SyncmatrixClient
    ) -> bool:
        """
    ...
    async def _create_provision_table(self, work_pool_name: str, client: SyncmatrixClient):
        return Panel(
            dedent(
                f"""\
                    Provisioning infrastructure for your work pool [blue]{work_pool_name}[/] will require:
                        ...
                        Updates in Syncmatrix workspace
    ...
    async def _customize_resource_names(
        self, work_pool_name: str, client: SyncmatrixClient
    ) -> bool:
    ...
    async def provision(
        ...
        Args:
            ...
            client (Optional[SyncmatrixClient]): An instance of SyncmatrixClient. If None, it will be injected.
        ...
        with Progress(console=self._console) as progress:
            ...
            update_current_profile(
                {SYNCMATRIX_DEFAULT_DOCKER_BUILD_NAMESPACE: registry["loginServer"]}
            )
            progress.advance(task)
