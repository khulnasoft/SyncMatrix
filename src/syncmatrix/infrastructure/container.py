...
It has been replaced by the Docker worker from the syncmatrix-docker package, which offers enhanced functionality and better performance.
...
import syncmatrix
from syncmatrix.blocks.core import Block, SecretStr
...
# Labels to apply to all containers started by Syncmatrix
CONTAINER_LABELS = {
    "io.syncmatrix.version": syncmatrix.__version__,
}
...
@deprecated_class(
    start_date="Mar 2024",
    help="Use the `DockerRegistryCredentials` class from syncmatrix-docker instead.",
)
...
class DockerRegistry(BaseDockerLogin):
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/infrastructure/#syncmatrix.infrastructure.docker.DockerRegistry"
...
@deprecated_class(
    start_date="Mar 2024",
    help="Use the Docker worker from syncmatrix-docker instead."
...
class DockerContainer(Infrastructure):
    ...
    Click [here](https://docs.khulnasoft.com/guides/deployment/docker) to see a tutorial.
    ...
    ## Connecting to a locally hosted Syncmatrix API
    ...
    image: str = Field(
        default_factory=get_syncmatrix_image_name,
        description="Tag of a Docker image to use. Defaults to the Syncmatrix image.",
    )
    ...
    _logo_url = "https://cdn.sanity.io/images/3ugk85nk/production/14a315b79990200db7341e42553e23650b34bb96-250x250.png"
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/infrastructure/#syncmatrix.infrastructure.DockerContainer"
    ...
    def _get_network_mode(self) -> Optional[str]:
        ...
        api_url = self.env.get("SYNCMATRIX_API_URL", SYNCMATRIX_API_URL.value())
    ...
    def _get_extra_hosts(self, docker_client) -> Dict[str, str]:
        ...
        if sys.platform == "linux" and (
            ...
            # a local address
            "SYNCMATRIX_API_URL" not in self.env
            or re.search(
                ".*(localhost)|(127.0.0.1)|(host.docker.internal).*",
                self.env["SYNCMATRIX_API_URL"],
            )
        ):
