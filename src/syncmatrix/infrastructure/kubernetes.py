...
It has been replaced by the Kubernetes worker from the syncmatrix-kubernetes package, which offers enhanced functionality and better performance.
...
from syncmatrix.blocks.kubernetes import KubernetesClusterConfig
from syncmatrix.exceptions import InfrastructureNotAvailable, InfrastructureNotFound
...
from syncmatrix.utilities.pydantic import JsonPatch
...
@deprecated_class(
    start_date="Mar 2024",
    help="Use the Kubernetes worker from syncmatrix-kubernetes instead."
...
class KubernetesJob(Infrastructure):
    ...
    For a guided tutorial, see [How to use Kubernetes with Syncmatrix](https://medium.com/the-syncmatrix-blog/how-to-use-kubernetes-with-syncmatrix-419b2e8b8cb2/).
    For more information, including examples for customizing the resulting manifest, see [`KubernetesJob` infrastructure concepts](https://docs.khulnasoft.com/concepts/infrastructure/#kubernetesjob).
    ...
    Attributes:
        ...
        image: An optional string specifying the image reference of a container image
            to use for the job, for example, docker.io/khulnasoft/syncmatrix:2-latest. The
            behavior is as described in https://kubernetes.io/docs/concepts/containers/images/#image-names.
            Defaults to the Syncmatrix image.
    ...
    _documentation_url = "https://docs.khulnasoft.com/api-ref/syncmatrix/infrastructure/#syncmatrix.infrastructure.KubernetesJob"
    ...
    image: Optional[str] = Field(
        ...
        description=(
            ...
            " `docker.io/khulnasoft/syncmatrix:2-latest`.The behavior is as described in"
            " the Kubernetes documentation and uses the latest version of Syncmatrix by"
    ...
    def default_image(cls, values):
        ...
        if not image and not job_image:
            values["image"] = get_syncmatrix_image_name()
    ...
    async def generate_work_pool_base_job_template(self):
        from syncmatrix.workers.utilities import (
    ...
    def _get_cluster_uid(self) -> str:
        ...
        SYNCMATRIX_KUBERNETES_CLUSTER_UID can be set in cases where the `kube-system`
        ...
        env_cluster_uid = os.environ.get("SYNCMATRIX_KUBERNETES_CLUSTER_UID")
        if env_cluster_uid:
    ...
    def _get_environment_variables(self):
        ...
        if (
            "SYNCMATRIX_API_URL" in env
            and "SYNCMATRIX_API_URL" not in self.env
            and self._api_dns_name
        ):
            env["SYNCMATRIX_API_URL"] = (
                env["SYNCMATRIX_API_URL"]
