...
from syncmatrix.exceptions import ObjectAlreadyExists
...
import syncmatrix
from syncmatrix.blocks.core import Block, BlockNotSavedError
...
MIN_COMPAT_SYNCMATRIX_VERSION = "2.0b12"


if TYPE_CHECKING:
    from syncmatrix.client.schemas.objects import Deployment, Flow, FlowRun
...
class Infrastructure(Block, abc.ABC):
    ...
    async def publish_as_work_pool(self, work_pool_name: Optional[str] = None):
        ...
        if SYNCMATRIX_UI_URL:
            console.print(
                ...
                f" {SYNCMATRIX_UI_URL.value()}/work-pools/work-pool/{work_pool.name}"
            )
        ...
        console.print(f"\t[blue]syncmatrix worker start --pool {work_pool.name}[/]\n")
    ...
    @property
    def logger(self):
        return get_logger(f"syncmatrix.infrastructure.{self.type}")

    @property
    def is_using_a_runner(self):
        return self.command is not None and "syncmatrix flow-run execute" in shlex.join(
            self.command
        )
    ...
    def _base_flow_run_command() -> List[str]:
        ...
        if experiment_enabled("enhanced_cancellation"):
            if (
                SYNCMATRIX_EXPERIMENTAL_WARN
                and SYNCMATRIX_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION
            ):
                warnings.warn(
            ...
            return ["syncmatrix", "flow-run", "execute"]

        return ["python", "-m", "syncmatrix.engine"]
    ...
    def _base_flow_run_environment(flow_run: "FlowRun") -> Dict[str, str]:
        ...
        """
        environment = {}
        environment["SYNCMATRIX__FLOW_RUN_ID"] = str(flow_run.id)
        return environment
    ...
    def _base_deployment_labels(deployment: "Deployment") -> Dict[str, str]:
        ...
        if deployment.updated is not None:
            labels["khulnasoft.com/deployment-updated"] = deployment.updated.in_timezone(
                "utc"
            ).to_iso8601_string()
        return labels
