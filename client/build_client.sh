...
if [ -z ${CI} ];
    then
        if [[ -z "${SYNCMATRIX_API_KEY}" ]] || [[ -z "${SYNCMATRIX_API_URL}" ]]; then
            echo "In order to run smoke tests locally, SYNCMATRIX_API_KEY and"\
            "SYNCMATRIX_API_URL must be set and valid for a Syncmatrix Cloud account.";
            exit 1;
        fi
        python -m venv venv;
