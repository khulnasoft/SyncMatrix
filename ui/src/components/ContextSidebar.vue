  ...
  <p-context-sidebar class="context-sidebar">
    <template #header>
      <router-link :to="routes.root()" class="context-sidebar__logo-link">
        <p-icon icon="Syncmatrix" class="context-sidebar__logo-icon" />
      </router-link>
    </template>
    ...
    <template #footer>
      <a href="https://www.khulnasoft.com/cloud-vs-oss?utm_source=oss&utm_medium=oss&utm_campaign=oss&utm_term=none&utm_content=none" target="_blank">
        <p-context-nav-item>
          <div>
            Ready to scale?
          </div>
...
<script lang="ts" setup>
  import { PContextSidebar, PContextNavItem } from '@syncmatrix/design'
  import { localization } from '@syncmatrix/ui-library'
