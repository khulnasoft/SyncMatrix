...
<script lang="ts" setup>
  import { media } from '@syncmatrix/design'
  import { useWorkspaceApi, PageHeadingWorkPoolQueue, CodeBanner, WorkPoolQueueDetails, WorkPoolQueueUpcomingFlowRunsList, FlowRunFilteredList, useFlowRunsFilter, useTabs } from '@syncmatrix/ui-library'
  import { useRouteParam, useRouteQueryParam, useSubscription } from '@syncmatrix/vue-compositions'
  ...
  const isAgentWorkPool = computed(() => workPool.value?.type === 'syncmatrix-agent')
  ...
  const codeBannerCliCommand = computed(() => `syncmatrix ${isAgentWorkPool.value ? 'agent' : 'worker'} start --pool "${workPoolName.value}" --work-queue "${workPoolQueueName.value}"`)
