...
<script lang="ts" setup>
  ...
  import { WorkQueueDetails, PageHeadingWorkQueue, FlowRunFilteredList, WorkQueueFlowRunsList, CodeBanner, localization, useWorkspaceApi, useFlowRunsFilter, SyncmatrixStateNames, useFlowRunsFilterFromRoute } from '@syncmatrix/ui-library'
  import { useSubscription, useRouteParam } from '@syncmatrix/vue-compositions'
  ...
  const workQueueCliCommand = computed(() => `syncmatrix agent start ${workQueue.value ? ` --work-queue "${workQueue.value.name}"` : ''}`)
