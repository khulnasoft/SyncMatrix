      ...
      <PGlobalSidebar class="app-router-view__mobile-menu">
        <template #upper-links>
          <router-link :to="appRoutes.root()">
            <p-icon icon="Syncmatrix" class="app-router-view__syncmatrix-icon" />
          </router-link>
        </template>
...
<script lang="ts" setup>
  import { PGlobalSidebar, PIcon, media, showToast } from '@syncmatrix/design'
  import { workspaceApiKey, canKey as designCanKey, createWorkspaceRoutes, workspaceRoutesKey } from '@syncmatrix/ui-library'
  ...
  const api = createSyncmatrixApi(config)
  const routes = createWorkspaceRoutes()

  provide(canKey, can)
  provide(designCanKey, can)
  provide(syncmatrixApiKey, api)
...
.app-router-view { @apply
  ...
  bg-no-repeat
  overflow-auto;
  --syncmatrix-scroll-margin: theme('spacing.20');
  height: 100vh;
...
.app-router-view__syncmatrix-icon { @apply
  w-7
  h-7
}
...
@screen lg {
  .app-router-view {
    --syncmatrix-scroll-margin: theme('spacing.2');
    display: grid;
