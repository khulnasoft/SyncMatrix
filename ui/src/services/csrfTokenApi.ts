...
import { randomId } from '@syncmatrix/design'
import { Api, AxiosInstanceSetupHook, SyncmatrixConfig } from '@syncmatrix/ui-library'
...
export class CsrfTokenApi extends Api {
    ...
    public constructor(apiConfig: SyncmatrixConfig, instanceSetupHook: AxiosInstanceSetupHook | null = null) {
    ...
    public async addCsrfHeaders(config: InternalAxiosRequestConfig) {
        if (this.csrfSupportEnabled) {
            ...
            config.headers['Syncmatrix-Csrf-Token'] = csrfToken.token
            config.headers['Syncmatrix-Csrf-Client'] = this.clientId
            config.headers['Syncmatrix-Csrf-Retry-Count'] = config.headers['Syncmatrix-Csrf-Retry-Count'] ?? '0'
        }
    }
...
export function setupCsrfInterceptor(csrfTokenApi: CreateActions<CsrfTokenApi>, axiosInstance: AxiosInstance) {
    ...
    axiosInstance.interceptors.response.use(undefined, async (error: AxiosError) => {
        if (isAxiosError(error) && csrfTokenApi.isInvalidCsrfToken(error)) {
            ...
            if (config && config.headers['Syncmatrix-Csrf-Retry-Count']) {
                const retryCount = parseInt(config.headers['Syncmatrix-Csrf-Retry-Count'], 10)
                if (retryCount < MAX_RETRIES) {
                    await csrfTokenApi.addCsrfHeaders(config)
                    config.headers['Syncmatrix-Csrf-Retry-Count'] = (retryCount + 1).toString()
