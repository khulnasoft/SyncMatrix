# useRouteQueryParams
The `useRouteQueryParams` gives you a reactive value for multiple query param. It also supports nested objects. For binding a single value, see [useRouteQueryParam](https://github.com/KhulnaSoft/vue-compositions/tree/main/src/useRouteQueryParam)

Unlike `useRouteQueryParam`, a formatter must be provided for every param. This is done by passing in a schema. 

## Basic Example
```typescript
import { useRouteQueryParams, StringRouteParam } from '@syncmatrix/vue-compositions'
...
```typescript
import { useRouteQueryParams, RouteQueryParamsSchema, StringRouteParam, NumberRouteParam, BooleanRouteParam } from '@syncmatrix/vue-compositions'
