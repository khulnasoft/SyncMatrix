  ...
  <ComponentPage title="Icons" :demos="[{ title: 'Icons' }]">
    <template #description>
      The following icons are a combination of <PLink to="https://heroicons.com/">
        Hero Icons
      </PLink> and custom icons added by Syncmatrix.
    </template>
...
<script lang="ts" setup>
  ...
  import { HeroIcon, SyncmatrixIcon } from '@/types'
  import * as heroIcons from '@heroicons/vue/24/outline'
  import { useDebouncedRef } from '@syncmatrix/vue-compositions'
  import { ref, computed } from 'vue'
  import CodeBlock from '@/demo/components/CodeBlock.vue'
  import ComponentPage from '@/demo/components/ComponentPage.vue'

  const syncmatrixIconKeys = Object.keys(syncmatrixIcons) as SyncmatrixIcon[]
  const heroIconKeys = Object.keys(heroIcons) as HeroIcon[]
  const icons = [...syncmatrixIconKeys, ...heroIconKeys]
