  ...
  <p-form>
    ...
    <p-label label="From the following resources">
      <EventResourceCombobox
        :selected="formData.match['syncmatrix.resource.id'] ?? []"
        multiple
      ...
      <template #content>
        <div class="automation-trigger-custom-event-input__evaluation-options-accordion-content">
          ...
          <p-label label="Filter for events related to">
            <EventResourceCombobox
              :selected="formData.matchRelated['syncmatrix.resource.id'] ?? []"
              multiple
...
<script setup lang="ts">
  import { asArray, toPluralString } from '@syncmatrix/design'
  import { ValidationRule, useValidation } from '@syncmatrix/vue-compositions'
  ...
  function updateMatchResourceIds(selectedResources: string[]): void {
    ...
    if (selectedResources.length === 0) {
      delete formData.value.match['syncmatrix.resource.id']
      return
    }

    formData.value.match['syncmatrix.resource.id'] = selectedResources
  }
  ...
  function updateMatchRelatedResourceIds(selectedResources: string[]): void {
    ...
    if (selectedResources.length === 0) {
      delete formData.value.matchRelated['syncmatrix.resource.id']
      return
    }
    formData.value.matchRelated['syncmatrix.resource.id'] = selectedResources
  }
