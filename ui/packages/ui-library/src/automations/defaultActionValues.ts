...
export function getDefaultNotificationSubject(template: AutomationTriggerTemplate): string {
  switch (template) {
    case 'deployment-status':
      return 'Syncmatrix deployment \'{{ flow.name }}/{{ deployment.name }}\' has entered status \'{{ deployment.status }}\''
    case 'flow-run-state':
      return 'Syncmatrix flow run notification'
    case 'work-pool-status':
      return 'Syncmatrix work pool \'{{ work_pool.name }}\' has entered status \'{{ work_pool.status }}\''
    case 'work-queue-status':
      return 'Syncmatrix work queue \'{{ work_queue.name }}\' has entered status \'{{ work_queue.status }}\''
    case 'custom':
      return 'A Syncmatrix automation \'{{ automation.name }}\' was triggered'
    default:
