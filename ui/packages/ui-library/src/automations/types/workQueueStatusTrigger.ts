...
export const { values: workQueueStatusEvent, isValue: isWorkQueueStatusEvent } = createTuple(['syncmatrix.work-queue.ready', 'syncmatrix.work-queue.not-ready', 'syncmatrix.work-queue.paused'])
...
export function isWorkQueueStatusTrigger(trigger: unknown): boolean {
  return isAutomationTriggerEvent(trigger) &&
         isMatchResource(trigger, syncmatrixResourceIds => syncmatrixResourceIds.every(value => value.startsWith('syncmatrix.work-queue'))) &&
