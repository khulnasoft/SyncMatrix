...
```ts
import { mocker } from '@syncmatrix/ui-library'
...
In order for the mocker to know how to create this new type, we need to define a [`MockFunction`](https://github.com/KhulnaSoft/ui-library/src/services/Mocker.ts).
```ts 
...
You're not done yet, just creating this mock function does NOT make it available to `mocker.create`. The next step is to add your mock function to that [predefined index](https://github.com/KhulnaSoft/ui-library/src/mocks/index.ts) I mentioned above.
