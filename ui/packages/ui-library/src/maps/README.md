...
```ts
import { mapper } from '@syncmatrix/ui-library'
...
In order for the mapper to know how to map from `BlueType` to `RedType`, we need to define a [`MapperFunction`](https://github.com/KhulnaSoft/ui-library/src/services/Mapper.ts).
```ts 
...
You're not done yet, just creating this map function does NOT make it available to `mapper.map`. The next step is to add your map function to that [predefined index](https://github.com/KhulnaSoft/ui-library/src/maps/index.ts) I mentioned above.
