import { CollectionItem, CollectionsResponse, SyncmatrixWorkerCollectionResponse, WorkerCollectionItem } from '@/models'
...
export class CollectionsApi extends Api {
  ...
  public getWorkerCollection(): Promise<WorkerCollectionItem[]> {
    return this.get<SyncmatrixWorkerCollectionResponse>('/views/aggregate-worker-metadata')
      .then(({ data }) => mapper.map('SyncmatrixWorkerCollectionResponse', data, 'WorkerCollectionItem'))
  }
}