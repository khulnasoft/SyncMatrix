...
export type SyncmatrixConfig = {
  baseUrl: string,
  token?: string,
}

type ConfigFunction<R, T extends SyncmatrixConfig = SyncmatrixConfig> = (config: T) => R
export type ApiBaseUrl<T extends SyncmatrixConfig = SyncmatrixConfig> = string | ConfigFunction<string, T>
export type ApiHeaders<T extends SyncmatrixConfig = SyncmatrixConfig> = RawAxiosRequestHeaders | ConfigFunction<RawAxiosRequestHeaders, T>

export const getSyncmatrixBaseUrl: ApiBaseUrl = (config) => config.baseUrl

export const getSyncmatrixUIHeaders: RawAxiosRequestHeaders = { 'X-SYNCMATRIX-UI': true }
...
export class Api<T extends SyncmatrixConfig = SyncmatrixConfig> {
  protected readonly apiConfig: MaybeGetter<T>
  protected apiHeaders: MaybeArray<ApiHeaders> = [getSyncmatrixUIHeaders, getAuthorizationHeaders]
  protected apiBaseUrl: ApiBaseUrl = getSyncmatrixBaseUrl
