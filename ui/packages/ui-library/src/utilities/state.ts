...
import { syncmatrixStateNameTypes, syncmatrixStateNames } from '@/types/states'
import { capitalize } from '@/utilities/strings'

export function mapStateNameToStateType(stateName: string = 'Unknown'): { name: string, type: StateType | null } {
  const syncmatrixStateName = syncmatrixStateNames.find(name => stateName.toLowerCase() === name.toLowerCase())

  if (syncmatrixStateName) {
    const stateType = syncmatrixStateNameTypes[syncmatrixStateName]

    return {
      name: syncmatrixStateName,
      type: stateType,
    }
  }
