...
<script lang="ts" setup>
  import { isDefined, isNotNullish } from '@syncmatrix/design'
  ...
  import { SchemaValue, getSyncmatrixKindFromValue } from '@/schemas/types/schemaValues'
  ...
  const props = defineProps<{
    ...
    // https://github.com/KhulnaSoft/ui-library/pull/2355
  ...
  const kind = computed(() => getSyncmatrixKindFromValue(() => props.value))
  ...
  const { errors: propertyErrors, setKind } = useSyncmatrixKindValue({ value, property: () => props.propertyForValidation ?? props.property })
