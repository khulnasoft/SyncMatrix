...
<script lang="ts" setup>
  import { ButtonGroupOption } from '@syncmatrix/design'
  ...
  import { SchemaValue, isSyncmatrixKindValue } from '@/schemas/types/schemaValues'
  ...
  const internalValue = computed(() => {
    if (isSyncmatrixKindValue(props.value)) {
      return props.value
    }
  ...
  async function updateValue(value: unknown): Promise<void> {
    ...
    // https://github.com/KhulnaSoft/syncmatrix/issues/14265
    if (value === undefined) {
