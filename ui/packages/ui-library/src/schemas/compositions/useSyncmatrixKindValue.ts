...
import { SyncmatrixKind, getSyncmatrixKindFromValue, isSyncmatrixKindJson } from '@/schemas/types/schemaValues'
...
export type UseSyncmatrixKindValueParameters = {
  property: MaybeRefOrGetter<SchemaProperty>,
  value: Ref<SchemaValue>,
}

export type UseSyncmatrixKindValue = {
  errors: Ref<SchemaValueError[]>,
  setKind: (to: SyncmatrixKind) => Promise<void>,
}

export function useSyncmatrixKindValue({ property, value: schemaValue }: UseSyncmatrixKindValueParameters): UseSyncmatrixKindValue {
  const valueMap: Partial<Record<SyncmatrixKind, SchemaValue>> = {}
  const errors = ref<SchemaValueError[]>([])
  const { schema } = useSchemaFormSettings()
  const api = useWorkspaceApi()

  async function setKind(to: SyncmatrixKind): Promise<void> {
    if (isSyncmatrixKindJson(schemaValue.value) && to === 'none') {
    ...
    const currentKind = getSyncmatrixKindFromValue(schemaValue.value)
    ...
    try {
      ...
      if (isInvalidSchemaValueTransformationError(error)) {
        ...
        schemaValue.value = valueMap[to] ?? { __syncmatrix_kind: to }
      }
