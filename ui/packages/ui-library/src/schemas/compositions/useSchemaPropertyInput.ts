...
import { isSyncmatrixKindValue } from '@/schemas/types/schemaValues'
...
export function useSchemaPropertyInput(schemaProperty: MaybeRefOrGetter<SchemaProperty>, propertyValue: Ref<SchemaValue>, propertyErrors: MaybeRefOrGetter<SchemaValueError[]>) {
  const input = computed(() => {
    ...
    if (!isSyncmatrixKindValue(propertyValue.value)) {
    ...
    if (isSyncmatrixKindValue(propertyValue.value, 'json')) {
    ...
    if (isSyncmatrixKindValue(propertyValue.value, 'jinja')) {
    ...
    if (isSyncmatrixKindValue(propertyValue.value, 'workspace_variable')) {
    ...
    if (isSyncmatrixKindValue(propertyValue.value, 'none')) {
      return withProps(SchemaFormPropertyInput, {
        property,
        value: propertyValue.value,
        errors,
