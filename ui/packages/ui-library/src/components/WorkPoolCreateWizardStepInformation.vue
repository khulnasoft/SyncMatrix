...
<script lang="ts" setup>
  import { useWizardStep } from '@syncmatrix/design'
  import { usePatchRef, useValidation, useValidationObserver } from '@syncmatrix/vue-compositions'
  ...
  const { state, error } = useValidation(name, 'Work pool name', value => {
    if (value?.toLowerCase().startsWith('syncmatrix')) {
      return 'Work pools starting with "syncmatrix" are reserved for internal use.'
    }
