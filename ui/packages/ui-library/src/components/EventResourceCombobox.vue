  ...
  const options = computed<RelatedResourceGroup[]>(() => [
    {
      ...
      options: automations.value.map(automation => ({
        ...
        value: `syncmatrix-cloud.automation.${automation.id}`,
        type: 'automation',
      })),
    },
    {
      ...
      options: blocks.value.map(block => ({
        label: block.name,
        value: `syncmatrix.block-document.${block.id}`,
        type: 'block-document',
      })),
    },
    {
      ...
      options: deployments.value.map(deployment => ({
        ...
        value: `syncmatrix.deployment.${deployment.id}`,
        type: 'deployment',
      })),
    },
    {
    ...
    },
    {
      ...
      options: workPools.value.map(workPool => ({
        label: workPool.name,
        value: `syncmatrix.work-pool.${workPool.id}`,
        type: 'work-pool',
      })),
    },
    {
      ...
      options: workQueues.value.map(workQueue => ({
        ...
        value: `syncmatrix.work-queue.${workQueue.id}`,
        type: 'work-queue',
      })),
    },
