...
<script setup lang="ts">
  ...
  const { events } = useWorkspaceEvents(() => {
    ...
    return {
      event: {
        name: ['syncmatrix-cloud.automation.action.executed', 'syncmatrix.automation.action.executed'],
      },
      resource: {
        idPrefix: ['syncmatrix-cloud.automation', 'syncmatrix.automation'],
      },
      related: {
        id: [`syncmatrix.flow-run.${props.flowRun.id}`],
      },
      occurred: {
        since,
        until,
  ...
  const automationId = computed(() => {
    ...
    const { id, role } = getResourceIdParts(event.resource['syncmatrix.resource.id'])
