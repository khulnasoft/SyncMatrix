...
<script lang="ts" setup>
  import { isArray, PSelect, SelectOption } from '@syncmatrix/design'
  ...
  const internalValue = computed({
    get() {
      ...
      const isAllExceptScheduled = props.selected.length === syncmatrixStateNames.length - 1
    ...
    set(value: string | string[] | null) {
      if (!value) {
        ...
        if (values.includes(allExceptScheduled)) {
          emits('update:selected', syncmatrixStateNames.filter((stateName) => stateName !== 'Scheduled'))
        } else {
  ...
  const options = computed<SelectOption[]>(() => {
    const stateNames = syncmatrixStateNames.map((stateName) => {
