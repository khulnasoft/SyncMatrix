...
<script lang="ts" setup>
  import { Icon } from '@syncmatrix/design'
  import { useRouteQueryParam } from '@syncmatrix/vue-compositions'
  ...
  function getTagLabel(resource: WorkspaceEventResource): string {
    return resource['syncmatrix.resource.id'].split('.').at(-1)!
  }

  function getTagFilterRoute(resource: WorkspaceEventResource): RouteLocationRaw {
    const related = toggle(relatedResources.value, resource['syncmatrix.resource.id'])
  ...
  function getTagFilterIcon(resource: WorkspaceEventResource): Icon {
    const filterExists = relatedResources.value.includes(resource['syncmatrix.resource.id'])
